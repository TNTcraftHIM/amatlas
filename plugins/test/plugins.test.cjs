...
// 极简 mock DOM:slot 元素带真实 innerHTML 子节点清除语义 / appendChild;createElement 造同形元素。appendChild 记 parentNode +
// removeChild 真摘除(round10:验 toast 自动消隐出 DOM)。innerHTML 重写必须像浏览器一样清空旧 children，才能锁住面板重绘后的持久关闭控件。
function mockEl() {
  var html = '';
  return { value: '', className: '', textContent: '', _attrs: {}, _kids: [],
    set innerHTML(value) { html = String(value); this._kids.forEach(function (child) { child.parentNode = null; }); this._kids = []; },
    get innerHTML() { return html; },
    appendChild: function (c) { if (c.parentNode && c.parentNode !== this && c.parentNode.removeChild) c.parentNode.removeChild(c); var i = this._kids.indexOf(c); if (i >= 0) this._kids.splice(i, 1); c.parentNode = this; this._kids.push(c); },
    removeChild: function (c) { var i = this._kids.indexOf(c); if (i >= 0) this._kids.splice(i, 1); c.parentNode = null; },
...
function mockDoc(map, withHead) {
  var doc = { querySelector: function (s) { return map[s] || null; }, createElement: function () { return mockEl(); } };
  if (withHead) {
    doc.head = mockEl();
    doc.head.insertBefore = function (c) { c.parentNode = this; this._kids.unshift(c); };
    doc.getElementById = function (id) { return doc.head._kids.filter(function (el) { return el.id === id; })[0] || null; };
  }
  return doc;
}
...
(function () {
  ...
  var e = eng(function (x) { x.use(save); }); e.start();
  ok('A5 UI 挂到约定插槽(button/panel/backdrop 三兄弟,后两者初始隐藏)', bar._kids.length === 3 && /💾/.test(bar._kids[0].textContent) && bar._kids[1].hidden === true && bar._kids[2].hidden === true);
  var btn = bar._kids[0], panel = bar._kids[1], backdrop = bar._kids[2];
  ok('A5b Save backdrop 是同 slot 第三项且 aria-hidden=true', !!backdrop && /amatlas-plugin-backdrop/.test(backdrop.className) && backdrop.parentNode === bar && panel._kids.indexOf(backdrop) < 0 && backdrop.getAttribute('aria-hidden') === 'true');
  btn.onclick();                                         // 点 💾 展开
  ok('A6 点 💾 → panel/backdrop 同步展开,点 panel 内部不关闭', panel.hidden === false && !!backdrop && backdrop.hidden === false && (function () { if (panel.onclick) panel.onclick(); return panel.hidden === false && backdrop.hidden === false; })());
  panel._kids[0]._kids[0].onclick();
  ok('A6b Save ✕ → panel/backdrop 同步隐藏', panel.hidden === true && !!backdrop && backdrop.hidden === true);
  btn.onclick(); if (backdrop && backdrop.onclick) backdrop.onclick();
  ok('A6c Save 重开后点 backdrop → panel/backdrop 同步隐藏', panel.hidden === true && !!backdrop && backdrop.hidden === true);
  btn.onclick();
...
(function () {
  ...
  save.saveTo(1, '待删');
  btn = bar._kids[0]; var backdrop = bar._kids[2]; btn.onclick(); row1._kids[3].onclick();
  ok('A19b 删除确认武装态可观察', /确认/.test(row1._kids[3].textContent));
  if (backdrop && backdrop.onclick) backdrop.onclick();
  if (panel.hidden) btn.onclick();
  ok('A19c Save backdrop 走 closePanel 清 pending,重开不再是确认态', !!backdrop && !/确认/.test(row1._kids[3].textContent));
})();
...
(function () {
  ...
  ok('B6 toggle:button/panel/backdrop 顺序稳定且后两者初始隐藏', bar._kids.length === 3 && /🗺️/.test(bar._kids[0].textContent) && bar._kids[1].hidden === true && bar._kids[2].hidden === true);
  var btn = bar._kids[0], panel = bar._kids[1], backdrop = bar._kids[2];
  ok('B6b Minimap backdrop 是同 slot 第三项且 aria-hidden=true', !!backdrop && /amatlas-plugin-backdrop/.test(backdrop.className) && backdrop.parentNode === bar && panel._kids.indexOf(backdrop) < 0 && backdrop.getAttribute('aria-hidden') === 'true');
  btn.onclick();                                            // 点开 → 画地图
  var mapBox = panel._kids[1];                              // 面板 = [✕ closeEl, mapBox 地图容器](svg 刷新只动 mapBox、不冲掉 ✕)
  ok('B7 点开 panel/backdrop → ✕ + 地图容器画 svg 高亮 home,点 panel 内部不关闭', panel.hidden === false && !!backdrop && backdrop.hidden === false && /✕/.test(panel._kids[0].textContent) && mapBox.innerHTML.indexOf('<svg') >= 0 && /data-node="home"[^>]*data-current="1"/.test(mapBox.innerHTML) && (function () { if (mapBox.onclick) mapBox.onclick(); return panel.hidden === false && backdrop.hidden === false; })());
  ...
  ok('B7c ✕ 点击同步关闭 panel/backdrop', (function () { panel._kids[0].onclick(); return panel.hidden === true && !!backdrop && backdrop.hidden === true; })());
  btn.onclick(); if (backdrop && backdrop.onclick) backdrop.onclick();
  ok('B7d 重开后点 backdrop 同步关闭 panel/backdrop', panel.hidden === true && !!backdrop && backdrop.hidden === true);
})();
...
(function () {
  ...
  ok('B6ia inline 模式不创建 backdrop', box._kids.every(function (el) { return !/amatlas-plugin-backdrop/.test(el.className); }));
  moveTo(e, 'garden');
  ...
  ok('B6fa toggle 无工具栏退化不创建 backdrop', box2._kids.every(function (el) { return !/amatlas-plugin-backdrop/.test(el.className); }));
})();
...
(function () {
  ...
  var e = eng(function (x) { x.use(ach); }); e.start();
  var btn = bar._kids[0], panel = bar._kids[1], backdrop = bar._kids[2];
  ok('F1 工具栏渲染 button/panel/backdrop,后两者初始隐藏', bar._kids.length === 3 && /🏆 成就 0\/2/.test(btn.textContent) && panel.hidden === true && !!backdrop && backdrop.hidden === true);
  ok('F1b Achievement backdrop 是同 slot 第三项且 aria-hidden=true', !!backdrop && /amatlas-plugin-backdrop/.test(backdrop.className) && backdrop.parentNode === bar && panel._kids.indexOf(backdrop) < 0 && backdrop.getAttribute('aria-hidden') === 'true');
  btn.onclick();
  ok('F2 点按钮 → panel/backdrop 展开、列出全部成就,点 panel 内部不关闭', panel.hidden === false && !!backdrop && backdrop.hidden === false && panel.innerHTML.indexOf('甲') >= 0 && panel.innerHTML.indexOf('乙') >= 0 && panel.innerHTML.indexOf('🔒') >= 0 && (function () { if (panel.onclick) panel.onclick(); return panel.hidden === false && backdrop.hidden === false; })());
  ...
  ok('F5a Achievement innerHTML 重绘像真实 DOM 清旧 children 后仍只复挂一个持久 ✕', panel._kids.length === 1 && /✕/.test(panel._kids[0].textContent));
  panel._kids[panel._kids.length - 1].onclick();
  ok('F5b Achievement renderList 复挂的 ✕ 同步关闭 panel/backdrop', panel.hidden === true && !!backdrop && backdrop.hidden === true);
  btn.onclick(); if (backdrop && backdrop.onclick) backdrop.onclick();
  ok('F5c Achievement 重开后点 backdrop 同步关闭 panel/backdrop', panel.hidden === true && !!backdrop && backdrop.hidden === true);
...
(function () {
  ...
  ok('I1 物品栏 button/panel/backdrop 顺序稳定、后两者初始隐藏 + 计数读 initState', bar._kids.length === 3 && /🎒/.test(bar._kids[0].textContent) && /物品 1/.test(bar._kids[0].textContent) && bar._kids[1].hidden === true && bar._kids[2].hidden === true);
  var btn = bar._kids[0], panel = bar._kids[1], backdrop = bar._kids[2];
  ok('I1b Inventory backdrop 是同 slot 第三项且 aria-hidden=true', !!backdrop && /amatlas-plugin-backdrop/.test(backdrop.className) && backdrop.parentNode === bar && panel._kids.indexOf(backdrop) < 0 && backdrop.getAttribute('aria-hidden') === 'true');
  btn.onclick();
  ok('I2 点 🎒 → panel/backdrop 展开 + world.items 显示名/图标,点 panel 内部不关闭', panel.hidden === false && !!backdrop && backdrop.hidden === false && /血红宝石/.test(panel.innerHTML) && /💎/.test(panel.innerHTML) && (function () { if (panel.onclick) panel.onclick(); return panel.hidden === false && backdrop.hidden === false; })());
  panel._kids[panel._kids.length - 1].onclick();
  ok('I2b Inventory renderList 复挂的 ✕ 同步关闭 panel/backdrop', panel.hidden === true && !!backdrop && backdrop.hidden === true);
  btn.onclick(); if (backdrop && backdrop.onclick) backdrop.onclick();
  ok('I2c Inventory 重开后点 backdrop 同步关闭 panel/backdrop', panel.hidden === true && !!backdrop && backdrop.hidden === true);
  btn.onclick();
...
(function () {
  ...
  // J7/J8 reset-first 样式隔离:reset 不得用按钮子集抢占完整 shared id；已有完整 shared 时也不重复注 reset 私有样式。
  var pluginFactories = [createSavePlugin, createMinimapPlugin, createAchievementPlugin, createInventoryPlugin];
  var sharedBlocks = pluginFactories.map(function (factory, index) {
    var bar = mockEl(), doc = mockDoc({ '#plugin-bar': bar }, true), plugin = index === 2 ? factory({ document: doc, achievements: [] }) : factory({ document: doc });
    eng(function (x) { x.use(plugin); }).start();
    var style = doc.getElementById('amatlas-plugin-shared');
    return style && style.textContent || '';
  });
  ok('J7a 四插件默认 shared CSS 逐字同源且自带 full-viewport backdrop 交互层', sharedBlocks.every(function (css) { return css === sharedBlocks[0]; }) &&
    /:where\(\.amatlas-plugin-panel\)\{[^}]*box-sizing:border-box;[^}]*width:92%/.test(sharedBlocks[0]) &&
    /:where\(\.amatlas-plugin-backdrop\)\{[^}]*position:fixed;[^}]*inset:0;[^}]*z-index:59;[^}]*pointer-events:auto;[^}]*touch-action:none/.test(sharedBlocks[0]) &&
    /:where\(\.amatlas-plugin-backdrop\)\[hidden\]\{display:none\}/.test(sharedBlocks[0]));
  var guide = fs.readFileSync(path.join(__dirname, '..', '..', '.claude', 'skills', 'text-adventure-game', 'references', 'game-design-guide.md'), 'utf8');
  ok('J7b 发布作者 guide 只教授插件自带 sibling backdrop，不再传播 panel::before', /\.amatlas-plugin-backdrop\s*\{[^}]*pointer-events\s*:\s*auto;[^}]*touch-action\s*:\s*none/.test(guide) && /\.amatlas-plugin-backdrop\[hidden\]\s*\{\s*display\s*:\s*none/.test(guide) && !/\.amatlas-plugin-panel::before/.test(guide));
  var resetFirstBar = mockEl(), resetFirstDoc = mockDoc({ '#plugin-bar': resetFirstBar }, true);
  var resetFirstEngine = eng(function (x) {
    x.use(createResetPlugin({ document: resetFirstDoc, confirm: false }));
    x.use(createInventoryPlugin({ document: resetFirstDoc }));
  }); resetFirstEngine.start();
  var resetStyle = resetFirstDoc.getElementById('amatlas-plugin-reset');
  var sharedStyle = resetFirstDoc.getElementById('amatlas-plugin-shared');
  ok('J7 reset-first 先注私有按钮 CSS,后续 panel 插件仍注完整 shared(panel+backdrop)', !!resetStyle && !!sharedStyle && /amatlas-plugin-panel/.test(sharedStyle.textContent) && /amatlas-plugin-backdrop/.test(sharedStyle.textContent));
  var panelFirstBar = mockEl(), panelFirstDoc = mockDoc({ '#plugin-bar': panelFirstBar }, true);
  var panelFirstEngine = eng(function (x) {
    x.use(createInventoryPlugin({ document: panelFirstDoc }));
    x.use(createResetPlugin({ document: panelFirstDoc, confirm: false }));
  }); panelFirstEngine.start();
  ok('J8 完整 shared 已存在时 reset 不再注私有样式', !!panelFirstDoc.getElementById('amatlas-plugin-shared') && !panelFirstDoc.getElementById('amatlas-plugin-reset'));
