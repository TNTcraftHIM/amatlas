...
(function (global, factory) {
  ...
  function createSavePlugin(opts) {
    ...
    var api = null, btnEl = null, panelEl = null, backdropEl = null, rows = {}, pending = {}, warned = false;
    ...
    function closePanel() {
      setPanelOpen(false);
      var k; for (k in pending) if (Object.prototype.hasOwnProperty.call(pending, k)) pending[k] = null;
      refresh();
    }
    function refresh() {
      if (!panelEl) return;
    ...
    function onLoad(id) {
      return function () {
        ...
        if (loadFrom(id)) { closePanel(); return; }  // 读档成功 → 收起面板(非破坏、无需确认)
    ...
    function mountUI() {
      ...
      panelEl = doc.createElement('div'); panelEl.className = 'amatlas-plugin-panel amatlas-save-panel';
      backdropEl = doc.createElement('div'); backdropEl.className = 'amatlas-plugin-backdrop'; backdropEl.setAttribute('aria-hidden', 'true');
      setPanelOpen(false);
      backdropEl.onclick = closePanel;
      btnEl.onclick = function () {
        if (!panelEl.hidden) { closePanel(); return; }   // 正在显示 → 关并清 pending
        setPanelOpen(true); if (ioStatus) ioStatus.textContent = ''; refresh();   // 开面板清导入提示(防重开显过期警告)
      };

      var head = doc.createElement('div'); head.className = 'amatlas-save-head'; head.textContent = '存档';
      var close = doc.createElement('button'); close.className = 'amatlas-plugin-close'; close.textContent = '✕'; close.setAttribute('title', '关闭'); close.onclick = closePanel; head.appendChild(close);   // ✕ 直接关浮窗 + 清 pending(rank7)
      ...
      im.onclick = function () {
        ...
        if (!ta.value) return;
        if (importSave(ta.value)) { closePanel(); return; }
      ...
      bar.appendChild(btnEl); bar.appendChild(panelEl); bar.appendChild(backdropEl);
      refresh();
    }
