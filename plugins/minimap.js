...
(function (global, factory) {
  ...
  function createMinimapPlugin(opts) {
    ...
    var api = null, btnEl = null, panelEl = null, backdropEl = null, closeEl = null, mapBox = null;
    ...
    function mountToggle() {                              // 抄 achievement:🗺️ 按钮(进 #plugin-bar)+ toggle 面板
      ...
      panelEl = doc.createElement('div'); panelEl.className = 'amatlas-plugin-panel amatlas-map-panel';
      backdropEl = doc.createElement('div'); backdropEl.className = 'amatlas-plugin-backdrop'; backdropEl.setAttribute('aria-hidden', 'true');
      setPanelOpen(false);
      closeEl = doc.createElement('button'); closeEl.className = 'amatlas-plugin-close'; closeEl.textContent = '✕'; closeEl.setAttribute('title', '关闭'); closeEl.onclick = function () { setPanelOpen(false); };   // ✕ 关浮窗(对齐 save/achievement;此前 minimap 漏了关闭按钮)
      ...
      btnEl.onclick = function () { var open = panelEl.hidden; setPanelOpen(open); if (open) mapBox.innerHTML = svg(); };
      backdropEl.onclick = function () { setPanelOpen(false); };
