...
(function (global, factory) {
  ...
  function createAchievementPlugin(opts) {
    ...
    var api = null, btnEl = null, panelEl = null, backdropEl = null, closeEl = null, stackEl = null, durWarned = false;
    ...
    function mountUI() {
      ...
      panelEl = doc.createElement('div'); panelEl.className = 'amatlas-plugin-panel amatlas-ach-panel';
      backdropEl = doc.createElement('div'); backdropEl.className = 'amatlas-plugin-backdrop'; backdropEl.setAttribute('aria-hidden', 'true');
      setPanelOpen(false);
      closeEl = doc.createElement('button'); closeEl.className = 'amatlas-plugin-close'; closeEl.textContent = '✕'; closeEl.setAttribute('title', '关闭'); closeEl.onclick = function () { setPanelOpen(false); };   // ✕ 直接关浮窗(renderList 后复挂)
      btnEl.onclick = function () { var open = panelEl.hidden; setPanelOpen(open); if (open) renderList(panelEl); };
