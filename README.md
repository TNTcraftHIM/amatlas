...
> **测试版说明**:Amatlas 目前是 **0.x 测试版本**,欢迎试用与反馈(GitHub Issues:https://github.com/TNTcraftHIM/amatlas/issues)。1.0 之前处于快速迭代期,**契约 / 接口可能有较大变动,甚至大型重构**,升级不保证平滑——升级前先读 `CHANGELOG.md` 的「破坏性」条目。
...
| 作者入口 | `CLAUDE.md` 首行导入 `AGENTS.md`，再叠加 `.claude/rules/` | `AGENTS.md`(自动读取,工具中立) |
| 技能手册 | `.claude/skills/text-adventure-game/` | `.agents/skills/text-adventure-game/`(自动生成的等价镜像) |
| 附加机制 | 7 条斜杠命令(`/new-game` 等)、Stop hook 强制审计、审查子代理 | 无本包斜杠命令 / hooks——直接用自然语言提需求，由 `AGENTS.md` 路由到对应 skill 的工作流 |

无论走哪条路,审计与构建都是同一套纯 Node 脚本(graph-audit、assembly-probe、build),产出同一种单文件 HTML。技能手册的**单一真相源是 `.claude/skills/`**;`.agents/skills/` 是生成镜像,**不要手改**(说明见 `.agents/GENERATED.md`)。仓库维护者在引擎包根修改源 skill 后运行 `node core/tooling/codex-parity.mjs --sync --root .` 生成镜像，再运行 `node core/tooling/codex-parity.mjs --check --root .` 防漂移。
...
**用 Codex?** 上面这些斜杠命令是 Claude Code 专属;Codex 用户直接说需求(例:「帮我做一个雨夜灯塔的文字冒险」),它会按 AGENTS.md 路由到对应 skill/example/reference,在 src/ 工作并跑同一套三道闸。
...
| 入口 | 用途 |
|---|---|
| `examples/showroom/index.html` | Embedded Gallery:用单一 iframe 窗口集中 UI skin、通用 Audio Workbench、maze3d visual/audio gallery、独立表现力展示、两份实验性横版切片、7 个 playable references 与 Origin 综合作品（共 15 个入口） |
...
| `examples/maze3d/index.html` | maze3d 统一入口:首个推荐 Recipe 3 是三条可换序线索支路、optional warp、partial pages 与可重试 sequence 的分支仪式库；basic / horror / puzzle / layers / fps-range 仍共用同一 `raycast-maze.js` runtime,不是五个模块。FPS v3 与空间谜题作者面见同目录 `references/maze3d-authoring.md` |
| `examples/maze3d/fps.html` | FPS 资源绕行玩家直达入口:从同一 `world.js` 派生 `start=fps_range`,复用正式 `raycast-maze.js`;先走北支路取得精确弹药，再回主轴取得近程 `scatter`、切回 `precision` 收尾，完整表现由 `maze.theme` 驱动 |
| `examples/maze3d/gallery.html` | maze3d Visual Gallery：六个协调 recipes、五张 FPS 主题卡（四个 family + crystal 特化）与原子素材的作者选材页 |
| `examples/maze3d/audio-gallery.html` | maze3d 私有声音试听 Gallery(需用户手势解锁声音) |
| `examples/rig-showcase/index.html` | cutscene 私有 rig / typewriter / 多角色对话的表现力展示与教学材料；不是第八个玩法模块，也不是正式可复制模块，作者 schema 以 `modules/cutscene/references/cutscene-authoring.md` 为准 |
| `examples/tabletop-demo/index.html` | R·02 灯塔检定：花 1 状态换气闸优势并走和平路线；保留状态则在警报中以资源加值取得带代价的部分成功、走强行接管路线。固定 seed 下同选择可复现，两路都 fail-forward 通关；公开存读档保 durable consequence 与 Continue，不把瞬时骰子帧冒充持久状态 |
| `examples/arcade-demo/index.html` | R·06 贪吃蛇门禁：自定义 example 私有实时模块嵌入普通 scene；正式目标吃满 5 果，三次失败解锁 fail-forward，任何时候可 abort。它证明现役 `manifest.modules` 自定义 kind 接缝，不代表 boot 内置 Arcade 或公共 Canvas schema |
...
`examples/cutscene-demo/` 是唯一正式 cutscene example:主体保持普通文字冒险,开局、关键剧情和结尾才由 cutscene 临时接管舞台。细节演出的常规语义入口是 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`，由 actor / anchor / shot / action 编译到现役 beats；`beat.motion` / `beat.cast` / `beat.speaker` 只作专家 escape hatch。高层混合玩法依赖主干是 core runtime → text-adventure runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game；现役 R·03 还按 manifest 加载 progressions/audio/save/minimap/achievement/reset，完整顺序以正式 `index.html` 与 reference 为准，完整写法与可运行 Rig 见 `modules/cutscene/references/cutscene-authoring.md` 和该 demo。

`examples/rig-showcase/` 复用同一个 cutscene runtime，只用于集中展示 rig、stage、口型和 typewriter 的表现上限；它在 Showroom 中单列为教学材料，不改变“七个正式可复制范本”的计数。

`examples/minimal-demo/` 是 R·05 自定义玩法最小起点：`counter` 只经 `manifest.modules` 显式装配，十次达标后由核心 `exits` 回接 boot 自动认领的普通 `scene`；它证明新 kind 不必进入 core 或 boot 内建表。其余 `arcade-demo` / `tabletop-demo` / `horror-demo` 也都由 `examples/showroom/index.html` 单向索引并嵌入打开；子页本身不反向依赖 Gallery。`arcade-demo` 是正式 R·06 私有实时模块范本，不是引擎内置 Arcade；`horror-demo` 是底层手写装配的 escape-hatch 对照，非新游戏默认起点。
...
2. 把旧项目的 `src/`，以及项目根的 `PROGRESS.md`、`canon.md`、`glossary.md` 拷进新目录。
3. **升级前先读 `CHANGELOG.md` 的「破坏性」条目**——某版若改了作者面字段(如 `world.id` 首版起为必填 UUID),按提示调整 `src/`。
4. 在新目录 `claude` + `/audit-game`,让 AI 按新版重审重建。

旧版无 marker 的 legacy `PROGRESS.md` 不会自动迁移或注入；先显式完整读取，再按 `docs/progress-format.md` 安全整理，不能根据截断片段猜测旧状态。
...
| `presenters/` | 呈现器(消费 View 快照、可叠加):`present-dom.js`(文字/选项)· `present-timeline.js`(cutscene motion/typewriter/rig 的纯编译与共享播放时钟)· `present-svg.js`(程序化场景/骰子)· `present-corridor.js`(走廊视觉)· `present-audio.js`(BGM/环境音/SFX)· `compose-music.js`(程序作曲:22 预设/MusicSpec/完整基线内的四段短呼吸/音色板)· `midi-music.js`(MIDI 导入:`{midi:'<base64>'}` 零依赖解析,GM 折表→音色库)· `present-dice3d.js`(可选 3D 骰) |
...
| `examples/` | 12 个 tracked 目录各守身份:`text-adventure-demo`/`tabletop-demo`/`cutscene-demo`/`minimal-demo`/`arcade-demo`/`maze3d`/`horror-demo` 是七个正式可玩 reference；`origin` 是 scene+encounter+rich cutscene+maze3d FPS v3 主题多武器的综合 dogfood；`rig-showcase` 是 cutscene 表现力教学材料；`sidescroller` 与 `sidescroller-frostline` 是复用同一私有 runtime、分别选择 `coast`/`frost` 协调表现套件的两个实验性客户；`showroom` 是 Gallery / Preview / Workbench 聚合入口。教学、实验与工作台不冒充正式世界或公共模块。 |
...
| 何时加载 | 内容 |
|---|---|
| 自动加载(每次会话) | `AGENTS.md`(工具中立边界)、`CLAUDE.md`(Claude 专属 overlay)、`.claude/rules/`(按唯一职责拆分的通用规则) |
...
**Codex 走同源镜像**:AGENTS.md(自动读取的工具中立入口)+ .agents/skills/(由 .claude/skills/ 单源生成,core/tooling/codex-parity.mjs --check 防漂移)。
...
完整游戏通常跨多个会话。作品当前态放在根 `PROGRESS.md` 的有界 marker 块，耐久世界事实放在 `canon.md`，格式与维护规则见 `docs/progress-format.md`。SessionStart 只回灌合法 current，PreCompact 只写本次恢复快照；legacy、损坏或不可读正文不会被自动注入或迁移。

是否 compact、何时换会话由使用者决定，不需要为工具包选择特定停点。继续当前会话、主动 compact 或新开会话都以同一份作品文件为准；新会话直接说「继续」即可。
