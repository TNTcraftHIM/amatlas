...
| `examples/maze3d/index.html` | maze3d 统一入口:同一 `raycast-maze.js` runtime 的 basic / horror / puzzle / layers / fps-range recipes,不是五个模块；FPS v3 的闭合作者面见同目录 `references/maze3d-authoring.md` |
| `examples/maze3d/fps.html` | FPS 生存场玩家直达入口:从同一 `world.js` 派生 `start=fps_range`,复用正式 `raycast-maze.js`;取得近程 `scatter`、切回精确 `precision` 收尾，完整表现由 `maze.theme` 驱动 |
