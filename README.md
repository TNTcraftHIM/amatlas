...
| 入口 | 用途 |
|---|---|
| `examples/showroom/index.html` | Embedded Gallery:用单一 iframe 窗口集中 UI skin、通用 Audio Workbench、maze3d visual/audio gallery、独立表现力展示、实验性横版切片、7 个 playable references 与 Origin 综合作品（共 14 个入口） |
...
| `examples/` | 11 个 tracked 目录各守身份:`text-adventure-demo`/`tabletop-demo`/`cutscene-demo`/`minimal-demo`/`arcade-demo`/`maze3d`/`horror-demo` 是七个正式可玩 reference；`origin` 是 scene+encounter+rich cutscene+maze3d FPS v3 主题多武器的综合 dogfood；`rig-showcase` 是 cutscene 表现力教学材料；`sidescroller` 是实验性私有 runtime 切片；`showroom` 是 Gallery / Preview / Workbench 聚合入口。教学、实验与工作台不冒充正式世界。 |
