...
| `examples/maze3d/index.html` | maze3d 统一入口:同一 `raycast-maze.js` runtime 的 basic / horror / puzzle / layers / fps-range recipes,不是五个模块；FPS 的闭合作者面见同目录 `references/maze3d-authoring.md` |
| `examples/maze3d/fps.html` | FPS 生存场玩家直达入口:从同一 `world.js` 派生 `start=fps_range`,复用正式 `raycast-maze.js`;先取弹、两枪守卫、穿过发光 D |
...
| `examples/` | 可运行样例(模块化源码 `world.js` + `game.js` + `index.html`):`text-adventure-demo`(主范本)· `tabletop-demo`(跑团)· `cutscene-demo`(唯一正式过场集成范本)· `minimal-demo`(最小模块起点)· `arcade-demo`(自定义模块示范)· `maze3d`(自定义 runtime 示例岛,非公共模块)· `sidescroller`(实验性横版可玩切片,私有 runtime)· `horror-demo`(presenter 演出 / 底层手写装配 escape-hatch 对照)· `rig-showcase`(cutscene 表现力教学材料,非新模块/正式范本)· `showroom`(Gallery / Preview / Workbench Hub) |
