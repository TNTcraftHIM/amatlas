...
(function (global, factory) {
  ...
  function createDomPresenter(opts) {
    ...
    function renderTypewriter(view, playback, box) {
      ...
      function removeRevealHandlers() {
        visualLines.forEach(function (line) { line.onclick = null; });
      }
      ...
      if (!instant && spans.length > timeline.revealCount(scaledPlan, 0)) {
        visualLines.forEach(function (line) { line.onclick = finish; });
      }
      ...
      (actions || []).forEach(function (a) {
        ...
        if (a.locked) {
          ...
          btn.onclick = function () {
            if (a.id === 'cutscene:next' && typewriterState && !typewriterState.complete) {
              typewriterState.showAll();
              return;
            }
            if (engine) engine.apply(a);  // 动作=状态转移,交核心
          };
        }
        box.appendChild(btn);
      });
