...
(function (global, factory) {
  ...
  function createDomPresenter(opts) {
    ...
    function setCutsceneNextName(button, revealing) {
    ...
    function refreshTypewriterNextControl(state) {
      if (state && state.nextButton) setCutsceneNextName(state.nextButton, !state.complete);
    }
    ...
    function bindCutscenePanel(action, nodeKind) {
      ...
      var box = $(S.look);
      if (!box) return;
      var binding = { box: box, action: action, press: null, down: null, move: null, up: null, cancel: null, click: null, events: [] };
    ...
    function renderChoices(actions, nodeKind) {
      var active = doc && doc.activeElement;
      var restoreCutsceneNextFocus = !!(active && /(^|\s)cutscene-next(?:\s|$)/.test(String(active.className || '')));
      clearCutscenePanelBinding();
      var box = $(S.choices); if (!box) return;
      box.textContent = '';
      if (typewriterState) typewriterState.nextButton = null;
      var availableNext = (actions || []).filter(function (action) { return action && action.id === 'cutscene:next' && !action.locked; });
      var cutsceneNextButton = null;
      (actions || []).forEach(function (a) {
        var btn = doc.createElement('button');
        btn.className = 'choice' + (a.kind === 'move' ? ' move' : '') + (a.locked ? ' locked' : '');
        btn.textContent = a.label || '';
        if (a.id === 'cutscene:next') {
          btn.className += ' cutscene-next';
          btn.setAttribute('data-cutscene-next', '');
          if (!a.locked) cutsceneNextButton = btn;
          if (typewriterState) typewriterState.nextButton = btn;
          setCutsceneNextName(btn, !!(typewriterState && !typewriterState.complete));
        }
        ...
        if (a.locked) {
          ...
          btn.onclick = function (event) {
            if (a.id === 'cutscene:next') {
              ...
              activateCutsceneNext(a);
            }
            else if (engine) engine.apply(a);  // 动作=状态转移,交核心
          };
        }
        box.appendChild(btn);
      });
      if (availableNext.length === 1) bindCutscenePanel(availableNext[0], nodeKind);
      if (restoreCutsceneNextFocus && cutsceneNextButton && typeof cutsceneNextButton.focus === 'function') cutsceneNextButton.focus();
    }
    ...
    function ensureStyle() {
      ...
      st.textContent = ':where(.choice-adv){display:inline-block;margin-left:.5em;padding:0 .45em;border-radius:.5em;'
        ...
        + ':where(html[data-node-kind="cutscene"]) #look.cutscene-next-panel{box-sizing:border-box;padding-inline-end:calc(4rem + env(safe-area-inset-right));'
        + 'cursor:pointer}'
        + ':where(html[data-node-kind="cutscene"]) #choices{z-index:2}'
        + ':where(html[data-node-kind="cutscene"]) #choices .cutscene-next{position:fixed;z-index:20;'
        + 'right:max(calc(12px + env(safe-area-inset-right)),calc(50vw - 368px));'
        + 'bottom:max(12px,env(safe-area-inset-bottom));width:48px;min-width:44px;min-height:44px;margin:0}'
        + ':where(html[data-node-kind="cutscene"]) .amatlas-toast-stack{bottom:calc(max(12px,env(safe-area-inset-bottom)) + 52px)}';
    ...
    function present(v) {
      ...
      renderLook(view);
      renderChoices(v.actions, v.nodeKind);
      renderStatus(view);
