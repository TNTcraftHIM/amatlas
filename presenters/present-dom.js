...
(function (global, factory) {
  ...
  function createDomPresenter(opts) {
    ...
    function typewriterSessionIsCurrent(state) {
      if (!state.manager || typeof state.manager.inspect !== 'function' ||
          typeof state.managerGeneration !== 'number') return true;
      var current = state.manager.inspect();
      if (current.generation !== state.managerGeneration) return false;
      return !state.registered || current.key === state.key;
    }
    ...
    function renderTypewriter(view, playback, box) {
      ...
      var instant = reduced || speed === 'instant';
      ...
      if (typewriterState && typewriterState.key === playback.key && typewriterState.plan === plan &&
          ...
          typewriterSessionIsCurrent(typewriterState)) {
      ...
      if (instant || spans.length === 0) {
        finish();
        if (typeof playbackManager.inspect === 'function')
          state.managerGeneration = playbackManager.inspect().generation;
        return;
      }
