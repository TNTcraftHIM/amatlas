...
(function () {
  ...
  // M8 幂等 re-start(R2 二轮 P1):直接连调 startBgs(不先 stopBgs)必须先停旧声景;修复前 runBgsBuild 把 bgsExtra 直接置 []
  //   丢弃旧节点引用而不停音 → 旧源/LFO 永久连 bgsMaster(泄漏+双播、再也停不掉)。反向变异:活跃节点数不随 re-start 累积。
  var ctxLk = makeMockCtx(), apLk = createAudioPresenter({ context: ctxLk });
  function activeNodes(c) { var L = c._log; var st = L.filter(function (x) { return x === 'src.start' || /^osc\.start/.test(x); }).length; var sp = L.filter(function (x) { return x === 'src.stop' || x === 'osc.stop'; }).length; return st - sp; }
  apLk.startBgs('rain');  var aL1 = activeNodes(ctxLk);
  apLk.startBgs('wind');  var aL2 = activeNodes(ctxLk);   // 裸 re-start(未先 stopBgs)
  apLk.startBgs('waves'); var aL3 = activeNodes(ctxLk);   // 再裸 re-start
  ok('M8-idem 裸连调 startBgs 先停旧声景,活跃节点不随 re-start 累积(修复前旧引用被丢=永久泄漏,aL2/aL3≈2·3 倍)', aL1 > 0 && aL2 <= aL1 + 2 && aL3 <= aL1 + 2);
  // M9 SfxSpec 不被上一段合成音乐的演奏 profile 污染(R2 二轮 P1):mkOsc/mkLp/envADSR 共享 module 级 voicePerformance,
  //   musicVoice 每音符置它、playSfxSpec 从不复位 → 残留 profile 静默缩放作者 gain/attack/release(实测 0.5→0.48)。
  //   反向变异:先放合成音乐再放同一 sfx,其增益包络值必须与「单独放该 sfx」一致(只比值、不比时刻,免受 currentTime 干扰)。
  var MSPEC = { mode: 'major', key: 'C', tempo: 43, progression: ['I', 'V', 'vi', 'IV'], instruments: ['bass', 'pad', 'arp', 'lead'], melody: 'flowing', feel: [] };
  var SFXSPEC = { type: 'sine', freq: 440, dur: 0.3, gain: 0.5, attack: 0.01, decay: 0.05, sustain: 0.5, release: 0.05 };
  function sfxGainVals(c) { return JSON.stringify(c._log.filter(function (x) { return /^gain\.linRamp:/.test(x); }).map(function (x) { return x.split(':')[1].split('@')[0]; })); }
  var cAlone = makeMockCtx(); createAudioPresenter({ context: cAlone }).present({ view: { audio: { sfx: [SFXSPEC] } } });
  var cAfter = makeMockCtx(); var pAfter = createAudioPresenter({ context: cAfter });
  pAfter.present({ view: { audio: { music: MSPEC } } });                  // 先放合成音乐 → 残留 voicePerformance
  cAfter._log.length = 0;
  pAfter.present({ view: { audio: { sfx: [SFXSPEC] } } });                // 再放同一 sfx
  ok('M-perf SfxSpec 增益包络与是否先放合成音乐无关(voicePerformance 不跨路串扰;修复前残留 profile 缩放 gain 0.5→0.48)', sfxGainVals(cAlone).length > 2 && sfxGainVals(cAlone) === sfxGainVals(cAfter));
...
// M21/M22 makeNoiseBuffer 缓存反向牙:临时源码探针计生成次数,并仅在测试副本里切换同一闭包的 ctx。
(function () {
  var sourcePath = path.join(__dirname, '..', 'present-audio.js');
  var src = fs.readFileSync(sourcePath, 'utf8');
  var fnNeedle = 'function makeNoiseBuffer(seconds, color) {';
  var createNeedle = 'var buf = ctx.createBuffer(1, len, sr);';
  if (src.indexOf(fnNeedle) < 0 || src.indexOf(createNeedle) < 0) {
    ok('M21-M22 makeNoiseBuffer 缓存探针源码锚点与 runtime 脱节,请同步更新测试', false);
  } else {
    var probed = src
      .replace(fnNeedle, fnNeedle + ' if (global.__presentAudioNoiseCtx) ctx = global.__presentAudioNoiseCtx;')
      .replace(createNeedle, createNeedle + ' global.__presentAudioNoiseGenerations.push({ ctx: ctx, seconds: seconds, color: color, buffer: buf });');
    var tmpPath = sourcePath + '.noise-cache-probe.tmp.cjs';
    try {
      fs.writeFileSync(tmpPath, probed);
      var probeModule = require(tmpPath);
      var ctx1 = makeMockCtx(), ctx2 = makeMockCtx();
      global.__presentAudioNoiseCtx = ctx1; global.__presentAudioNoiseGenerations = [];
      var probePresenter = probeModule.createAudioPresenter({ context: ctx1 });
      probePresenter.startBgs('rain'); probePresenter.startBgs('rain');
      var firstCtxWhite = global.__presentAudioNoiseGenerations.filter(function (x) { return x.ctx === ctx1 && x.seconds === 3 && x.color === 'white'; });
      ok('M21 makeNoiseBuffer 同 ctx 同 (3,white) 多层+重启只生成一次', firstCtxWhite.length === 1);
      global.__presentAudioNoiseCtx = ctx2;
      probePresenter.startBgs('rain'); probePresenter.stopBgs();
      var secondCtxWhite = global.__presentAudioNoiseGenerations.filter(function (x) { return x.ctx === ctx2 && x.seconds === 3 && x.color === 'white'; });
      ok('M22 makeNoiseBuffer ctx 变更后缓存失效并重新生成 buffer', secondCtxWhite.length === 1 && firstCtxWhite[0].buffer !== secondCtxWhite[0].buffer);
    } catch (e) {
      ok('M21-M22 makeNoiseBuffer 缓存探针运行失败: ' + (e && (e.stack || e.message) || e), false);
    } finally {
      try { delete require.cache[require.resolve(tmpPath)]; } catch (e2) {}
      try { fs.unlinkSync(tmpPath); } catch (e3) {}
      try { delete global.__presentAudioNoiseCtx; delete global.__presentAudioNoiseGenerations; } catch (e4) {}
    }
  }
})();
