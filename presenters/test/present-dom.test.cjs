...
function makeEl(tag) {
  ...
  var el = {
    tagName: tag, className: '', disabled: false, onclick: null,
...
// P. 固定 native next 与正文面板必须共用同一两击 helper；同一物理 click 不能补全文后又越拍。
(function () {
  ...
  var next = d._c['#choices'].children[0];
  var spans = byClass(look, 'amatlas-typewriter-grapheme');
  ok('P1 原生 next 保持 button 并获得稳定 class/data hook', next.tagName === 'button' && /(?:^|\s)cutscene-next(?:\s|$)/.test(next.className) && attr(next, 'data-cutscene-next') === '');
  panelClick(look);
  ok('P2 正文第一击只补全文，不 apply', visibleCount(spans) === spans.length && appliedPanel.length === 0 && attr(next, 'aria-label') === '继续 / 下一段');
  panelClick(look);
  ok('P3 正文第二击只推进一次', appliedPanel.length === 1 && appliedPanel[0].id === 'cutscene:next');

  var stopped = false;
  next.onclick({ stopPropagation: function () { stopped = true; } });
  ok('P4 native next click 阻止冒泡并复用 helper', stopped && appliedPanel.length === 2);
})();
...
(function () {
  ...
  look = d._c['#look'];
  ok('Q4 末拍只剩作者 links 时不绑定面板推进/便利点击样式', look.onclick === null && look.onpointerdown === null && !/(?:^|\s)cutscene-next-panel(?:\s|$)/.test(look.className));
  presenter.present(plainSnap([nextAction], 'scene'));
  look = d._c['#look'];
  ok('Q5 普通 scene 即使伪造同 id 也不绑定面板推进', look.onclick === null && look.onpointerdown === null);
})();

// R. 重渲染/离开 cutscene 必须替换并清理 handler，不累计双触发。
(function () {
  var d = makeDoc(), appliedCleanup = [];
  var look = d._c['#look'], sentinelClicks = 0, sentinelDowns = 0;
  ...
  ok('R2 cutscene 内既有属性 handler 与 panel listener 共存', look.onclick === sentinelClick && look.onpointerdown === sentinelDown && sentinelClicks === 2 && sentinelDowns === 2);
  presenter.present({ pos: { map: 'm', node: 'plain' }, nodeKind: 'scene', view: { title: 'plain', body: [] }, actions: [] });
  ok('R3 离开 cutscene 后只移除自身 listener，既有 handler 原样保留', look.listenerCount('click') === 0 && look.listenerCount('pointerdown') === 0 && look.onclick === sentinelClick && look.onpointerdown === sentinelDown && !/(?:^|\s)cutscene-next-panel(?:\s|$)/.test(look.className));
  panelClick(look);
  ok('R4 离场后 sentinel 仍执行且 panel 不再推进', sentinelClicks === 3 && sentinelDowns === 3 && appliedCleanup.length === 2);
})();

// S. 同节点换拍会重建 native next：只有旧 next 持焦点时才把焦点转移给新 next。
(function () {
  ...
  var first = d._c['#choices'].children[0];
  first.focus();
  first.onclick({ stopPropagation: function () {} });
  ok('S1 首次 Enter 只补全文且焦点仍在原 next', phase === 0 && d.activeElement === first && attr(first, 'aria-label') === '继续 / 下一段');
  first.onclick({ stopPropagation: function () {} });
  var second = d._c['#choices'].children[0];
  ok('S2 第二次 Enter 换拍后焦点转到重建的新 next', phase === 1 && second !== first && d.activeElement === second && second.focusCount === 1);
  second.onclick({ stopPropagation: function () {} });
  var third = d._c['#choices'].children[0];
  ok('S3 第三次 Enter 可继续换拍且焦点继续跟随 native next', phase === 2 && third !== second && d.activeElement === third);

  d.activeElement = d.body;
  presenter.present(snaps[1]);
  panelClick(d._c['#look']);
  ok('S4 panel click 在 next 未聚焦时不主动抢焦点', d.activeElement === d.body);
})();

// T. 极简旧 stub 没有 add/removeEventListener 时，属性回退也必须共存并原样恢复。
(function () {
  var d = makeDoc(), look = d._c['#look'], appliedFallback = 0, sentinel = 0;
  ...
  panelClick(look);
  ok('T1 无 listener API 时属性回退同时调用 sentinel 与 panel', sentinel === 1 && appliedFallback === 1 && look.onclick !== oldClick);
  presenter.present({ pos: { map: 'm', node: 'plain' }, nodeKind: 'scene', view: { title: 'plain', body: [] }, actions: [] });
  ok('T2 属性回退离场后逐字恢复既有 handler', look.onclick === oldClick);
  panelClick(look);
  ok('T3 恢复后的 sentinel 仍执行且 panel 不残留', sentinel === 2 && appliedFallback === 1);
})();

// U. fixed cutscene next 已脱离文档流：正文只保留横向避让，不再预留块尾空挡。
(function () {
  var d = makeDoc();
  d.head = makeEl('head');
  ...
  var choicesRule = (css.match(/#choices\{([^}]*)\}/) || [])[1] || '';
  var nextRule = (css.match(/#choices \.cutscene-next\{([^}]*)\}/) || [])[1] || '';

  ok('U1 cutscene 正文不再保留 3.5rem padding-block-end', !!lookRule && !/padding-block-end\s*:/.test(lookRule) && !/3\.5rem/.test(lookRule));
  ok('U2 cutscene 正文仍以 4rem + right safe-area 横向避让 fixed next', /padding-inline-end\s*:\s*calc\(4rem \+ env\(safe-area-inset-right\)\)/.test(lookRule));
  ok('U3 native next 仍 fixed 且保持 48px 宽、44px 最小触控尺寸', /position\s*:\s*fixed/.test(nextRule) && /width\s*:\s*48px/.test(nextRule) && /min-width\s*:\s*44px/.test(nextRule) && /min-height\s*:\s*44px/.test(nextRule));
  ok('U4 native next 的 right/bottom safe-area 避让仍在', /right\s*:[^;]*safe-area-inset-right/.test(nextRule) && /bottom\s*:[^;]*safe-area-inset-bottom/.test(nextRule));
  ok('U5 cutscene choices 建立层叠上下文，使 fixed next 不被后渲染的 status 截获点击', /z-index\s*:\s*2/.test(choicesRule));
})();
