...
const Timeline = require('../present-timeline.js');
...
// TT. Phase 2 cutscene motion:stable SVG layers + shared timeline attribute patching.
(function () {
  function rafHarness() {
    var nextId = 1, queue = {}, requests = 0;
    return {
      request: function (callback) { var id = nextId++; queue[id] = callback; requests++; return id; },
      cancel: function (id) { delete queue[id]; },
  ...
  function motionDom() {
    var base = '', baseWrites = 0, artWrites = 0, attrWrites = {}, root = null;
    var doc = {};
    function Node(tag) {
      this.tagName = tag;
      ...
      Object.defineProperty(this, 'innerHTML', {
        ...
        set: function (value) { html = value; artWrites++; }
      });
    }
    Node.prototype.setAttribute = function (name, value) {
      this.attrs[name] = String(value);
      attrWrites[name] = (attrWrites[name] || 0) + 1;
    };
    ...
    doc.createElementNS = function (ns, tag) { return new Node(tag); };
    function find(node, attr, value) {
      if (!node) return null;
      if (node.attrs[attr] === value) return node;
      for (var i = 0; i < node.children.length; i++) {
        var found = find(node.children[i], attr, value);
        if (found) return found;
      }
      return null;
    }
    var container = {
      querySelector: function (selector) { return selector === 'svg' ? root : null; },
      _base: function () { return base; },
      _writes: function () { return baseWrites; },
      _artWrites: function () { return artWrites; },
      _attrWrites: function (name) { return attrWrites[name] || 0; },
      _attrSnapshot: function () { var out = {}; for (var k in attrWrites) out[k] = attrWrites[k]; return out; },
      _layer: function (id) { return find(root, 'data-amatlas-motion-layer', id); },
      _rigStage: function (castId) { return find(root, 'data-amatlas-rig-cast', castId || 'actor'); },
      _rigPart: function (castId, id) {
        if (id === undefined) { id = castId; castId = 'actor'; }
        return find(find(root, 'data-amatlas-rig-cast', castId), 'data-amatlas-rig-part', id);
      },
      _rigState: function (castId, partId, stateId) {
        if (stateId === undefined) { stateId = partId; partId = castId; castId = 'actor'; }
        var part = find(find(root, 'data-amatlas-rig-cast', castId), 'data-amatlas-rig-part', partId);
        return find(part, 'data-amatlas-rig-state', stateId);
      },
      _rigOrder: function (castId) {
        var stage = find(root, 'data-amatlas-rig-cast', castId || 'actor');
        return stage ? stage.children.map(function (node) { return node.attrs['data-amatlas-rig-part']; }) : [];
      },
      _castOrder: function () {
        return root ? root.children.filter(function (node) { return node.attrs['data-amatlas-rig-cast']; })
          .map(function (node) { return node.attrs['data-amatlas-rig-cast']; }) : [];
      },
      _rootStages: function () {
        return root ? root.children.map(function (node) {
          return node.attrs['data-amatlas-motion-key'] ? 'motion'
            : node.attrs['data-amatlas-rig-cast'] ? 'cast:' + node.attrs['data-amatlas-rig-cast'] : 'other';
        }) : [];
      },
  ...
  function playback(key) {
    return {
      ...
      key: key,
      durationMs: 200,
      motion: Timeline.normalizeMotionPlan({
        layers: [
          { id: 'ship', art: 'ship', x: 10, y: 20, scale: 0.5, rotate: -5, opacity: 0.5 },
          { id: 'lamp', art: 'lantern', x: 4, y: 8, scale: 0.4, opacity: 0 }
        ],
        tracks: [
          { target: 'ship', property: 'x', keys: [ { at: 0.2, value: 30 } ] },
          { target: 'ship', property: 'scale', keys: [ { at: 0.2, value: 1 } ] },
          { target: 'ship', property: 'rotate', keys: [ { at: 0.2, value: 5 } ] },
          { target: 'ship', property: 'opacity', keys: [ { at: 0.2, value: 1 } ] },
          { target: 'lamp', property: 'x', keys: [ { at: 0.2, value: 24 } ] },
          { target: 'lamp', property: 'opacity', keys: [ { at: 0.2, value: 1 } ] }
        ]
      }, { durationMs: 200 })
    };
  }
  function axisPlayback(key) {
    return {
      version: 1,
      key: key,
      durationMs: 200,
      motion: Timeline.normalizeMotionPlan({
        layers: [
          { id: 'drop', art: 'rock', x: 160, y: 20, scale: 0.5, opacity: 1 },
          { id: 'legacy', art: 'ship', x: 40, y: 80, scale: 0.4, opacity: 1 },
          { id: 'single', art: 'lantern', x: 260, y: 80, scale: 0.4, opacity: 1 }
        ],
        tracks: [
          { target: 'drop', property: 'y', keys: [{ at: 0.2, value: 100 }] },
          { target: 'drop', property: 'scaleX', keys: [
            { at: 0.1, value: 0.8 }, { at: 0.2, value: 0.6 }
          ] },
          { target: 'drop', property: 'scaleY', keys: [
            { at: 0.1, value: 0.3 }, { at: 0.2, value: 0.4 }
          ] },
          { target: 'legacy', property: 'scale', keys: [{ at: 0.2, value: 0.8 }] },
          { target: 'single', property: 'scaleX', keys: [{ at: 0.2, value: 0.7 }] }
        ]
      }, { durationMs: 200 })
    };
  }
  function customSnap(playbackPlan, scene, posNode) {
    return { view: { scene: scene, body: [{ type: 'prose', text: '完整正文' }], cutscenePlayback: playbackPlan },
      pos: { map: 'm', node: posNode || 'cut' } };
  }
  function snap(key, scene, posNode) {
    return { view: { scene: scene, body: [{ type: 'prose', text: '完整正文' }], cutscenePlayback: playback(key) }, pos: { map: 'm', node: posNode || 'cut' } };
  }
  function rigPlan(rootX, drawOrder, mouthKeys, rootPivotX, rootEndX) {
    return Timeline.normalizeRigPlan({
      // child 在 parent 前声明，drawOrder 又与 topo 相反。
      parts: [
        { id: 'mouth', parent: 'body', art: [{ shape: 'line', x1: -2, y1: 0, x2: 2, y2: 0, stroke: '#fff' }],
          pivot: { x: 0, y: 0 }, rest: { x: 10, y: 0, rotate: 0, scaleX: 1, scaleY: 1, opacity: 0.5 } },
        { id: 'body', parent: null, art: 'figure', pivot: { x: rootPivotX == null ? 0 : rootPivotX, y: 0 },
          rest: { x: rootX == null ? 100 : rootX, y: 50, rotate: 0, scaleX: 1, scaleY: 1, opacity: 0.8 } }
      ],
      drawOrder: drawOrder || ['mouth', 'body'],
      tracks: [
        { target: 'body', property: 'rotate', keys: [{ at: 0.2, value: 90 }] },
        { target: 'mouth', property: 'rotate', keys: [{ at: 0.2, value: 0 }] }
      ].concat(rootEndX == null ? [] : [
        { target: 'body', property: 'x', keys: [{ at: 0.2, value: rootEndX }] }
      ]),
      variants: [{ target: 'mouth', base: 'rest', states: [
        { id: 'A', art: [{ shape: 'ellipse', cx: 0, cy: 0, rx: 2, ry: 1.5, fill: '#fff' }] },
        { id: 'O', art: [{ shape: 'ellipse', cx: 0, cy: 0, rx: 1, ry: 2, fill: 'none', stroke: '#fff' }] }
      ], keys: mouthKeys === undefined ? [
        { at: 0.05, value: 'A' },
        { at: 0.1, value: 'O' },
        { at: 0.15, value: 'rest' }
      ] : mouthKeys }], secondary: []
  ...
  function rigPlayback(key, rootX) {
    var out = playback(key);
    out.cast = [{ id: 'actor', rig: rigPlan(rootX) }];
    return out;
  }
  function svgMatrix(node) {
    var match = /^matrix\(([^)]+)\)$/.exec(node && node.getAttribute('transform'));
    return match ? match[1].trim().split(/[ ,]+/).map(Number) : null;
  }
  ...
  function rigSnap(playbackPlan, scene, posNode) {
    return { view: { scene: scene, body: [{ type: 'prose', text: '完整正文' }], cutscenePlayback: playbackPlan },
      pos: { map: 'm', node: posNode || 'cut' } };
  }
  ...
  var dom = motionDom();
  var presenter = createSvgPresenter({ container: dom, timelineManager: manager });
  var scene = { region: 'night', mood: 'tense' };
  var first = snap('m/cut#0', scene);
  presenter.present(first);
  var ship = dom._layer('ship'), initialId = ship.getAttribute('id');
  ok('TT1 motion 创建稳定 ship/lamp <g>，初始 transform 顺序固定 translate→rotate→scale',
    !!ship && !!dom._layer('lamp') && ship.getAttribute('transform') === 'translate(10.000 20.000) rotate(-5.000) scale(0.500)');
  ok('TT2 animation <g> 不进入被比较的 base SVG 字节',
    dom._base() === buildSceneSVG(scene) && dom._base().indexOf('data-amatlas-motion') < 0);
  var artWrites = dom._artWrites();
  presenter.present(first);
  ok('TT3 同 playback key 重渲染不重建 base/layer art，也不重启共享 rAF',
    dom._writes() === 1 && dom._artWrites() === artWrites && dom._layer('ship').getAttribute('id') === initialId && raf.requests() === 1);
  raf.fireNext(0);
  raf.fireNext(100);
  ok('TT4 帧推进只 patch transform/opacity，不写 container.innerHTML 或 layer.innerHTML',
    dom._layer('ship').getAttribute('transform') === 'translate(20.000 20.000) rotate(0.000) scale(0.750)'
      && dom._layer('ship').getAttribute('opacity') === '0.750' && dom._writes() === 1 && dom._artWrites() === artWrites);

  var oldShip = dom._layer('ship');
  var second = snap('m/cut#1', scene);
  presenter.present(second);
  ok('TT5 换 key 先把旧 layer patch 回 t=0，再以稳定新 id 建新 layer 的 t=0',
    oldShip.getAttribute('transform') === 'translate(10.000 20.000) rotate(-5.000) scale(0.500)'
      && dom._layer('ship').getAttribute('transform') === 'translate(10.000 20.000) rotate(-5.000) scale(0.500)'
      && dom._layer('ship').getAttribute('id') !== initialId);
  ok('TT6 beat/key 切换且 base scene 字节相同时不重建 base，ambient SMIL 相位不重启',
    dom._writes() === 1 && dom._base().indexOf('<animate') >= 0 && raf.pending() === 1);

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var reducedRaf = rafHarness();
  var reducedManager = Timeline.createPlaybackManager({ requestAnimationFrame: reducedRaf.request, cancelAnimationFrame: reducedRaf.cancel });
  var reducedDom = motionDom();
  createSvgPresenter({ container: reducedDom, timelineManager: reducedManager }).present(snap('m/reduced#0', scene, 'reduced'));
  delete global.window;
  ok('TT7 reduced-motion 不启 rAF，motion 落 durationMs 终态且 ambient SMIL 仍由 stripSmil 剥除',
    reducedRaf.requests() === 0
      && reducedDom._layer('ship').getAttribute('transform') === 'translate(30.000 20.000) rotate(5.000) scale(1.000)'
      && reducedDom._layer('ship').getAttribute('opacity') === '1.000' && reducedDom._base().indexOf('<animate') < 0);

  var leaveRaf = rafHarness();
  var leaveManager = Timeline.createPlaybackManager({ requestAnimationFrame: leaveRaf.request, cancelAnimationFrame: leaveRaf.cancel });
  var leaveDom = motionDom();
  var leavePresenter = createSvgPresenter({ container: leaveDom, timelineManager: leaveManager });
  leavePresenter.present(snap('m/leave#0', scene, 'cut'));
  var leavingLayer = leaveDom._layer('ship'), staleLeave = leaveRaf.takeNext();
  leavePresenter.present({ view: { scene: scene }, pos: { map: 'm', node: 'shore' } });
  var leavePatches = leaveDom._attrWrites('transform');
  staleLeave(150);
  ok('TT8 离场移除 overlay、取消 session，已出队旧 rAF 因 generation 不再 patch',
    !leaveDom._layer('ship') && leaveManager.inspect().key === null && leaveDom._attrWrites('transform') === leavePatches
      && leavingLayer.getAttribute('transform') === 'translate(10.000 20.000) rotate(-5.000) scale(0.500)');

  var restoreRaf = rafHarness();
  var restoreManager = Timeline.createPlaybackManager({ requestAnimationFrame: restoreRaf.request, cancelAnimationFrame: restoreRaf.cancel });
  var restoreDom = motionDom(), lifecycleModule = null, installedPresent = null;
  var restorePresenter = createSvgPresenter({ container: restoreDom, timelineManager: restoreManager });
  restorePresenter.install({
    registerModule: function (mod) { lifecycleModule = mod; },
    addPresenter: function (fn) { installedPresent = fn; }
  });
  installedPresent(snap('m/restore#0', scene, 'cut'));
  var staleRestore = restoreRaf.takeNext();
  lifecycleModule.systems.filter(function (system) { return system.on === 'restore'; })[0].run({}, { phase: 'deactivate' });
  var restorePatches = restoreDom._attrWrites('transform');
  staleRestore(175);
  ok('TT9 restore(deactivate) 即使 pos/key 相同也代次取消，晚到旧 rAF 不再 patch',
    lifecycleModule.systems.some(function (system) { return system.on === 'enter'; }) && restoreManager.inspect().key === null && restoreDom._attrWrites('transform') === restorePatches
      && restoreDom._layer('ship').getAttribute('transform') === 'translate(10.000 20.000) rotate(-5.000) scale(0.500)');

  var missingDuration = playback('m/missing-duration#0');
  missingDuration.durationMs = null;
  ok('TT10 SVG presenter rejects a non-finite playback envelope instead of deriving motion duration locally', throws(function () {
    createSvgPresenter({ container: motionDom(), timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap(missingDuration, scene, 'missing-duration'));
  }));

  var axisRaf = rafHarness();
  var axisManager = Timeline.createPlaybackManager({ requestAnimationFrame: axisRaf.request, cancelAnimationFrame: axisRaf.cancel });
  var axisDom = motionDom();
  var axisPresenter = createSvgPresenter({ container: axisDom, timelineManager: axisManager });
  var firstAxisSnap = customSnap(axisPlayback('m/axis#0'), scene, 'axis');
  axisPresenter.present(firstAxisSnap);
  var firstDrop = axisDom._layer('drop'), firstDropId = firstDrop.getAttribute('id');
  ok('P4a-S1 axis layer starts with two-parameter scale while legacy uniform layer keeps exact scale(s)',
    firstDrop.getAttribute('transform') === 'translate(160.000 20.000) rotate(0.000) scale(0.500 0.500)'
      && axisDom._layer('legacy').getAttribute('transform') === 'translate(40.000 80.000) rotate(0.000) scale(0.400)'
      && axisDom._layer('single').getAttribute('transform') === 'translate(260.000 80.000) rotate(0.000) scale(0.400 0.400)');
  var axisArtWrites = axisDom._artWrites(), axisBaseWrites = axisDom._writes();
  axisPresenter.present(firstAxisSnap);
  ok('P4a-S2 same axis playback key reuses layer art and the single shared rAF',
    axisDom._layer('drop') === firstDrop && axisDom._layer('drop').getAttribute('id') === firstDropId
      && axisDom._artWrites() === axisArtWrites && axisDom._writes() === axisBaseWrites && axisRaf.requests() === 1);
  axisRaf.fireNext(0);
  axisRaf.fireNext(100);
  ok('P4a-S3 frame patch keeps X/Y distinct and a missing Y track at the layer.scale base',
    axisDom._layer('drop').getAttribute('transform') === 'translate(160.000 60.000) rotate(0.000) scale(0.800 0.300)'
      && axisDom._layer('legacy').getAttribute('transform') === 'translate(40.000 80.000) rotate(0.000) scale(0.600)'
      && axisDom._layer('single').getAttribute('transform') === 'translate(260.000 80.000) rotate(0.000) scale(0.550 0.400)'
      && axisDom._artWrites() === axisArtWrites && axisDom._writes() === axisBaseWrites);
  var staleAxis = axisRaf.takeNext();
  var oldAxisDrop = axisDom._layer('drop');
  axisPresenter.present(customSnap(axisPlayback('m/axis#1'), scene, 'axis'));
  var axisPatches = axisDom._attrWrites('transform');
  staleAxis(175);
  ok('P4a-S4 key switch resets and detaches the old axis layer; stale generation cannot patch the replacement',
    oldAxisDrop.getAttribute('transform') === 'translate(160.000 20.000) rotate(0.000) scale(0.500 0.500)'
      && axisDom._layer('drop') !== oldAxisDrop
      && axisDom._layer('drop').getAttribute('transform') === 'translate(160.000 20.000) rotate(0.000) scale(0.500 0.500)'
      && axisDom._attrWrites('transform') === axisPatches && axisRaf.pending() === 1);

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var axisReducedRaf = rafHarness();
  var axisReducedDom = motionDom();
  createSvgPresenter({ container: axisReducedDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: axisReducedRaf.request, cancelAnimationFrame: axisReducedRaf.cancel
  }) }).present(customSnap(axisPlayback('m/axis-reduced#0'), scene, 'axis-reduced'));
  delete global.window;
  ok('P4a-S5 reduced-motion applies the axis terminal sample immediately and requests no rAF',
    axisReducedRaf.requests() === 0
      && axisReducedDom._layer('drop').getAttribute('transform') === 'translate(160.000 100.000) rotate(0.000) scale(0.600 0.400)'
      && axisReducedDom._layer('legacy').getAttribute('transform') === 'translate(40.000 80.000) rotate(0.000) scale(0.800)'
      && axisReducedDom._layer('single').getAttribute('transform') === 'translate(260.000 80.000) rotate(0.000) scale(0.700 0.400)');

  var axisCastDom = motionDom();
  var axisCastPlayback = axisPlayback('m/axis-cast#0');
  axisCastPlayback.cast = [{ id: 'actor', rig: rigPlan(100) }];
  createSvgPresenter({ container: axisCastDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
    .present(customSnap(axisCastPlayback, scene, 'axis-cast'));
  ok('P4a-S6 axis flat motion stage remains before cast and both share the existing playback manager',
    axisCastDom._rootStages().join(',') === 'motion,cast:actor'
      && axisCastDom._layer('drop').getAttribute('transform') === 'translate(160.000 100.000) rotate(0.000) scale(0.600 0.400)');

  var rigRaf = rafHarness();
  var rigManager = Timeline.createPlaybackManager({ requestAnimationFrame: rigRaf.request, cancelAnimationFrame: rigRaf.cancel });
  var rigDom = motionDom();
  var rigPresenter = createSvgPresenter({ container: rigDom, timelineManager: rigManager });
  var bothPlayback = rigPlayback('m/rig#0');
  var bothSnap = rigSnap(bothPlayback, scene, 'rig');
  rigPresenter.present(bothSnap);
  var firstMouth = rigDom._rigPart('mouth');
  var restState = rigDom._rigState('mouth', 'rest');
  var aState = rigDom._rigState('mouth', 'A');
  var oState = rigDom._rigState('mouth', 'O');
  ok('UU1 flat motion stage 在 cast stage 前；part 按 drawOrder 为稳定 sibling，parent 关系不进入 DOM nesting',
    rigDom._rootStages().join(',') === 'motion,cast:actor' && rigDom._rigOrder().join(',') === 'mouth,body'
      && firstMouth.parentNode === rigDom._rigPart('body').parentNode
      && firstMouth.children.length === 3 && rigDom._rigPart('body').children.length === 0
      && restState.parentNode === firstMouth && aState.parentNode === firstMouth && oState.parentNode === firstMouth);
  ok('UU2 rig 初始 world matrix/opacity 由 FK sampler patch，drawOrder 不影响父子合成',
    firstMouth.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 110.000 50.000)'
      && firstMouth.getAttribute('opacity') === '0.400');
  ok('UU2b omitted stage facing is identity and preserves the legacy outer stage attributes',
    rigDom._rigStage('actor').getAttribute('transform') === undefined);
  ok('UU3 base/rest 与 A/O state 在创建期各建一次稳定内层组，初态只显示 base',
    restState.getAttribute('display') === 'inline' && aState.getAttribute('display') === 'none'
      && oState.getAttribute('display') === 'none');
  var rigArtWrites = rigDom._artWrites(), rigBaseWrites = rigDom._writes();
  var firstMouthId = firstMouth.getAttribute('id'), initialDisplayWrites = rigDom._attrWrites('display');
  rigPresenter.present(bothSnap);
  ok('UU4 同 key/plan/root 复用 part/state DOM 与 art，并且 motion+rig 仍只请求一张 rAF',
    rigDom._rigPart('mouth') === firstMouth && rigDom._rigPart('mouth').getAttribute('id') === firstMouthId
      && rigDom._rigState('mouth', 'rest') === restState && rigDom._rigState('mouth', 'A') === aState
      && rigDom._rigState('mouth', 'O') === oState && rigDom._artWrites() === rigArtWrites
      && rigDom._writes() === rigBaseWrites && rigRaf.requests() === 1);
  var attrsBeforeFrame = rigDom._attrSnapshot();
  rigRaf.fireNext(0);
  rigRaf.fireNext(49);
  ok('UU5 non-boundary frames do not write display', rigDom._attrWrites('display') === initialDisplayWrites);
  rigRaf.fireNext(50);
  ok('UU6 state boundary performs exactly two display writes and shows A',
    rigDom._attrWrites('display') === initialDisplayWrites + 2 && restState.getAttribute('display') === 'none'
      && aState.getAttribute('display') === 'inline' && oState.getAttribute('display') === 'none');
  rigRaf.fireNext(51);
  ok('UU7 frame after boundary does not repeat display writes', rigDom._attrWrites('display') === initialDisplayWrites + 2);
  rigRaf.fireNext(100);
  var attrsAfterFrame = rigDom._attrSnapshot();
  var unchangedAttrs = Object.keys(attrsAfterFrame).filter(function (name) {
    return name !== 'transform' && name !== 'opacity' && name !== 'display';
  })
    .every(function (name) { return attrsAfterFrame[name] === attrsBeforeFrame[name]; });
  ok('UU8 帧 patch 只写 transform/opacity 与边界 display，不重建 base/outer part/state art',
    rigDom._rigPart('mouth').getAttribute('transform') === 'matrix(0.707 0.707 -0.707 0.707 107.071 57.071)'
      && rigDom._attrWrites('display') === initialDisplayWrites + 4
      && oState.getAttribute('display') === 'inline' && unchangedAttrs
      && rigDom._artWrites() === rigArtWrites && rigDom._writes() === rigBaseWrites);
  var displayBeforeRerender = rigDom._attrWrites('display');
  rigPresenter.present(bothSnap);
  ok('UU9 同 key rerender 保持当前 O 口型，不重置到 rest、不替换 state DOM',
    rigDom._rigState('mouth', 'O') === oState && oState.getAttribute('display') === 'inline'
      && restState.getAttribute('display') === 'none' && rigDom._attrWrites('display') === displayBeforeRerender
      && rigDom._artWrites() === rigArtWrites);

  var changedPlanSnap = rigSnap({
    version: 1, key: bothPlayback.key, durationMs: 200,
    motion: bothPlayback.motion, text: null, cast: [{ id: 'actor', rig: rigPlan(120) }]
  }, scene, 'rig');
  rigPresenter.present(changedPlanSnap);
  ok('UU10 同 key 但 cast plan identity 改变会重建 stage；顺序仍是 motion→cast',
    rigDom._rigPart('mouth') !== firstMouth && rigDom._rigPart('mouth').getAttribute('transform').indexOf('127.071 57.071') >= 0
      && rigDom._rootStages().join(',') === 'motion,cast:actor');

  var oldChangedMouth = rigDom._rigPart('mouth');
  var changedKeyPlayback = rigPlayback('m/rig#1');
  rigPresenter.present(rigSnap(changedKeyPlayback, scene, 'rig'));
  ok('UU11 换 key 先把旧 cast patch 回 t=0，再建新 sibling nodes；base SVG 字节不重建',
    oldChangedMouth.getAttribute('transform').indexOf('130.000 50.000') >= 0
      && rigDom._rigPart('mouth') !== oldChangedMouth && rigDom._writes() === rigBaseWrites && rigRaf.pending() === 1);

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var rigReducedRaf = rafHarness();
  var rigReducedDom = motionDom();
  createSvgPresenter({ container: rigReducedDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: rigReducedRaf.request, cancelAnimationFrame: rigReducedRaf.cancel
  }) }).present(rigSnap(rigPlayback('m/rig-reduced#0'), scene, 'rig-reduced'));
  delete global.window;
  ok('UU12 reduced-motion 不起第二张/任何 rAF，每个 cast rig 精确落 durationMs poster 的 rest 口型',
    rigReducedRaf.requests() === 0
      && rigReducedDom._rigPart('mouth').getAttribute('transform') === 'matrix(0.000 1.000 -1.000 0.000 100.000 60.000)'
      && rigReducedDom._rigState('mouth', 'rest').getAttribute('display') === 'inline'
      && rigReducedDom._rigState('mouth', 'O').getAttribute('display') === 'none'
      && rigReducedDom._base().indexOf('<animate') < 0);

  var facingRaf = rafHarness();
  var facingManager = Timeline.createPlaybackManager({ requestAnimationFrame: facingRaf.request, cancelAnimationFrame: facingRaf.cancel });
  var facingDom = motionDom();
  var facingPresenter = createSvgPresenter({ container: facingDom, timelineManager: facingManager });
  var facingPlan = rigPlan(100, null, [], 10, 140);
  var authoredDom = motionDom();
  var authoredPlayback = playback('m/as-authored#0');
  authoredPlayback.cast = [{ id: 'actor', stage: { facing: 'as-authored' }, rig: facingPlan }];
  createSvgPresenter({ container: authoredDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
    .present(rigSnap(authoredPlayback, scene, 'as-authored'));
  ok('C3a-0 explicit as-authored facing is also an outer-stage identity',
    authoredDom._rigStage('actor').getAttribute('transform') === undefined);
  var facingPlayback = playback('m/facing#0');
  facingPlayback.cast = [{ id: 'actor', stage: { facing: 'mirror-x' }, rig: facingPlan }];
  var facingSnap = rigSnap(facingPlayback, scene, 'facing');
  facingPresenter.present(facingSnap);
  var facingStage = facingDom._rigStage('actor');
  var facingBody = facingDom._rigPart('actor', 'body');
  var facingArtWrites = facingDom._artWrites();
  var initialFacingTransform = facingStage.getAttribute('transform');
  facingPresenter.present(facingSnap);
  ok('C3a-1 mirror-x reuses the outer stage and all part DOM while sharing one pending rAF',
    facingDom._rigStage('actor') === facingStage && facingDom._rigPart('actor', 'body') === facingBody
      && facingDom._artWrites() === facingArtWrites && facingRaf.requests() === 1 && facingRaf.pending() === 1);
  facingRaf.fireNext(0);
  facingRaf.fireNext(100);
  var sampledFacing = Timeline.sampleRig(facingPlan, 100);
  var sampledRoot = sampledFacing.parts.filter(function (part) { return part.id === 'body'; })[0];
  var F = svgMatrix(facingStage), M = svgMatrix(facingBody);
  var FM = multiplyMatrices(F, M), MF = multiplyMatrices(M, F);
  var pivotWorldX = sampledRoot.matrix[0] * 10 + sampledRoot.matrix[4];
  var pivotWorldY = sampledRoot.matrix[1] * 10 + sampledRoot.matrix[5];
  var mirroredPivot = matrixPoint(FM, 10, 0);
  ok('C3a-2 outer wrapper is patched from each sample current root pivot without rebuilding a part',
    initialFacingTransform === 'matrix(-1.000 0.000 0.000 1.000 220.000 0.000)'
      && facingStage.getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 260.000 0.000)'
      && facingDom._rigStage('actor') === facingStage && facingDom._rigPart('actor', 'body') === facingBody
      && facingDom._artWrites() === facingArtWrites && facingRaf.pending() === 1);
  ok('C3a-3 mirror is F*M rather than M*F and leaves a non-zero moving root pivot in place',
    F && M && near(F[4], 2 * pivotWorldX)
      && near(FM[4], 2 * pivotWorldX - M[4]) && !near(FM[4], MF[4])
      && near(mirroredPivot.x, pivotWorldX) && near(mirroredPivot.y, pivotWorldY));
  ok('C3a-4 mirror never mutates sampled FK part matrices',
    M.every(function (value, index) { return near(value, sampledRoot.matrix[index]); }));

  var enterOnlyDom = motionDom();
  var enterOnlyError = null;
  var enterOnlyPlayback = playback('m/enter-only#0');
  enterOnlyPlayback.cast = [{ id: 'actor', stage: { enter: {
    offset: { x: 64, y: -12 }, durationMs: 100, ease: 'linear'
  } }, rig: facingPlan }];
  try {
    createSvgPresenter({ container: enterOnlyDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap(enterOnlyPlayback, scene, 'enter-only'));
  } catch (e) { enterOnlyError = e; }
  ok('C3b-S0 defensive private-envelope validation requires own stage.facing',
    enterOnlyError && String(enterOnlyError.message).indexOf('cutscenePlayback.cast[0].stage.facing') >= 0
      && enterOnlyDom._castOrder().length === 0);

  var enterRaf = rafHarness();
  var enterManager = Timeline.createPlaybackManager({ requestAnimationFrame: enterRaf.request, cancelAnimationFrame: enterRaf.cancel });
  var enterDom = motionDom();
  var enterPresenter = createSvgPresenter({ container: enterDom, timelineManager: enterManager });
  var enterPlayback = playback('m/enter#0');
  enterPlayback.cast = [{ id: 'actor', stage: { facing: 'as-authored', enter: {
    offset: { x: 64, y: -12 }, durationMs: 100, ease: 'linear'
  } }, rig: facingPlan }];
  var enterSnap = rigSnap(enterPlayback, scene, 'enter');
  enterPresenter.present(enterSnap);
  var enterStage = enterDom._rigStage('actor');
  var enterBody = enterDom._rigPart('actor', 'body');
  var enterArtWrites = enterDom._artWrites();
  ok('C3b-S1 t=0 non-mirrored stage is T only, with enter opacity on the outer stage',
    enterStage.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 64.000 -12.000)'
      && enterStage.getAttribute('opacity') === '0.000' && enterBody.getAttribute('opacity') === '0.800');
  enterPresenter.present(enterSnap);
  ok('C3b-S2 same key reuses stage/part art and enter shares the single playback rAF',
    enterDom._rigStage('actor') === enterStage && enterDom._rigPart('actor', 'body') === enterBody
      && enterDom._artWrites() === enterArtWrites && enterRaf.requests() === 1 && enterRaf.pending() === 1);
  enterRaf.fireNext(0);
  enterRaf.fireNext(50);
  ok('C3b-S3 midpoint patches T and outer opacity without changing FK part opacity or rebuilding art',
    enterStage.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 32.000 -6.000)'
      && enterStage.getAttribute('opacity') === '0.500' && enterBody.getAttribute('opacity') === '0.800'
      && enterDom._artWrites() === enterArtWrites && enterRaf.pending() === 1);
  enterRaf.fireNext(100);
  ok('C3b-S4 exact enter end is identity T plus opacity 1 while the beat keeps the same single rAF',
    enterStage.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 0.000 0.000)'
      && enterStage.getAttribute('opacity') === '1.000' && enterRaf.pending() === 1);

  var enterMirrorRaf = rafHarness();
  var enterMirrorDom = motionDom();
  var enterMirrorPlayback = playback('m/enter-mirror#0');
  enterMirrorPlayback.cast = [{ id: 'actor', stage: { facing: 'mirror-x', enter: {
    offset: { x: 40, y: 20 }, durationMs: 200, ease: 'linear'
  } }, rig: facingPlan }];
  createSvgPresenter({ container: enterMirrorDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: enterMirrorRaf.request, cancelAnimationFrame: enterMirrorRaf.cancel
  }) }).present(rigSnap(enterMirrorPlayback, scene, 'enter-mirror'));
  enterMirrorRaf.fireNext(0);
  enterMirrorRaf.fireNext(100);
  var enterMirrorStage = enterMirrorDom._rigStage('actor');
  var enterMirrorBody = enterMirrorDom._rigPart('actor', 'body');
  var enterMirrorMatrix = svgMatrix(enterMirrorStage);
  var enterMirrorSample = Timeline.sampleRig(facingPlan, 100);
  var enterMirrorRoot = enterMirrorSample.parts.filter(function (part) { return part.id === 'body'; })[0];
  var enterMirrorPivotX = enterMirrorRoot.matrix[0] * 10 + enterMirrorRoot.matrix[4];
  var enterT = [1, 0, 0, 1, 20, 10];
  var enterF = [-1, 0, 0, 1, 2 * enterMirrorPivotX, 0];
  var expectedTF = multiplyMatrices(enterT, enterF);
  var wrongFT = multiplyMatrices(enterF, enterT);
  ok('C3b-S5 mirrored enter composes strict T(stageOffset)*F(rootPivotX), not F*T',
    enterMirrorMatrix.every(function (value, index) { return near(value, expectedTF[index]); })
      && !near(enterMirrorMatrix[4], wrongFT[4]) && enterMirrorStage.getAttribute('opacity') === '0.500');
  ok('C3b-S6 T*F remains outside the unchanged FK matrix and part opacity',
    svgMatrix(enterMirrorBody).every(function (value, index) { return near(value, enterMirrorRoot.matrix[index]); })
      && near(Number(enterMirrorBody.getAttribute('opacity')), enterMirrorRoot.opacity));

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var enterReducedRaf = rafHarness();
  var enterReducedDom = motionDom();
  var enterReducedPlayback = playback('m/enter-reduced#0');
  enterReducedPlayback.cast = [{ id: 'actor', stage: { facing: 'mirror-x', enter: {
    offset: { x: 64, y: 12 }, durationMs: 100, ease: 'ease-out'
  } }, rig: facingPlan }];
  createSvgPresenter({ container: enterReducedDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: enterReducedRaf.request, cancelAnimationFrame: enterReducedRaf.cancel
  }) }).present(rigSnap(enterReducedPlayback, scene, 'enter-reduced'));
  delete global.window;
  ok('C3b-S7 reduced-motion directly applies completed enter state without starting an rAF',
    enterReducedRaf.requests() === 0 && enterReducedDom._rigStage('actor').getAttribute('opacity') === '1.000'
      && enterReducedDom._rigStage('actor').getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 300.000 0.000)');

  var malformedExitDom = motionDom();
  var malformedExitPlayback = playback('m/bad-exit#0');
  malformedExitPlayback.cast = [{ id: 'actor', stage: { facing: 'as-authored', exit: {
    offset: { x: 64, y: 0 }, durationMs: 100, startMs: 100, ease: 'linear', at: 100
  } }, rig: facingPlan }];
  ok('C3c-S0 SVG defensively rejects a non-normalized/extended exit before creating cast DOM', throws(function () {
    createSvgPresenter({ container: malformedExitDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap(malformedExitPlayback, scene, 'bad-exit'));
  }) && malformedExitDom._castOrder().length === 0);
  var wrongTailDom = motionDom();
  var wrongTailPlayback = playback('m/wrong-tail#0');
  wrongTailPlayback.cast = [{ id: 'actor', stage: { facing: 'as-authored', exit: {
    offset: { x: 64, y: 0 }, durationMs: 100, startMs: 90, ease: 'linear'
  } }, rig: facingPlan }];
  ok('C3c-S0b SVG defensively requires the exit endpoint to equal the rig/beat duration', throws(function () {
    createSvgPresenter({ container: wrongTailDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap(wrongTailPlayback, scene, 'wrong-tail'));
  }) && wrongTailDom._castOrder().length === 0);

  var exitRaf = rafHarness();
  var exitManager = Timeline.createPlaybackManager({ requestAnimationFrame: exitRaf.request, cancelAnimationFrame: exitRaf.cancel });
  var exitDom = motionDom();
  var exitPresenter = createSvgPresenter({ container: exitDom, timelineManager: exitManager });
  var exitPlayback = playback('m/exit#0');
  exitPlayback.cast = [{ id: 'actor', stage: { facing: 'mirror-x',
    enter: { offset: { x: -40, y: -20 }, durationMs: 100, ease: 'linear' },
    exit: { offset: { x: 64, y: 20 }, durationMs: 100, startMs: 100, ease: 'linear' }
  }, rig: facingPlan }];
  var exitSnap = rigSnap(exitPlayback, scene, 'exit');
  exitPresenter.present(exitSnap);
  var exitStage = exitDom._rigStage('actor');
  var exitBody = exitDom._rigPart('actor', 'body');
  var exitArtWrites = exitDom._artWrites();
  exitPresenter.present(exitSnap);
  ok('C3c-S1 same key reuses exit stage/part art and keeps one shared playback rAF',
    exitDom._rigStage('actor') === exitStage && exitDom._rigPart('actor', 'body') === exitBody
      && exitDom._artWrites() === exitArtWrites && exitRaf.requests() === 1 && exitRaf.pending() === 1);
  exitRaf.fireNext(0);
  exitRaf.fireNext(50);
  ok('C3c-S2 enter window composes through the same outer stage before exit starts',
    exitStage.getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 220.000 -10.000)'
      && exitStage.getAttribute('opacity') === '0.500' && exitBody.getAttribute('opacity') === '0.800');
  exitRaf.fireNext(100);
  ok('C3c-S3 exactly touching enter/exit windows meet at identity offset and opacity one',
    exitStage.getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 260.000 0.000)'
      && exitStage.getAttribute('opacity') === '1.000');
  exitRaf.fireNext(150);
  var exitMatrix = svgMatrix(exitStage);
  var exitSample = Timeline.sampleRig(facingPlan, 150);
  var exitRoot = exitSample.parts.filter(function (part) { return part.id === 'body'; })[0];
  var exitPivotX = exitRoot.matrix[0] * 10 + exitRoot.matrix[2] * 0 + exitRoot.matrix[4];
  var expectedExitTF = multiplyMatrices([1, 0, 0, 1, 32, 10], [-1, 0, 0, 1, 2 * exitPivotX, 0]);
  ok('C3c-S4 exit midpoint composes T(total enter+exit offset)*F(current root pivot) with multiplied opacity',
    exitMatrix.every(function (value, index) { return near(value, expectedExitTF[index]); })
      && exitStage.getAttribute('opacity') === '0.500');
  ok('C3c-S5 exit stage composition leaves FK part matrices and opacity unchanged',
    svgMatrix(exitBody).every(function (value, index) { return near(value, exitRoot.matrix[index]); })
      && near(Number(exitBody.getAttribute('opacity')), exitRoot.opacity) && exitDom._artWrites() === exitArtWrites);
  exitRaf.fireNext(200);
  ok('C3c-S6 exit endpoint holds the authored offset at zero outer opacity',
    exitStage.getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 364.000 20.000)'
      && exitStage.getAttribute('opacity') === '0.000');

  var exitSwitchRaf = rafHarness();
  var exitSwitchManager = Timeline.createPlaybackManager({ requestAnimationFrame: exitSwitchRaf.request,
    cancelAnimationFrame: exitSwitchRaf.cancel });
  var exitSwitchDom = motionDom();
  var exitSwitchPresenter = createSvgPresenter({ container: exitSwitchDom, timelineManager: exitSwitchManager });
  var exitSwitchFirst = playback('m/exit-switch#0');
  exitSwitchFirst.cast = [{ id: 'actor', stage: { facing: 'as-authored', exit: {
    offset: { x: 64, y: 0 }, durationMs: 100, startMs: 100, ease: 'linear'
  } }, rig: facingPlan }];
  exitSwitchPresenter.present(rigSnap(exitSwitchFirst, scene, 'exit-switch'));
  var detachedExitStage = exitSwitchDom._rigStage('actor');
  var staleExitFrame = exitSwitchRaf.takeNext();
  var exitSwitchSecond = playback('m/exit-switch#1');
  exitSwitchSecond.cast = [{ id: 'actor', stage: { facing: 'as-authored', exit: {
    offset: { x: 64, y: 0 }, durationMs: 100, startMs: 100, ease: 'linear'
  } }, rig: facingPlan }];
  exitSwitchPresenter.present(rigSnap(exitSwitchSecond, scene, 'exit-switch'));
  var replacementExitStage = exitSwitchDom._rigStage('actor');
  var writesBeforeStaleExit = exitSwitchDom._attrWrites('transform');
  var replacementBeforeStaleExit = replacementExitStage.getAttribute('transform');
  staleExitFrame(150);
  ok('C3c-S7 key switch synchronously detaches the old exit stage before present returns',
    detachedExitStage.parentNode === null && replacementExitStage !== detachedExitStage
      && replacementExitStage.getAttribute('data-amatlas-rig-key') === 'm/exit-switch#1');
  ok('C3c-S8 stale old-generation callback cannot patch the detached or replacement exit stage',
    exitSwitchDom._attrWrites('transform') === writesBeforeStaleExit
      && replacementExitStage.getAttribute('transform') === replacementBeforeStaleExit);

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var exitReducedRaf = rafHarness();
  var exitReducedDom = motionDom();
  var exitReducedPlayback = playback('m/exit-reduced#0');
  exitReducedPlayback.cast = [{ id: 'actor', stage: { facing: 'mirror-x', exit: {
    offset: { x: 64, y: 20 }, durationMs: 100, startMs: 100, ease: 'ease-in'
  } }, rig: facingPlan }];
  createSvgPresenter({ container: exitReducedDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: exitReducedRaf.request, cancelAnimationFrame: exitReducedRaf.cancel
  }) }).present(rigSnap(exitReducedPlayback, scene, 'exit-reduced'));
  delete global.window;
  ok('C3c-S9 reduced-motion samples the hidden exit endpoint immediately without any rAF',
    exitReducedRaf.requests() === 0 && exitReducedDom._rigStage('actor').getAttribute('opacity') === '0.000'
      && exitReducedDom._rigStage('actor').getAttribute('transform') === 'matrix(-1.000 0.000 0.000 1.000 364.000 20.000)');

  var rigLeaveRaf = rafHarness();
  var rigLeaveManager = Timeline.createPlaybackManager({ requestAnimationFrame: rigLeaveRaf.request, cancelAnimationFrame: rigLeaveRaf.cancel });
  var rigLeaveDom = motionDom();
  var rigLeavePresenter = createSvgPresenter({ container: rigLeaveDom, timelineManager: rigLeaveManager });
  rigLeavePresenter.present(rigSnap(rigPlayback('m/rig-leave#0'), scene, 'rig-leave'));
  var leavingMouth = rigLeaveDom._rigPart('mouth'), staleRigLeave = rigLeaveRaf.takeNext();
  rigLeavePresenter.present({ view: { scene: scene }, pos: { map: 'm', node: 'shore' } });
  var rigLeaveWrites = rigLeaveDom._attrWrites('transform');
  staleRigLeave(150);
  ok('UU13 cast 离场移除 stage、取消共享 session，已出队旧 generation 不再写',
    !rigLeaveDom._rigPart('mouth') && rigLeaveManager.inspect().key === null
      && rigLeaveDom._attrWrites('transform') === rigLeaveWrites
      && leavingMouth.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 110.000 50.000)');

  var rigRestoreRaf = rafHarness();
  var rigRestoreManager = Timeline.createPlaybackManager({ requestAnimationFrame: rigRestoreRaf.request, cancelAnimationFrame: rigRestoreRaf.cancel });
  var rigRestoreDom = motionDom(), rigLifecycle = null, rigInstalledPresent = null;
  var rigRestorePresenter = createSvgPresenter({ container: rigRestoreDom, timelineManager: rigRestoreManager });
  rigRestorePresenter.install({
    registerModule: function (mod) { rigLifecycle = mod; },
    addPresenter: function (fn) { rigInstalledPresent = fn; }
  });
  rigInstalledPresent(rigSnap(rigPlayback('m/rig-restore#0'), scene, 'rig-restore'));
  var staleRigRestore = rigRestoreRaf.takeNext(), restoreMouth = rigRestoreDom._rigPart('mouth');
  rigLifecycle.systems.filter(function (system) { return system.on === 'restore'; })[0].run({}, { phase: 'deactivate' });
  var rigRestoreWrites = rigRestoreDom._attrWrites('transform');
  staleRigRestore(175);
  ok('UU14 restore(deactivate) 对 cast 同样按 generation 取消，晚到旧 rAF 无写入',
    rigRestoreManager.inspect().key === null && rigRestoreDom._attrWrites('transform') === rigRestoreWrites
      && restoreMouth.getAttribute('transform') === 'matrix(1.000 0.000 0.000 1.000 110.000 50.000)');

  var multiRaf = rafHarness();
  var multiManager = Timeline.createPlaybackManager({ requestAnimationFrame: multiRaf.request, cancelAnimationFrame: multiRaf.cancel });
  var multiDom = motionDom();
  var multiPresenter = createSvgPresenter({ container: multiDom, timelineManager: multiManager });
  var zetaPlan = rigPlan(80);
  var alphaPlan = rigPlan(180, ['body', 'mouth']);
  var multiPlayback = playback('m/multi#0');
  multiPlayback.cast = [{ id: 'zeta', rig: zetaPlan }, { id: 'alpha', rig: alphaPlan }];
  var originalSampleRig = Timeline.sampleRig;
  var sampledPlans = [];
  Timeline.sampleRig = function (plan, tMs) { sampledPlans.push({ plan: plan, tMs: tMs }); return originalSampleRig(plan, tMs); };
  try { multiPresenter.present(rigSnap(multiPlayback, scene, 'multi')); }
  finally { Timeline.sampleRig = originalSampleRig; }
  var zetaBody = multiDom._rigPart('zeta', 'body');
  var alphaBody = multiDom._rigPart('alpha', 'body');
  ok('UU15 cast stage 顺序严格等于作者数组顺序，且每个 rig 独立遵守自己的 drawOrder',
    multiDom._rootStages().join(',') === 'motion,cast:zeta,cast:alpha'
      && multiDom._castOrder().join(',') === 'zeta,alpha'
      && multiDom._rigOrder('zeta').join(',') === 'mouth,body'
      && multiDom._rigOrder('alpha').join(',') === 'body,mouth');
  ok('UU16 两个 rig 的同名 body 保留在独立 stage，DOM id 同时包含 playback key/cast id/part id',
    zetaBody && alphaBody && zetaBody !== alphaBody && zetaBody.parentNode !== alphaBody.parentNode
      && zetaBody.getAttribute('id') !== alphaBody.getAttribute('id')
      && zetaBody.getAttribute('id').indexOf('-zeta-body') >= 0
      && alphaBody.getAttribute('id').indexOf('-alpha-body') >= 0);
  ok('UU17 初始组合 patch 对每个 cast member 各调用一次 sampleRig，且 motion+两 rig 仍只请求一张 rAF',
    sampledPlans.filter(function (sample) { return sample.plan === zetaPlan; }).length === 2
      && sampledPlans.filter(function (sample) { return sample.plan === alphaPlan; }).length === 2
      && multiRaf.requests() === 1);
  var sampleCountBeforeFrame = sampledPlans.length;
  Timeline.sampleRig = function (plan, tMs) { sampledPlans.push({ plan: plan, tMs: tMs }); return originalSampleRig(plan, tMs); };
  try { multiRaf.fireNext(0); } finally { Timeline.sampleRig = originalSampleRig; }
  ok('UU18 每帧按 cast 顺序逐成员 sampleRig，不能把两 rig 合成一次采样',
    sampledPlans.length === sampleCountBeforeFrame + 2
      && sampledPlans[sampleCountBeforeFrame].plan === zetaPlan
      && sampledPlans[sampleCountBeforeFrame + 1].plan === alphaPlan);
  var multiArtWrites = multiDom._artWrites();
  var zetaStage = multiDom._rigStage('zeta'), alphaStage = multiDom._rigStage('alpha');
  multiPresenter.present(rigSnap(multiPlayback, scene, 'multi'));
  ok('UU19 同 key/cast plan/root 复用全部 stage，每帧和重渲染均不改 innerHTML',
    multiDom._rigStage('zeta') === zetaStage && multiDom._rigStage('alpha') === alphaStage
      && multiDom._artWrites() === multiArtWrites && multiRaf.requests() === 2);

  global.window = { matchMedia: function (q) { return { matches: /prefers-reduced-motion/.test(q) }; } };
  var multiReducedRaf = rafHarness(), multiReducedDom = motionDom();
  var multiReducedPlayback = playback('m/multi-reduced#0');
  multiReducedPlayback.cast = [{ id: 'zeta', rig: zetaPlan }, { id: 'alpha', rig: alphaPlan }];
  createSvgPresenter({ container: multiReducedDom, timelineManager: Timeline.createPlaybackManager({
    requestAnimationFrame: multiReducedRaf.request, cancelAnimationFrame: multiReducedRaf.cancel
  }) }).present(rigSnap(multiReducedPlayback, scene, 'multi-reduced'));
  delete global.window;
  ok('UU20 reduced-motion 对每个 rig 采样 durationMs 终态且不创建角色级 rAF',
    multiReducedRaf.requests() === 0
      && multiReducedDom._rigPart('zeta', 'mouth').getAttribute('transform') === 'matrix(0.000 1.000 -1.000 0.000 80.000 60.000)'
      && multiReducedDom._rigPart('alpha', 'mouth').getAttribute('transform') === 'matrix(0.000 1.000 -1.000 0.000 180.000 60.000)');

  var dialogueRaf = rafHarness();
  var dialogueManager = Timeline.createPlaybackManager({ requestAnimationFrame: dialogueRaf.request, cancelAnimationFrame: dialogueRaf.cancel });
  var dialogueDom = motionDom();
  var dialoguePresenter = createSvgPresenter({ container: dialogueDom, timelineManager: dialogueManager });
  var dialoguePlayback = playback('m/dialogue#0');
  dialoguePlayback.speaker = 'warden';
  dialoguePlayback.text = Timeline.compileReveal({ mode: 'typewriter', lines: [
    { cps: 20, chunks: [{ text: 'AB' }] }
  ] });
  dialoguePlayback.cast = [
    { id: 'warden', rig: rigPlan(100, null, [
      { at: 0.05, value: 'A' }, { at: 0.1, value: 'O' }, { at: 0.2, value: 'rest' }
    ]) },
    { id: 'traveler', rig: rigPlan(220, null, []) }
  ];
  dialoguePresenter.present(rigSnap(dialoguePlayback, scene, 'dialogue'));
  dialogueRaf.fireNext(0);
  dialogueRaf.fireNext(75);
  ok('UU21 C2 说话拍只采样 speaker 的 A/O keys，非 speaker mouth 空 keys 保持 rest，且两角色共用一张 rAF',
    dialogueDom._rigState('warden', 'mouth', 'A').getAttribute('display') === 'inline'
      && dialogueDom._rigState('warden', 'mouth', 'rest').getAttribute('display') === 'none'
      && dialogueDom._rigState('traveler', 'mouth', 'rest').getAttribute('display') === 'inline'
      && dialogueDom._rigState('traveler', 'mouth', 'A').getAttribute('display') === 'none'
      && dialogueRaf.requests() === 3 && dialogueRaf.pending() === 1);
  dialogueRaf.fireNext(200);
  ok('UU22 对话终点 speaker 与非 speaker mouth 都是 rest，presenter 不从字幕 grapheme 推断口型',
    dialogueDom._rigState('warden', 'mouth', 'rest').getAttribute('display') === 'inline'
      && dialogueDom._rigState('traveler', 'mouth', 'rest').getAttribute('display') === 'inline'
      && dialogueRaf.pending() === 0);

  var capturedPatches = [];
  var captureManager = {
    prepare: function () {}, unregister: function () {},
    register: function (options) { capturedPatches.push(options.patch); options.patch(0); }
  };
  var generationDom = motionDom();
  var generationPresenter = createSvgPresenter({ container: generationDom, timelineManager: captureManager });
  generationPresenter.present(rigSnap({ version: 1, key: 'generation#0', durationMs: 200,
    motion: null, text: null, cast: [{ id: 'actor', rig: rigPlan(100) }] }, scene, 'generation'));
  var stalePatch = capturedPatches[0];
  generationPresenter.present(rigSnap({ version: 1, key: 'generation#1', durationMs: 200,
    motion: null, text: null, cast: [{ id: 'actor', rig: rigPlan(120) }] }, scene, 'generation'));
  var newGenerationMouth = generationDom._rigPart('mouth');
  var newGenerationTransform = newGenerationMouth.getAttribute('transform');
  stalePatch(100);
  ok('UU21 旧 generation 回调捕获旧 state，不能经可变 presenter state 写入新 stage',
    generationDom._rigPart('mouth') === newGenerationMouth
      && newGenerationMouth.getAttribute('transform') === newGenerationTransform);

  var badArtPlan = Timeline.normalizeRigPlan({
    parts: [{ id: 'bad', parent: null, art: 'not-a-preset', pivot: { x: 0, y: 0 },
      rest: { x: 100, y: 50, rotate: 0, scaleX: 1, scaleY: 1, opacity: 1 } }],
    drawOrder: ['bad'], tracks: [], variants: [], secondary: []
  }, { durationMs: 200 });
  var badArtDom = motionDom();
  ok('UU15 未知 base rig preset fail-loud，且预检失败不留下半棵 rig DOM', throws(function () {
    createSvgPresenter({ container: badArtDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap({ version: 1, key: 'bad#0', durationMs: 200, motion: null, text: null,
        cast: [{ id: 'actor', rig: badArtPlan }] }, scene, 'bad'));
  }) && badArtDom._castOrder().length === 0);
  ok('UU16 声明 cast 却缺 SVG presenter 容器/root 必须 fail-loud', throws(function () {
    createSvgPresenter({ timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap(rigPlayback('missing-root#0'), scene, 'bad'));
  }));
  var unknownStatePlan = Timeline.normalizeRigPlan({
    parts: [{ id: 'face', parent: null, art: 'figure', pivot: { x: 0, y: 0 },
      rest: { x: 100, y: 50, rotate: 0, scaleX: 1, scaleY: 1, opacity: 1 } }],
    drawOrder: ['face'], tracks: [], variants: [{ target: 'face', base: 'rest',
      states: [{ id: 'bad', art: 'not-a-preset' }], keys: [] }], secondary: []
  }, { durationMs: 200 });
  var unknownStateDom = motionDom();
  ok('UU17 unknown variant state preset fails before creating any rig DOM/art', throws(function () {
    createSvgPresenter({ container: unknownStateDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap({ version: 1, key: 'bad-state#0', durationMs: 200, motion: null, text: null,
        cast: [{ id: 'actor', rig: unknownStatePlan }] }, scene, 'bad-state'));
  }) && unknownStateDom._castOrder().length === 0 && unknownStateDom._artWrites() === 0);
  var tooManyPrimitives = [];
  for (var pi = 0; pi < 256; pi++) tooManyPrimitives.push({ shape: 'circle', cx: pi, cy: 0, r: 1, fill: '#fff' });
  var artBudgetPlan = Timeline.normalizeRigPlan({
    parts: [{ id: 'dense', parent: null, art: [{ shape: 'circle', cx: 0, cy: 0, r: 1, fill: '#fff' }], pivot: { x: 0, y: 0 },
      rest: { x: 100, y: 50, rotate: 0, scaleX: 1, scaleY: 1, opacity: 1 } }],
    drawOrder: ['dense'], tracks: [], variants: [{ target: 'dense', base: 'base',
      states: [{ id: 'dense-state', art: tooManyPrimitives }], keys: [] }], secondary: []
  }, { durationMs: 200 });
  var artBudgetDom = motionDom();
  ok('UU18 base + variant primitive 总数超过 256 时先拒绝，零 rig DOM/art 写入', throws(function () {
    createSvgPresenter({ container: artBudgetDom, timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap({ version: 1, key: 'budget#0', durationMs: 200, motion: null, text: null,
        cast: [{ id: 'actor', rig: artBudgetPlan }] }, scene, 'budget'));
  }) && artBudgetDom._castOrder().length === 0 && artBudgetDom._artWrites() === 0);

  function denseRigPlan(primitiveCount, rootX) {
    var art = [];
    for (var i = 0; i < primitiveCount; i++)
      art.push({ shape: 'circle', cx: i % 20, cy: Math.floor(i / 20), r: 1, fill: '#fff' });
    return Timeline.normalizeRigPlan({
      parts: [{ id: 'body', parent: null, art: art, pivot: { x: 0, y: 0 },
        rest: { x: rootX, y: 50, rotate: 0, scaleX: 1, scaleY: 1, opacity: 1 } }],
      drawOrder: ['body'], tracks: [], variants: [], secondary: []
    }, { durationMs: 200 });
  }
  var aggregatePrimitiveDom = motionDom();
  ok('UU22 三个合法 per-rig art 计划聚合超过 512 时在创建任一 cast stage 前拒绝', throws(function () {
    createSvgPresenter({ container: aggregatePrimitiveDom,
      timelineManager: Timeline.createPlaybackManager({ requestAnimationFrame: null }) })
      .present(rigSnap({ version: 1, key: 'aggregate-budget#0', durationMs: 200, motion: null, text: null,
        cast: [
          { id: 'first', rig: denseRigPlan(171, 60) },
          { id: 'second', rig: denseRigPlan(171, 160) },
          { id: 'third', rig: denseRigPlan(171, 260) }
        ] }, scene, 'aggregate-budget'));
  }) && aggregatePrimitiveDom._castOrder().length === 0 && aggregatePrimitiveDom._artWrites() === 0);
})();
