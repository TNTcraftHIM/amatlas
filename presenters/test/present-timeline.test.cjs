...
var immutableMotionSource = {
  layers: [{ id: 'immutable', art: [{ shape: 'circle', cx: 0, cy: 0, r: 2, fill: '#fff' }], x: 4 }],
  tracks: [{ target: 'immutable', property: 'x', keys: [{ at: 1, value: 8 }] }]
};
var immutableMotion = T.normalizeMotionPlan(immutableMotionSource, { durationMs: 1000, path: 'immutable.motion' });
var immutableRevealSource = { mode: 'typewriter', lines: [{ cps: 5, chunks: [{ text: '冻结' }] }] };
var immutableReveal = T.compileReveal(immutableRevealSource, { path: 'immutable.text' });
immutableMotionSource.layers[0].art[0].r = 99;
immutableMotionSource.tracks[0].keys[0].value = 99;
immutableRevealSource.lines[0].chunks[0].text = '篡改';
var motionMutationRejected = false, revealMutationRejected = false;
try { immutableMotion.tracks[0].keys[0].value = 77; } catch (error) { motionMutationRejected = error instanceof TypeError; }
try { immutableReveal.lines[0].chunks[0].text = '改'; } catch (error) { revealMutationRejected = error instanceof TypeError; }
ok(Object.isFrozen(immutableMotion) && Object.isFrozen(immutableMotion.layers) &&
  Object.isFrozen(immutableMotion.layers[0]) && Object.isFrozen(immutableMotion.layers[0].art) &&
  Object.isFrozen(immutableMotion.layers[0].art[0]) && Object.isFrozen(immutableMotion.tracks) &&
  Object.isFrozen(immutableMotion.tracks[0].baseKey) && Object.isFrozen(immutableMotion.tracks[0].keys[0]) &&
  immutableMotion.layers[0].art[0].r === 2 && immutableMotion.tracks[0].keys[0].value === 8 && motionMutationRejected,
  'C10 normalized motion recursively freezes cloned art/tracks and rejects strict mutation after source changes');
ok(Object.isFrozen(immutableReveal) && Object.isFrozen(immutableReveal.lines) &&
  Object.isFrozen(immutableReveal.lines[0]) && Object.isFrozen(immutableReveal.lines[0].chunks) &&
  Object.isFrozen(immutableReveal.lines[0].chunks[0]) && Object.isFrozen(immutableReveal.graphemes) &&
  Object.isFrozen(immutableReveal.graphemes[0]) && immutableReveal.lines[0].chunks[0].text === '冻结' &&
  immutableReveal.graphemes.map(function (part) { return part.text; }).join('') === '冻结' && revealMutationRejected,
  'C11 normalized reveal recursively freezes cloned text/graphemes and rejects strict mutation after source changes');
...
var ieeeMsPlan = T.normalizeMotionPlan({ layers: [{ id: 'ieee', art: 'ship' }], tracks: [
  { target: 'ieee', property: 'x', keys: [{ at: 1.001, value: 1 }] }
] }, { durationMs: 1001, path: 'ieee.motion' });
ok(ieeeMsPlan.tracks[0].keys[0].atMs === 1001,
  'D4a three-decimal seconds survive IEEE-754 representation and normalize to the intended integer ms');
