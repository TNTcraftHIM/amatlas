...
// R2D. R2 二轮呈现层审计:MusicSpec 顶层类型闸补全(array/基本类型)+ stealth GENRE_DNA 兄弟补齐
(function () {
  ok('R2D1 composeMusic 顶层拒数组(镜像 AmbientSpec/SfxSpec 类型闸;array typeof==="object" 曾漏网被当 spec 静默处理)', throws(function () { composeMusic([1, 2], 0); }));
  ok('R2D2 normalizeSpec 直接拒数组(结构违约 → throw)', throws(function () { normalizeSpec([1, 2]); }));
  ok('R2D3 resolveMusic(布尔等基本类型)→ warn-once(不再静默回退默认曲,对齐 unknown 预设名 warn)', (function () {
    resolveMusic._warnedType = false;
    var w = [], ow = console.warn; console.warn = function (m) { w.push(String(m)); };
    var r = resolveMusic(true); console.warn = ow;
    return r && typeof r === 'object' && w.length > 0 && /回退默认曲/.test(w[0]);
  })());
  ok('R2D4 反向变异:合法对象 spec / 字符串预设 / 对象 spec 不被类型闸误伤', !throws(function () { composeMusic({ mode: 'major', key: 'C', instruments: ['pad'] }, 0); }) && typeof resolveMusic('gentle') === 'object' && typeof resolveMusic({ mode: 'major', key: 'C' }) === 'object');
  function feelN(feel) { return composeMusic({ mode: 'minor', key: 'A', tempo: 80, progression: ['i', 'iv', 'i', 'iv'], instruments: ['pad', 'bass', 'arp'], melody: 'flowing', feel: feel }, 0).events.length; }
  ok('R2D5 feel:[stealth] 得完整曲式:事件数 == sneaking 兄弟、!= 无 DNA 的 BASE(GENRE_DNA 独漏 stealth 的接缝已补)', feelN(['stealth']) === feelN(['sneaking']) && feelN(['stealth']) !== feelN(['zzz-none']));
})();
