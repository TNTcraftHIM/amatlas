...
(function (global, factory) {
  if (typeof module !== 'undefined' && module.exports) module.exports = factory(require('./present-timeline.js'));
    ...
    atlas.SvgPresenter = factory(atlas.Timeline);
  }
  ...
  function buildSceneSVG(scene) {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (shape === 'dice') {
        ...
        // fail-loud + 注入闸(R2 二轮 P0):sides 契约是「正整数」,却曾原样拼进 data-sides 属性(见下 push)——
        //   同分支兄弟属性 region/mood/ref 都过 esc()、唯 sides 裸奔;一个 `sides:'6" onmouseover=...'` 能逃逸属性、
        //   注入事件处理器(经 present() 的 innerHTML 写入即真实可执行)。正整数天然无注入字符 → 校验即闭合;非正整数=作者违约 → 抛。
        var sides = null;
        if (e.sides != null) {
          if (typeof e.sides !== 'number' || !Number.isInteger(e.sides) || e.sides <= 0) {
            throw new Error('[amatlas] scene.elements 的 dice.sides 必须是正整数(契约 §4.2 `sides?:正整数`),收到 ' + JSON.stringify(e.sides) + ':非正整数会被误渲染,字符串更会作为属性值裸拼进 SVG(注入面)。');
          }
          sides = e.sides;
        }
  ...
  var svgPresenterSerial = 0;

  function motionNumber(value) {
    if (typeof value !== 'number' || !isFinite(value)) throw new Error('[amatlas] motion 采样值必须是有限数');
    if (Math.abs(value) < 0.0005) value = 0;
    return value.toFixed(3);
  }
  function motionLayerId(playbackKey, layerId) {
    var token = '';
    for (var i = 0; i < playbackKey.length; i++) token += ('0000' + playbackKey.charCodeAt(i).toString(16)).slice(-4);
    return 'amatlas-motion-' + token + '-' + layerId;
  }
  function rigLayerId(playbackKey, castId, partId) {
    var token = '';
    for (var i = 0; i < playbackKey.length; i++) token += ('0000' + playbackKey.charCodeAt(i).toString(16)).slice(-4);
    return 'amatlas-rig-' + token + '-' + castId + '-' + partId;
  }
  function findSvgRoot(el) {
    if (el && String(el.tagName || '').toLowerCase() === 'svg') return el;
    return el && typeof el.querySelector === 'function' ? el.querySelector('svg') : null;
  }
  function removeMotionState(state) {
    if (!state || !state.overlay || !state.overlay.parentNode) return;
    state.overlay.parentNode.removeChild(state.overlay);
  }
  function removeRigState(state) {
    if (!state || !state.overlay || !state.overlay.parentNode) return;
    state.overlay.parentNode.removeChild(state.overlay);
  }
  function removeCastState(state) {
    if (!state || !Array.isArray(state.rigStates)) return;
    state.rigStates.forEach(removeRigState);
  }
  function createMotionState(root, playback) {
    var doc = root && root.ownerDocument;
    if (!doc || typeof doc.createElementNS !== 'function' || typeof root.appendChild !== 'function')
      throw new Error('[amatlas] cutscene motion 需要可写的 SVG DOM(createElementNS/appendChild)');
    if (!Timeline || typeof Timeline.sampleMotion !== 'function')
      throw new Error('[amatlas] cutscene motion 已声明，但未加载 presenters/present-timeline.js');
    var plan = playback.motion;
    if (!plan || !Array.isArray(plan.layers) || !Array.isArray(plan.tracks))
      throw new Error('[amatlas] view.cutscenePlayback.motion 不是规范化 motion plan');

    var overlay = doc.createElementNS(SVG_NS, 'g');
    overlay.setAttribute('id', motionLayerId(playback.key, 'stage'));
    overlay.setAttribute('data-amatlas-motion-key', playback.key);
    overlay.setAttribute('pointer-events', 'none');
    var nodes = Object.create(null);
    plan.layers.forEach(function (layer, index) {
      var art = renderElementArt(layer.art, 'cutscenePlayback.motion.layers[' + index + '].art');
      if (art == null)
        throw new Error('[amatlas] cutscenePlayback.motion.layers[' + index + '].art 是未知预设，motion layer 无 glyph 可退化');
      var node = doc.createElementNS(SVG_NS, 'g');
      node.setAttribute('id', motionLayerId(playback.key, layer.id));
      node.setAttribute('data-amatlas-motion-layer', layer.id);
      node.innerHTML = art;
    ...
    var state = { key: playback.key, plan: plan, root: root, overlay: overlay, nodes: nodes };
    state.patch = function (tMs) {
      var frame = Timeline.sampleMotion(state.plan, tMs);
      state.plan.layers.forEach(function (layer) {
        var values = frame[layer.id];
        var node = state.nodes[layer.id];
        var hasAxisScale = Object.prototype.hasOwnProperty.call(values, 'scaleX') ||
          Object.prototype.hasOwnProperty.call(values, 'scaleY');
        var scale = hasAxisScale
          ? motionNumber(values.scaleX) + ' ' + motionNumber(values.scaleY)
          : motionNumber(values.scale);
        node.setAttribute('transform', 'translate(' + motionNumber(values.x) + ' ' + motionNumber(values.y)
          + ') rotate(' + motionNumber(values.rotate) + ') scale(' + scale + ')');
        node.setAttribute('opacity', motionNumber(values.opacity));
      });
    };
    return state;
  }

  function prepareRigArts(plan, rigPath) {
    rigPath = rigPath || 'cutscenePlayback.cast[?].rig';
    if (!plan || !Array.isArray(plan.parts) || !Array.isArray(plan.drawOrder) ||
        !Array.isArray(plan.tracks) || !Array.isArray(plan.topoParts) ||
        !Array.isArray(plan.variants) || !Array.isArray(plan.secondary) ||
        typeof plan.durationMs !== 'number' || !isFinite(plan.durationMs) || plan.durationMs <= 0)
      throw new Error('[amatlas] ' + rigPath + ' 不是规范化 rig plan');
    if (!Timeline || typeof Timeline.sampleRig !== 'function')
      throw new Error('[amatlas] cutscene rig 已声明，但未加载带 sampleRig 的 presenters/present-timeline.js');

    var partById = Object.create(null);
    var drawingsById = Object.create(null);
    var primitiveCount = 0;
    function prepareDrawing(art, where) {
      var spec;
      if (typeof art === 'string') {
        if (!Object.prototype.hasOwnProperty.call(ART_PRESETS, art))
          throw new Error('[amatlas] ' + where + ': 未知 art 预设名 "' + art + '"，rig 不允许 glyph 退化');
        spec = ART_PRESETS[art];
      } else if (Array.isArray(art)) spec = art;
      else throw new Error('[amatlas] ' + where + ': 必须是 preset 名或 art-spec 图元数组');
      primitiveCount += spec.length;
      if (primitiveCount > 256)
        throw new Error('[amatlas] ' + rigPath + ' art primitives 超过硬上限 256；收到 ' + primitiveCount);
      var rendered = typeof art === 'string'
        ? renderArtSpec(spec, where + "('" + art + "')")
        : renderElementArt(art, where);
      if (rendered == null) throw new Error('[amatlas] ' + where + ': rig art 无法渲染');
      return rendered;
    }
    plan.parts.forEach(function (part, index) {
      var where = rigPath + '.parts[' + index + '].art';
      if (!part || typeof part.id !== 'string' || !part.id || Object.prototype.hasOwnProperty.call(partById, part.id))
        throw new Error('[amatlas] ' + rigPath + '.parts[' + index + ']: part id 非法或重复');
      partById[part.id] = part;
      drawingsById[part.id] = { baseArt: prepareDrawing(part.art, where), slot: null };
    });
    if (plan.drawOrder.length !== plan.parts.length)
      throw new Error('[amatlas] ' + rigPath + '.drawOrder 不是全部 part 的精确排列');
    var seen = Object.create(null);
    plan.drawOrder.forEach(function (id, index) {
      if (!Object.prototype.hasOwnProperty.call(partById, id))
        throw new Error('[amatlas] ' + rigPath + '.drawOrder[' + index + '] 含未知 part "' + id + '"');
      if (Object.prototype.hasOwnProperty.call(seen, id))
        throw new Error('[amatlas] ' + rigPath + '.drawOrder[' + index + '] 重复 part "' + id + '"');
      seen[id] = 1;
    });
    var slotTargetSeen = Object.create(null);
    plan.variants.forEach(function (slot, index) {
      var where = rigPath + '.variants[' + index + ']';
      if (!slot || typeof slot.target !== 'string' || !Object.prototype.hasOwnProperty.call(partById, slot.target))
        throw new Error('[amatlas] ' + where + '.target: variant target 不存在');
      if (Object.prototype.hasOwnProperty.call(slotTargetSeen, slot.target))
        throw new Error('[amatlas] ' + where + '.target: 同一 target 最多一个 variant slot');
      slotTargetSeen[slot.target] = 1;
      if (typeof slot.base !== 'string' || !slot.base || !Array.isArray(slot.states) || slot.states.length === 0)
        throw new Error('[amatlas] ' + where + ': 不是规范化 variant slot');
      var stateSeen = Object.create(null);
      stateSeen[slot.base] = 1;
      var preparedStates = [{ id: slot.base, art: drawingsById[slot.target].baseArt }];
      slot.states.forEach(function (state, stateIndex) {
        var stateWhere = where + '.states[' + stateIndex + ']';
        if (!state || typeof state.id !== 'string' || !state.id || Object.prototype.hasOwnProperty.call(stateSeen, state.id))
          throw new Error('[amatlas] ' + stateWhere + '.id: variant state id 非法或重复 base/state');
        stateSeen[state.id] = 1;
        preparedStates.push({ id: state.id, art: prepareDrawing(state.art, stateWhere + '.art') });
      });
      drawingsById[slot.target].slot = { base: slot.base, states: preparedStates };
    });
    Timeline.sampleRig(plan, 0);
    return { partById: partById, drawingsById: drawingsById, primitiveCount: primitiveCount };
  }

  function prepareCastArts(cast) {
    if (!Array.isArray(cast) || cast.length === 0)
      throw new Error('[amatlas] view.cutscenePlayback.cast 不是规范化非空 cast plan');
    var seen = Object.create(null);
    var totalPrimitives = 0;
    return cast.map(function (member, index) {
      var memberPath = 'cutscenePlayback.cast[' + index + ']';
      if (!member || typeof member !== 'object' || Array.isArray(member) ||
          typeof member.id !== 'string' || !member.id || !member.rig)
        throw new Error('[amatlas] ' + memberPath + ' 不是规范化 cast member');
      Object.keys(member).forEach(function (key) {
        if (key !== 'id' && key !== 'rig' && key !== 'stage')
          throw new Error('[amatlas] ' + memberPath + '.' + key + ': 未知字段');
      });
      if (Object.prototype.hasOwnProperty.call(seen, member.id))
        throw new Error('[amatlas] ' + memberPath + '.id 重复 cast id "' + member.id + '"');
      seen[member.id] = 1;
      var facing = 'as-authored';
      var enter = null;
      var exit = null;
      if (Object.prototype.hasOwnProperty.call(member, 'stage')) {
        var stage = member.stage;
        if (!stage || typeof stage !== 'object' || Array.isArray(stage))
          throw new Error('[amatlas] ' + memberPath + '.stage 必须是规范化对象');
        Object.keys(stage).forEach(function (key) {
          if (key !== 'facing' && key !== 'enter' && key !== 'exit')
            throw new Error('[amatlas] ' + memberPath + '.stage.' + key + ': 未知字段');
        });
        if (!Object.prototype.hasOwnProperty.call(stage, 'facing'))
          throw new Error('[amatlas] ' + memberPath + '.stage.facing: 缺少必需字段');
        if (stage.facing !== 'as-authored' && stage.facing !== 'mirror-x')
          throw new Error('[amatlas] ' + memberPath + '.stage.facing: 未知 facing；允许 as-authored/mirror-x');
        facing = stage.facing;
        if (Object.prototype.hasOwnProperty.call(stage, 'enter')) {
          if (!Timeline || typeof Timeline.sampleStageEnter !== 'function')
            throw new Error('[amatlas] cutscene stage.enter 已声明，但未加载带 sampleStageEnter 的 presenters/present-timeline.js');
          Timeline.sampleStageEnter(stage.enter, 0);
          enter = stage.enter;
        }
        if (Object.prototype.hasOwnProperty.call(stage, 'exit')) {
          if (!Timeline || typeof Timeline.sampleStageExit !== 'function')
            throw new Error('[amatlas] cutscene stage.exit 已声明，但未加载带 sampleStageExit 的 presenters/present-timeline.js');
          Timeline.sampleStageExit(stage.exit, 0);
          if (stage.exit.startMs + stage.exit.durationMs !== member.rig.durationMs)
            throw new Error('[amatlas] ' + memberPath + '.stage.exit.startMs: exit 尾窗终点必须等于 rig durationMs');
          if (enter && enter.durationMs > stage.exit.startMs)
            throw new Error('[amatlas] ' + memberPath + '.stage.exit.startMs: enter 与 exit 尾窗不得重叠');
          exit = stage.exit;
        }
      }
      var prepared = prepareRigArts(member.rig, memberPath + '.rig');
      prepared.facing = facing;
      prepared.enter = enter;
      prepared.exit = exit;
      totalPrimitives += prepared.primitiveCount;
      if (totalPrimitives > 512)
        throw new Error('[amatlas] cutscenePlayback.cast art primitives 聚合总数超过硬上限 512；收到 ' + totalPrimitives);
      return prepared;
    });
  }

  function createRigState(root, playbackKey, member, prepared) {
    var doc = root && root.ownerDocument;
    if (!doc || typeof doc.createElementNS !== 'function' || typeof root.appendChild !== 'function')
      throw new Error('[amatlas] cutscene rig 需要可写的 SVG DOM(createElementNS/appendChild)');
    var plan = member.rig;
    prepared = prepared || prepareRigArts(plan);
    var rootPart = null;
    plan.parts.forEach(function (part) {
      if (part.parent === null) {
        if (rootPart) throw new Error('[amatlas] rig 必须恰好有一个根 part');
        rootPart = part;
      }
    });
    if (!rootPart) throw new Error('[amatlas] rig 缺少根 part');
    var overlay = doc.createElementNS(SVG_NS, 'g');
    overlay.setAttribute('id', rigLayerId(playbackKey, member.id, 'stage'));
    overlay.setAttribute('data-amatlas-rig-cast', member.id);
    overlay.setAttribute('data-amatlas-rig-key', playbackKey);
    overlay.setAttribute('pointer-events', 'none');
    var nodes = Object.create(null);
    var variantNodes = Object.create(null);
    var currentVariants = Object.create(null);
    plan.drawOrder.forEach(function (id) {
      var node = doc.createElementNS(SVG_NS, 'g');
      node.setAttribute('id', rigLayerId(playbackKey, member.id, id));
      node.setAttribute('data-amatlas-rig-part', id);
      var drawing = prepared.drawingsById[id];
      if (!drawing) throw new Error('[amatlas] rig 缺少已预检 drawing: ' + id);
      if (drawing.slot) {
        var stateNodes = Object.create(null);
        drawing.slot.states.forEach(function (state) {
          var stateNode = doc.createElementNS(SVG_NS, 'g');
          stateNode.setAttribute('id', rigLayerId(playbackKey, member.id, id + '-state-' + state.id));
          stateNode.setAttribute('data-amatlas-rig-state', state.id);
          stateNode.setAttribute('display', state.id === drawing.slot.base ? 'inline' : 'none');
          stateNode.innerHTML = state.art;
          node.appendChild(stateNode);
          stateNodes[state.id] = stateNode;
        });
        variantNodes[id] = stateNodes;
        currentVariants[id] = drawing.slot.base;
      } else {
        node.innerHTML = drawing.baseArt;
        variantNodes[id] = null;
        currentVariants[id] = null;
      }
      overlay.appendChild(node);
      nodes[id] = node;
    });
    ...
    var state = { key: playbackKey, castId: member.id, plan: plan, root: root, overlay: overlay, nodes: nodes,
      variantNodes: variantNodes, currentVariants: currentVariants, rootPart: rootPart,
      facing: prepared.facing, enter: prepared.enter || null, exit: prepared.exit || null };
    state.patch = function (tMs) {
      var frame = Timeline.sampleRig(state.plan, tMs);
      var enterFrame = state.enter ? Timeline.sampleStageEnter(state.enter, tMs) : null;
      var exitFrame = state.exit ? Timeline.sampleStageExit(state.exit, tMs) : null;
      var hasStageFrame = !!(enterFrame || exitFrame);
      var stageOffsetX = (enterFrame ? enterFrame.offset.x : 0) + (exitFrame ? exitFrame.offset.x : 0);
      var stageOffsetY = (enterFrame ? enterFrame.offset.y : 0) + (exitFrame ? exitFrame.offset.y : 0);
      var stageOpacity = (enterFrame ? enterFrame.opacity : 1) * (exitFrame ? exitFrame.opacity : 1);
      if (state.facing === 'mirror-x') {
        var rootFrame = null;
        frame.parts.forEach(function (part) { if (part.id === state.rootPart.id) rootFrame = part; });
        if (!rootFrame) throw new Error('[amatlas] rig frame 缺少根 part "' + state.rootPart.id + '"');
        var pivot = state.rootPart.pivot;
        var pivotWorldX = rootFrame.matrix[0] * pivot.x + rootFrame.matrix[2] * pivot.y + rootFrame.matrix[4];
        // The outer stage composes T*F around the sampled root pivot; every FK part matrix stays untouched.
        state.overlay.setAttribute('transform', 'matrix(-1.000 0.000 0.000 1.000 '
          + motionNumber(stageOffsetX + 2 * pivotWorldX) + ' ' + motionNumber(stageOffsetY) + ')');
      } else if (hasStageFrame) {
        state.overlay.setAttribute('transform', 'matrix(1.000 0.000 0.000 1.000 '
          + motionNumber(stageOffsetX) + ' ' + motionNumber(stageOffsetY) + ')');
      }
      if (hasStageFrame) state.overlay.setAttribute('opacity', motionNumber(stageOpacity));
      frame.parts.forEach(function (part) {
        var node = state.nodes[part.id];
        if (!node) throw new Error('[amatlas] rig frame 含未知 part "' + part.id + '"');
        var partVariantNodes = state.variantNodes[part.id];
        if (part.variant === null) {
          if (partVariantNodes) throw new Error('[amatlas] rig frame 缺少 part "' + part.id + '" 的 variant state');
        } else {
          if (!partVariantNodes || !partVariantNodes[part.variant])
            throw new Error('[amatlas] rig frame 含未知 variant "' + part.variant + '" for part "' + part.id + '"');
          var current = state.currentVariants[part.id];
          if (current !== part.variant) {
            if (!partVariantNodes[current]) throw new Error('[amatlas] rig 当前 variant 不存在: ' + current);
            partVariantNodes[current].setAttribute('display', 'none');
            partVariantNodes[part.variant].setAttribute('display', 'inline');
            state.currentVariants[part.id] = part.variant;
          }
        }
        node.setAttribute('transform', 'matrix(' + part.matrix.map(motionNumber).join(' ') + ')');
        node.setAttribute('opacity', motionNumber(part.opacity));
      });
    };
    return state;
  }

  function createCastState(root, playback, preparedCast) {
    var state = { key: playback.key, plan: playback.cast, root: root, rigStates: [] };
    try {
      playback.cast.forEach(function (member, index) {
        state.rigStates.push(createRigState(root, playback.key, member, preparedCast[index]));
      });
    } catch (e) {
      removeCastState(state);
      throw e;
    }
    state.patch = function (tMs) {
      state.rigStates.forEach(function (rigState) { rigState.patch(tMs); });
    };
    return state;
  }
  ...
  function createSvgPresenter(opts) {
    ...
    var timelineManager = opts.timelineManager || (Timeline && Timeline.playback);
    var timelineConsumerId = 'svg-presenter-' + (++svgPresenterSerial);
    ...
    function present(snap) {
      var el = resolve();
      var view = snap && snap.view;
      var scene = view && view.scene;
      var playback = view && view.cutscenePlayback;
      var hasMotion = !!(playback && playback.motion);
      var hasCast = !!(playback && playback.cast);
      var hasTimeline = hasMotion || hasCast;
      if (!el) {
        if (hasCast) throw new Error('[amatlas] cutscene cast 已声明，但找不到 SVG presenter 容器/root');
        return;                                          // 无插槽 → 普通 scene/motion 沿用既有纯文字退化
      }
      var reduced = prefersReduced();
      if (hasTimeline) {
        if (!timelineManager || typeof timelineManager.register !== 'function' || typeof timelineManager.prepare !== 'function')
          throw new Error('[amatlas] cutscene motion/cast 已声明，但 presenter timeline 会话管理器不可用');
        ...
        timelineManager.prepare(playback.key);           // 换 key 时在旧 overlay 仍存在时先回 patch(t=0)
      }
      if (!hasTimeline && (motionState || castState)) {
        if (timelineManager && typeof timelineManager.unregister === 'function')
          timelineManager.unregister(timelineConsumerId, true);
        removeMotionState(motionState);
        removeCastState(castState);
        motionState = null;
        castState = null;
      } else if (hasTimeline) {
        if (motionState && (!hasMotion || motionState.key !== playback.key)) {
          removeMotionState(motionState);
          motionState = null;
        }
        if (castState && (!hasCast || castState.key !== playback.key)) {
          removeCastState(castState);
          castState = null;
        }
      }

      // motion/cast 可独立于 scene 作者意图存在；给它一张中性 base 舞台，普通无 scene 路径仍严格输出空串。
      if (hasTimeline && (!scene || typeof scene !== 'object' || Array.isArray(scene))) scene = {};
      var svg = buildSceneSVG(scene);                   // 无 scene → '' → 清空(避免残留上一场景)
      if (gradeAmt != null) svg = gradeSVG(svg, gradeAmt);   // opt-in HSL 冷暖分级后处理(默认 OFF → 字节不变)
      if (reduced) svg = stripSmil(svg);                // 元素基线属性即停止态(M 组既有约定:停止态可见)
      ...
      var baseChanged = !(posKey === lastPosKey && svg === lastSvg);
      if (baseChanged) el.innerHTML = svg;
      lastSvg = svg;

      if (hasTimeline) {
        var root = findSvgRoot(el);
        if (!root) throw new Error('[amatlas] cutscene motion/cast 找不到 base scene 的 <svg> 根节点');
        var rebuildMotion = hasMotion && (!motionState || motionState.key !== playback.key ||
          motionState.plan !== playback.motion || motionState.root !== root);
        var rebuildCast = hasCast && (!castState || castState.key !== playback.key ||
          castState.plan !== playback.cast || castState.root !== root);
        // cast stages 必须始终在 flat motion stage 后；motion 重建时同步重挂 cast 以保持 sibling stage 顺序。
        if (rebuildMotion && castState && castState.root === root) rebuildCast = true;
        var preparedCast = rebuildCast ? prepareCastArts(playback.cast) : null;
        if (rebuildMotion) {
          removeMotionState(motionState);
          motionState = createMotionState(root, playback);
        }
        if (rebuildCast) {
          removeCastState(castState);
          castState = createCastState(root, playback, preparedCast);
        }
        ...
        if (typeof durationMs !== 'number' || !isFinite(durationMs) || durationMs < 0 || Math.round(durationMs) !== durationMs)
          throw new Error('[amatlas] cutscene motion/cast durationMs 必须是非负整数毫秒');
        if (hasCast && durationMs <= 0)
          throw new Error('[amatlas] cutscene cast durationMs 必须是正整数毫秒');
        var registeredMotionState = motionState;
        var registeredCastState = castState;
        timelineManager.register({
          consumerId: timelineConsumerId,
          key: playback.key,
          durationMs: durationMs,
          patch: function (tMs) {
            if (registeredMotionState) registeredMotionState.patch(tMs);
            if (registeredCastState) registeredCastState.patch(tMs);
          },
    ...
    return {
      ...
      install: function (api) {
        if (timelineManager && typeof timelineManager.bindLifecycle === 'function') timelineManager.bindLifecycle(api);
        api.addPresenter(present);
      },  // S11-b-ex:返回 use-able 插件 → engine.use(createSvgPresenter(opts));已删 .plugin
      present: present,
