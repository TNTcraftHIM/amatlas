...
> **0.x 测试版本**:1.0 前契约 / 接口可能有较大变动,甚至大型重构,升级不保证平滑;破坏性变更以 CHANGELOG.md 为准。

本文件给 Codex 等 AI 工具读，帮你用 Amatlas 引擎做互动游戏，**不依赖 Claude Code 专属机制**（无斜杠命令、无 hooks）。Claude Code 用户请看 `CLAUDE.md`；本文件与之互补，不重复。

> 说明:本文引用的作者手册在 `.agents/skills/`——Codex 原生从 cwd 向上扫描的 skill 位置(可用 `/skills` 列出、隐式匹配)。它是 `.claude/skills/`(Claude Code 单一真相源)的**自动生成镜像**,两者都随包、内容等价;手册正文里偶见的 `.claude/...` 交叉引用在包内同样可解析。不要手改 `.agents/skills/`。
...
| 过场演出（时间轴） | `'cutscene'` | 常规语义入口 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`；正式范例 `examples/cutscene-demo/`，手册见 `modules/cutscene/references/cutscene-authoring.md` |
| 第一人称迷宫 / 怪物追逐 / 第一人称 FPS | 自定义 `'maze3d'` | `examples/maze3d/` 的 basic/horror/puzzle/layers/fps-range 五个 recipes；先读 `examples/maze3d/references/maze3d-authoring.md` |
...
富过场优先把 actor / anchor / shot / action 交给公共 compiler；低层 `beat.motion` / `beat.cast` / `beat.speaker` 是专家 escape hatch，只在 beat 边界与高层编译结果混排。对象形 typewriter 需要 timeline + DOM；写 motion/cast/speaker 还必须加载 SVG、保留 `#scene`，且不能在 manifest 里设 `present.svg:false`。正式顺序照 `examples/cutscene-demo/index.html`：core runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game。

`maze3d` 是已经随包提供的**自定义 runtime**，不是“引擎没有的玩法”，也不是 boot 内置模块。做 FPS 时照 `fps-range` Recipe 5 的 v3 闭合 `maze.combat` 数据面，只写 `player/loadout/equipped/guard/pickups`，武器 kind 只用固定 `precision` / `scatter`，表现由 `maze.theme` 自动驱动；把 `examples/maze3d/raycast-maze.js` 复制到 `src/`、index.html 引它并保留 `#maze3d-stage`，再在 game.js 用 `manifest.modules:[A.Maze3d.createMaze3dModule({stageId:'maze3d-stage'})]` 显式装配。不要另造 `fps` 模块，也不要只写 kind 后指望 boot 自动拉取。
...
| `modules/cutscene/references/cutscene-authoring.md` | 过场演出写法（高层 compilePerformance / 专家低层 beat / 逐拍快进 / 状态一致性） |
