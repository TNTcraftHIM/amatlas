...
富过场的脚本也要按作者语义闭合：对象形 typewriter 需要 `present-timeline.js` + DOM；写 `motion/cast/speaker` 还必须加载 `present-svg.js`、保留 `#scene`，且不能在 manifest 里设 `present.svg:false`。正式顺序照 `examples/cutscene-demo/index.html`。
