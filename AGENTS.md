...
`maze3d` 是已经随包提供的**自定义 runtime**，不是“引擎没有的玩法”，也不是 boot 内置模块。做 FPS 时照 `fps-range` Recipe 5 的闭合 `maze.combat` 数据面，只写 `player/pistol/guard/supplies` 定稿形状；把 `examples/maze3d/raycast-maze.js` 复制到 `src/`、index.html 引它并保留 `#maze3d-stage`，再在 game.js 用 `manifest.modules:[A.Maze3d.createMaze3dModule({stageId:'maze3d-stage'})]` 显式装配。不要另造 `fps` 模块，也不要只写 kind 后指望 boot 自动拉取。
