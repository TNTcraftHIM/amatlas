...
细节演出的默认作者入口是 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`。作者写 actor、anchor、shot 与语义动作，compiler 确定性排程并 lower 成现役 `beats`；不要先手算骨骼角度、关键帧或文字时间。下面是唯一的最小高层正例，可复制到正式 demo 的 `world.js` 中与其 `makePerformanceStaticRig()` 配套：
...
compiler 会 fail-loud。典型错误码只需先认识这五类：
...
高层 compiler 必须在 timeline 后、world 前加载；正式完整顺序如下，不要恢复旧的 runtime → DOM → timeline → SVG 顺序：
...
`presenters/present-timeline.js` 负责 schema 校验、秒到整数毫秒编译、纯采样与唯一 playback manager；它自身不操作 DOM。内置 SVG/DOM presenters 注册为同一 manager 的 consumer，共享一张表现层 rAF：SVG patch motion/cast stage，DOM patch typewriter；cutscene 自己的拍推进时钟仍是另一项职责，不因 stage 动画新增时钟。脚本闭包按作者语义配套：对象形 typewriter 需要 timeline + DOM；声明 motion/cast/speaker 还必须加载 SVG、保留 `#scene` 且不得用 `present.svg:false` 禁用。正式顺序统一为 core runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game。缺 `Intl.Segmenter` 或 timeline 会由 runtime fail-loud；官方 A.boot 路径若声明 motion/cast 却缺 SVG executor/root，则由 assembly-probe 作为 P0 拒绝，不再让文字正常而角色静默消失。
