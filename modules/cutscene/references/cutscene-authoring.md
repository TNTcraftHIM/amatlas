...
## 常规语义入口：`compilePerformance`

细节演出的默认作者入口是 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`。作者写 actor、anchor、shot 与语义动作，compiler 确定性排程并 lower 成现役 `beats`；不要先手算骨骼角度、关键帧或文字时间。下面是唯一的最小高层 spec 正例；它**不是独立运行页**，必须与正式 demo 同文件的 `makePerformanceStaticRig()` / profile 及下方完整脚本装配配套。可直接运行的起点是复制 `examples/cutscene-demo/{world.js,game.js,index.html}` 再删减内容：

```js
var performance = Amatlas.CutsceneAuthoring.compilePerformance({
  version: 1,
  actors: [{
    id: 'guide',
    rig: makePerformanceStaticRig(),
    profile: {
      kind: 'cutout-biped-v1', authoredFacing: 'right', root: 'torso',
  ...
  anchors: [
    { id: 'start', x: 96, y: 108, facing: 'right' },
    { id: 'mark', x: 160, y: 108, facing: 'right' },
    { id: 'beacon', x: 250, y: 64, capacity: 0 }
  ],
  shots: [{
    id: 'walk_and_point',
    cast: [{ actor: 'guide', at: 'start' }],
    sequence: [
      { do: 'walkTo', actor: 'guide', to: 'mark', dur: 2.4, steps: 4 },
      { do: 'pointAt', actor: 'guide', target: 'beacon', hand: 'right', dur: 0.6 }
    ]
  }]
});

var intro = {
  kind: 'cutscene', title: '引路', beats: performance.beats,
  links: [{ to: 'harbor', label: '继续' }]
};
```

这里有意不展开完整 Rig。直接看 `examples/cutscene-demo/world.js` 获取可运行的 `cutout-biped-v1` 静态 Rig、`say` / `caption` 与装配全貌；本手册不复制另一份巨型 Rig。`say` 和 `caption` 都申请 `global.text`，因此整 shot 独占；多轮对话要拆成多个 shot，不要把两份正文并行塞进同一 shot。

闭合动作与编排形状速查：

```text
enter / walkTo / lookAt / pointAt / say / caption / wait / exit
sequence:[action, ...]
parallel:[action, ...]  // 只允许一层、join-all；同资源冲突会 fail-loud
```

compiler 会 fail-loud。典型错误码先认识这六类：

| 错误码 | 先检查什么 |
|---|---|
| `PERF_SCHEMA` | 字段、闭合形状、时间精度或有限数 |
| `PERF_REF` | actor / anchor / profile part 引用是否存在 |
| `PERF_CAPABILITY` | Rig 是否能由 profile 结构性证明该动作能力 |
| `PERF_STATE` | actor 是否 onstage、朝向与 enter / exit 时序是否合法 |
| `PERF_CONFLICT` | parallel 资源、anchor capacity 或 shot-exclusive 文本是否冲突 |
| `PERF_LIMIT` | 单 shot action 数、总时长或其它有界预算是否超限 |

按 `error.path` 定位作者字段，再读 `error.detail` 的结构化冲突双方或期望值后修数据；不要绕过、吞掉或改写错误码。单 shot 最多 `512` 个 action（parallel 子 action 逐个计数），更长编排拆 shot。完整动作规则与资源表由 compiler 错误和 focused tests 维护，本手册只教稳定心智。

### 脚本闭包与验证

高层 compiler 必须在 timeline 后、world 前加载；混合 `scene + cutscene` 的依赖主干如下。`text-adventure/runtime/renderer.js` 必须在 boot 前可用，否则正式 world 的普通 `scene` 无 owner、启动即失败：
...
<script src="../../presenters/present-timeline.js"></script>
<script src="../../modules/cutscene/authoring/compile-performance.js"></script>
...
其中 core → text-adventure → cutscene → DOM → timeline → compiler → SVG → world → boot → game 是**玩法依赖主干**，不是现役 R·03 的完整脚本清单；上面 17 段才与 `examples/cutscene-demo/index.html` 完整对账。删 manifest 中某项能力时，才可同步删对应 presenter/plugin。

在引擎根目录至少跑 focused compiler test 与三道闸：

```bash
node modules/cutscene/test/compile-performance.test.cjs
node core/tooling/graph-audit.mjs examples/cutscene-demo/world.js
node core/tooling/assembly-probe.mjs examples/cutscene-demo/index.html
node pipeline/build/build.mjs examples/cutscene-demo/index.html
```

## 专家低层 escape hatch（按拍）

`beat.motion` / `beat.cast` / `beat.speaker` 是专家 escape hatch，适合高层动作包尚不能表达的单拍特例。高层编译结果与低层 beat 只在 beat 边界混排；单拍内不要同时维护两套作者真相。下面保留现役低层起步写法，供这种有意特例使用：
...
`presenters/present-timeline.js` 负责 schema 校验、秒到整数毫秒编译、纯采样与唯一 playback manager；它自身不操作 DOM。内置 SVG/DOM presenters 注册为同一 manager 的 consumer，共享一张表现层 rAF：SVG patch motion/cast stage，DOM patch typewriter；cutscene 自己的拍推进时钟仍是另一项职责，不因 stage 动画新增时钟。脚本闭包按作者语义配套：对象形 typewriter 需要 timeline + DOM；声明 motion/cast/speaker 还必须加载 SVG、保留 `#scene` 且不得用 `present.svg:false` 禁用。混合玩法依赖主干统一为 core runtime → text-adventure runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game；现役 R·03 的完整 presenter/plugin 清单仍以本页“脚本闭包与验证”和正式 `index.html` 为准。缺 `Intl.Segmenter` 或 timeline 会由 runtime fail-loud；官方 A.boot 路径若声明 motion/cast 却缺 SVG executor/root，则由 assembly-probe 作为 P0 拒绝，不再让文字正常而角色静默消失。
