...
- exit 只是视觉效果，不写 state、不执行 `run`、不发完成事件，也不门控「▸」或末拍出口。逐拍快进可以直接跳过剩余退场；对象形正文的“立即显示”只完成文字揭示，不换 playback key，退场时钟继续。
...
5. **文字节奏与动画数据**
   ...
   - 当前对象形文本已由 `present-dom` 逐字绘制，并与 SVG motion/cast 共用表现层 playback manager；标准 `▸` 在揭示未完成时先立即显示全文，下一击才换拍。不要另写第二套打字机、按钮或 rAF。
