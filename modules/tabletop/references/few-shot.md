...
## 5. 资源改变后续 tier + 部分成功 + 结果档

正式 R·02 的收口写法：可选准备花资源，后续 `advantage` / `bonus` 读取**同一个**角色卡池；警报抬高动态 DC，只差一点时走带代价的 `partial`。

```js
{ id:'scan', label:'凝神感知周围', skill:'感知', dc:6, dice:'2d6',
  cost:{res:'状态', amount:1},
  available:function(S){ return !S.flags.scanned; },
  success:{text:'状态 −1。你看懂了结构。', set:{scanned:true, knows:true}},
  fail:{text:'状态 −1。仍没看懂。', set:{scanned:true}} }

{ id:'force', label:'撬开舱门', skill:'体魄', dc:9, dice:'2d6',
  cost:{res:'状态', amount:0},
  advantage:function(S){ return !!S.flags.knows; },
  success:{text:'安静撬开。', set:{gateResolved:true, gateOpen:true}, to:'core'},
  fail:{text:'砸开并触发警报。', set:{gateResolved:true, alarmed:true}, clock:2, to:'core'} }

{ id:'talk', label:'与 AI 谈判', skill:'交涉', dice:'2d6',
  dc:function(S){ return S.flags.alarmed ? 11 : 8; },
  bonus:function(S){ return S.sheet.resources.状态 >= 3 ? 1 : 0; },
  partialBand:1,
  success:{text:'它信任了你。', set:{aiTrusted:true}, to:'peace'},
  partial:{text:'只开放应急接管。', set:{emergencyOverride:true}, clock:1, to:'force'},
  fail:{text:'谈判破裂。', to:'force'} }
```

- `partial` 是 `total<DC` 但落在 `[DC-partialBand, DC)` 的带代价出路；不是普通成功。完整设计边界见 `tabletop-design.md` §6。
- `bonus` 读 `S.sheet.resources.状态`，不要另造顶层同名资源；删掉资源读侧必须会改变可观察 tier，否则资源只是装饰数字。
- 有 `to` 的检定先在源节点显示结果，本帧只给「继续 →」。`_ttPending`、资源、flags、clock、RNG 会随档；骰值/check 行/outcome 是瞬时帧，不跨读档复放。
- 因此源节点的 `look(S)` 应读取 `gateOpen/alarmed/aiTrusted/emergencyOverride` 等 durable facts。玩家在结果帧存档、读回后仍看得懂刚发生了什么，同时不会重复掷骰或扣费。
