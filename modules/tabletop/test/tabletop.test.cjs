...
(function () {
  ...
  ok('L2 check 的命令式后果别名全族 → 抛(后果不再被静默忽略)', ['onSuccess', 'onFailure', 'on_success', 'on_failure', 'on_succeed', 'on_fail', 'modifiers'].every(function (field) {
    var check = { id: 'b', label: '撬门', skill: '体魄', dc: 8, dice: '2d6' };
    check[field] = { text: '不会生效' };
    return throws(function () { engChecks([check]).view(); });
  }));
