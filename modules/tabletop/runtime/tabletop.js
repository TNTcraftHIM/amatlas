    ...
    var mod = {
      ...
      actions: function (state, node) {
        ...
        (node.checks || []).forEach(function (c, idx) {
          ...
          if (c.onSuccess != null || c.onFailure != null || c.on_success != null || c.on_failure != null || c.on_succeed != null || c.on_fail != null || c.modifiers != null) {
            throw new Error('[amatlas] tabletop:检定「' + c.label + '」用了非契约后果字段 onSuccess/onFailure/on_success/on_failure/on_succeed/on_fail/modifiers,但检定后果字段是 success/fail 对象(如 success:{flag:"x",text:"…",clock:1});这些别名会被忽略 → 检定无任何后果。改用 success/fail。');
          }
