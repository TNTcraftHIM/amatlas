    ...
    var mod = {
      ...
      actions: function (state, node) {
        ...
        (node.checks || []).forEach(function (c, idx) {
          ...
          if (c.success && ((c.success.flag && state.flags[c.success.flag]) || (c.success.set && Object.keys(c.success.set).length && Object.keys(c.success.set).every(function (sk) { return state.flags[sk]; })))) {                  // 已成功(`flag` 已置 **或** `set` 的键都已置)→ 不再重掷。set 写 state.flags 同 flag(:136),守卫一致对待——治「成功只写 success.set〔无 flag〕的检定 cost-bearing 却无 flag/to 守卫 → 通过后仍可无限重点扣资源」软锁(showcase《逝音录》decode 实锤:success.set:{filesDecrypted}+cost 精力、可刷空精力软锁后续结局)
