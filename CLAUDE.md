...
- **`maze3d` 私有 runtime** — 第一人称迷宫/追逐/FPS 已有 `examples/maze3d/` 的 basic/horror/puzzle/layers/fps-range 五个 recipes；它不是 boot 内置或公共模块。FPS v3 只接受固定 `precision` / `scatter` 与 `player/loadout/equipped/guard/pickups`，完整表现由 `maze.theme` 驱动。先走 `/new-game` / skill 类型路由并读 `examples/maze3d/references/maze3d-authoring.md`，显式复制 runtime + stage + `manifest.modules`，不要误判成引擎没有的玩法。
