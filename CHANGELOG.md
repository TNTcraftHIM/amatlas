...
本批继续打磨随包的可玩套件与正式作者旅程，并修正插件模态和键盘焦点接缝；公共 core、`module-interface.md`、内置模块 runtime、presenter、boot、build、存档格式与公共作者 API 均不变。Maze3D 与横版新增作者字段继续属于 example-private 数据面。

### 新增
- **横版表现 profile 与两客户闭环**：唯一私有横版 runtime 现提供 `coast` / `frost` 两套闭合程序化背景、地形、角色/目标轮廓、反馈与 SFX；两款作品继续共用同一 fixed-tick 玩法实现，并补齐 source/built、桌面/390px、死亡重开与清关出口证据。
- **Maze3D 选材与可玩套件**：Visual Gallery 增加六张协调 recipe 与五张 FPS theme 卡；正式世界把协调视觉数据 dogfood 到现役节点，并收口资源绕行 FPS、三线索分支谜题和安静支路→动态唤醒→持钥回程的追逐逃生闭环。全部能力仍由既有私有 `raycast-maze.js` 与 `maze.theme` 数据面拥有，不新增公共 FPS/Maze 模块。

### 改进
- **五份正式 reference 作者/玩家旅程**：Tabletop 以资源准备/保留形成两条固定 seed 的 fail-forward 后果路线；Arcade 明确正式五果、三次失败分流与 abort；Cutscene 用高层 performance compiler 走到末拍并回接普通 scene；Minimal 证明 live custom module→core exit→普通 scene；Horror 证明手写 `createEngine + use`、显式音频停止、terminal 持久化/reset 与纯 DOM 退化。Showroom、README、Claude/Codex 作者入口和 source/built/移动端回归与这些真实路径对账。
- **插件模态与键盘焦点**：Save、Minimap、Inventory、Achievement 改用真实 sibling backdrop，可靠拦住面板外输入并保持面板可操作；浮窗补 `box-sizing:border-box`，Reset 不再以按钮样式抢占共享面板样式 ID。场景标题保留程序焦点但只在键盘 `:focus-visible` 时显示清晰焦点环，并留出滚动边距。

### 修正
- **Horror reset 文案**：未挂手动 SavePlugin 的手写 Horror reference 不再错误声称“手动存档槽保留”。

### 工程
- **正式旅程与反向变异**：七种 focused browser 模式互斥 fail-closed；新增 Maze3D 套件及五份 reference 的 formal、graph、assembly、build、source/built/Showroom/mobile/save/reset/reload/reduced-motion 证据，不以私有 `_engine` 探针代替公开玩家事实。
...
[0.4.4]: https://github.com/TNTcraftHIM/amatlas/releases/tag/v0.4.4
