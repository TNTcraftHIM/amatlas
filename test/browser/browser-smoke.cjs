...
(async () => {
  ...
  try {
    ...
    // ── 场景 4:FPS 玩家直达页——ready 等待→拾弹→冷却回执→两枪→D；阵亡→一键重试 fresh ──
    {
      ...
      const errors = await withPage(browser, async (page) => {
        ...
        ok('S10 FPS built 直达入口未 ready 长等待仍保持 fresh/静止', /待命/.test(initialHint) && /生命 60\/60/.test(waitingHint) && /弹药 1/.test(waitingHint) && /敌人 1/.test(waitingHint) && !/攻击预兆|受伤|阵亡/.test(waitingHint), initialHint + ' → ' + waitingHint);
        ...
        ok('S11 FPS built 公开移动拾取 +3 弹药', /弹药 4/.test(pickupHint) && /\+3 弹药/.test(pickupHint), initialHint + ' → ' + pickupHint);
        if (!/弹药 4/.test(pickupHint) || !/\+3 弹药/.test(pickupHint)) throw new Error('FPS built 正向闭环未真实触发 +3 弹药拾取:' + pickupHint);
        ...
        ok('S12 FPS built 第一枪真实耗弹并命中守卫', /弹药 3/.test(firstShotHint) && /敌人 1/.test(firstShotHint) && /命中/.test(firstShotHint), firstShotHint);
        if (!/弹药 3/.test(firstShotHint) || !/敌人 1/.test(firstShotHint)) throw new Error('FPS built 第一枪未进入公开命中状态:' + firstShotHint);
        await fireButton.click();
        await page.waitForTimeout(80);
        const cooldownHint = await page.locator('#maze3d-stage .amatlas-maze-hint').innerText();
        ok('S13 FPS built cooldown press 不耗弹且显示确定性回执', /弹药 3/.test(cooldownHint) && /敌人 1/.test(cooldownHint) && /手枪回位中/.test(cooldownHint), cooldownHint);
        if (!/手枪回位中/.test(cooldownHint) || !/弹药 3/.test(cooldownHint)) throw new Error('FPS built cooldown press 未给公开回执或错误耗弹:' + cooldownHint);
        await page.waitForTimeout(320);
        await fireButton.click();
        await page.waitForTimeout(180);
        const killedHint = await page.locator('#maze3d-stage .amatlas-maze-hint').innerText();
        ok('S14 FPS built 冷却后第二枪真实击倒最后守卫', /弹药 2/.test(killedHint) && /敌人 0/.test(killedHint) && /守卫.*已击倒/.test(killedHint), killedHint);
        if (!/敌人 0/.test(killedHint)) throw new Error('FPS built 第二枪未清场:' + killedHint);
        ...
        ok('S15 FPS built 真实拾弹两枪后同一 D 解锁并收口 maze session', beforeDoor.canvas && !beforeDoor.exitEnabled && opened.exitEnabled && opened.canvasStopped && /敌人 0/.test(killedHint), JSON.stringify({ beforeDoor, opened }));
        ...
        await page.waitForFunction(() => {
          const hint = document.querySelector('#maze3d-stage .amatlas-maze-hint');
          return document.querySelector('#maze3d-stage canvas') && hint && /待命/.test(hint.textContent) && /生命 60\/60/.test(hint.textContent) && /弹药 1/.test(hint.textContent) && /敌人 1/.test(hint.textContent);
        }, null, { timeout: 8000 });
        const retryFresh = await page.locator('#maze3d-stage .amatlas-maze-hint').innerText();
        ok('S18 FPS built 一键重试同节点重建 fresh 60 HP/1 ammo/1 guard/2 supplies', /待命/.test(retryFresh) && /生命 60\/60/.test(retryFresh) && /弹药 1/.test(retryFresh) && /敌人 1/.test(retryFresh) && /Recipe 5/.test(await page.locator('#place').innerText()), retryFresh);
      });
        ...
        await page.addInitScript(() => {
          ...
          Object.defineProperty(window, 'MAZE3D_WORLD', {
            ...
            set(world) {
              ...
              node.maze.combat.player.ammo = 9;
              node.maze.combat.guard = {
                x: 4, y: 2, face: 'skull', hp: 100, hitRadius: 0.23,
                ai: { sight: 9, hear: 8, attackRange: 0.50, moveSpeed: 1.1, damage: 20, windup: 0.10, cooldown: 9 }
              };
              node.maze.combat.supplies = [];
              delete node.maze.combat.player.maxAmmo;
              node.maze.events = [{
