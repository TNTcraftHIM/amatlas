  ...
  try {
    // ── 场景 1:showroom embedded Gallery——打开真实子页、关闭卸载、零页面错误 ──
    {
      ...
      const errors = await withPage(browser, async (page) => {
        ...
        await visualFrame.locator('canvas').nth(91).waitFor({ state: 'visible', timeout: 8000 });
        const visualPreview = await visualFrame.locator('body').evaluate(() => ({
        ...
        ok('S3a maze3d Visual Gallery 的 92 个静态 canvas 不进 Tab 顺序、不抢关闭按钮焦点、不把 iframe 滚到页尾',
          visualPreview.canvases === 92 && visualPreview.tabbableCanvases === 0 && visualPreview.activeTag === 'BODY' && visualPreview.scrollY === 0 && visualHostFocus === 'demoport-close',
    ...
    // ── 场景 1c:cutscene-demo Phase 3 dogfood——同一旅程覆盖 source + built ──
    for (const entry of [
      { label: 'source', url: sourceUrl('cutscene-demo') },
      { label: 'built', url: builtUrl('cutscene-demo') }
    ]) {
      const errors = await withPage(browser, async (page) => {
        ...
        const rigSample = () => page.evaluate(() => {
          ...
          const snap = window._engine && window._engine.view();
          const playback = snap && snap.view && snap.view.cutscenePlayback;
          return {
            key: document.querySelector('[data-amatlas-rig-cast="warden"]') && document.querySelector('[data-amatlas-rig-cast="warden"]').getAttribute('data-amatlas-rig-key'),
            torso: local('torso'),
            ...
            playbackKey: playback && playback.key,
            speaker: playback && playback.speaker,
            visible: spans.filter((node) => node.style.visibility !== 'hidden').length,
            total: spans.length,
            activeMouth: activeMouth
          };
        });
        const advanceToCompiledWalk = async () => {
          await nextPanel().click();       // #0 plain -> #1 low-level typewriter/motion.
          await completeAndAdvance();      // #1 -> #2 low-level escape-hatch motion.
          await nextPanel().click();       // #2 -> #3 compiled walk/point/caption.
        ...
        await nextPanel().waitFor({ state: 'visible', timeout: 8000 });
        await advanceToCompiledWalk();
        const walkStart = await rigSample();
        await page.waitForTimeout(1050);
        const walkMiddle = await rigSample();
        await page.waitForTimeout(1500);
        const pointMiddle = await rigSample();
        ok('S4j ' + entry.label + ' 高层 walk 真实经过起点/中点/终点且进入 point',
          walkStart.torso && walkStart.torso.x < 100 && walkMiddle.torso.x > 100 && walkMiddle.torso.x < 155 &&
          Math.abs(pointMiddle.torso.x - 160) < 1 && pointMiddle.arm.rotate < -20,
        ...
        const captionPartial = await rigSample();
        await nextPanel().click();
        const captionDone = await rigSample();
        ok('S4k ' + entry.label + ' point 收口且 caption 首击只补全文、不越拍',
          captionPartial.visible > 0 && captionPartial.visible < captionPartial.total &&
          captionPartial.label === '显示全部文字' && captionDone.visible === captionDone.total &&
          captionDone.playbackKey === 'coast/intro#3' && captionDone.arm.rotate < -70 &&
          captionDone.label === '继续 / 下一段' &&
          /守灯人沿退潮线走到潮标旁，抬手指向雾里的灯塔。/.test(captionDone.text),
        ...
        await page.waitForFunction(() => {
          ...
          const mouth = document.querySelector(
            '[data-amatlas-rig-part="mouth"] [data-amatlas-rig-state="A"][display="inline"], ' +
            '[data-amatlas-rig-part="mouth"] [data-amatlas-rig-state="O"][display="inline"]'
          );
          return spans.length > 4 && visible > 0 && visible < spans.length && mouth;
        }, null, { timeout: 2600 });
        const sayPartial = await rigSample();
        const sameBeatSave = await page.evaluate(() => window._engine.serialize());
        await nextPanel().click();
        const sayDone = await rigSample();
        ok('S4l ' + entry.label + ' say 逐字中间态带 speaker mouth，首击只补全文',
          sayPartial.speaker === 'warden' && sayPartial.visible > 0 && sayPartial.visible < sayPartial.total &&
          sayPartial.activeMouth.some((state) => state === 'A' || state === 'O') &&
          sayDone.playbackKey === 'coast/intro#4' && sayDone.visible === sayDone.total &&
          sayDone.label === '继续 / 下一段' &&
          await page.locator('[data-amatlas-rig-part="mouth"] [data-amatlas-rig-state="A"]').count() === 1,
          JSON.stringify({ sayPartial, sayDone }));

        const restored = await page.evaluate((save) => {
          const loaded = window._engine.load(save);
          const inspect = window.Amatlas.Timeline.playback.inspect();
          const look = document.querySelector('#look[data-cutscene-next]');
          const cast = document.querySelector('[data-amatlas-rig-cast="warden"]');
          const snap = window._engine.view();
          return {
            loaded,
            key: snap.view.cutscenePlayback && snap.view.cutscenePlayback.key,
            pos: snap.pos,
            text: (look || {}).textContent || '',
            label: look && look.getAttribute('aria-label'),
            consumers: inspect.consumerCount,
            running: inspect.running,
            cast: !!cast
          };
        }, sameBeatSave);
        await page.waitForTimeout(260);
        const restoredProgress = await rigSample();
        ok('S4m ' + entry.label + ' 同节点 restore 从 #0 重播并清旧 compiler stage/consumer',
          restored.loaded && restored.key === undefined && restored.pos.node === 'intro' &&
          /黑潮退去时/.test(restored.text) && restored.label === '继续 / 下一段' &&
          !restored.cast && restored.consumers === 0 && !restored.running &&
          restoredProgress.playbackKey === undefined && restoredProgress.key === null && restoredProgress.total === 0,
          JSON.stringify({ restored, restoredProgress }));

        await page.emulateMedia({ reducedMotion: 'reduce' });
        await advanceToCompiledWalk();
        const reducedWalk = await rigSample();
        await completeAndAdvance();
        await page.waitForFunction(() => document.querySelector('[data-amatlas-rig-key$="#4"]'), null, { timeout: 5000 });
        const reducedSay = await rigSample();
        await completeAndAdvance();         // #4 -> #5。
        await nextPanel().click();          // #5 -> #6 hold；末拍已暴露作者 links。
        await page.locator('#choices > button.choice').filter({ hasText: '重看序章' }).click();
        await page.waitForFunction(() => {
          const look = document.querySelector('#look[data-cutscene-next]');
          return window._engine && window._engine.state.pos.node === 'intro' && look &&
            /黑潮退去时/.test(look.textContent || '') && !document.querySelector('[data-amatlas-rig-cast]');
        }, null, { timeout: 5000 });
        const replay = await rigSample();
        const replayTimeline = await page.evaluate(() => window.Amatlas.Timeline.playback.inspect());
        ok('S4n ' + entry.label + ' reduced-motion 给稳定 poster；同节点 replay 清旧 session',
          reducedWalk.torso && Math.abs(reducedWalk.torso.x - 160) < 1 && reducedWalk.arm.rotate < -70 &&
          reducedWalk.visible === reducedWalk.total && reducedWalk.label === '继续 / 下一段' &&
          /守灯人沿退潮线走到潮标旁，抬手指向雾里的灯塔。/.test(reducedWalk.text) &&
          reducedSay.speaker === 'warden' && reducedSay.visible === reducedSay.total &&
          /第三次回潮前，跟紧这道光。/.test(reducedSay.text) && reducedSay.label === '继续 / 下一段' &&
          replay.playbackKey === undefined && replay.key === null && replayTimeline.key === null,
          JSON.stringify({ reducedWalk, reducedSay, replay, replayTimeline }));
      });
      ok('S4o cutscene-demo ' + entry.label + ' 高层旅程零页面错误/console.error/requestfailed', errors.length === 0, errors.join(' | '));
    }
    ...
    // ── 场景 6:sidescroller 子步 A——built 跑、跳、卷屏与 coarse 控件 ──
    {
      ...
      const errors = await withPage(browser, async (page) => {
        ...
        await page.waitForFunction(() => /目标已解除/.test((document.querySelector('.amatlas-sidescroller-hud') || {}).textContent || '') && Array.prototype.slice.call(document.querySelectorAll('#choices button')).some((node) => /穿过解除封锁/.test(node.textContent)), null, { timeout: 5000 });
        ...
        ok('S24 sidescroller built 三枪击毁固定哨戒炮后单写 clear 并暴露标准出口', cleared.flag && /清关出口已开放/.test(cleared.hud) && cleared.choices.some((text) => /穿过解除封锁/.test(text)) && cleared.choices.some((text) => /撤离试验段/.test(text)), JSON.stringify(cleared));
      });
    ...
    for (const entry of [
      ...
      const errors = await withPage(browser, async (page) => {
        ...
        const playerTop = () => page.locator('#sidescroller-stage canvas').evaluate((canvas) => {
          ...
          for (let y = 0; y < canvas.height; y++) for (let x = 0; x < canvas.width; x++) {
            ...
            if (data[i] === 224 && data[i + 1] === 170 && data[i + 2] === 99 && data[i + 3] === 255) { top = Math.min(top, y); }
          }
        ...
        await page.waitForFunction(() => /目标已解除/.test((document.querySelector('.amatlas-sidescroller-hud') || {}).textContent || '') && Array.prototype.slice.call(document.querySelectorAll('#choices button')).some((node) => /货运升降台/.test(node.textContent)), null, { timeout: 6000 });
      ...
      const restartErrors = await withPage(browser, async (page) => {
        ...
        ok('S29a frostline ' + entry.label + ' 死亡时held触控未释放也在重开边界清零', /生命 4 \/ 4/.test(restarted.hud) && restarted.x === 51, JSON.stringify(restarted));
