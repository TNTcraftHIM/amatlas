...
var runnerSource = fs.readFileSync(RUNNER, 'utf8');
...
function run(file, timeout) {
  ...
  // Windows 满载时 taskkill /t 的进程树清理可能跨过 3s；外层 watchdog 只负责
  // 防测试本身永久挂住，必须明显宽于被测 runner 的 50/80ms 内层阈值和清理尾延迟。
  var r = spawnSync(process.execPath, [RUNNER], { encoding: 'utf8', env: env, timeout: 10000 });
  return { status: r.status, signal: r.signal, error: r.error, out: (r.stdout || '') + (r.stderr || '') };
}

console.log('test runner 超时验证');
ok('A0 maze 完整变异套件保留 600s 局部预算，其它测试仍守默认预算',
  /examples\/maze3d\/test\/raycast-maze\.test\.cjs', 600000/.test(runnerSource) &&
    /process\.env\.ATLAS_TEST_TIMEOUT_MS \? TEST_TIMEOUT_MS : \(t\[2\] \|\| TEST_TIMEOUT_MS\)/.test(runnerSource),
  'maze budget/local override missing');
