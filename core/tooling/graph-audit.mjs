...
export function auditWorld(world) {
  ...
  for (const mapId of Object.keys(maps)) {
    ...
    for (const nodeId of Object.keys(nodes)) {
      ...
      const nestedBadChecks = [];
      if (Array.isArray(node.checks)) {
        node.checks.forEach((check, checkIndex) => {
          if (!check || typeof check !== 'object') return;
          ['on_success', 'on_failure', 'on_succeed', 'on_fail', 'modifiers'].forEach((field) => {
            if (Object.prototype.hasOwnProperty.call(check, field)) nestedBadChecks.push(`checks[${checkIndex}].${field}`);
          });
        });
      }
      const badCheckPaths = badCheck.concat(nestedBadChecks);
      if (badCheckPaths.length) {
        issues.push(`[可疑][P1] 跑团检定格式错:节点 '${key}' 用了非契约字段 ${badCheckPaths.map((f) => '`' + f + '`').join('/')}(引擎不认 → 检定静默失效、节点常显不可达)。`
      ...
      // cutscene 专属静态牙(R2 二轮 P0):kind:'cutscene' 节点用 exits 声明出口 → 引擎首次进入即抛(cutscene parseBeats fail-loud);
      //   但运行时抛只在【真正走到该分支】才现,而 assembly-probe/smoke 贪心只走首个分支 → 坏成品可过 graph/probe/build/smoke 四闸、
      //   却对端玩家在一条可达分支硬崩。与既有对称兄弟(exit 写 link 字段 :162 / link 写 available :186)同款,把运行时 fail-loud 提前到静态期。
      if (node.kind === 'cutscene' && Array.isArray(node.exits) && node.exits.length) {
        issues.push(`[确认][P0] cutscene 出口放错字段:节点 '${key}'(kind:'cutscene')用了 exits 声明出口 —— cutscene 的出口必须写进 links(引擎首次进入该节点即抛;自动游玩贪心走首分支常测不到,上线后玩家点到该分支才崩)。把出口改成 links:[{to,label,requires?}](available→requires)。`);
      }
      ...
      for (const m of crawlerIssues(key, node)) issues.push(m);
...
function maze3dIssues(key, node) {
  const out = [];
  if (!node || node.kind !== 'maze3d') return out;   // 只认 maze3d。此前的「或 node.maze 有 grid」析取会把 crawler(kind:'maze')的开放词汇 grid 误套 '#.DK' 白名单(全部真 maze3d 节点都带 kind:'maze3d' → 删此析取零覆盖损失,只除掉对 crawler 的误伤);crawler 数据校验改由 crawlerIssues 按其自身格式做。
...
// ---------- crawler(kind:'maze')数据校验:按 crawler **自身**格式 fail-loud(≠ maze3d 的 '#.DK' 闭词汇)----------
// crawler 运行时(crawler/runtime/crawler.js)对坏 node.maze 只静默退化(:168 `if(!node||!node.maze)return;`、
//   render/actions 到处 `maze.grid||[]` / `maze.start||{...}`)→ 缺/错数据不抛,玩家进去只能原地转向、永远走不出去
//   =确定性 soft-lock。maze3dIssues 收紧到 kind==='maze3d' 后 crawler 不再被误拦、但也没了任何静态闸——补这一条。
// crawler 词汇:grid 只有 '#'=墙、其余任意字符=地板+内容标记(isFloor: cellAt!=='#',crawler.js:50);内容/出口由
//   node.maze.cells['x,y'] 驱动(exit.to 可达性已由 outEdges 读 cells 覆盖,不在此重复)→ 本闸只查「grid 形状 + start 落点」。
function crawlerIssues(key, node) {
  const out = [];
  if (!node || node.kind !== 'maze') return out;
  const p0 = (msg) => out.push(`[确认][P0] 迷宫(crawler)数据错误:节点 '${key}' ${msg}`);
  const p1 = (msg) => out.push(`[可疑][P1] 迷宫(crawler)可疑数据:节点 '${key}' ${msg}`);
  const isInt = (v) => Number.isInteger(v);
  const dirOk = (v) => typeof v === 'string' && /^[NESW]$/i.test(v);   // letterToDir 大小写不敏感(crawler.js:31)

  const maze = node.maze;
  if (!maze || typeof maze !== 'object' || Array.isArray(maze)) {
    p0("kind:'maze' 节点必须有 node.maze 对象(含 grid);缺了 runtime 只静默退化成整墙、玩家原地转向永远走不出去(确定性 soft-lock,原本四闸全判绿)。");
    return out;
  }
  const grid = maze.grid;
  if (!Array.isArray(grid) || !grid.length) {
    p0("maze.grid 必须是非空字符串数组(每行一个字符串:'#'=墙,其余任意字符=地板+内容标记)。缺/空 grid 时 runtime 静默退化成整墙 → 确定性 soft-lock。");
    return out;
  }
  let W = null, shapeBad = false;
  for (let y = 0; y < grid.length; y++) {
    ...
    if (typeof row !== 'string' || !row.length) { p0(`maze.grid[${y}] 必须是非空字符串。`); shapeBad = true; continue; }
    if (W == null) W = row.length;
    else if (row.length !== W) { p0(`maze.grid 必须是矩形:第 ${y} 行宽 ${row.length},应为 ${W}。短行会被 runtime 当成隐形墙(cellAt 越界返 '#',crawler.js:46),请手工补齐而不是让引擎猜。`); shapeBad = true; }
  }
  if (shapeBad) return out;
  ...
  const inBounds = (x, y) => y >= 0 && y < H && x >= 0 && x < W;
  const cellAt = (x, y) => inBounds(x, y) ? grid[y][x] : '#';
  // crawler start 可省略(runtime 默认 (0,0,'N'),crawler.js:125-128)→ 按其默认取有效落点再校验。
  const st = (maze.start && typeof maze.start === 'object' && !Array.isArray(maze.start)) ? maze.start : {};
  const sx = st.x != null ? st.x : 0;
  const sy = st.y != null ? st.y : 0;
  if (!isInt(sx) || !isInt(sy)) p0(`maze.start.x/.y 必须是整数格坐标(省略则默认 0),得到 x=${JSON.stringify(sx)} y=${JSON.stringify(sy)}。`);
  else if (!inBounds(sx, sy)) p0(`maze.start (${sx},${sy}) 超出 grid 范围(${W}×${H})。`);
  else if (cellAt(sx, sy) === '#') p0(`maze.start (${sx},${sy}) 落在墙 '#' 上,玩家出生即卡在墙里、寸步难行。`);
  if (st.dir != null && !dirOk(st.dir)) p1(`maze.start.dir=${JSON.stringify(st.dir)} 不是 N/E/S/W;runtime 会退回默认朝向 N(crawler.js:32),作者朝向意图会丢失。`);
  ...
  for (const comp of sccs) {
    ...
    for (const k of comp) {
      ...
      for (const c of (Array.isArray(node.checks) ? node.checks : [])) {
        ...
        // 触发:任一「会被扣 cost、又不逃出口袋」的结果档 → grind-trap。fail 恒查(未声明=原地留在口袋);
        //   crit/partial/fumble 仅在**已声明**时查(未声明会降级到 fail、已由 fail 支覆盖;否则 leavesSCC(undefined)=false
        //   会把每个检定误报)。补对称接缝:line 689 取边 / line 728 anyEscape 都五档对称,原触发只独看 fail → 已声明 fumble/partial/crit 原地的口袋漏判(round: pocket_fumble)。
        const stranded = !leavesSCC(c.fail, mapId)
          || (c.crit && !leavesSCC(c.crit, mapId))
          || (c.partial && !leavesSCC(c.partial, mapId))
          || (c.fumble && !leavesSCC(c.fumble, mapId));
        if (stranded) { esc = { node: k, res: (c.cost && c.cost.res) || '资源', label: c.label || c.id || '' }; break; }  // 有档不逃 → grind-trap
      }
      if (esc) break;
    }
    if (esc) {
...
function deadStateKeyIssues(source, declared, world) {
  if (typeof source !== 'string' || !source) return [];
  source = source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');   // 先剥 JS 注释(对齐 deadFlagIssues):否则注释里的 `S.xxx` 被裸正则当真读 → 误报死 state 键;同族接缝加固。
  ...
  // maze3d 节点的 winKey/scareKey 由 runtime 在 canvas 内写(走到 D 门=winKey、被怪抓=scareKey,静态图看不见)→
  //   把【真 maze3d 节点】声明的 winKey/scareKey 视为"已写",避免外层 links.requires 读 S.X 被误报死 state
  //   (dogfood:承星者迷宫 winKey 写 corridorLit、外层场景门控读它)。**改对象级、仅 kind==='maze3d'**:原为对整份
  //   combined 的裸文本正则,任意节点/注释插一句同名 `winKey:'X'` 即抵消真 P0(红队实测 decoyMeta:{winKey:'…'} 或注释绕过)
  //   → 收紧到解析后 world 里【真 maze3d 节点】的顶层属性;与 deadFlagIssues 的 `flag:'X'` 家族同理但补齐 kind 作用域。
  if (world && world.maps) {
    for (const mp of Object.keys(world.maps)) {
      const nodes = (world.maps[mp] && world.maps[mp].nodes) || {};
      for (const nd of Object.keys(nodes)) {
        const node = nodes[nd];
        if (!node || node.kind !== 'maze3d') continue;
        if (typeof node.winKey === 'string' && node.winKey) written.add(node.winKey);
        if (typeof node.scareKey === 'string' && node.scareKey) written.add(node.scareKey);
      }
    }
  }
...
export function auditProject(world, worldSource, gameSource, hasGameJs) {
  ...
  audit.issues.push(...deadStateKeyIssues(combined, declared, world));
