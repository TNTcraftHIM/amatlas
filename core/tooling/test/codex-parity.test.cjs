...
try {
  ...
  (function () {
    ...
    ok('10b AGENTS 按能力名路由正式 example/skill/reference，不拥有客户端路径或复制命令模板',
      /examples\/text-adventure-demo\//.test(agents) && /text-adventure-game/.test(agents) && /references\//.test(agents) &&
    ...
    ok('10c2 AGENTS 测试版口径覆盖整个 0.x，不能把当前作者入口写死为 0.1.x',
      /\*\*0\.x 测试版本\*\*/.test(agents) && !/测试版本\(0\.1\.x\)/.test(agents));
    ...
    ok('10e 现役 cutscene 作者入口只教正文面板推进，不得恢复独立 ▸ 按钮口径',
      /首拍\/中间拍由正文面板承担逐拍即时快进/.test(skill) &&
      /DOM 不显示独立 `▸`/.test(skill) &&
      !/首拍\/中间拍(?:动作面)?只有「▸」/.test(skill));
    var claudeSkill = fs.readFileSync(path.join(engineRoot, '.claude', 'skills', 'text-adventure-game', 'SKILL.md'), 'utf8');
    var readme = fs.readFileSync(path.join(engineRoot, 'README.md'), 'utf8');
    var cutsceneRef = fs.readFileSync(path.join(engineRoot, 'modules', 'cutscene', 'references', 'cutscene-authoring.md'), 'utf8');
    var cutsceneHtml = fs.readFileSync(path.join(engineRoot, 'examples', 'cutscene-demo', 'index.html'), 'utf8');
    ok('10f 顶层路由到 cutscene reference，reference/skills/README 认领公共高层演出入口',
      /modules\/cutscene\/references\/cutscene-authoring\.md/.test(agents) &&
      [cutsceneRef, claudeSkill, skill, readme].every(function (source) {
        return /CutsceneAuthoring\.compilePerformance/.test(source) && /常规语义入口/.test(source) && /专家 escape hatch/.test(source);
      }));
    var formalProseOrder = /core runtime[^\n]*text-adventure runtime[^\n]*cutscene runtime[^\n]*DOM[^\n]*timeline[^\n]*compile-performance[^\n]*SVG[^\n]*world[^\n]*boot[^\n]*game/;
    var formalScriptOrder = /core\/runtime\/engine-core\.js[\s\S]*text-adventure\/runtime\/renderer\.js[\s\S]*cutscene\/runtime\/cutscene\.js[\s\S]*present-dom\.js[\s\S]*present-timeline\.js[\s\S]*compile-performance\.js[\s\S]*present-svg\.js[\s\S]*world\.js[\s\S]*boot\.js[\s\S]*game\.js/;
    var expectedCutsceneScripts = [
      '../../core/runtime/engine-core.js', '../../modules/text-adventure/runtime/renderer.js', '../../modules/cutscene/runtime/cutscene.js',
      '../../presenters/present-dom.js', '../../presenters/present-timeline.js', '../../modules/cutscene/authoring/compile-performance.js',
      '../../presenters/present-svg.js', '../../presenters/progressions.js', '../../presenters/compose-music.js', '../../presenters/present-audio.js',
      '../../plugins/save.js', '../../plugins/minimap.js', '../../plugins/achievement.js', '../../plugins/reset.js',
      'world.js', '../../preset/boot.js', 'game.js'
    ];
    var cutsceneScripts = Array.from(cutsceneHtml.matchAll(/<script\s+src="([^"]+)"/g), function (match) { return match[1]; });
    var cutsceneRefBlock = (cutsceneRef.match(/```html\s*([\s\S]*?)```/) || ['', ''])[1];
    var cutsceneRefScripts = Array.from(cutsceneRefBlock.matchAll(/<script\s+src="([^"]+)"/g), function (match) { return match[1]; });
    ok('10g cutscene reference 与 Claude/Codex skill 锁混合玩法十段依赖主干，reference/正式页锁同一17段完整顺序，AGENTS只路由',
      /正式装配顺序见该 reference/.test(agents) && formalScriptOrder.test(cutsceneRef) && formalProseOrder.test(claudeSkill) && formalProseOrder.test(skill) &&
      /不是现役 R·03.*完整脚本清单/.test(cutsceneRef) &&
      cutsceneScripts.length === expectedCutsceneScripts.length && cutsceneScripts.every(function (script, index) { return script === expectedCutsceneScripts[index]; }) &&
      cutsceneRefScripts.length === cutsceneScripts.length && cutsceneRefScripts.every(function (script, index) { return script === cutsceneScripts[index]; }));
    ok('10h README 明示 .claude skill 单一真相源与包根 sync/check 命令',
      /单一真相源是 `.claude\/skills\/`/.test(readme) &&
      /node core\/tooling\/codex-parity\.mjs --sync --root \./.test(readme) &&
      /node core\/tooling\/codex-parity\.mjs --check --root \./.test(readme) &&
      !/node engine\/core\/tooling\/codex-parity\.mjs/.test(readme) && /不要手改/.test(readme));
  })();
