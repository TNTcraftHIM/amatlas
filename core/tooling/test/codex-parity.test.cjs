...
try {
  ...
  (function () {
    ...
    var routeFiles = [agents, claudeSkill, skill, readme];
    ok('10f README/AGENTS/Claude/Codex skill 路由到公共高层演出入口', routeFiles.every(function (source) {
    ...
    ok('10g AGENTS 与 Claude/Codex skill 锁正式九段加载顺序',
      formalOrder.test(agents) && formalOrder.test(claudeSkill) && formalOrder.test(skill));
    ok('10h README 明示 .claude skill 单一真相源与唯一同步命令',
      /单一真相源是 `.claude\/skills\/`/.test(readme) &&
      /node engine\/core\/tooling\/codex-parity\.mjs --sync --root engine/.test(readme) &&
      /不要手改/.test(readme));
  })();
