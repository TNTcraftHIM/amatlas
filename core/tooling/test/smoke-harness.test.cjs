...
import(pathToFileURL(TOOL).href).then(async function (mod) {
  ...
  var fusedNext = await mod.runSmoke(
    '<!doctype html><body><main id="app">这是一段足够长的过场正文，用于验证融合推进面板就是正式可点击入口。' +
    '<div id="look" role="button" tabindex="0" data-cutscene-next="">继续演出</div></main>' +
    '<script>document.getElementById("look").addEventListener("click",function(){document.getElementById("app").textContent="已经推进到下一拍，正文随之改变。";});</script></body>',
    { settleMs: 0, actionWaitMs: 0 }
  );
  ok('A5 现役 cutscene 融合正文面板必须算可点击入口并能被驱动', fusedNext.fail === 0 && fusedNext.lines.some(function (line) {
    return /✅ 3\. 渲染了可点击入口/.test(line);
  }) && fusedNext.lines.some(function (line) {
    return /✅ 4\. 点一下能切换且不崩/.test(line);
  }), fusedNext.lines.join(' | '));
  ...
  try {
    ...
    ok('A6 仓库外解包时 NODE_PATH 可提供 jsdom，不要求把 node_modules 拷进 payload', external.status === 0,
