...
// DK. 死 state 键 —— maze3d winKey/scareKey 豁免【对象级、仅 kind:'maze3d'】两端锁(R2 二轮批3:原裸文本正则可被诱饵/注释绕过)
var dkm = run(path.join(__dirname, 'fixtures', 'dead-state-key-maze3d-world.js'));
ok('DK1 真 maze3d 节点声明 winKey → 外层读 S.corridorLit 豁免(不误报死键)', !/死 state 键 'corridorLit'/.test(dkm.out), dkm.out.replace(/\n/g, ' ').slice(0, 200));
ok('DK2 真 maze3d 节点声明 scareKey → 外层读 S.doorArmed 豁免(不误报死键)', !/死 state 键 'doorArmed'/.test(dkm.out), dkm.out.replace(/\n/g, ' ').slice(0, 200));
ok("DK3 非 maze3d 节点诱饵同名 winKey 不再抵消 → S.decoyFlag 仍报死 state 键 P0(spoof 端,退出码 1)", dkm.status === 1 && /\[确认\]\[P0\][^\n]*死 state 键 'decoyFlag'/.test(dkm.out), 'status=' + dkm.status + ' ' + dkm.out.replace(/\n/g, ' ').slice(0, 260));
...
ok('K2 nested 把 on_success/on_failure 写进 checks[] → 报完整内层路径', /跑团检定格式错[^\n]*nested[^\n]*checks\[0\]\.on_success[^\n]*checks\[0\]\.on_failure/.test(be.out), be.out.replace(/\n/g, ' ').slice(0, 300));
ok('K3 safe 用对格式(checks/exits)→ 不误报', !/跑团检定格式错[^\n]*safe/.test(be.out), be.out.replace(/\n/g, ' ').slice(0, 160));
ok('K4 格式错为 P1、不硬拦(退出码 0)', be.status === 0, 'status=' + be.status);
...
ok('W6 fumble 原地口袋 {fum_a,fum_b} → 报软锁口袋(success/fail 都逃、旧闸只看 fail 会放行;已声明 fumble 无 to 留口袋 + 扣体力 → 对称接缝补齐)', /软锁口袋[^\n]*fum_a[^\n]*fum_b/.test(slp.out), slp.out.replace(/\n/g, ' ').match(/软锁口袋[^\n]{0,90}/g));
ok('W7 fumble 也逃口袋 {esc_a,esc_b} 不报(已声明 fumble 但 fumble.to 也离开 → 守零误报:只抓「已声明且不逃」)', !/软锁口袋[^\n]*esc_a/.test(slp.out) && !/软锁口袋[^\n]*esc_b/.test(slp.out), slp.out.replace(/\n/g, ' ').slice(0, 260));
...
// CR. crawler(kind:'maze')数据 fail-loud + maze3d 误套两端锁(R2 二轮批3:守卫双向错位——有 grid 误报、缺 maze 漏报)
var crBad = run(path.join(__dirname, 'fixtures', 'crawler-bad-world.js'));
ok('CR1 缺 node.maze 的 kind:maze 节点 → P0(不再只当 P2 死胡同放过)', crBad.status === 1 && /迷宫\(crawler\)数据错误:节点 'dungeon\/nomaze'[^\n]*必须有 node\.maze/.test(crBad.out), 'status=' + crBad.status + ' ' + crBad.out.replace(/\n/g, ' ').slice(0, 300));
ok('CR2 start 落墙 → P0 点名坐标', /迷宫\(crawler\)数据错误:节点 'dungeon\/wallstart'[^\n]*\(0,0\) 落在墙/.test(crBad.out), crBad.out.replace(/\n/g, ' ').slice(0, 300));
ok('CR3 非矩形 grid → P0 点名行宽', /迷宫\(crawler\)数据错误:节点 'dungeon\/raggedgrid'[^\n]*必须是矩形/.test(crBad.out), crBad.out.replace(/\n/g, ' ').slice(0, 300));
var crGood = run(path.join(__dirname, 'fixtures', 'crawler-good-world.js'));
ok("CR4 合法 crawler(开放词汇 'E' 内容标记)→ 不再误报 maze3d 的「字符非法」(接缝零误报端)", !/字符 "E" 非法/.test(crGood.out) && !/maze3d 数据错误[^\n]*cellar/.test(crGood.out), crGood.out.replace(/\n/g, ' ').slice(0, 240));
ok('CR5 合法 crawler grid/start → crawlerIssues 零 P0(退出码 0;仅 hall 有意结局 P2)', crGood.status === 0 && !/迷宫\(crawler\)数据错误/.test(crGood.out), 'status=' + crGood.status + ' ' + crGood.out.replace(/\n/g, ' ').slice(0, 240));
...
var cse = run(path.join(__dirname, 'fixtures', 'cutscene-exits-world.js'));
ok('CS7 cutscene 节点用 exits 声明出口 → [确认][P0] + 退出码 1(R2 二轮:运行时 fail-loud 提前到静态期,防四闸贪心联合放行崩游戏)', cse.status === 1 && /\[确认\]\[P0\][^\n]*cutscene 出口放错字段[^\n]*'m\/bad'/.test(cse.out), 'status=' + cse.status + ' ' + cse.out.replace(/\n/g, ' ').slice(0, 260));
ok('CS8 反向变异:健康 cutscene(出口用 links)不误报「出口放错字段」(接缝零误报端)', !/cutscene 出口放错字段/.test(csw.out), csw.out.replace(/\n/g, ' ').slice(0, 160));
