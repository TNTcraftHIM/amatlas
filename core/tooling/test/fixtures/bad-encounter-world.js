  ...
  return {
    ...
    maps: { m: { name: 'M', nodes: {
      ...
      trial: { kind: 'encounter', look: '检定',
        ...
        exits: [ { label: '前进', to: 'nested' } ] },
      // ✗ 同族错位:别名写进 checks[] 内层也会被 runtime 忽略 → 静态闸必须点名完整路径
      nested: { kind: 'encounter', look: '嵌套错拼',
        checks: [ { id: 'c', label: '检定', skill: 'x', dc: 8, dice: '2d6', on_success: { text: '成' }, on_failure: { text: '败' } } ],
