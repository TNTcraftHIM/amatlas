...
var TIMELINE = path.join(ENGINE, 'presenters', 'present-timeline.js');
var PERFORMANCE_COMPILER = path.join(ENGINE, 'modules', 'cutscene', 'authoring', 'compile-performance.js');
var SAVE = path.join(ENGINE, 'plugins', 'save.js');
var SVG = path.join(ENGINE, 'presenters', 'present-svg.js');
var PROGRESSIONS = path.join(ENGINE, 'presenters', 'progressions.js');
var COMPOSE_MUSIC = path.join(ENGINE, 'presenters', 'compose-music.js');
var AUDIO = path.join(ENGINE, 'presenters', 'present-audio.js');
var ACH = path.join(ENGINE, 'plugins', 'achievement.js');
var INV = path.join(ENGINE, 'plugins', 'inventory.js');
var MINIMAP = path.join(ENGINE, 'plugins', 'minimap.js');
var RESET = path.join(ENGINE, 'plugins', 'reset.js');
var MAZE = path.join(ENGINE, 'examples', 'maze3d', 'raycast-maze.js');
var CUTSCENE_DEMO_WORLD = path.join(ENGINE, 'examples', 'cutscene-demo', 'world.js');
var CUTSCENE_DEMO_GAME = path.join(ENGINE, 'examples', 'cutscene-demo', 'game.js');
...
try {
  ...
  // E4-E8 maze3d 的 stageId 也是显式强意图；默认挂载点同样是玩法唯一 canvas 容器，缺失都会让整项玩法静默消失。
  function mazeProbe(stageIdSource, stageMarkup, combatSource) {
    fresh();
    write('world.js', '(function(g,f){if(typeof module!=="undefined"&&module.exports)module.exports=f();else g.MY_WORLD=f();})(typeof globalThis!=="undefined"?globalThis:this,function(){return {id:"14141414-1414-4414-8414-141414141414",initState:{won:false,dead:false},start:{map:"m",node:"maze"},maps:{m:{nodes:{maze:{kind:"maze3d",look:"迷宫",winKey:"won",maze:{grid:["#####","#..D#","#####"],start:{x:1,y:1,dir:"E"}' + (combatSource || '') + '},links:[{label:"重来",to:"maze"}]}}}}};});');
    write('game.js', '(function(){window._engine=window.Amatlas.boot(window.MY_WORLD,{modules:[window.Amatlas.Maze3d.createMaze3dModule(' + stageIdSource + ')],present:{svg:false,audio:false},storage:null});})();');
    write('index.html', '<!doctype html><html><body><div id="look"></div><div id="choices"></div>' + stageMarkup
      + '<script src="' + rel(CORE) + '"></script><script src="' + rel(RENDERER) + '"></script><script src="' + rel(MAZE) + '"></script><script src="' + rel(DOM) + '"></script>'
      + '<script src="world.js"></script><script src="' + rel(BOOT) + '"></script><script src="game.js"></script></body></html>');
    return probe(path.join(TMP, 'index.html'));
  }
  var e4 = mazeProbe('{stageId:"missing-stage",controls:false,audio:false,staticPreview:true}', '<div id="actual-stage"></div>');
  ok('E4 maze3d 显式 stageId 缺 DOM → P0，不再静默零 canvas', e4.status === 1 && /maze3d 挂载点缺失/.test(e4.out) && /#missing-stage/.test(e4.out), 'status=' + e4.status + ' ' + e4.out.replace(/\n/g, ' ').slice(0, 240));
  var e5b = mazeProbe('{}', '<div id="actual-stage"></div>');
  ok('E5 maze3d 省略 stageId 但缺默认 #maze3d-stage → P0', e5b.status === 1 && /maze3d 挂载点缺失/.test(e5b.out) && /#maze3d-stage/.test(e5b.out), 'status=' + e5b.status + ' ' + e5b.out.replace(/\n/g, ' ').slice(0, 240));
  var e6 = mazeProbe('{stageId:"actual-stage",controls:false,audio:false,staticPreview:true}', '<div id="actual-stage"></div>');
  ok('E6 maze3d stageId 有对应 DOM → 不误报挂载点 P0', e6.status === 0 && !/maze3d 挂载点缺失/.test(e6.out), 'status=' + e6.status + ' ' + e6.out.replace(/\n/g, ' ').slice(0, 220));
  var e7 = mazeProbe('{stageId:"actual-stage"}', '<div id="actual-stage"></div>', ',combat:{deathKey:"dead",player:{maxHealth:10,health:10},loadout:[{kind:"laser",ammo:1,maxAmmo:2}],equipped:"laser"}');
  ok('E7 非法 FPS combat kind 在零 DOM assembly-probe 也 fail-loud P0', e7.status === 1 && /maze\.combat\.loadout\[0\]\.kind/.test(e7.out) && /precision.*scatter/.test(e7.out), 'status=' + e7.status + ' ' + e7.out.replace(/\n/g, ' ').slice(0, 260));
  var e8 = mazeProbe('{stageId:"actual-stage"}', '<div id="actual-stage"></div>', ',combat:{deathKey:"dead",player:{maxHealth:10,health:10},loadout:[{kind:"precision",ammo:1,maxAmmo:2}],equipped:"precision"}');
  ok('E8 合法 precision combat 在同一零 DOM 探针通过', e8.status === 0 && !/maze combat/.test(e8.out), 'status=' + e8.status + ' ' + e8.out.replace(/\n/g, ' ').slice(0, 220));
  fresh();
  write('world.js', W2);
  write('game.js', '(function(){var opts={stageId:window.RUNTIME_STAGE};window._engine=window.Amatlas.boot(window.MY_WORLD,{modules:[window.Amatlas.Maze3d.createMaze3dModule(opts)]});})();');
  write('index.html', '<!doctype html><body><div id="look"></div><div id="choices"></div><script src="' + rel(CORE) + '"></script><script src="' + rel(RENDERER) + '"></script><script src="' + rel(MAZE) + '"></script><script src="' + rel(DOM) + '"></script><script src="world.js"></script><script src="' + rel(BOOT) + '"></script><script src="game.js"></script></body>');
  var e9 = probe(path.join(TMP, 'index.html'));
  ok('E9 createMaze3dModule(options 变量/动态 stageId)静态不可知 → 不假报默认 #maze3d-stage P0', e9.status === 0 && !/maze3d 挂载点缺失/.test(e9.out), 'status=' + e9.status + ' ' + e9.out.replace(/\n/g, ' ').slice(0, 240));
  ...
  ok('R1 9 拍 cutscene 前 8 拍仅 runtime next，probe 逐拍走到末拍出口且 P0=0', r9.status === 0 && !/\[确认\]\[P0\]/.test(r9.out), 'status=' + r9.status + ' ' + r9.out.replace(/\n/g, ' ').slice(0, 240));
  ok('R2 probe 连续 apply runtime next 穿过 beat5.run，再从末拍出口离开', !/运行时 NaN|运行时崩/.test(r9.out), r9.out.replace(/\n/g, ' ').slice(0, 160));
  ...
  // S. 富 cutscene executor 闭包：对象 typewriter 只需 timeline+DOM；motion/cast 还必须有启用的 SVG executor + #scene。
  //    这是 A.boot 官方作者路径的窄闸，不把自定义 presenter/手写装配强行套进来。
  function richCutsceneFixture(beatSource, options) {
    options = options || {};
    fresh();
    write('world.js', '(function(g,f){if(typeof module!=="undefined"&&module.exports)module.exports=f();else g.MY_WORLD=f();})(typeof globalThis!=="undefined"?globalThis:this,function(){return {id:"14141414-1414-4414-8414-141414141414",start:{map:"m",node:"intro"},maps:{m:{name:"M",nodes:{intro:{kind:"cutscene",title:"富过场",beats:[' + beatSource + '],links:[{label:"重播",to:"intro"}]}}}}};});');
    write('game.js', '(function(){function boot(){window._engine=window.Amatlas.boot(window.MY_WORLD,' + (options.manifest || '{}') + ');}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot);else boot();})();');
    var scripts = [rel(CORE), rel(CUTSCENE), rel(DOM)];
    if (options.timeline !== false) scripts.push(rel(TIMELINE));
    if (options.svg) scripts.push(rel(SVG));
    scripts.push('world.js', rel(BOOT), 'game.js');
    write('index.html', '<!doctype html><html><head><style>.choice{color:red}</style></head><body><main id="app">' +
      (options.scene ? '<div id="scene"></div>' : '') + '<div id="look"></div><div id="choices"></div></main>' +
      scripts.map(function (s) { return '<script src="' + s + '"></script>'; }).join('') + '</body></html>');
    return probe(path.join(TMP, 'index.html'));
  }
  var typewriterBeat = '{dur:2,text:{mode:"typewriter",lines:[{cps:8,chunks:[{text:"雾先分开。"}]}]}}';
  var motionBeat = '{dur:2,text:"船影掠过。",motion:{layers:[{id:"ship",art:"ship",x:0,y:10,scale:1,rotate:0,opacity:1}],tracks:[{target:"ship",property:"x",keys:[{at:1,value:100}]}]}}';
  var castBeat = '{dur:2,text:"守望者出现。",cast:[{id:"guide",rig:{parts:[{id:"body",parent:null,art:"figure",pivot:{x:0,y:0},rest:{x:160,y:90,rotate:0,scaleX:1,scaleY:1,opacity:1}}],drawOrder:["body"],tracks:[],variants:[],secondary:[]}}]}';

  var s1 = richCutsceneFixture(typewriterBeat, {});
  ok('S1 A.boot 纯 typewriter + timeline + DOM、无 SVG 合法通过', s1.status === 0 && !/富过场视觉 executor/.test(s1.out), 'status=' + s1.status + ' ' + s1.out.replace(/\n/g, ' ').slice(0, 220));
  var s2 = richCutsceneFixture(motionBeat, {});
  ok('S2 A.boot motion + timeline 但漏 SVG script → 富过场 executor P0', s2.status === 1 && /富过场视觉 executor/.test(s2.out) && /present-svg\.js/.test(s2.out), 'status=' + s2.status + ' ' + s2.out.replace(/\n/g, ' ').slice(0, 240));
  var s3 = richCutsceneFixture(motionBeat, { svg: true, scene: true });
  ok('S3 A.boot motion + timeline + SVG + #scene → P0=0', s3.status === 0 && !/\[确认\]\[P0\]/.test(s3.out), 'status=' + s3.status + ' ' + s3.out.replace(/\n/g, ' ').slice(0, 220));
  var s4 = richCutsceneFixture(motionBeat, { svg: true, scene: true, manifest: '{present:{svg:false}}' });
  ok('S4 A.boot motion/cast 显式 present.svg:false → 富过场 executor P0', s4.status === 1 && /富过场视觉 executor/.test(s4.out) && /svg:false/.test(s4.out), 'status=' + s4.status + ' ' + s4.out.replace(/\n/g, ' ').slice(0, 240));
  var s5 = richCutsceneFixture(motionBeat, { svg: true });
  ok('S5 A.boot motion/cast 有 SVG script 但缺 #scene → 富过场 executor P0', s5.status === 1 && /富过场视觉 executor/.test(s5.out) && /#scene/.test(s5.out), 'status=' + s5.status + ' ' + s5.out.replace(/\n/g, ' ').slice(0, 240));
  var s6 = richCutsceneFixture(motionBeat, { timeline: false, svg: true, scene: true });
  ok('S6 rich beat 缺 timeline 保持 runtime 精确 P0', s6.status === 1 && /present-timeline\.js/.test(s6.out) && /装配崩:加载 game\.js 时抛错/.test(s6.out), 'status=' + s6.status + ' ' + s6.out.replace(/\n/g, ' ').slice(0, 240));
  var s7 = richCutsceneFixture(castBeat, {});
  ok('S7 A.boot cast + timeline 但漏 SVG script → 与 motion 同族 P0', s7.status === 1 && /富过场视觉 executor/.test(s7.out) && /present-svg\.js/.test(s7.out), 'status=' + s7.status + ' ' + s7.out.replace(/\n/g, ' ').slice(0, 240));
  var s8 = richCutsceneFixture(typewriterBeat + ',' + motionBeat, {});
  ok('S8 首拍纯 typewriter、后续拍 motion：probe 自动推进后仍抓缺 SVG P0', s8.status === 1 && /富过场视觉 executor/.test(s8.out) && /present-svg\.js/.test(s8.out), 'status=' + s8.status + ' ' + s8.out.replace(/\n/g, ' ').slice(0, 240));

  function branchedRichCutsceneFixture(options) {
    options = options || {};
    fresh();
    write('world.js', '(function(g,f){if(typeof module!=="undefined"&&module.exports)module.exports=f();else g.MY_WORLD=f();})(typeof globalThis!=="undefined"?globalThis:this,function(){return {id:"14141414-1414-4414-8414-141414141414",start:{map:"m",node:"start"},maps:{m:{nodes:{start:{kind:"scene",look:"选择",links:[{to:"safe",label:"安全首分支"},{to:"rich",label:"富过场次分支"}]},safe:{kind:"scene",look:"循环",links:[{to:"start",label:"回首分支"}]},rich:{kind:"cutscene",title:"富过场",beats:[' + motionBeat + '],links:[{to:"start",label:"结束"}]}}}}};});');
    write('game.js', '(function(){window._engine=window.Amatlas.boot(window.MY_WORLD,' + (options.manifest || '{present:{audio:false},storage:null}') + ');})();');
    var scripts = [rel(CORE), rel(RENDERER), rel(CUTSCENE), rel(DOM), rel(TIMELINE)];
    if (options.svg) scripts.push(rel(SVG));
    scripts.push('world.js', rel(BOOT), 'game.js');
    write('index.html', '<!doctype html><html><body>' + (options.scene ? '<div id="scene"></div>' : '') + '<div id="look"></div><div id="choices"></div>' + scripts.map(function (s) { return '<script src="' + s + '"></script>'; }).join('') + '</body></html>');
    return probe(path.join(TMP, 'index.html'));
  }
  var s9 = branchedRichCutsceneFixture({});
  ok('S9 rich cutscene 只在第二移动分支、贪心首分支循环也必须预检缺 SVG P0', s9.status === 1 && /富过场视觉 executor[^\n]*结构可达 rich cutscene/.test(s9.out) && /present-svg\.js/.test(s9.out), 'status=' + s9.status + ' ' + s9.out.replace(/\n/g, ' ').slice(0, 280));
  var s10 = branchedRichCutsceneFixture({ svg: true, scene: true });
  ok('S10 第二分支 rich cutscene 配齐 SVG + #scene → 不误报', s10.status === 0 && !/富过场视觉 executor/.test(s10.out), 'status=' + s10.status + ' ' + s10.out.replace(/\n/g, ' ').slice(0, 240));

  // T. Phase 3 公共 compiler 装配闭包：在 probe 的同一 browser-like vm context 里真实编译 world，
  //    再让 probe 从 engine.world 看见 lowering 后的 rich cast。禁止用 compiler/world 源码 grep 代替运行语义。
  function compiledCutsceneFixture(options) {
    options = options || {};
    fresh();
    write('world.js', '(function(g){var C=g.Amatlas&&g.Amatlas.CutsceneAuthoring;if(!C||typeof C.compilePerformance!=="function")throw new Error("cutscene performance compiler missing before world.js");function p(id,parent,rest){return {id:id,parent:parent,art:"ship",pivot:{x:0,y:0},rest:Object.assign({x:0,y:0,rotate:0,scaleX:1,scaleY:1,opacity:1},rest||{})};}var rig={parts:[p("torso",null,{x:80,y:108}),p("leg_l","torso",{x:-7,y:14}),p("leg_r","torso",{x:7,y:14}),p("arm_l_upper","torso",{x:-13,y:-14}),p("arm_l_fore","arm_l_upper",{y:20}),p("hand_l","arm_l_fore",{y:18}),p("arm_r_upper","torso",{x:13,y:-14}),p("arm_r_fore","arm_r_upper",{y:20}),p("hand_r","arm_r_fore",{y:18}),p("eyes","torso",{y:-30})],drawOrder:["leg_l","leg_r","arm_l_upper","arm_l_fore","hand_l","torso","eyes","arm_r_upper","arm_r_fore","hand_r"],tracks:[],variants:[],secondary:[{type:"blink",target:"eyes",property:"scaleY",closedValue:0.08,windowMs:1800,durationMs:100,chance:0.65,seed:7}]};var out=C.compilePerformance({version:1,actors:[{id:"warden",rig:rig,profile:{kind:"cutout-biped-v1",authoredFacing:"right",root:"torso",legs:{left:"leg_l",right:"leg_r"},arms:{left:{upper:"arm_l_upper",fore:"arm_l_fore",hand:"hand_l"},right:{upper:"arm_r_upper",fore:"arm_r_fore",hand:"hand_r"}}}}],anchors:[{id:"start",x:80,y:108,facing:"right"},{id:"mark",x:136,y:108,facing:"right"},{id:"focus",x:240,y:72,capacity:0}],shots:[{id:"compiled",cast:[{actor:"warden",at:"start"}],sequence:[{do:"walkTo",actor:"warden",to:"mark",dur:0.8,steps:2},{do:"pointAt",actor:"warden",target:"focus",hand:"right",dur:0.2},{do:"caption",lines:[{cps:10,chunks:[{text:"编译完成。"}]}]}]}]},{path:"world.maps.m.nodes.intro.performance"});g.MY_WORLD={id:"14141414-1414-4414-8414-141414141414",start:{map:"m",node:"intro"},maps:{m:{nodes:{intro:{kind:"cutscene",title:"编译过场",beats:out.beats,links:[{label:"重播",to:"intro"}]}}}}};})(typeof globalThis!=="undefined"?globalThis:this);');
    write('game.js', '(function(){window._engine=window.Amatlas.boot(window.MY_WORLD,{present:{audio:false},storage:null});})();');
    var scripts = [rel(CORE), rel(CUTSCENE), rel(DOM)];
    if (options.compilerBeforeTimeline) scripts.push(rel(PERFORMANCE_COMPILER));
    if (options.timeline !== false) scripts.push(rel(TIMELINE));
    if (options.compiler !== false && !options.compilerBeforeTimeline && !options.compilerAfterWorld) scripts.push(rel(PERFORMANCE_COMPILER));
    if (options.svg !== false) scripts.push(rel(SVG));
    scripts.push('world.js');
    if (options.compilerAfterWorld) scripts.push(rel(PERFORMANCE_COMPILER));
    scripts.push(rel(BOOT), 'game.js');
    write('index.html', '<!doctype html><html><head><style>.choice{color:red}</style></head><body><main id="app">' +
      (options.scene === false ? '' : '<div id="scene"></div>') + '<div id="look"></div><div id="choices"></div></main>' +
      scripts.map(function (s) { return '<script src="' + s + '"></script>'; }).join('') + '</body></html>');
    return probe(path.join(TMP, 'index.html'));
  }

  var t1 = compiledCutsceneFixture({ compiler: false });
  ok('T1 compiler script 缺失 → 同 context 真实 world 在加载期 P0', t1.status === 1 && /compiler missing before world\.js/.test(t1.out), 'status=' + t1.status + ' ' + t1.out.replace(/\n/g, ' ').slice(0, 240));
  var t2 = compiledCutsceneFixture({ compilerBeforeTimeline: true });
  ok('T2 compiler 早于 timeline → 加载期 P0 并给正式顺序', t2.status === 1 && /present-timeline\.js -> compile-performance\.js -> world\.js/.test(t2.out), 'status=' + t2.status + ' ' + t2.out.replace(/\n/g, ' ').slice(0, 260));
  var t3 = compiledCutsceneFixture({ compilerAfterWorld: true });
  ok('T3 compiler 晚于 world → world 真实求值先失败 P0', t3.status === 1 && /compiler missing before world\.js/.test(t3.out), 'status=' + t3.status + ' ' + t3.out.replace(/\n/g, ' ').slice(0, 240));
  var t4 = compiledCutsceneFixture({ timeline: false });
  ok('T4 compiled world 漏 timeline → compiler 加载期 P0', t4.status === 1 && /present-timeline\.js -> compile-performance\.js -> world\.js/.test(t4.out), 'status=' + t4.status + ' ' + t4.out.replace(/\n/g, ' ').slice(0, 260));
  var t5 = compiledCutsceneFixture({ svg: false });
  ok('T5 真实 compiled rich world 漏 SVG → probe 从 engine.world 抓 P0', t5.status === 1 && /富过场视觉 executor/.test(t5.out) && /present-svg\.js/.test(t5.out), 'status=' + t5.status + ' ' + t5.out.replace(/\n/g, ' ').slice(0, 280));
  var t6 = compiledCutsceneFixture({ scene: false });
  ok('T6 真实 compiled rich world 漏 #scene → probe 从 engine.world 抓 P0', t6.status === 1 && /富过场视觉 executor/.test(t6.out) && /#scene/.test(t6.out), 'status=' + t6.status + ' ' + t6.out.replace(/\n/g, ' ').slice(0, 280));
  var t7 = compiledCutsceneFixture({});
  ok('T7 timeline → compiler → world + SVG/#scene 闭合 → 真实 compiled world 通过', t7.status === 0 && !/\[确认\]\[P0\]/.test(t7.out), 'status=' + t7.status + ' ' + t7.out.replace(/\n/g, ' ').slice(0, 260));

  function cutsceneDemoFixture(includeCompiler) {
    fresh();
    var scripts = [rel(CORE), rel(RENDERER), rel(CUTSCENE), rel(DOM), rel(TIMELINE)];
    if (includeCompiler) scripts.push(rel(PERFORMANCE_COMPILER));
    scripts.push(rel(SVG), rel(PROGRESSIONS), rel(COMPOSE_MUSIC), rel(AUDIO), rel(SAVE), rel(MINIMAP), rel(ACH), rel(RESET),
      rel(CUTSCENE_DEMO_WORLD), rel(BOOT), rel(CUTSCENE_DEMO_GAME));
    write('index.html', '<!doctype html><html><head><style>.choice{color:red}</style></head><body><main id="app"><div id="plugin-bar"></div><div id="mapname"></div><h1 id="place"></h1><div id="scene"></div><div id="look"></div><div id="choices"></div><div id="status"></div></main><div id="plugin-minimap"></div><div id="plugin-overlay"></div>' +
      scripts.map(function (s) { return '<script src="' + s + '"></script>'; }).join('') + '</body></html>');
    return probe(path.join(TMP, 'index.html'));
  }
  var t8 = cutsceneDemoFixture(false);
  ok('T8 正式 cutscene-demo 漏 compiler → world dogfood 加载期 P0', t8.status === 1 && /compiler missing|CutsceneAuthoring|compilePerformance/.test(t8.out), 'status=' + t8.status + ' ' + t8.out.replace(/\n/g, ' ').slice(0, 280));
  var t9 = cutsceneDemoFixture(true);
  ok('T9 正式 cutscene-demo 配齐 timeline → compiler → world → 装配通过', t9.status === 0 && !/\[确认\]\[P0\]/.test(t9.out), 'status=' + t9.status + ' ' + t9.out.replace(/\n/g, ' ').slice(0, 260));
} finally {
