...
// 提取每个工厂调用的实参区，避免跨过 `createXxx({})` 的右括号去误抓后文同名字段。
// 与 bootArgsText 同属静态提示层：动态 options 变量无法可靠配对时让位，runtime/真机继续兜底。
function callArgsTexts(src, factoryName) {
  const out = [], re = new RegExp('\\b' + factoryName + '\\s*\\(', 'g');
  let m;
  while ((m = re.exec(src))) {
    let depth = 1, i = re.lastIndex; const start = i;
    for (; i < src.length && depth > 0; i++) { const c = src[i]; if (c === '(') depth++; else if (c === ')') depth--; }
...
export function runProbe(indexPath) {
  ...
  const assembly = { ids: new Set(), usesBoot: false, hasSvgScript: false, svgDisabled: false };
  ...
  //    → 该能力**静默失效**(引擎找不到挂载点不报错、只 no-op;showcase 把 minimap/成就/状态条/重开写没的根因)。P1 提醒。
  {
    ...
    assembly.ids = ids;
    assembly.usesBoot = usesBoot;
    assembly.hasSvgScript = hasScript(/present-svg/);
    assembly.svgDisabled = /svg\s*:\s*false/.test(manifestText);
    ...
    // maze3d stageId 不带 #，但也是该玩法唯一 canvas 容器。显式拼错或省略默认容器都会让整项玩法静默零 canvas，故独立按 P0 配对。
    const mazeFactoryArgs = callArgsTexts(gameSrc, 'createMaze3dModule');
    const mazeArgs = mazeFactoryArgs.length ? mazeFactoryArgs[0].trim() : null;
    const mazeStageMatch = mazeArgs != null && /\bstageId\s*:\s*["']([\w-]+)["']/.exec(mazeArgs);
    const mazeHasDynamicStage = mazeArgs != null && !mazeStageMatch && /\bstageId\s*:/.test(mazeArgs);
    const mazeUsesLiteralOptions = mazeArgs === '' || (mazeArgs != null && /^\{[\s\S]*\}$/.test(mazeArgs));
    // 只有静态可知时才作 P0：显式字符串值，或无参/对象字面量确实省略 stageId。变量 options / 动态 stageId 有合法反例，必须让位给真机。
    const mazeStageId = mazeStageMatch ? mazeStageMatch[1] : ((!mazeHasDynamicStage && mazeUsesLiteralOptions) ? 'maze3d-stage' : null);
    ...
    if (mazeStageId && !ids.has(mazeStageId)) p0.push(`maze3d 挂载点缺失:createMaze3dModule(${mazeStageMatch ? `stageId:'${mazeStageId}'` : '省略 stageId，使用默认值'}) 需要 index.html 的 #${mazeStageId}，但该 id 不存在 → 整项 maze3d 玩法静默零 canvas。补 <div id="${mazeStageId}"></div>，或修正 stageId 拼写。`);
  ...
  // 富 cutscene 的 motion/cast 是明确视觉作者意图；在官方 A.boot 路径中，它们只能由 bundled SvgPresenter 消费。
  // 不能依赖贪心自动游玩恰好进入该节点：次分支可永远被首分支循环遮住。对【实际 world 对象的结构可达节点】预检，
  // 不 grep 源码、不执行分支副作用；且只约束 A.boot，保留手写/自定义 presenter escape hatch。
  function reachableRichCutscenes(world) {
    if (!world || !world.start || !world.maps) return [];
    const queue = [{ map: world.start.map, node: world.start.node }], seen = new Set(), rich = [];
    const push = (to, map) => {
      if (typeof to === 'string') queue.push({ map, node: to });
      else if (to && typeof to === 'object' && typeof to.map === 'string' && typeof to.node === 'string') queue.push({ map: to.map, node: to.node });
    };
    while (queue.length) {
      const ref = queue.shift(), key = ref.map + '/' + ref.node;
      if (seen.has(key)) continue;
      seen.add(key);
      const map = world.maps[ref.map], node = map && map.nodes && map.nodes[ref.node];
      if (!node) continue;
      if (node.kind === 'cutscene' && Array.isArray(node.beats) && node.beats.some((beat) => beat && (beat.motion || beat.cast))) rich.push(key);
      (node.links || []).forEach((edge) => { if (edge) push(edge.to, ref.map); });
      (node.exits || []).forEach((edge) => { if (edge) push(edge.to, ref.map); });
      (node.checks || []).forEach((check) => {
        if (!check) return;
        ['success', 'fail', 'crit', 'fumble', 'partial'].forEach((outcome) => { if (check[outcome]) push(check[outcome].to, ref.map); });
      });
    }
    return rich;
  }
  const richNodes = assembly.usesBoot ? reachableRichCutscenes(eng.world) : [];
  if (richNodes.length) {
    const where = `结构可达 rich cutscene ${richNodes.map((key) => `'${key}'`).join(' / ')} 声明 motion/cast`;
    if (!assembly.hasSvgScript) {
      p0.push(`富过场视觉 executor 缺失:${where}，但 index.html 没加载 presenters/present-svg.js → 即使贪心探针没走到该分支，端玩家进入后角色或运动仍会静默消失。为 rich cutscene 加载 present-timeline.js + present-svg.js，并保留 #scene。`);
    } else if (assembly.svgDisabled) {
      p0.push(`富过场视觉 executor 被禁用:${where}，但 A.boot manifest 写了 present.svg:false → 角色或运动静默消失。删除 svg:false，或删除这些拍的 motion/cast 作者意图。`);
    } else if (!assembly.ids.has('scene')) {
      p0.push(`富过场视觉 executor 挂载点缺失:${where}，SVG 已加载但 index.html 没有 #scene → 角色或运动静默消失。按正式骨架补 <div id="scene"></div>。`);
    }
  }
