...
export function runProbe(indexPath) {
  ...
  // 富 cutscene 的 motion/cast 是明确视觉作者意图；在官方 A.boot 路径中，它们只能由 bundled SvgPresenter
  // 消费。boot 对缺 SVG script/显式 svg:false 原本宽容，会留下正文/按钮都活着、角色或运动静默消失的假成品。
  // 只查【真实 view 的当前可达 playback】而非 grep world 字段，且只约束 A.boot，保留手写/自定义 presenter escape hatch。
  function checkRichCutsceneExecutor(envelope) {
    const playback = envelope && envelope.view && envelope.view.cutscenePlayback;
    if (!assembly.usesBoot || !(playback && (playback.motion || playback.cast))) return false;
    if (!assembly.hasSvgScript) {
      p0.push('富过场视觉 executor 缺失:当前可达 cutscenePlayback 声明 motion/cast，但 index.html 没加载 presenters/present-svg.js → 正文/按钮仍运行，角色或运动却静默消失。为 rich cutscene 加载 present-timeline.js + present-svg.js，并保留 #scene。');
    } else if (assembly.svgDisabled) {
      p0.push('富过场视觉 executor 被禁用:当前可达 cutscenePlayback 声明 motion/cast，但 A.boot manifest 写了 present.svg:false → 角色或运动静默消失。删除 svg:false，或删除该拍的 motion/cast 作者意图。');
    } else if (!assembly.ids.has('scene')) {
      p0.push('富过场视觉 executor 挂载点缺失:当前可达 cutscenePlayback 声明 motion/cast，SVG 已加载但 index.html 没有 #scene → 角色或运动静默消失。按正式骨架补 <div id="scene"></div>。');
    } else return false;
    return true;
  }
  checkRichCutsceneExecutor(view);
  ...
  for (let step = 0; step < 30; step++) {
    ...
    if (checkRichCutsceneExecutor(snap)) break;
