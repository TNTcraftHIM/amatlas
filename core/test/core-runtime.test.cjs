...
// G'''. systems[].on/run fail-loud(R2 二轮):事件名拼错/漏 run 的 system 会永久静默不触发 → 注册期即抛,不拖到运行时或永不触发。
(function () {
  function mk() { return createEngine(makeWorld(), { storage: null }); }
  function throwsOn(fn) { try { fn(); return false; } catch (e) { return true; } }
  // 反向变异「原 case 仍过」:合法 system(on 非空串 + run 函数)必须注册成功且 enter 时真触发
  const eOk = mk(); let ran = 0;
  eOk.registerModule({ id: 'good-sys', nodeKinds: ['demo'], systems: [{ on: 'enter', run: function () { ran++; } }], render: function () { return { title: 'ok', body: [] }; } });
  eOk.start();
  ok('G12 合法 system(on 非空串+run 函数)仍注册成功且 enter 时真触发(反向变异:别误伤合法模块)', ran >= 1);
  // 「新 case 不过」:缺 run / on 非串 / on 空串 → registerModule 当场抛,而不是静默注册一个永不触发的死 system
  ok('G12a system 缺 run 函数 → registerModule 抛(不留「触发时才炸」的哑弹)', throwsOn(function () {
    mk().registerModule({ id: 'no-run', nodeKinds: ['demo'], systems: [{ on: 'enter' }], render: function () { return {}; } });
  }));
  ok('G12b system.on 缺失/非字符串 → registerModule 抛(否则监听死事件、永久静默)', throwsOn(function () {
    mk().registerModule({ id: 'bad-on', nodeKinds: ['demo'], systems: [{ run: function () {} }], render: function () { return {}; } });
  }) && throwsOn(function () {
    mk().registerModule({ id: 'num-on', nodeKinds: ['demo'], systems: [{ on: 3, run: function () {} }], render: function () { return {}; } });
  }));
  ok('G12c system.on 空串 → registerModule 抛', throwsOn(function () {
    mk().registerModule({ id: 'empty-on', nodeKinds: ['demo'], systems: [{ on: '', run: function () {} }], render: function () { return {}; } });
  }));
})();
...
(function () {
  ...
  // W9 rngSeed(R2 二轮红队 P0):引擎自有 int32,与 clock.t/seen 同族但曾独漏 isFinite/整数校验 →
  //   合法 JSON 1e400→Infinity(或非整数)混入后,rng() 的 `|0`(ToInt32)静默清零/截断 → 破坏「同档复现」。反向变异两端都锁。
  const rngRef = e.state, rngBytes = e.serialize();
  // Infinity 无法经 JSON.stringify 注入(→null),故对合法档文本把 rngSeed 值替成 1e400 字面量(合法 JSON token,解析成 Infinity)
  const infEnvelope = e.serialize().replace(/"rngSeed":-?\d+/, '"rngSeed":1e400');
  ok('W9a rngSeed=Infinity(1e400)→ load=false 且 state 原子不动(补齐 clock.t/seen 早有的 isFinite 挡板)', e.load(infEnvelope) === false && e.state === rngRef && e.serialize() === rngBytes);
  const floatEnvelope = JSON.parse(e.serialize()); floatEnvelope.state.rngSeed = 3.7;
  ok('W9b rngSeed 非整数 3.7 → load=false 且 state 原子不动(|0 会截断→序列漂移)', e.load(JSON.stringify(floatEnvelope)) === false && e.state === rngRef && e.serialize() === rngBytes);
  const negIntEnvelope = JSON.parse(e.serialize()); negIntEnvelope.state.rngSeed = -5;
  ok('W9c rngSeed 合法负整数 -5(rng 的 |0 可产生负 int32)→ load=true 且不被误伤(反向变异验牙:别把新判据收过头)', e.load(JSON.stringify(negIntEnvelope)) === true && e.state.rngSeed === -5);
})();

// PP. 原型污染硬化(R2 二轮 P2):存档码/导入码里的 __proto__ 键不得篡改候选 state 的原型链(load/importCode 是外部输入)
(function () {
  const e = createEngine(makeWorld(), { storage: null }); e.registerModule(makeStub()); e.start();
  const cleanBytes = e.serialize();
  // JSON.parse 把 "__proto__" 造成 state 的自有键;修复前 hydrate 的 `s[k]=v` 会触发原型 setter、换掉 state 原型
  const evil = cleanBytes.replace('"flags":{}', '"flags":{},"__proto__":{"polluted":true}');
  e.load(evil);
  ok('PP1 存档里的 __proto__ 键被跳过 → state 原型链不被篡改、全局 Object.prototype 不受污染', ({}).polluted === undefined && e.state.polluted === undefined && Object.getPrototypeOf(e.state) === Object.prototype);
  ok('PP2 无 __proto__ 的合法档仍正常 load 往返(硬化不误伤)', e.load(cleanBytes) === true && Object.getPrototypeOf(e.state) === Object.prototype);
})();
