...
(function (global, factory) {
  ...
  function createEngine(world, opts) {
    ...
    function registerModule(mod) {
      ...
      try {
        (mod.systems || []).forEach(function (s) {
    ...
    function hydrate(data) {
      var s = freshState();
      if (data && data.state) for (var k in data.state) if (Object.prototype.hasOwnProperty.call(data.state, k)) s[k] = data.state[k];
    ...
    function badShape(data) {
      ...
      if (st.rngSeed === null || (st.rngSeed !== undefined && typeof st.rngSeed !== 'number')) return 'rngSeed 形状非法';
      return null;
    }
