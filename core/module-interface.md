...
>
> **v31→v32 变更(cutscene `link.run` 字符串回应对称)**:cutscene 包装出口不再丢弃 `link.run(state)` 返回的非空 string。无回应的移动仍立即导航；有回应的移动先结算 run，把文本作为源过场的一次性 event 帧显示，并把目的地写入 state `_cutscenePending`，本帧只给「继续 →」，点击后走标准 core 导航。pending 目的地入档，刷新/读档不丢继续动作；进入任意新节点时清空。此行为与 text-adventure/tabletop 的“回应先可见，再导航”对称，回应文本本身仍是瞬时帧产物、不入档。
>
...
## 五、服务契约(核心提供,模块/系统调用)
- **RNG**:`api.rng()`→[0,1) 确定性(推进 `state.rngSeed`,mulberry32);`api.dice(n,sides)`。**累加器入档** → 同档逐抽复现、测试可断言。
...
- **事件总线**:`api.emit(type,payload)` / `api.on(type,fn)`(同步 Observer,非队列；事件名是开放字符串，`__proto__` 等也按普通键处理)。核心内置事件:`enter`(payload `{pos,node,first}`,移动/启动后)、`action`(payload `{action}`,每次 apply)。系统间**经事件组合**,互不直接依赖(跑团把"骰子→检定→后果→时钟"用事件串起即此价值)。
...
**存档边界三守则(易用性审计批)**:① `load` 与 `loadLocal` 走**同一道坏档形状闸**(版本不识/引擎自有字段形状非法 → false + 当前状态不动；`clock` 若出现必须是含有限非负 `t` 的非数组对象，不能用 `[]/{}` 绕过；自动续档是最高频读档通道,不豁免);② `load` 只有在 hydrate+render+自动续档回写全成功后才返回 true；回写失败返回 false 并恢复读档前内存 state(否则「读档成功→刷新」=静默回滚到旧磁盘档);③ `storage` 解析全链 try/catch(隐私模式下连 `typeof localStorage` 求值都抛 SecurityError → 降级 null 不白屏),显式 `storage:null`(关持久化)被尊重、不被 `||` 吞。
