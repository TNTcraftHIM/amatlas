...
(function (global, factory) {
  ...
  function createMaze3dModule(opts) {
    ...
    function validateCombatSpec(maze, grid, start) {
      if (!combatHas(maze, 'combat')) return null;
      var combat = maze.combat, root = 'maze.combat';
      combatObject(combat, root);
      combatFields(combat, ['exitRequires', 'deathKey', 'player', 'pistol', 'guard', 'supplies'], root);
      combatRequired(combat, ['deathKey', 'player', 'pistol'], root);
      ...
      combatFields(player, ['maxHealth', 'health', 'ammo', 'maxAmmo'], playerPath);
      combatRequired(player, ['maxHealth', 'health', 'ammo'], playerPath);
      ...
      var ammo = combatNumber(player.ammo, playerPath + '.ammo', false, true);

      var pistol = combat.pistol, pistolPath = root + '.pistol';
      combatObject(pistol, pistolPath);
      combatFields(pistol, ['damage', 'range', 'cooldown', 'recoil'], pistolPath);
      combatRequired(pistol, ['damage', 'range', 'cooldown', 'recoil'], pistolPath);
      var pistolCopy = {
        damage: combatNumber(pistol.damage, pistolPath + '.damage', true, false),
        range: combatNumber(pistol.range, pistolPath + '.range', true, false),
        cooldown: combatNumber(pistol.cooldown, pistolPath + '.cooldown', false, false),
        recoil: combatNumber(pistol.recoil, pistolPath + '.recoil', false, false)
      };
      ...
      if (combatHas(combat, 'guard')) {
        ...
        combatFields(guard, ['x', 'y', 'face', 'hp', 'hitRadius', 'ai'], guardPath);
        ...
        var guardFace = null;
        if (combatHas(guard, 'face')) guardFace = combatOptionalName(guard.face, guardPath + '.face');
        ...
        if (guardFace !== null) guardState.face = guardFace;
      ...
      var suppliesPath = root + '.supplies', supplies = combatHas(combat, 'supplies') ? combat.supplies : [];
      if (!Array.isArray(supplies)) combatFail(suppliesPath, '必须是数组');
      var seenSupplyCells = {}, hasAmmoSupply = false, supplyCopy = [];
      for (var si = 0; si < supplies.length; si++) {
        var supplyPath = suppliesPath + '[' + si + ']';
        if (!(si in supplies)) combatFail(supplyPath, '不能是数组空槽');
        var supply = supplies[si];
        combatObject(supply, supplyPath);
        combatFields(supply, ['x', 'y', 'kind', 'amount'], supplyPath);
        combatRequired(supply, ['x', 'y', 'kind', 'amount'], supplyPath);
        var sx = combatNumber(supply.x, supplyPath + '.x', false, true);
        var sy = combatNumber(supply.y, supplyPath + '.y', false, true);
        if (cellAt(grid, sx, sy) !== '.') combatFail(supplyPath, '必须位于 grid 可走 . 格,得到 (' + sx + ',' + sy + ')');
        if (sx === start.x && sy === start.y) combatFail(supplyPath, '不能与 maze.start 同格 (' + sx + ',' + sy + ')');
        for (var smi = 0; smi < monsterCopy.length; smi++) if (monsterCopy[smi].x === sx && monsterCopy[smi].y === sy) combatFail(supplyPath, '不能与 ' + guardPath + ' 同格 (' + sx + ',' + sy + ')');
        var supplyCell = sx + ',' + sy;
        if (combatHas(seenSupplyCells, supplyCell)) combatFail(supplyPath, '坐标必须唯一;与 ' + seenSupplyCells[supplyCell] + ' 同格 (' + sx + ',' + sy + ')');
        seenSupplyCells[supplyCell] = supplyPath;
        if (Array.isArray(maze.events)) for (var sei = 0; sei < maze.events.length; sei++) {
          var mazeEvent = maze.events[sei];
          if (mazeEvent && mazeEvent.x === sx && mazeEvent.y === sy) combatFail(supplyPath, '不能与 maze.events[' + sei + '] 同格 (' + sx + ',' + sy + ')');
        }
        if (supply.kind !== 'ammo' && supply.kind !== 'health') combatFail(supplyPath + '.kind', "必须是 'ammo' 或 'health'");
        var supplyAmount = combatNumber(supply.amount, supplyPath + '.amount', true, true);
        var supplyState = { sx: sx + 0.5, sy: sy + 0.5, kind: supply.kind, amount: supplyAmount, consumed: false, touching: false, isCombatSupply: true, idx: si };
        if (supply.kind === 'ammo') hasAmmoSupply = true;
        supplyCopy.push(supplyState);
      }

      var maxAmmo = null, maxAmmoPath = playerPath + '.maxAmmo';
      if (!hasAmmoSupply) {
        if (combatHas(player, 'maxAmmo')) combatFail(maxAmmoPath, '仅在 supplies 含 ammo 时允许声明');
      } else {
        if (!combatHas(player, 'maxAmmo')) combatFail(maxAmmoPath, '有 ammo supply 时是必填字段');
        maxAmmo = combatNumber(player.maxAmmo, maxAmmoPath, true, true);
        if (ammo > maxAmmo) combatFail(playerPath + '.ammo', '不能大于 ' + maxAmmoPath);
      }
      var playerState = { maxHealth: maxHealth, health: health, ammo: ammo };
      if (maxAmmo !== null) playerState.maxAmmo = maxAmmo;
      return { exitRequires: exitRequires, deathKey: deathKey, player: playerState, pistol: pistolCopy, monsters: monsterCopy, supplies: supplyCopy };
    }
    // ── /FPS combat 数据闸 ──
    ...
    function combatShotSfx() {                                  // combat 私有确认音:仅有效射击提交边沿触发；复用 hbMaster/静音/stop 生命周期。
      ...
      try {
        ...
        o.type = 'square'; o.frequency.setValueAtTime(168, t); if (o.frequency.exponentialRampToValueAtTime) o.frequency.exponentialRampToValueAtTime(72, t + 0.085);
        gn.gain.setValueAtTime(0.16, t); gn.gain.exponentialRampToValueAtTime(0.0001, t + 0.095);
        o.connect(gn); gn.connect(hbMaster); o.start(t); o.stop(t + 0.1); return true;
    ...
    function startMaze(node) {
      ...
      var combatSession = validateCombatSpec(maze, grid, start);   // 必须在 canvas 创建前 fail-loud；返回值已深分离于作者 spec。
      ...
      if (combatSession) {
        ...
        g.pistol = combatSession.pistol;
        g.combatMonsters = combatSession.monsters;
        g.combatSupplies = combatSession.supplies;
        g.supplyNotice = null;
        g.combatReady = false;
      ...
      function resolveSupplyContacts() {
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen) return false;
        var supplies = g.combatSupplies || [], changed = false;
        for (var i = 0; i < supplies.length; i++) {
          var supply = supplies[i];
          if (!supply || supply.consumed) continue;
          var dx = g.px - supply.sx, dy = g.py - supply.sy, touching = dx * dx + dy * dy < 0.25;
          if (!touching) { supply.touching = false; continue; }
          var firstTouch = !supply.touching;
          supply.touching = true;
          var current = supply.kind === 'health' ? g.player.health : g.player.ammo;
          var cap = supply.kind === 'health' ? g.player.maxHealth : g.player.maxAmmo;
          var next = Math.min(cap, current + supply.amount);
          if (next === current) {
            if (firstTouch) showSupplyNotice(supply.kind === 'health' ? '生命已满' : '弹药已满');
            continue;
          }
          if (supply.kind === 'health') g.player.health = next;
          else g.player.ammo = next;
          supply.consumed = true;
          showSupplyNotice('+' + (next - current) + (supply.kind === 'health' ? ' 生命' : ' 弹药'));
          changed = true;
        }
        return changed;
      }
      ...
      function fireCombatWeapon() {
        ...
        if (g.tw + 1e-9 < g.nextFireAt) {
          g.combatNotice = { text: '手枪回位中', at: g.tw, until: g.tw + 0.48 };
          return false;
        }
        var ammo = g.player.ammo;
        if (!(ammo > 0)) {
          g.lastShot = { kind: 'empty', at: g.tw, weapon: 'pistol' };
          return false;
        }
        g.combatNotice = null;
        g.player.ammo = ammo - 1;
        g.shotSeq++;
        g.lastGunshot = { seq: g.shotSeq, at: g.tw, x: g.px, y: g.py };
        combatShotSfx();
        g.nextFireAt = g.tw + weapon.cooldown;
        g.muzzleUntil = g.tw + 0.055;
        g.recoilUntil = g.tw + 0.18;
        var dx = Math.cos(g.a), dy = Math.sin(g.a), wall = castRay(g.px, g.py, g.a);
        var hit = resolveHitscan(g.px, g.py, dx, dy, weapon.range, wall.dist, g.combatMonsters || []);
        if (!hit) {
          g.lastShot = { kind: 'miss', at: g.tw, weapon: 'pistol', shotSeq: g.shotSeq };
          return true;
        }
        var target = hit.target;
        target.hp = Math.max(0, target.hp - weapon.damage);
        target.painUntil = g.tw + 0.14;
        if (target.hp === 0) { target.dead = true; target.active = false; target.killedAt = g.tw; setCombatMonsterState(target, 'dead'); }
        g.lastShot = { kind: target.dead ? 'kill' : 'hit', at: g.tw, weapon: 'pistol', shotSeq: g.shotSeq, targetId: target.id, hp: target.hp, maxHp: target.maxHp };
        return true;
      }
      ...
      try { if (ENABLE_CONTROLS) {
        ...
        if (g.player) {
          ...
          fireBtn.textContent = '开火'; fireBtn.setAttribute('aria-label', '开火'); fireBtn.setAttribute('title', '开火');
      ...
      function projectSprite(sp) {
        ...
        if (sp.isKey || sp.isCombatSupply || (sp.isItem && !sp.isDecor && !sp.isFloorPickup && !sp.isWallPickup)) {            // 钥匙/坐标事件道具/combat supply:独立 token,底部锚到地面线并共享真实 z-buffer。
      ...
      function combatSupplyLayers(p) {                          // Phase 2a 固定 ammo/medkit 像素形态;不接受作者 art/preset。
        var cx = p.screenX, floorY = p.floorY, w = p.w, h = p.h, L = [];
        if (p.sp.kind === 'ammo') {
          L.push({ x: cx - w * 0.46, y: floorY - h * 0.58, w: w * 0.92, h: h * 0.50, rgb: [42, 58, 68] });
          L.push({ x: cx - w * 0.40, y: floorY - h * 0.51, w: w * 0.80, h: h * 0.10, rgb: [74, 104, 116] });
          for (var i = 0; i < 3; i++) {
            L.push({ x: cx - w * 0.29 + i * w * 0.22, y: floorY - h * 0.46, w: w * 0.10, h: h * 0.27, rgb: [238, 190, 58] });
            L.push({ x: cx - w * 0.29 + i * w * 0.22, y: floorY - h * 0.49, w: w * 0.10, h: h * 0.05, rgb: [255, 232, 126] });
          }
        } else {
          L.push({ x: cx - w * 0.46, y: floorY - h * 0.64, w: w * 0.92, h: h * 0.56, rgb: [224, 232, 236] });
          L.push({ x: cx - w * 0.36, y: floorY - h * 0.70, w: w * 0.72, h: h * 0.10, rgb: [116, 132, 140] });
          L.push({ x: cx - w * 0.09, y: floorY - h * 0.56, w: w * 0.18, h: h * 0.38, rgb: [210, 48, 56] });
          L.push({ x: cx - w * 0.25, y: floorY - h * 0.45, w: w * 0.50, h: h * 0.15, rgb: [210, 48, 56] });
        }
        return L;
      }
      ...
      function drawSprites(zbuf) {                               // 墙画完后叠加;逐列 z-test(精灵比该列墙近才画 → 墙遮挡精灵)
        if (!g.monsters.length && !(g.combatMonsters && g.combatMonsters.length) && !(g.combatSupplies && g.combatSupplies.length) && !(g.items && g.items.length) && !evList.length && !(g.pillars && g.pillars.length) && !(g.decors && g.decors.length)) return;
        var list = [], s, m, pr;
        ...
        if (g.combatSupplies) for (s = 0; s < g.combatSupplies.length; s++) { var supply = g.combatSupplies[s]; if (supply.consumed) continue; pr = projectSprite(supply); if (pr) list.push(pr); }
        ...
        for (var k = 0; k < list.length; k++) {
          ...
          if (p.sp.isKey || p.sp.isCombatSupply || (p.sp.isItem && !p.sp.isFloorPickup && !p.sp.isWallPickup)) ffog = Math.max(0.5, ffog);   // floor 物品(钥匙/事件道具/supply)可见性下限;嵌入式隐藏物保持低调
          ...
          if ((p.sp.isKey || p.sp.isCombatSupply || (p.sp.isItem && !p.sp.isMarker && !p.sp.isDecor && !p.sp.isFloorPickup && !p.sp.isWallPickup)) && p.floorY != null) {   // pickup 接触阴影:独立 token 仍保持“能拿”高可见性,但脚点压在地面上
          ...
          var layers;
          if (p.sp.isCombatSupply) {
            layers = combatSupplyLayers(p);
          ...
          } else if (p.sp.combatTarget && p.sp.dead) {
            var corpseY = floorYAtDepth(p.depth), corpseW = Math.max(5, p.w * 0.78), corpseH = Math.max(2, p.h * 0.10);
            layers = [
              { x: p.screenX - corpseW / 2, y: corpseY - corpseH, w: corpseW, h: corpseH, rgb: [58, 24, 24] },
              { x: p.screenX - corpseW * 0.38, y: corpseY - corpseH * 1.55, w: corpseW * 0.76, h: Math.max(1, corpseH * 0.35), rgb: [118, 58, 42] }
            ];   // 击倒态压到地面线,不假装已有完整 dead sprite atlas。
          } else {
          ...
          for (var Li = 0; Li < layers.length; Li++) {
            ...
            ctx.fillStyle = painFlash ? 'rgb(255,238,206)' : attackTelegraph ? 'rgb(255,112,58)' : fogColor(ly.rgb, fog);   // 白闪=受击；橙红=攻击前摇预兆。
      ...
      function combatFaceLabel() {
        return { normal: '警戒', alert: '警觉', hurt: '受伤', dead: '阵亡' }[g.player && g.player.faceState] || '警戒';
      }
      function drawCombatView() {
        if (!g.player || g.won || g.caught) return;
        updateCombatFaceState();
        var weapon = g.pistol, ammo = g.player.ammo, cx = CW / 2, cy = CH / 2;
        var kick = (!reducedMotion && g.tw < g.recoilUntil) ? Math.max(0, (g.recoilUntil - g.tw) / 0.18) * weapon.recoil * 4 : 0;
        ...
        // 有限 face 状态:固定左下像素头像；状态与 HUD 同源,不暴露公共 View 字段。
        var faceX = 10, faceY = CH - 43, faceColor = g.player.faceState === 'dead' ? 'rgb(92,28,32)' : g.player.faceState === 'hurt' ? 'rgb(205,92,70)' : g.player.faceState === 'alert' ? 'rgb(230,166,72)' : 'rgb(144,158,150)';
        ...
        // 准星:暗描边 + 亮内线,中心留空,不遮命中点。
        ctx.fillStyle = 'rgba(0,0,0,.82)';
        ctx.fillRect(cx - 12, cy - 1, 8, 3); ctx.fillRect(cx + 4, cy - 1, 8, 3); ctx.fillRect(cx - 1, cy - 12, 3, 8); ctx.fillRect(cx - 1, cy + 4, 3, 8);
        ctx.fillStyle = 'rgb(238,232,206)';
        ctx.fillRect(cx - 11, cy, 7, 1); ctx.fillRect(cx + 4, cy, 7, 1); ctx.fillRect(cx, cy - 11, 1, 7); ctx.fillRect(cx, cy + 4, 1, 7);
        // pistol screen-space 地板:程序化底部剪影,不进入世界 z-buffer。
        var wy = CH - 58 + kick, ww = 92, wx = cx - ww / 2;
        ctx.fillStyle = 'rgb(36,40,46)'; ctx.fillRect(wx, wy + 18, ww, 40);
        ctx.fillStyle = 'rgb(92,100,108)'; ctx.fillRect(wx + 12, wy + 6, ww - 24, 24);
        ctx.fillStyle = 'rgb(142,150,154)'; ctx.fillRect(wx + 20, wy + 8, ww - 40, 5);
        ctx.fillStyle = 'rgb(24,27,31)'; ctx.fillRect(cx - 13, wy + 34, 26, 30);
        if (g.tw < g.muzzleUntil) {   // 枪口闪保留在 reduced-motion:它是输入因果,不是装饰性持续运动。
          ctx.fillStyle = 'rgb(255,238,154)'; ctx.fillRect(cx - 8, wy - 12, 16, 18); ctx.fillRect(cx - 18, wy - 7, 36, 8);
          ctx.fillStyle = 'rgb(255,148,54)'; ctx.fillRect(cx - 4, wy - 18, 8, 16);
        }
        if (g.lastShot && (g.lastShot.kind === 'hit' || g.lastShot.kind === 'kill') && g.tw - g.lastShot.at < 0.16) {
          ctx.fillStyle = g.lastShot.kind === 'kill' ? 'rgb(255,194,78)' : 'rgb(255,246,220)';
          ctx.fillRect(cx - 9, cy - 9, 3, 3); ctx.fillRect(cx + 6, cy - 9, 3, 3); ctx.fillRect(cx - 9, cy + 6, 3, 3); ctx.fillRect(cx + 6, cy + 6, 3, 3);
        }
        var statusText = '生命 ' + g.player.health + '/' + g.player.maxHealth + '  ' + combatFaceLabel() + '  弹药 ' + ammo + '  敌人 ' + combatAliveCount();
        ctx.font = '12px system-ui,sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'bottom'; ctx.fillStyle = 'rgb(238,243,248)';
        if (ctx.strokeText) { ctx.strokeStyle = 'rgba(0,0,0,.9)'; ctx.lineWidth = 3; ctx.strokeText(statusText, CW - 10, CH - 9); }
        ctx.fillText(statusText, CW - 10, CH - 9);
        if (g.supplyNotice && g.tw < g.supplyNotice.until) {
          ctx.fillStyle = 'rgb(255,232,148)';
      ...
      function render() {
        ...
        if (!g.caught && !g.won && !(g.player && g.player.dead)) {
          ...
          var objHint = g.player ? (!g.combatReady ? '● 待命:移动、转向或开火开始' : clearLockedDoor ? '● 出口封锁:剩余 ' + combatAliveCount() + ' 名守卫' : combatWindupCount() ? '● 攻击预兆:守卫正在蓄力' : combatAliveCount() ? '● 生存场:击倒守卫并活着离开' : '● 守卫已倒下,前往出口')
        ...
        if (g.player && !g.won && !g.caught) {
          var ca0 = g.player.ammo, alive0 = combatAliveCount();
          combatHud = g.player.dead ? '阵亡 · 从下面的选项重试或返回'
            : !g.combatReady ? '待命 · 生命 ' + g.player.health + '/' + g.player.maxHealth + ' · 弹药 ' + ca0 + ' · 敌人 ' + alive0 + ' · 按移动/转向/Space/开火开始'
              : (keyLockedDoor && clearLockedDoor) ? '出口封锁 · 先取得 ' + kE + '，并击倒剩余 ' + alive0 + ' 名守卫'
              : clearLockedDoor ? '出口封锁 · 仍有 ' + alive0 + ' 名守卫'
                : keyLockedDoor ? '🔒 门锁着 · 先找到那把 ' + kE
                  : '生存场 · 生命 ' + g.player.health + '/' + g.player.maxHealth + ' · ' + combatFaceLabel() + ' · 弹药 ' + ca0 + ' · 敌人 ' + alive0;
          ...
          if (!g.player.dead && combatWindupCount()) combatHud += ' · 攻击预兆:守卫正在蓄力';
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'empty') combatHud += ' · 弹药耗尽';
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'miss') combatHud += ' · 未命中';
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'hit') combatHud += ' · 命中守卫 (' + g.lastShot.hp + '/' + g.lastShot.maxHp + ')';
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'kill') combatHud += ' · 守卫已击倒';
        }
      ...
      function loop(ts) {
        ...
        if (!STATIC_PREVIEW && !g.won && !g.caught && !g.puzzleOpen && !(g.player && g.player.dead)) {
          ...
          resolveSupplyContacts();        // Phase 2a:移动/K 接触后、坐标事件与敌人 attack commit 前结算 session-local supply。
