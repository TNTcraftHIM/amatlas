/* ════════════════════════════════════════════════════════════════════════
   Amatlas origin · 引擎启动胶水(origin/game.js)
   ════════════════════════════════════════════════════════════════════════
   ⚠ 这是引擎综合验证沙盒(dogfood),不是起步模板。
   起步请复制 text-adventure-demo/ 或 tabletop-demo/ 目录。
   origin 演示多模块共存、自定义皮肤与综合 manifest,超出普通游戏所需。
   ════════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function boot() {
    ...
    var engine = window.Amatlas.boot(WORLD, {
      ...
      status: function (S) {
        ...
        if (S.firstWorldTrialWon) bits.push({ label: '第一场噪声', value: '已穿过' });
        else if (S.firstWorldTrialDeath) bits.push({ label: '第一场噪声', value: '等待重试' });
        return bits;
      },
      save: true,
      ...
      achievements: [
        ...
        },
        {
          id: 'first-trial',
          title: '守住第一条规律',
          description: '击倒未定噪声凝成的守卫，并把第一条规律送到出口',
          hidden: true,
          on: 'action',
          when: function (S) { return !!S.firstWorldTrialWon; }
        },
        {
          id: 'first-world',
          title: '点亮第一个世界',
