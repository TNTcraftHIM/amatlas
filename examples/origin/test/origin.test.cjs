...
function inspect(world, html, originMaze, referenceMaze) {
  ...
  if (!/\.amatlas-plugin-backdrop\s*\{/.test(html) || /\.amatlas-plugin-panel::before\s*\{/.test(html)) errors.push('plugin-backdrop-theme');
  var childLayer = (html.match(/#app\s*>\s*\*\s*\{[^}]*\}/) || [''])[0];
  var pluginLayer = (html.match(/#plugin-bar\s*\{[^}]*\}/) || [''])[0];
  if (/z-index\s*:/.test(childLayer) || !/z-index\s*:\s*2\s*;/.test(pluginLayer)) errors.push('plugin-modal-layer');
  ...
  var pickups = combat && combat.pickups || [], weaponPickup = pickups[0], healthPickup = pickups[1];
  if (!combat || !combat.guard || combat.guard.x !== 6 || combat.guard.y !== 3 || combat.guard.hp !== 60 || own(combat.guard, 'face') || combat.guard.ai.sight !== 0 || combat.guard.ai.hear !== 8 || combat.guard.ai.moveSpeed !== 0 || pickups.length !== 2 ||
      !weaponPickup || keys(weaponPickup) !== 'ammo,kind,maxAmmo,weapon,x,y' || weaponPickup.x !== 5 || weaponPickup.y !== 3 || weaponPickup.kind !== 'weapon' || weaponPickup.weapon !== 'scatter' || weaponPickup.ammo !== 1 || weaponPickup.maxAmmo !== 4 ||
      !healthPickup || keys(healthPickup) !== 'amount,kind,x,y' || healthPickup.x !== 2 || healthPickup.y !== 2 || healthPickup.kind !== 'health' || healthPickup.amount !== 20 || pickups.some(function (pickup) { return own(pickup, 'id'); }) ||
      (combat.pickups || []).some(function (pickup) { return pickup.kind === 'ammo'; }))
    errors.push('combat-balance');
  var trialMaze = trial && trial.maze || {}, trialAudio = trial && trial.audio || {};
  var closeGrid = ['#############', '#.....#.....#', '#.....#.....#', '#..........D#', '#.....#.....#', '#.....#.....#', '#############'];
  if (JSON.stringify(trialMaze.grid) !== JSON.stringify(closeGrid) || !trialMaze.start || trialMaze.start.x !== 1 || trialMaze.start.y !== 3 || trialMaze.start.dir !== 'E' || trialMaze.maxDecor !== 2)
    errors.push('trial-close-breach');
...
ok('O1b Origin 回廊保持 sacral BGM，trial 文案只读成星图刻针/星环共振器/未定噪声',
  ...
  themeCopy);
ok('O1c Origin theme 已迁到显式 backdrop selector 且不再保留 panel::before', /\.amatlas-plugin-backdrop\s*\{/.test(html) && !/\.amatlas-plugin-panel::before\s*\{/.test(html));
ok('O1d Origin plugin-bar 独占正文上一层且普通 #app 子项不自建 stacking context', inspect(freshWorld(), html, mazeA, mazeB).indexOf('plugin-modal-layer') < 0);
...
var oldBackdropSelector = inspect(freshWorld(), html.replace(/\.amatlas-plugin-backdrop\s*\{/, '.amatlas-plugin-panel::before {'), mazeA, mazeB);
ok('M9 反向把 Origin backdrop selector 改回 panel::before 精确转红', oldBackdropSelector.indexOf('plugin-backdrop-theme') >= 0, oldBackdropSelector.join(','));

var trappedModal = inspect(freshWorld(), html.replace(/(#app\s*>\s*\*\s*\{[^}]*)/, '$1\n    z-index: 1;').replace(/(#plugin-bar\s*\{[^}]*?)\s*z-index\s*:\s*2\s*;/, '$1'), mazeA, mazeB);
ok('M10 反向让同级正文压住 plugin-bar 模态层时精确转红', trappedModal.indexOf('plugin-modal-layer') >= 0, trappedModal.join(','));
