...
(function (global, factory) {
  ...
  function createSidescrollerModule(opts) {
    ...
    function start(node) {
      ...
      var restart = doc.createElement('button'); restart.textContent = '重开本局'; restart.className = 'amatlas-touchpad-key amatlas-touchpad-key--pill'; restart.setAttribute('type', 'button'); restart.setAttribute('aria-label', '重开本局'); restart.style.minHeight = '44px'; restart.addEventListener('click', function () { if (activeSim && activeSim.dead) restartSimulation(plan, activeSim); }); controls.appendChild(restart);
      ...
      function draw() {
        ...
        hud.textContent = sim.clear ? '哨戒炮已摧毁 · 海堤门解除封锁'
          : sim.dead ? '生命 0 / ' + plan.player.health + ' · 已阵亡，按“重开本局”'
          : '生命 ' + sim.hp + ' / ' + plan.player.health + ' · 哨戒炮 ' + (sim.sentry ? sim.sentry.hp : 0) + ' · x ' + Math.round(sim.xQ / Q) + ' · tick ' + sim.tick;
      }
      function frame(ts) {
    ...
    var mod = {
      ...
      render: function (state, node) { return { title: node.title || '横版试验', body: [{ type: 'prose', text: '方向键或 A/D 移动，W/↑跳跃，空格开火；也可使用屏上按钮。击毁哨戒炮后海堤门才会解除封锁。' }], status: [] }; },
