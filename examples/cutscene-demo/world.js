...
(function (global, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = factory(require('../../modules/cutscene/authoring/compile-performance.js'));
  } else {
    global.CUTSCENE_DEMO_WORLD = factory(global.Amatlas && global.Amatlas.CutsceneAuthoring);
  }
})(typeof globalThis !== 'undefined' ? globalThis : this, function (CutsceneAuthoring) {
  'use strict';

  if (!CutsceneAuthoring || typeof CutsceneAuthoring.compilePerformance !== 'function') {
    throw new Error('[cutscene-demo] CutsceneAuthoring.compilePerformance missing before world.js; load ' +
      'present-timeline.js -> modules/cutscene/authoring/compile-performance.js -> world.js');
  }
  ...
  function makePerformanceStaticRig() {
    return {
  ...
  var INTRO_PERFORMANCE = CutsceneAuthoring.compilePerformance({
    version: 1,
    actors: [{
      id: 'warden',
    ...
    anchors: [
      { id: 'mist_edge', x: 88, y: 105, facing: 'right' },
      { id: 'tide_mark', x: 160, y: 105, facing: 'right' },
      { id: 'tower_light', x: 260, y: 58, capacity: 0 }
    ],
    shots: [
      {
        id: 'walk_point_caption',
        cast: [{ actor: 'warden', at: 'mist_edge' }],
        sequence: [
          { do: 'walkTo', actor: 'warden', to: 'tide_mark', dur: 2.4, steps: 4 },
          { do: 'pointAt', actor: 'warden', target: 'tower_light', hand: 'right', dur: 0.6 },
          { do: 'caption', lines: [
            { cps: 15, chunks: [{ text: '守灯人沿退潮线走到潮标旁，抬手指向雾里的灯塔。', pauseAfter: 0.18 }] }
          ] }
        ]
      },
      {
        id: 'say_warning',
        cast: [{ actor: 'warden', at: 'tide_mark' }],
        sequence: [
          { do: 'say', actor: 'warden', mouth: ['A', 'O', 'A', 'O'], lines: [
            { cps: 12, chunks: [{ text: '第三次回潮前，跟紧这道光。', pauseAfter: 0.2 }] }
          ] }
        ]
      }
    ]
  }, { path: 'CUTSCENE_DEMO_WORLD.maps.coast.nodes.intro.performance' });
      ...
      coast: {
        ...
        nodes: {
          ...
          intro: {
            ...
            ].concat(INTRO_PERFORMANCE.beats, [
              { dur: 4,
            ...
            links: [ { to: 'shore', label: '踏上沙滩' }, { to: 'intro', label: '重看序章' } ]
          },
