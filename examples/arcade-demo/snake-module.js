  ...
  function createSnakeModule(opts) {
    ...
    function startGame(node) {
      ...
      function draw() {
        ...
        ctx.fillStyle = '#e8edf4'; ctx.font = '13px system-ui,sans-serif';
        ...
        ctx.fillText(hud, 8, canvas.height - 8);
      }
      ...
      controls.style.cssText = 'display:flex;flex-wrap:wrap;justify-content:center;gap:8px;margin-top:10px';
      function controlButton(label, aria, fn) {
        var b = hostDoc.createElement('button'); b.textContent = label; b.setAttribute('type', 'button'); b.setAttribute('aria-label', aria);
        b.style.minWidth = '44px'; b.style.minHeight = '44px'; b.style.touchAction = 'manipulation';
        b.addEventListener('pointerdown', function (e) { if (e && e.preventDefault) e.preventDefault(); fn(); });
        return b;
      }
      controls.appendChild(controlButton('←', '向左', function () { setDirection({ x: -1, y: 0 }); }));
      controls.appendChild(controlButton('↑', '向上', function () { setDirection({ x: 0, y: -1 }); }));
      controls.appendChild(controlButton('↓', '向下', function () { setDirection({ x: 0, y: 1 }); }));
      controls.appendChild(controlButton('→', '向右', function () { setDirection({ x: 1, y: 0 }); }));
      controls.appendChild(controlButton('重开本局', '重开本局', restartLocal));
