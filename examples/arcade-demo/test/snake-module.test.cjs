...
(function () {   // A11 真鼠标单击=pointerdown+click 双事件:click 只该给键盘(detail===0),否则"重开"双触发→双 newGame/双抽 rngSeed
  var h = makeHarness();
  var rngN = 0; h.api.rng = function () { rngN++; return 0; };   // 计数 placeFood 的 api.rng() 抽取(确定性 rng=0)
  h.api.fire({ kind: 'arcade', failKey: 'snakeFails', lockAfter: 9, goal: 9 });
  var restart = h.button('重开本局');
  var b0 = rngN; if (restart && restart._h.pointerdown) restart._h.pointerdown({ preventDefault: function () {} }); var per = rngN - b0;
  ok(!!restart && per > 0, 'A11a 一次重开(pointerdown)消耗 rng(placeFood 抽种) drew=' + per);
  var b1 = rngN; if (restart && restart._h.pointerdown) restart._h.pointerdown({ preventDefault: function () {} }); if (restart && restart._h.click) restart._h.click({ detail: 1 }); var mouse = rngN - b1;
  ok(mouse === per, 'A11b 鼠标单击"重开"(pointerdown+click detail≥1)只重开一次,rng 抽数=单次 ' + per + ' 非双倍(变异=click 无条件触发→' + (2 * per) + ') drew=' + mouse);
  var b2 = rngN; if (restart && restart._h.click) restart._h.click({ detail: 0 });
  ok(rngN - b2 === per, 'A11c 键盘 Enter/Space 的 click(detail===0)仍触发重开,可达性不回归 drew=' + (rngN - b2));
})();
