...
function resetEnv(opts) {
  opts = opts || {};
  fillRects = 0; drawTexts = []; rafCb = null; rafPending = {}; rafNext = 1; winH = {}; docH = {}; winEvents = makeEventTarget(winH); docEvents = makeEventTarget(docH); canvasCreateCount = 0; cancelCount = 0; spokeCount = 0; pwCount = 0; audioStarts = 0; translateCount = 0; mockConvolvers = []; mockFilters = []; mockOscillators = [];
...
(function () {   // ZA2a v3 recipe 必须有可获得第二武器、定向弹药、单 guard、死亡出口和真正清 durable deathKey 的重试。
  ...
  ok(enter && enter.run && combat.exitRequires === 'clear' && guard && guard.hp === 60 && !Object.prototype.hasOwnProperty.call(guard, 'id') && !Object.prototype.hasOwnProperty.call(guard, 'face') && guard.ai.moveSpeed === 1.1 && guard.ai.sight === 0 && guard.ai.hear === 8 && guard.ai.attackRange === 1.35 && guard.ai.damage === 20 && guard.ai.windup === 0.55 && guard.ai.cooldown === 0.75 &&
    ...
    ammoPickup.x === 3 && ammoPickup.y === 3 && ammoPickup.kind === 'ammo' && ammoPickup.weapon === 'precision' && ammoPickup.amount === 3 &&
    medkit.x === 1 && medkit.y === 2 && medkit.kind === 'health' && medkit.amount === 20 &&
    ...
    "ZA2a Recipe 5 是 v3 precision 2/6 + scatter pickup 1/4 + 定向 precision ammo +3 + health，60HP guard 且重试清胜负键 state=" + JSON.stringify(state));
  ok(fps.maze.grid[start.y][start.x] === '.' && fps.maze.grid[medkit.y][medkit.x] === '.' &&
    Math.abs(medkit.x - start.x) + Math.abs(medkit.y - start.y) === 1,
    'ZA2a2 health pickup 是起点侧邻可走格');
  ok(weaponPickup.y === start.y && ammoPickup.y === start.y && weaponPickup.x < ammoPickup.x,
    'ZA2a3 主轴先获得 scatter 并自动装备，再经过 precision 定向 ammo');
...
(function () {   // ZA10b FPS 路由:Claude/Codex/README/Showroom 都须认识现有私有 runtime,但不得冒充 boot 内置模块。
  ...
  var newGameMiss = hasAll(newGameMd, routeNeedles);
  ...
  var claudeMiss = hasAll(engineClaudeMd, ['maze3d` 私有 runtime', 'fps-range', 'maze3d-authoring.md', 'manifest.modules', '不要误判成引擎没有的玩法', 'precision', 'scatter', 'pickups']);
  var readmeMiss = hasAll(engineReadme, ['basic / horror / puzzle / layers / fps-range', '不是五个模块', 'examples/maze3d/fps.html', 'start=fps_range', 'precision', 'scatter', 'maze.theme']);
  var showroomMiss = hasAll(showroomHtml, ['FPS 生存场 · 玩家直达', 'examples/maze3d/fps.html', '正式 raycast runtime', '统一 world', '精确手枪', '近程霰弹枪', '主题']);
  var changelogMiss = hasAll(engineChangelog, ['FPS 私有作者契约 v3', 'clean cut', 'precision', 'scatter', 'Origin', 'cutscene']);
  ok(!newGameMiss.length && !skillMiss.length && !agentsMiss.length && !claudeMiss.length && !readmeMiss.length && !showroomMiss.length && !changelogMiss.length,
    'ZA10b 所有 AI/人类入口可发现 v3 FPS 私有装配,且不误承诺公共模块 newGame=' + newGameMiss.join(',') + ' skill=' + skillMiss.join(',') + ' agents=' + agentsMiss.join(',') + ' claude=' + claudeMiss.join(',') + ' readme=' + readmeMiss.join(',') + ' showroom=' + showroomMiss.join(',') + ' changelog=' + changelogMiss.join(','));
})();
