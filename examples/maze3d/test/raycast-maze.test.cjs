...
(function () {   // A6 gallery 静态预览选项:只画首帧,不挂输入/按钮/HUD/音频,一页多 canvas 不烧 CPU/不抢键盘。
  ...
  ok(previewCanvas && previewCanvas.getAttribute('tabindex') === null && focusCount === 0,
    'A6 staticPreview/controls:false 的无操作 canvas 不进 Tab 顺序也不主动抢焦点(变异=Showroom 打开 Gallery 跳页尾+92 个死 Tab 站) tabindex=' + (previewCanvas && previewCanvas.getAttribute('tabindex')) + ' focus=' + focusCount);
