...
var fillRects, drawTexts, rafCb, rafPending, rafNext, winH, docH, winEvents, docEvents, stageEl, canvasCreateCount, cancelCount, spokeCount, pwCount, audioStarts, translateCount;   // winEvents/docEvents=真实多 listener EventTarget spy；winH/docH 保留便捷 dispatch 入口   // canvasCreateCount=combat validator 是否真在 canvas 创建前 fail-loud；audioStarts=osc/buffersource.start 调用数(脚步/开门/张力/拾取 出声证据);translateCount=ctx.translate 调用数(震屏/头部晃动证据);pwCount=createPeriodicWave 调用数(glottalWave 声门源=formant 念白唯一来源 → 隔离"是否真合成了念白")
...
function resetEnv(opts) {
  opts = opts || {};
  fillRects = 0; drawTexts = []; rafCb = null; rafPending = {}; rafNext = 1; winH = {}; docH = {}; winEvents = makeEventTarget(winH); docEvents = makeEventTarget(docH); canvasCreateCount = 0; cancelCount = 0; spokeCount = 0; pwCount = 0; audioStarts = 0; translateCount = 0; mockConvolvers = []; mockFilters = [];
...
section('D0b FPS Phase 0b combat 数据闸 + 会话骨架');
var COMBAT_GRID = ['#######', '#.....#', '#.....#', '#.....#', '#######'];
function validCombatSpec() {
  return {
    deathKey: 'armoryDied',
    player: { maxHealth: 100, health: 80, ammo: 12 },
    pistol: { damage: 20, range: 12, cooldown: 0.32, recoil: 2 },
    guard: { x: 4, y: 2, face: 'zombie', hp: 40, hitRadius: 0.23,
...
function validSupplySpec() {
  ...
  spec.player.ammo = 2;
  spec.player.maxAmmo = 6;
  spec.supplies = [
    { x: 2, y: 2, kind: 'ammo', amount: 3 },
...
(function () {   // 合法 combat 与合法旧 maze 均必须走真实 critical enter/startMaze。
  ...
  zero.player.health = 0; zero.player.ammo = 0;
  zero.pistol.cooldown = 0; zero.pistol.recoil = 0;
  zero.guard.ai.sight = 0; zero.guard.ai.hear = 0; zero.guard.ai.moveSpeed = 0; zero.guard.ai.windup = 0; zero.guard.ai.cooldown = 0; delete zero.guard.face;
  var zeroLegal = startCombat(zero);
  ok(!zeroLegal.error, 'D0b2 语义上可为 0 的 health/ammo/cooldown/recoil/sight/hear/moveSpeed/windup/AI cooldown 合法');
...
(function () {   // combat 与每层嵌套对象都必须真 plain object。
  ...
  [
    ...
    ['pistol', function (s) { s.pistol = []; }, 'maze.combat.pistol'],
...
(function () {   // 冻结白名单：后续字段与未消费能力明确拒绝；每层 typo 都报完整路径。
  ['pickups', 'doors', 'objectives', 'legacyKeyMode', 'weapon'].forEach(function (field) {
  ...
  [
    ...
    ['pistol.extra', function (s) { s.pistol.extra = 1; }, 'maze.combat.pistol.extra'],
    ['pistol.sfx', function (s) { s.pistol.sfx = 'pistol'; }, 'maze.combat.pistol.sfx'],
...
(function () {   // 所有冻结必填字段逐项锁路径；face 是仅有的可选叶子。
  var required = [
    [['deathKey'], 'maze.combat.deathKey'], [['player'], 'maze.combat.player'], [['pistol'], 'maze.combat.pistol'],
    [['player', 'maxHealth'], 'maze.combat.player.maxHealth'], [['player', 'health'], 'maze.combat.player.health'], [['player', 'ammo'], 'maze.combat.player.ammo'],
    [['pistol', 'damage'], 'maze.combat.pistol.damage'], [['pistol', 'range'], 'maze.combat.pistol.range'],
    [['pistol', 'cooldown'], 'maze.combat.pistol.cooldown'], [['pistol', 'recoil'], 'maze.combat.pistol.recoil'],
...
(function () {   // 每一个 number 叶子都单独证明拒绝 Infinity；边界另锁正数/非负数/整数语义。
  var numbers = [
    [['player', 'maxHealth'], 'maze.combat.player.maxHealth'], [['player', 'health'], 'maze.combat.player.health'], [['player', 'ammo'], 'maze.combat.player.ammo'],
    [['pistol', 'damage'], 'maze.combat.pistol.damage'], [['pistol', 'range'], 'maze.combat.pistol.range'], [['pistol', 'cooldown'], 'maze.combat.pistol.cooldown'], [['pistol', 'recoil'], 'maze.combat.pistol.recoil'],
  ...
  var bounds = [
    [['player', 'maxHealth'], 0, 'maze.combat.player.maxHealth'], [['player', 'health'], -1, 'maze.combat.player.health'], [['player', 'ammo'], -1, 'maze.combat.player.ammo'],
    [['pistol', 'damage'], 0, 'maze.combat.pistol.damage'], [['pistol', 'range'], 0, 'maze.combat.pistol.range'], [['pistol', 'cooldown'], -1, 'maze.combat.pistol.cooldown'], [['pistol', 'recoil'], -1, 'maze.combat.pistol.recoil'],
  ...
  badCombat('ammo 必须整数', function (s) { s.player.ammo = 1.5; }, 'maze.combat.player.ammo');
...
(function () {   // 旧复数/引用形状必须精确报错；guard 继续守坐标与普通 monsters 双真相。
  ...
  badCombat('旧对象 ammo', function (s) { s.player.ammo = { bullets: 12 }; }, 'maze.combat.player.ammo');
  badCombat('旧对象 maxAmmo', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.player.maxAmmo = { bullets: 6 }; }, 'maze.combat.player.maxAmmo');
  badCombat('旧 pistol.ammo 引用字段', function (s) { s.pistol.ammo = 'bullets'; }, 'maze.combat.pistol.ammo');
  badCombat('旧 pistol.spread 占位', function (s) { s.pistol.spread = 0; }, 'maze.combat.pistol.spread');
  ...
  badCombat('face 坏 shape', function (s) { s.guard.face = 7; }, 'maze.combat.guard.face');
})();
...
(function () {   // 临时源码探针只观察真实 startMaze 里的 g；产品 API 仍不暴露私有会话。
  ...
  if (src.indexOf(needle) < 0 || (src.match(/function validateCombatSpec\(/g) || []).length !== 1) {
    ok(false, 'D0b 会话探针锚点/validateCombatSpec 数量与 runtime 脱节');
    return;
  }
  ...
  try {
    ...
    var separated = g && g.player !== spec.player && g.pistol !== spec.pistol &&
      g.combatMonsters !== spec.guard && g.combatMonsters[0] !== spec.guard && g.combatMonsters[0].ai !== spec.guard.ai;
    ok(separated && g.combatDeathKey === legalDeathKey && g.player.health === 80 && g.player.ammo === 12 &&
      g.pistol.damage === 20 && Object.keys(g.pistol).sort().join(',') === 'cooldown,damage,range,recoil' &&
      g.combatMonsters.length === 1 && g.combatMonsters[0].id === 'guard' && g.guard === undefined && g.weapons === undefined &&
      g.combatMonsters[0].sx === 4.5 && g.combatMonsters[0].maxHp === 40 && g.combatMonsters[0].active === true && g.combatMonsters[0].dead === false,
      'D0b6 v2 会话深分离；g.pistol 与唯一 g.combatMonsters owner 精确，内部 guard id 固定且不另建 g.guard/g.weapons');
    ...
    g.player.health = 1; g.player.ammo = 0; g.pistol.damage = 1; g.combatMonsters[0].hp = 1; g.combatMonsters[0].ai.sight = 1;
    pump(5);
    ok(JSON.stringify(spec) === before && Object.keys(api.state).length === 0, 'D0b7 反向改会话深层对象不污染作者 spec；本步不写伤害/deathKey 到 Atlas state');
    var publicApi = require(MAZE);
    ok(publicApi.validateCombatSpec === undefined, 'D0b8 combat 校验/会话骨架保持 maze 私有,不扩 module.exports/公共契约');
  } catch (e) {
...
section('D0c FPS Phase 2a supplies/maxAmmo 数据闸');
(function () {
  ...
  ok(!started.error && !rendered.threw && JSON.stringify(legal) === before,
    'D0c1 ammo+health supplies 合法启动且 validator 不污染作者 spec');
  var empty = validCombatSpec(); empty.supplies = [];
  var healthOnly = validCombatSpec(); healthOnly.supplies = [{ x: 2, y: 1, kind: 'health', amount: 10 }];
  ok(!startCombat(empty).error && !startCombat(healthOnly).error,
    'D0c2 空 supplies/health-only 无 maxAmmo 均合法');

  badCombat('supplies 非数组', function (s) { s.supplies = {}; }, 'maze.combat.supplies');
  badCombat('supplies 稀疏数组', function (s) { s.supplies = new Array(1); }, 'maze.combat.supplies[0]');
  badCombat('supply 非 plain object', function (s) { s.supplies = [[]]; }, 'maze.combat.supplies[0]');
  badCombat('supply 未知字段/art 逃生口', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.supplies[0].art = 'clip.png'; }, 'maze.combat.supplies[0].art');
  ['x', 'y', 'kind', 'amount'].forEach(function (field) {
    badCombat('supply 缺必填 ' + field, function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; delete s.supplies[0][field]; }, 'maze.combat.supplies[0].' + field);
  });
  badCombat('旧 supply.id 无消费者字段', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.supplies[0].id = 'clip'; }, 'maze.combat.supplies[0].id');
  badCombat('旧 supply.ammo 引用字段', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.supplies[0].ammo = 'bullets'; }, 'maze.combat.supplies[0].ammo');
  badCombat('supply kind 闭枚举', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'armor', amount: 2 }]; }, 'maze.combat.supplies[0].kind');
  badCombat('supply amount 正整数', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 0 }]; }, 'maze.combat.supplies[0].amount');
  badCombat('supply amount 禁小数', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 1.5 }]; }, 'maze.combat.supplies[0].amount');
  badCombat('supply x/y 必须整数', function (s) { s.supplies = [{ x: 2.5, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0].x');
  badCombat('supply 坐标唯一', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 2 }, { x: 2, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[1]');

  badCombat('supply 在墙格', function (s) { s.supplies = [{ x: 0, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]');
  badCombat('supply 在 K 格', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]', { grid: ['#######', '#.K...#', '#.....#', '#.....#', '#######'] });
  badCombat('supply 在 D 格', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]', { grid: ['#######', '#.D...#', '#.....#', '#.....#', '#######'] });
  badCombat('supply 与 start 同格', function (s) { s.supplies = [{ x: 1, y: 2, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]');
  badCombat('supply 与 guard 同格', function (s) { s.supplies = [{ x: 4, y: 2, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]');
  badCombat('supply 与 maze.events 同格', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 2 }]; }, 'maze.combat.supplies[0]', { events: [{ x: 2, y: 1, hint: 'event' }] });

  badCombat('ammo supply 缺 maxAmmo', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; delete s.player.maxAmmo; }, 'maze.combat.player.maxAmmo');
  badCombat('maxAmmo cap 正整数', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.player.maxAmmo = 0; }, 'maze.combat.player.maxAmmo');
  badCombat('maxAmmo cap 禁小数', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.player.maxAmmo = 6.5; }, 'maze.combat.player.maxAmmo');
  badCombat('初始 ammo 不得超过 cap', function (s) { var x = validSupplySpec(); s.player = x.player; s.supplies = x.supplies; s.player.ammo = 7; }, 'maze.combat.player.ammo');
  badCombat('无 ammo supply 禁 maxAmmo', function (s) { s.player.maxAmmo = 20; }, 'maze.combat.player.maxAmmo');
  badCombat('health-only supplies 禁 maxAmmo', function (s) { s.supplies = [{ x: 2, y: 1, kind: 'health', amount: 2 }]; s.player.maxAmmo = 20; }, 'maze.combat.player.maxAmmo');
})();
...
(function () {
  ...
  try {
    ...
    ok(!firstError && prevented && g.combatReady === true && g.shotSeq === 1 && g.player.ammo === 11 && g.combatMonsters[0].hp === 20 && !g.combatMonsters[0].dead,
    ...
    ok(g.shotSeq === 1 && g.player.ammo === 11 && g.combatMonsters[0].hp === 20,
      'D1d 键盘 repeat 不排第二发');
    ...
    ok(!killError && g.shotSeq === 2 && g.player.ammo === 10 && g.combatMonsters[0].hp === 0 && g.combatMonsters[0].dead === true && g.combatMonsters[0].active === false && g.lastShot.kind === 'kill',
      'D1f keyup 后第二发恰好击倒 40HP 静态目标,HP clamp 到 0 且 dead=true/active=false');
    keyRelease(); run.step(22); keyFire(false); run.step(1);
    ok(g.shotSeq === 3 && g.player.ammo === 9 && g.combatMonsters[0].hp === 0 && g.lastShot.kind === 'miss' && /敌人 0/.test(findHud()),
    ...
    var heldAfterDrop = cooling.g.shotSeq === 1 && cooling.g.player.ammo === 11 && cooling.g.combatMonsters[0].hp === 20 && /手枪回位中/.test(cooldownHud);
    keyRelease(); keyFire(false); cooling.step(1);
    ok(heldAfterDrop && cooling.g.shotSeq === 2 && cooling.g.player.ammo === 10 && cooling.g.combatMonsters[0].hp === 0 && !/手枪回位中/.test(findHud()),
      'D1g1 cooldown-rejected press 不耗弹、不缓冲且短显“手枪回位中”;冷却结束后仍须 keyup+新 keydown 才开火 hud=' + JSON.stringify(cooldownHud));

    var blockedSpec = validCombatSpec();
    var blocked = begin(blockedSpec, ['#######', '#.....#', '#..#..#', '#.....#', '#######']);
    keyFire(false); blocked.step(1);
    ok(blocked.g.shotSeq === 1 && blocked.g.player.ammo === 11 && blocked.g.combatMonsters[0].hp === 40 && blocked.g.lastShot.kind === 'miss',
      'D1h 墙遮挡目标 → 子弹消耗但目标 HP 不变');

    var emptySpec = validCombatSpec(); emptySpec.player.ammo = 0;
    var empty = begin(emptySpec); keyFire(false); empty.step(1);
    ok(empty.g.shotSeq === 0 && empty.g.player.ammo === 0 && empty.g.combatMonsters[0].hp === 40 && empty.g.lastShot.kind === 'empty',
    ...
    locked.step(1);
    ok(lockCalls === 1 && locked.g.combatReady === true && locked.g.shotSeq === 0 && locked.g.player.ammo === 12,
    ...
    if (cv && cv._h && cv._h.mousedown) cv._h.mousedown({ button: 0, preventDefault: function () {} });
    locked.step(1);
    ok(locked.g.shotSeq === 1 && locked.g.player.ammo === 11,
      'D1k 首击释放后,已锁 Pointer Lock 的下一次主键 press 才开火');

    var fallbackSpec = validCombatSpec(); fallbackSpec.pistol.cooldown = 0;
    ...
    if (fallbackCanvas && fallbackCanvas._h && fallbackCanvas._h.mousedown) fallbackCanvas._h.mousedown({ button: 0, preventDefault: function () {} });
    fallback.step(1);
    ok(fallbackHeld && fallback.g.shotSeq === 3 && fallback.g.player.ammo === 9,
      'D1k1 无 requestPointerLock fallback 可开火;重复 mousedown 被锁存且 pointerleave/pointercancel 各自释放后可再按');

    var touchSpec = validCombatSpec(); touchSpec.pistol.cooldown = 0; touchSpec.guard.hp = 100;
    ...
    if (touchBtn && touchBtn._h && touchBtn._h.pointerdown) touchBtn._h.pointerdown({ pointerId: 7, pointerType: 'touch', preventDefault: function () {} });
    touch.step(1);
    var touchHeld = touch.g.shotSeq === 1 && touch.g.player.ammo === 11;
    ...
    touch.step(1);
    ok(touchHeld && noPointerClickDouble && touch.g.shotSeq === 2 && touch.g.player.ammo === 10,
    ...
    ok(!silentFallbackError && silentFallback.g.shotSeq === 1 && silentFallback.g.player.ammo === 11,
      'D1k5 无 AudioContext 时有效开火静默退化且玩法照常提交');

    var sourcesSpec = validCombatSpec(); sourcesSpec.pistol.cooldown = 0; sourcesSpec.guard.hp = 100;
    ...
    keyRelease(); keyFire(false); sources.step(1);
    ...
    blurred.step(1);
    ok(blurred.g.shotSeq === 0 && blurred.g.player.ammo === 12 && blurred.g.fireQueued === false && Object.keys(blurred.g.fireHeld || {}).length === 0,
    ...
    ok(gone && rebound && lifeOld.fireQueued === false && Object.keys(lifeOld.fireHeld || {}).length === 0 && lifeNew !== lifeOld && lifeNew.player.ammo === 11 && lifeNew.combatMonsters[0].hp === 20 && Object.keys(lifeNew.fireHeld || {}).length === 1,
...
(function () {
  ...
  try {
    ...
    ok(!timingError && tm.state === 'attack-windup' && timing.g.player.health === 50 && timing.g.player.faceState === 'alert' && /攻击预兆/.test(findHud()),
    ...
    ok(hurt.g.damageFlashUntil > hurt.g.tw && hurt.g.player.hurtUntil > hurt.g.tw && hurt.g.player.faceState === 'hurt' && redVisible && /生命 40\/50/.test(findHud()) && /受伤/.test(findHud()) && normalTranslate > 0,
    ...
    keyFire();
    var fatalError = until(fatal, function () { return fatal.g.player.dead; }, 30), deadX = fatal.g.px, deadY = fatal.g.py, deadA = fatal.g.a, deadAmmo = fatal.g.player.ammo, deadShots = fatal.g.shotSeq;
    fatal.step(40);
    ...
    ok(fatal.g.fireQueued === false && Object.keys(fatal.g.fireHeld || {}).length === 0 && fatal.g.px === deadX && fatal.g.py === deadY && fatal.g.a === deadA && fatal.g.player.ammo === deadAmmo && fatal.g.shotSeq === deadShots,
...
(function () {
  ...
  try {
    ...
    keyUp('w'); var ammoBefore = windup.g.player.ammo, hpBefore = windupGuard.hp;
    keyDown(' '); keyUp(' '); windupError = windupError || windup.step(1);
    ok(!windupError && windupGuard.state === 'attack-windup' && windup.g.shotSeq === 1 && windup.g.player.ammo === ammoBefore - 1 && windupGuard.hp === hpBefore - 20,
      'D1c2 guard windup 期间一次单击仍恰提交一枪并命中 ammo=' + ammoBefore + '→' + windup.g.player.ammo + ' hp=' + hpBefore + '→' + windupGuard.hp);
...
// ════ D2. FPS Phase 2a session-local ammo/health supplies════════════════
section('D2 FPS Phase 2a supplies session/lifecycle/presentation');
(function () {
  var src = fs.readFileSync(MAZE, 'utf8'), needle = '      // ── /FPS Phase 0b combat session skeleton ──';
  if (src.indexOf(needle) < 0 || (src.match(/function resolveSupplyContacts\(/g) || []).length !== 1 ||
      (src.match(/function combatSupplyLayers\(/g) || []).length !== 1) {
    ok(false, 'D2 源码探针锚点/补给结算/固定形态函数数量与 runtime 脱节');
    return;
  }
  var probed = src.replace(needle, '      global.__mazeCombatSession = g;\n' + needle);
  ...
  try {
    ...
    function standOn(run, index) { var supply = run.g.combatSupplies[index]; run.g.px = supply.sx; run.g.py = supply.sy; }
    function healthOnlySpec(health, maxHealth) {
      var spec = validCombatSpec();
      spec.player.health = health; spec.player.maxHealth = maxHealth;
      spec.supplies = [{ x: 2, y: 1, kind: 'health', amount: 20 }];
      return spec;
    }

    var author = validSupplySpec(), authorBefore = JSON.stringify(author), separatedRun = begin(author), separated = separatedRun.g;
    ok(separated.player !== author.player && separated.combatSupplies !== author.supplies && separated.combatSupplies[0] !== author.supplies[0] && separated.combatSupplies[0].sx === 2.5 &&
      separated.player.ammo === 2 && separated.player.maxAmmo === 6 && separated.combatSupplies[0].id === undefined && separated.combatSupplies[0].ammo === undefined &&
      separated.combatSupplies[0].consumed === false && separated.combatSupplies[0].touching === false && separated.items.indexOf(separated.combatSupplies[0]) < 0,
      'D2a validator 深分离 g.player/g.combatSupplies(sx/sy+latches),标量 cap 且 session supply 不恢复 id/ammo');
    separated.player.maxAmmo = 3; separated.combatSupplies[0].consumed = true;
    ok(JSON.stringify(author) === authorBefore && separatedRun.applyCalls() === 0,
      'D2b 反向改 session 深层补给/cap 不污染作者 spec，也不经 api.apply');

    var ammoSpec = validSupplySpec(); ammoSpec.player.ammo = 5;
    var ammo = begin(ammoSpec); standOn(ammo, 0); var ammoError = ammo.step(1);
    ok(!ammoError && ammo.g.player.ammo === 6 && ammo.g.combatSupplies[0].consumed === true && ammo.g.supplyNotice.text === '+1 弹药' &&
      /\+1 弹药/.test(findHud()) && drawTexts.indexOf('+1 弹药') >= 0 && ammo.applyCalls() === 0,
      'D2c ammo clamp 到 maxAmmo、资源+consumed 同帧提交，DOM/canvas HUD 可观察且 Atlas state 无写入 hud=' + JSON.stringify(findHud()));
    ammo.g.player.ammo = 0; ammo.step(4);
    ok(ammo.g.player.ammo === 0 && ammo.g.combatSupplies[0].consumed === true,
      'D2d consumed latch 一次性：消费后再耗弹、继续接触也不复活/重复增加');

    var health = begin(healthOnlySpec(90, 100)); standOn(health, 0); health.step(1);
    ok(health.g.player.health === 100 && health.g.combatSupplies[0].consumed === true && health.g.supplyNotice.text === '+10 生命' && /\+10 生命/.test(findHud()),
    ...
    full.step(2);
    ok(full.g.player.health === 100 && full.g.combatSupplies[0].consumed === false && full.g.combatSupplies[0].touching === true &&
    ...
    var leftRadius = full.g.combatSupplies[0].touching === false;
    ...
    ok(full.g.player.health === 100 && full.g.combatSupplies[0].consumed === true,
      'D2h 满血接触未消费后，仍站在补给上受伤即可取用，不要求物体复活或额外边沿');

    var fullAmmoSpec = validSupplySpec(); fullAmmoSpec.player.ammo = 6;
    var fullAmmo = begin(fullAmmoSpec); standOn(fullAmmo, 0); fullAmmo.step(1);
    ok(fullAmmo.g.player.ammo === 6 && !fullAmmo.g.combatSupplies[0].consumed && fullAmmo.g.supplyNotice.text === '弹药已满',
    ...
    ['dead', 'won', 'puzzle'].forEach(function (mode) {
      ...
      paused.step(2);
      ok(paused.g.player.health === 80 && paused.g.combatSupplies[0].consumed === false && paused.g.supplyNotice === null,
        'D2j ' + mode + ' 态不结算 supply');
    });
    ...
    ordered.step(1);
    ok(ordered.g.combatSupplies[0].consumed && ordered.g.player.health === 30 && liveMon.state === 'attack-recover',
      'D2k 同帧 supply 先把 45 clamp 到 50，再由 AI commit 扣 20→30，顺序早于 enemy attack');

    var lifeSpec = validSupplySpec(); lifeSpec.player.ammo = 5;
    ...
    ok(fresh && fresh !== oldSession && fresh.player.ammo === 5 && fresh.player.health === 80 && fresh.combatSupplies.every(function (s) { return !s.consumed && !s.touching; }) &&
      ...
      'D2l leave/re-enter/retry 重建初始资源与 supplies，listener/rAF 仍单份');
    ...
    ok(!canvasMounted() && !takeRaf() && global.__mazeCombatSession === undefined,
      'D2m durable deathKey=true restore 不偷开 fresh supply session');

    function deterministicTrace() {
      var spec = validSupplySpec(); spec.player.ammo = 5; spec.player.health = 90;
      var run = begin(spec), trace = [];
      standOn(run, 0); run.step(1); trace.push([run.g.tw, run.g.player.ammo, run.g.player.health, run.g.combatSupplies[0].consumed, run.g.combatSupplies[1].consumed]);
      standOn(run, 1); run.step(3); trace.push([run.g.tw, run.g.player.ammo, run.g.player.health, run.g.combatSupplies[0].consumed, run.g.combatSupplies[1].consumed]);
      return trace;
    }
    ...
    function supplyPixels(kind, behind) {
      var spec = validCombatSpec(); delete spec.guard;
      var x = behind ? 4 : 2;
      if (kind === 'ammo') { spec.player.ammo = 2; spec.player.maxAmmo = 6; spec.supplies = [{ x: x, y: 2, kind: 'ammo', amount: 3 }]; }
      else { spec.supplies = [{ x: x, y: 2, kind: 'health', amount: 20 }]; }
    ...
    var ammoHidden = supplyPixels('ammo', true), medVisible = supplyPixels('health', false), ammoVisible = supplyPixels('ammo', false);
    ...
    ok(ammoVisible.run.g.combatSupplies[0].consumed && recCols.indexOf('rgb(238,190,58)') < 0,
      'D2p supply 在 contact commit 同帧从 sprite pass 消失');
    ...
    ok(!legacySupplyText && !/生命已满|弹药已满/.test(findHud()), 'D2q 无 combat 的旧 recipe 不出现 supply UI/行为');
    var publicApi = require(MAZE);
    ok(publicApi.resolveSupplyContacts === undefined && publicApi.combatSupplyLayers === undefined,
      'D2r supply 结算/表现保持 maze3d 私有，不扩公共 API');
  } catch (e) {
    recOn = false;
    ok(false, 'D2 supplies 源码探针运行失败: ' + (e && (e.stack || e.message) || e));
  } finally {
    recOn = false;
    try { delete require.cache[require.resolve(tmpPath)]; } catch (e2) {}
    try { fs.unlinkSync(tmpPath); } catch (e3) {}
...
(function () {
  ...
  try {
    ...
    ok(/出口封锁/.test(findHud()) && /1 名守卫/.test(findHud()) && drawTexts.some(function (text) { return /出口封锁:剩余 1 名守卫/.test(text); }),
...
(function () {   // ZA1 统一 recipes 入口与 FPS 直达入口并存,两者加载同一 runtime/world。
  var miss = hasAll(indexHtml, ['统一入口', '不是新模块', 'raycast-maze.js', 'fps.html', 'W / S', 'A / D']);
  ok(!miss.length, 'ZA1 index.html 保留五 recipes 入口、优先指向 FPS 直达页，并加载同一 runtime miss=' + miss.join(','));
...
(function () {   // ZA1c 玩家 FPS 直达页必须复用同一 runtime/world，只派生 start 后装配。
  var htmlMiss = hasAll(fpsHtml, ['玩家直达', '先取正前方弹药', '准星', '两枪', '发光 D', 'W / S', 'A / D', '← / →', 'Space', '首次点击画面会开始追逐并锁定鼠标', '但不会开枪', '<details', 'raycast-maze.js', 'world.js', 'fps-game.js']);
...
(function () {   // ZA2 unified world 覆盖 basic/horror/puzzle/layers/fps-range 五个 recipe,且文案锁同一 runtime。
  var miss = hasAll(worldJs, ['Recipe 1 · 基础迷宫', 'Recipe 2 · 地底回廊', 'Recipe 3 · 机关游乐场', 'Recipe 4 · 失落矿井', 'Recipe 5 · FPS 最小生存场', "kind: 'maze3d'", "to: 'fps_range'", "to: 'fps_range_death'", '同一个 maze3d runtime', '不是五个模块']);
  ok(!miss.length, 'ZA2 world.js 覆盖五个 maze3d recipes,并锁“同一 runtime/不是五个模块”口径 miss=' + miss.join(','));
})();
(function () {   // ZA2a Phase 2a recipe 必须有精确补给压力场、单 guard、死亡出口和真正清 durable deathKey 的重试。
  ...
  var combat = fps.maze.combat, guard = combat.guard, supplies = combat.supplies, clip = supplies[0], medkit = supplies[1], state = { fpsRangeEscaped: true, fpsRangeDeath: true };
  var start = fps.maze.start, contactRadius = 0.5, defaultMove = 2.6, weapon = combat.pistol;
  if (!guard || !weapon) { ok(false, 'ZA2a 正式 Recipe 5 尚未迁移到 v2 单数 pistol/guard'); return; }
  var medkitDistance = Math.hypot(medkit.x - start.x, medkit.y - start.y);
  var medkitTravelTime = Math.max(0, medkitDistance - contactRadius) / defaultMove;
  var clipDistance = Math.hypot(clip.x - start.x, clip.y - start.y);
  var clipTravelTime = Math.max(0, clipDistance - contactRadius) / defaultMove;
  var initialGuardDistance = Math.hypot(guard.x - start.x, guard.y - start.y);
  var guardDistanceAtClip = initialGuardDistance - (defaultMove + guard.ai.moveSpeed) * clipTravelTime;
  var firstGuardCommitTime = clipTravelTime + Math.max(0, guardDistanceAtClip - guard.ai.attackRange) / guard.ai.moveSpeed + guard.ai.windup;
  var twoShotsReadyTime = clipTravelTime + weapon.cooldown;
  if (directRetry && directRetry.run) directRetry.run(state);
  ok(enter && enter.run && combat.exitRequires === 'clear' && guard && guard.hp === 40 && !Object.prototype.hasOwnProperty.call(guard, 'id') && guard.ai.moveSpeed === 1.1 && guard.ai.moveSpeed > 0 && guard.ai.moveSpeed < defaultMove && guard.ai.sight > 0 && guard.ai.attackRange === 1.35 && guard.ai.damage === 20 && guard.ai.windup === 0.55 && guard.ai.cooldown === 0.75 &&
    combat.player.maxHealth === 60 && combat.player.health === 60 && combat.player.ammo === 1 && combat.player.maxAmmo === 6 &&
    weapon.damage === 20 && weapon.range === 12 && weapon.cooldown === 0.32 && weapon.recoil === 2 && Object.keys(weapon).sort().join(',') === 'cooldown,damage,range,recoil' && supplies.length === 2 &&
    clip.x === 2 && clip.y === 3 && clip.kind === 'ammo' && clip.amount === 3 && !Object.prototype.hasOwnProperty.call(clip, 'id') && !Object.prototype.hasOwnProperty.call(clip, 'ammo') &&
    medkit.x === 1 && medkit.y === 2 && medkit.kind === 'health' && medkit.amount === 20 && !Object.prototype.hasOwnProperty.call(medkit, 'id') && !Object.prototype.hasOwnProperty.call(medkit, 'ammo') &&
    !combat.weapons && !combat.monsters && !combat.weapon && !combat.player.weapon &&
    fps.maze.grid[clip.y][clip.x] === '.' && fps.maze.grid[medkit.y][medkit.x] === '.' && fps.maze.grid.some(function (row) { return row.indexOf('D') >= 0; }) && !combat.doors && !combat.objectives &&
    /var MOVE = opts\.moveSpeed \|\| 2\.6;/.test(runtimeJs) && fps.look.indexOf('静止不追逐') < 0 && fps.look.indexOf('追逐') >= 0 &&
    ...
    "ZA2a Recipe 5 是 exitRequires:'clear'、1/6 bullets、前方 +3 ammo、侧路 +20 health、低于 MOVE 2.6 的单近程追击 guard 与派生清场 D 的 Phase 2a-e 压力场;当前节点一键重试清胜负键并 to 同节点，阵亡页保留可选查看 state=" + JSON.stringify(state));
  ok(fps.maze.grid[start.y][start.x] === '.' && fps.maze.grid[medkit.y][medkit.x] === '.' &&
    Math.abs(medkit.x - start.x) + Math.abs(medkit.y - start.y) === 1 && medkitTravelTime < guard.ai.windup,
    'ZA2a2 medkit 是起点侧邻可走格,按 0.5 接触半径/default MOVE 2.6 可在首次 windup 前到达 travel=' + medkitTravelTime.toFixed(3) + ' windup=' + guard.ai.windup);
  ok(clip.y === start.y && clip.x > start.x && clipTravelTime > 0 && twoShotsReadyTime < firstGuardCommitTime,
    'ZA2a3 静态时间预算:存在正前方 ammo 接触后两枪在 guard 首击 commit 前完成的窗口,不要求每局边跑边打 ammoAt=' + clipTravelTime.toFixed(3) + ' twoShotsAt=' + twoShotsReadyTime.toFixed(3) + ' guardCommitAt=' + firstGuardCommitTime.toFixed(3));
  ok(death && death.kind === 'scene' && death.links.some(function (link) { return link.to === 'hub'; }), 'ZA2a4 death node 同时提供重试与返回,不会 soft-lock');
})();
(function () {   // ZA2e 旧四 recipe 不获得 combat/ready/supply 作者面；只有 fps_range 使用私有 combat supplies。
  ...
  var polluted = legacyMazeIds.filter(function (id) { var maze = nodes[id] && nodes[id].maze; return maze && (maze.combat || maze.supplies || maze.ready); });
  ok(!polluted.length && nodes.fps_range.maze.combat.supplies.length === 2,
    'ZA2e basic/horror/puzzle/layers recipes 不出现 combat/ready/supply UI 或数据面，ready gate 只由 runtime 对 combat 派生 polluted=' + polluted.join(','));
})();
...
(function () {   // ZA10a FPS 作者面:完整三节点 recipe 必须能被真实 graph 审计,并讲清分账/严格准入/停止线。
  var miss = hasAll(authoringMd, [
    'Recipe 5 · FPS 最小生存场', '`maze.combat`', "exitRequires: 'clear'", "deathKey: 'rangeDeath'",
    'maxAmmo: 6', 'pistol:', 'guard:', "kind: 'ammo'", "kind: 'health'", 'maze.monsters', 'session-local',
    '玩家 HP、弹药、敌人 HP / AI 状态、补给 consumed', '只有 `winKey` / `deathKey`',
    '不加普通门、objective、第二敌人 archetype、作者可配置战斗音频或另一把武器',
    'FPS_RECIPE_INIT_STATE', 'function resetRange(S)', 'range_done:', 'range_death:',
    '只抄 `fps_range` 会留下死链和无法清除的 durable deathKey',
    '`combat={deathKey,player,pistol,guard?,supplies?,exitRequires?}`',
    '`pistol` 只接受 `damage/range/cooldown/recoil`', '`guard` 整体可选',
    '`supplies[]` 不接受 `id` 或 `ammo`',
  ...
  ok(!miss.length && recipeOk && authoringMd.indexOf('sfx:') < 0 && recipe.indexOf('weapons:') < 0 && recipe.indexOf('monsters:') < 0 && recipe.indexOf('spread:') < 0,
    'ZA10a 作者手册公开完整可审计 combat recipe、状态分账/停止线,不宣传未消费能力 miss=' + miss.join(',') + ' audit=' + recipeDetail.trim());
  ok(/player:\s*\{[\s\S]*ammo:\s*1,[\s\S]*maxAmmo:\s*6/.test(recipe) && /pistol:\s*\{[\s\S]*damage:\s*20/.test(recipe) && /guard:\s*\{[\s\S]*hitRadius:\s*0\.34/.test(recipe),
    'ZA10a2 recipe 锁 v2 标量 ammo + 单数 pistol/guard 作者形状');
})();
(function () {   // ZA10b FPS 路由:Claude/Codex/README/Showroom 都须认识现有私有 runtime,但不得冒充 boot 内置模块。
  var routeNeedles = ['FPS', 'fps-range', 'examples/maze3d/references/maze3d-authoring.md', '自定义', 'manifest.modules'];
  var newGameMiss = hasAll(newGameMd, routeNeedles.concat(['player/pistol/guard/supplies']));
  var skillMiss = hasAll(skillMd, ['第一人称 FPS', 'fps-range', 'maze3d-authoring.md', '不是新模块', 'player/pistol/guard/supplies']);
  var agentsMiss = hasAll(agentsMd, ['第一人称 FPS', 'fps-range', 'maze3d-authoring.md', '自定义 runtime', 'manifest.modules', 'player/pistol/guard/supplies']);
  var claudeMiss = hasAll(engineClaudeMd, ['maze3d` 私有 runtime', 'fps-range', 'maze3d-authoring.md', 'manifest.modules', '不要误判成引擎没有的玩法']);
  var readmeMiss = hasAll(engineReadme, ['basic / horror / puzzle / layers / fps-range', '不是五个模块', 'examples/maze3d/fps.html', 'start=fps_range']);
  var showroomMiss = hasAll(showroomHtml, ['FPS 生存场 · 玩家直达', 'examples/maze3d/fps.html', '正式 raycast runtime', '统一 world']);
  ok(!newGameMiss.length && !skillMiss.length && !agentsMiss.length && !claudeMiss.length && !readmeMiss.length && !showroomMiss.length,
    'ZA10b 所有 AI/人类入口可发现 FPS 私有装配,且不误承诺 boot auto newGame=' + newGameMiss.join(',') + ' skill=' + skillMiss.join(',') + ' agents=' + agentsMiss.join(',') + ' claude=' + claudeMiss.join(',') + ' readme=' + readmeMiss.join(',') + ' showroom=' + showroomMiss.join(','));
})();
  ...
  var cases = [
    {
      ...
      from: "combatFields(combat, ['exitRequires', 'deathKey', 'player', 'pistol', 'guard', 'supplies'], root);",
      to: "combatFields(combat, ['exitRequires', 'deathKey', 'player', 'pistol', 'guard', 'supplies', 'weapons'], root);",
      red: '✗ D0b 旧 weapons 根字段'
    },
    {
      name: 'accept-old-monsters',
      from: "combatFields(combat, ['exitRequires', 'deathKey', 'player', 'pistol', 'guard', 'supplies'], root);",
      to: "combatFields(combat, ['exitRequires', 'deathKey', 'player', 'pistol', 'guard', 'supplies', 'monsters'], root);",
      red: '✗ D0b 旧 monsters 根字段'
    },
    {
      name: 'accept-object-ammo',
      from: "var ammo = combatNumber(player.ammo, playerPath + '.ammo', false, true);",
      to: "var ammo = isPlainCombatObject(player.ammo) ? 12 : combatNumber(player.ammo, playerPath + '.ammo', false, true);",
      red: '✗ D0b 旧对象 ammo'
    },
    {
      name: 'accept-old-spread',
      from: "combatFields(pistol, ['damage', 'range', 'cooldown', 'recoil'], pistolPath);",
      to: "combatFields(pistol, ['damage', 'range', 'cooldown', 'recoil', 'spread'], pistolPath);",
      red: '✗ D0b 旧 pistol.spread 占位'
    },
    {
      name: 'accept-supply-id',
      from: "combatFields(supply, ['x', 'y', 'kind', 'amount'], supplyPath);",
      to: "combatFields(supply, ['x', 'y', 'kind', 'amount', 'id'], supplyPath);",
      red: '✗ D0b 旧 supply.id 无消费者字段'
    },
    {
      name: 'accept-supply-ammo',
      from: "combatFields(supply, ['x', 'y', 'kind', 'amount'], supplyPath);",
      to: "combatFields(supply, ['x', 'y', 'kind', 'amount', 'ammo'], supplyPath);",
      red: '✗ D0b 旧 supply.ammo 引用字段'
    }
  ];
...
(function () {   // ZA10d2 committed 反向变异：runtime 必须只消费标量 ammo/g.pistol，并维持唯一 0..1 combatMonsters owner。
  if (process.env.ATLAS_SKIP_COMBAT_SCHEMA_MUTATIONS === '1') return;
  var source = fs.readFileSync(MAZE, 'utf8');
  var cases = [
    {
      name: 'runtime-object-ammo',
      from: 'var ammo = g.player.ammo;',
      to: 'var ammo = g.player.ammo.bullets;',
      red: '✗ D1a 首次 Space 输入原子 ready'
    },
    {
      name: 'runtime-weapons-map',
      from: 'var weapon = g.pistol;',
      to: 'var weapon = g.weapons && g.weapons.pistol;',
      red: '✗ D1a 首次 Space 输入原子 ready'
    },
    {
