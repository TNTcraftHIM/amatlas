/* ══════════════════════════════════════════════════════════════════════
   ...
   · fps-range:Phase 2a 私有 combat 数据 + session-local 补给 + 单 guard 生存闭环。
      ...
      m: {
        ...
        nodes: {
          ...
          fps_range: {
            ...
            look: '守卫封锁着走廊。先拿正前方的 +3 弹药,再用准星两枪击倒 40 HP 守卫,最后穿过发光 D。首次移动、转向、开火或点击画面才会让追逐与攻击计时开始;初始画面可以安全阅读。首次点击画面只开始追逐并锁定鼠标,不会开枪。\n\n侧路有 +20 health 医疗包:满血接触不会消失,受伤后再取才消费。有效开火与受伤各有固定短音;冷却中重复按下不会耗弹或缓冲,HUD 会短显“手枪回位中”。',
            wonText: '生存场出口打开。你完成了取弹、准星两枪、击倒单名守卫并穿过既有 D 出口的完整闭环。',
            maze: {
              grid: [
                '#############',
                '#...........#',
              ...
              combat: {
                ...
                player: { maxHealth: 60, health: 60, ammo: 1, maxAmmo: 6 },
                pistol: { damage: 20, range: 12, cooldown: 0.32, recoil: 2 },
                guard: { x: 7, y: 3, face: 'skull', hp: 40, hitRadius: 0.34,
                  ai: { sight: 9, hear: 8, attackRange: 1.35, moveSpeed: 1.1, damage: 20, windup: 0.55, cooldown: 0.75 } },
                supplies: [
          ...
          fps_range_done: {
            kind: 'scene',
            title: 'FPS Phase 2a · 生存场脱离',
            look: '你在守卫下一次攻击落下前穿过出口。枪械、敌人 HP、玩家资源、补给消费态和 AI 计时都只存在于本轮 maze3d 会话;跨层只留下通关事实。',
            scene: { region: 'station', mood: 'calm' },
            links: [backToHub()]
          },
          fps_range_death: {
            kind: 'scene',
            title: 'FPS Phase 2a · 阵亡',
            look: '这一轮已经结束。死亡事实只写入一次;重试会清除 durable deathKey,重新建立 60 HP、1/6 发弹药、两件未消费补给和完整 40 HP 守卫。',
