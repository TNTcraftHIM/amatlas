    ...
    function startMaze(node) {
      ...
      function syncInteractButton() {                                // 触屏上下文按钮=同一主动 intent 的可见入口;显隐/文案只读当前上下文,不写玩法状态。
        ...
        if (target) {
          ...
          interactBtn.setAttribute('aria-label', key ? '拾取' : (use ? '互动' : '检视'));
        }
      }
      ...
      function openPuzzleOverlay(evx) {                            // R1-b4 三种模板共用一个私有 DOM 表单;所有操作有 ≥44px 触屏按钮,code 另支持数字键盘。
        ...
        if (puz.kind === 'code') {
          ...
          controls.appendChild(puzzleButton('⌫', '退格', function () { changePuzzleInput('code', 'back'); }));
        } else if (puz.kind === 'sequence') {
      ...
      try { if (ENABLE_CONTROLS) {
        ...
        // ── 四向按钮(坦克模式):桌面 escape hatch + 触屏默认控件(用户要求保留;触屏全屏后让位摇杆) ──
        var ctrls = hostDoc.createElement('div');
        ctrls.className = 'amatlas-maze-controls';
        ctrls.style.cssText = 'position:absolute;left:0;right:0;bottom:8px;display:flex;justify-content:center;gap:10px;pointer-events:none;z-index:2';
        var mkBtn = function (label, dir) {
          var b = hostDoc.createElement('button');
          b.textContent = label; b.setAttribute('aria-label', dir);
          b.style.cssText = 'pointer-events:auto;width:52px;height:46px;font-size:20px;line-height:1;border-radius:9px;border:1px solid rgba(255,255,255,.28);background:rgba(16,24,36,.72);color:#dde6f0;cursor:pointer;touch-action:none;-webkit-user-select:none;user-select:none';
          var set = function (e) { unlockAudio(); keys[dir] = 1; if (e && e.preventDefault) e.preventDefault(); };
          var clr = function () { keys[dir] = 0; };
        ...
        ctrls.appendChild(mkBtn('↰', 'left')); ctrls.appendChild(mkBtn('▲', 'fwd')); ctrls.appendChild(mkBtn('▼', 'back')); ctrls.appendChild(mkBtn('↱', 'right'));
        var ib = hostDoc.createElement('button');
        ib.textContent = '查看'; ib.setAttribute('aria-label', '检视');
        ib.style.cssText = 'position:absolute;right:10px;bottom:62px;z-index:3;pointer-events:auto;display:none;min-width:56px;height:46px;padding:0 12px;font-size:14px;line-height:1;border-radius:9px;border:1px solid rgba(255,255,255,.30);background:rgba(16,24,36,.78);color:#eef3f8;cursor:pointer;touch-action:none;-webkit-user-select:none;user-select:none';
        ...
        if (isTouch) {
          ...
          var syncTouchMode = function () { var fs = isFs(); ovr.style.display = fs ? 'block' : 'none'; ctrls.style.display = fs ? 'none' : 'flex'; if (!fs) clearTouchIntent(); };
        ...
        if (opts.fullscreen !== false) {   // 统一伪全屏:总给沉浸按钮(不依赖系统全屏 API,iOS/安卓/桌面一致;opts.fullscreen:false 关)
          if (hostDoc.getElementById && !hostDoc.getElementById('amatlas-maze-fs-style') && hostDoc.head) {
            ...
            fst.textContent = '.amatlas-maze-pseudofs{position:fixed;inset:0;z-index:9999;background:#000;display:flex;flex-direction:column;align-items:center;justify-content:flex-start}.amatlas-maze-pseudofs canvas{image-rendering:pixelated;display:block;flex:none}@media (orientation:landscape){.amatlas-maze-pseudofs{justify-content:center}}';   // 统一伪全屏布局:竖屏 canvas 顶部宽铺满(max-height 64vh 留下方≥36vh 操作区,手指在画面外戳)、横屏 canvas 居中高铺满(摇杆半透明叠,无伤大雅)
          ...
          fsBtn.textContent = '⛶'; fsBtn.setAttribute('aria-label', '全屏');
          fsBtn.style.cssText = 'position:absolute;top:8px;right:8px;z-index:3;pointer-events:auto;width:52px;height:46px;font-size:18px;border-radius:9px;border:1px solid rgba(255,255,255,.28);background:rgba(16,24,36,.72);color:#dde6f0;cursor:pointer;touch-action:none;-webkit-user-select:none;user-select:none';
          fsBtn.addEventListener('pointerdown', function (e) { unlockAudio(); if (e && e.preventDefault) e.preventDefault(); if (isFs()) exitFsIfMine(); else enterFs(); });
        ...
        if (!g.caught && !g.won) {
          var objHint = (g.needKey && !g.hasKey) ? '◎ 先找那把 ' + kE
            : (g.needKey && g.hasKey) ? '◎ ' + kE + ' 在手,去开门'
              : '◎ 寻找出口';
          ctx.save(); ctx.globalAlpha = 0.5; ctx.fillStyle = 'rgb(238,243,248)'; ctx.font = '12px system-ui,sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
          ctx.fillText(objHint, 9, 9); ctx.restore();   // headless ctx 是 stub,save/restore/fillText 退化无操作
          if (g.canMouseLook) {   // 桌面 mouselook 提示(左上目标锚下方、触屏不显;避开右上角全屏按钮 DOM 浮层):点画面锁鼠标转向 / 已锁则 ESC 退出
            ctx.save(); ctx.globalAlpha = 0.4; ctx.fillStyle = 'rgb(238,243,248)'; ctx.font = '11px system-ui,sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
            ctx.fillText(g.mouseLook ? '🖱 ESC 退出鼠标转向' : '🖱 点击锁定鼠标转向', 9, 28); ctx.restore();
          }
        }
