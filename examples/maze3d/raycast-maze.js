...
(function (global, factory) {
  ...
  // ── FPS Phase 0a hitscan 几何核(私有纯函数;只算交点/遮挡/择优,不接输入、伤害或表现)──
  function rayCircleHit(ox, oy, dx, dy, cx, cy, radius) {
    if (typeof ox !== 'number' || !isFinite(ox) || typeof oy !== 'number' || !isFinite(oy) ||
        typeof dx !== 'number' || !isFinite(dx) || typeof dy !== 'number' || !isFinite(dy) ||
        typeof cx !== 'number' || !isFinite(cx) || typeof cy !== 'number' || !isFinite(cy) ||
        typeof radius !== 'number' || !isFinite(radius) || radius <= 0) return null;
    var dirLen = Math.sqrt(dx * dx + dy * dy); if (!(dirLen > 0)) return null;
    var ux = dx / dirLen, uy = dy / dirLen, rx = cx - ox, ry = cy - oy;
    var along = rx * ux + ry * uy, radiusSq = radius * radius;
    if (along < 0) return null;   // 中心在射线身后即 miss；不让跨过原点的大圆命中背后目标。
    var perpSq = rx * rx + ry * ry - along * along; if (perpSq > radiusSq) return null;
    var halfChord = Math.sqrt(Math.max(0, radiusSq - perpSq));
    var nearT = along - halfChord; return nearT > 0 ? nearT : 0;
  }
  function resolveHitscan(ox, oy, dx, dy, range, wallT, targets) {
    if (typeof range !== 'number' || !isFinite(range) || range < 0 ||
        typeof wallT !== 'number' || !isFinite(wallT) || wallT < 0 || !Array.isArray(targets)) return null;
    var best = null;
    for (var i = 0; i < targets.length; i++) {
      var target = targets[i]; if (!target || typeof target !== 'object' || Array.isArray(target)) continue;
      if ((target.active != null && typeof target.active !== 'boolean') || (target.dead != null && typeof target.dead !== 'boolean') || target.active === false || target.dead === true) continue;
      var id = target.id, idType = typeof id;
      if (!((idType === 'string' && id.trim()) || (idType === 'number' && isFinite(id)))) continue;
      var t = rayCircleHit(ox, oy, dx, dy, target.sx, target.sy, target.hitRadius);
      if (t === null || t > range || t >= wallT) continue;
      var nearer = !best || t < best.t, tied = best && t === best.t;
      var lowerId = tied && (idType === typeof best.id ? id < best.id : idType === 'number');
      if (nearer || lowerId) best = { id: id, target: target, t: t };
    }
    return best;
  }
  // ── /FPS Phase 0a hitscan 几何核 ──
  ...
  function createMaze3dModule(opts) {
    ...
    var RAD = 0.22;                       // 玩家碰撞半径(格)
    var COMBAT_GUARD_RAD = 0.28;          // 存活 combat guard 固定身体半径；与 hitscan hitRadius 分账。
    var COMBAT_GUARD_DISTANCE = RAD + COMBAT_GUARD_RAD;
    ...
    // ── FPS combat 数据闸(maze 私有;闭 schema 校验并生成深分离会话骨架)──
    function combatHas(obj, key) { return Object.prototype.hasOwnProperty.call(obj, key); }
    function combatPath(base, key) { return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(key) ? base + '.' + key : base + '[' + JSON.stringify(key) + ']'; }
    function combatFail(path, detail) { throw new Error('[maze combat] ' + path + ' ' + detail); }
    function isPlainCombatObject(value) {
      if (!value || Object.prototype.toString.call(value) !== '[object Object]') return false;
      var proto = Object.getPrototypeOf(value);
      return proto === Object.prototype || proto === null;
    }
    function combatObject(value, path) { if (!isPlainCombatObject(value)) combatFail(path, '必须是 plain object'); }
    function combatFields(value, allowed, path) {
      var names = Object.getOwnPropertyNames(value);
      for (var i = 0; i < names.length; i++) if (allowed.indexOf(names[i]) < 0) combatFail(combatPath(path, names[i]), '是未知字段');
      if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(value); if (symbols.length) combatFail(path + '[' + String(symbols[0]) + ']', '是未知字段'); }
    }
    function combatRequired(value, required, path) {
      for (var i = 0; i < required.length; i++) if (!combatHas(value, required[i])) combatFail(combatPath(path, required[i]), '是必填字段');
    }
    function combatNumber(value, path, positive, integer) {
      if (typeof value !== 'number' || !isFinite(value)) combatFail(path, '必须是 finite number');
      if (positive ? value <= 0 : value < 0) combatFail(path, positive ? '必须是正数' : '必须是非负数');
      if (integer && value !== Math.floor(value)) combatFail(path, '必须是整数');
      return value;
    }
    function combatOptionalName(value, path) {
      if (typeof value !== 'string' || !value.trim()) combatFail(path, '必须是非空字符串');
      return value;
    }
    var RESERVED_STATE_FACT_KEYS = { pos: 1, clock: 1, rngSeed: 1, seen: 1, flags: 1, _once: 1 };
    function stateFactKeyConflict(value, state) {
      if (RESERVED_STATE_FACT_KEYS[value]) return '不能与引擎自有状态字段或内部键同名';
      if (value === '__proto__' || value === 'constructor' || value === 'prototype' || Object.prototype.hasOwnProperty.call(Object.prototype, value)) return '不能使用原型继承/污染键';
      if (state && Object.prototype.hasOwnProperty.call(state, value) && typeof state[value] !== 'boolean') return '已存在于 Amatlas state，但不是 boolean 剧情事实';
      return null;
    }
    function validateMazeStateFactKey(value, path, state) {
      if (typeof value !== 'string' || !value.trim()) throw new Error('[maze state] ' + path + ' 必须是非空字符串');
      var conflict = stateFactKeyConflict(value, state);
      if (conflict) throw new Error('[maze state] ' + path + ' ' + conflict + ': ' + JSON.stringify(value));
      return value;
    }
    function combatStateFactKey(value, path) {
      if (typeof value !== 'string' || !value.trim()) combatFail(path, '必须是非空字符串');
      var conflict = stateFactKeyConflict(value, api && api.state);
      if (conflict) combatFail(path, conflict + ': ' + JSON.stringify(value));
      return value;
    }
    var WEAPON_PRESETS = {
      precision: { kind: 'precision', damage: 20, range: 12, cooldown: 0.32, recoil: 2, angles: [0] },
      scatter: { kind: 'scatter', damage: 8, range: 6, cooldown: 0.78, recoil: 5, angles: [-2.5 * Math.PI / 180, -1.25 * Math.PI / 180, 0, 1.25 * Math.PI / 180, 2.5 * Math.PI / 180] }
    };
    var COMBAT_THEME_FAMILY = {
      '': 'ordnance', metal: 'ordnance', station: 'ordnance', industrial: 'ordnance', clinic: 'ordnance', submarine: 'ordnance',
      cave: 'relic', dungeon: 'relic', shoji: 'relic', tomb: 'relic',
      flesh: 'organic',
      ice: 'energy', crystal: 'energy', neon: 'energy'
    };
    var COMBAT_PRESENTATIONS = {
      ordnance: {
        family: 'ordnance', weapons: { precision: '精确手枪', scatter: '近程霰弹枪' }, shortWeapons: { precision: '手枪', scatter: '霰弹枪' },
        health: '生命', ammo: '弹药', target: '装甲目标', fire: '开火', face: { normal: '警戒', alert: '警觉', hurt: '受伤', dead: '阵亡' },
        copy: { mode: '生存场', start: '按移动/转向/Space/开火开始', windup: '攻击预兆:装甲目标正在蓄力', empty: '弹药耗尽', miss: '未命中', hit: '命中装甲目标', kill: '装甲目标已击倒', fight: '击倒装甲目标并活着离开', clear: '装甲目标已倒下,前往出口' },
        colors: { primary: [88, 102, 116], secondary: [154, 166, 176], bright: [238, 232, 206], accent: [238, 190, 58], dark: [28, 34, 40], hit: [255, 246, 220], kill: [255, 194, 78] },
        pickup: { weapon: [118, 132, 148], ammo: [238, 190, 58], health: [210, 48, 56] },
        sfx: { wave: 'square', precision: 168, scatter: 118, empty: 82, switch: 210, pickup: 420 }
      },
      relic: {
        family: 'relic', weapons: { precision: '符文刻针', scatter: '扇形圣印' }, shortWeapons: { precision: '刻针', scatter: '圣印' },
        health: '生命', ammo: '符砂', target: '遗物守像', fire: '开火', face: { normal: '守印', alert: '示警', hurt: '裂纹', dead: '阵亡' },
        copy: { mode: '守印场', start: '按移动/转向/Space/开火开始', windup: '示警:遗物守像正在聚力', empty: '符砂耗尽', miss: '圣印偏离', hit: '刻中遗物守像', kill: '遗物守像已归寂', fight: '镇定遗物守像并抵达出口', clear: '遗物守像已归寂,前往出口' },
        colors: { primary: [154, 118, 64], secondary: [206, 166, 86], bright: [242, 226, 174], accent: [124, 184, 142], dark: [48, 39, 31], hit: [246, 236, 190], kill: [226, 184, 88] },
        pickup: { weapon: [168, 132, 72], ammo: [206, 166, 86], health: [124, 184, 142] },
        sfx: { wave: 'triangle', precision: 236, scatter: 168, empty: 116, switch: 294, pickup: 520 }
      },
      organic: {
        family: 'organic', weapons: { precision: '骨针', scatter: '孢囊扩散器' }, shortWeapons: { precision: '骨针', scatter: '孢囊' },
        health: '活性', ammo: '孢质', target: '组织体', fire: '开火', face: { normal: '搏动', alert: '收缩', hurt: '创口', dead: '阵亡' },
        copy: { mode: '活体场', start: '按移动/转向/Space/开火开始', windup: '收缩预兆:组织体正在聚压', empty: '孢质耗尽', miss: '孢流落空', hit: '刺入组织体', kill: '组织体已失活', fight: '使组织体失活并抵达出口', clear: '组织体已失活,前往出口' },
        colors: { primary: [132, 46, 68], secondary: [174, 78, 98], bright: [232, 188, 154], accent: [220, 94, 126], dark: [50, 18, 28], hit: [255, 214, 196], kill: [232, 108, 132] },
        pickup: { weapon: [174, 78, 98], ammo: [220, 94, 126], health: [232, 188, 154] },
        sfx: { wave: 'sine', precision: 126, scatter: 88, empty: 62, switch: 176, pickup: 320 }
      },
      energy: {
        family: 'energy', weapons: { precision: '聚焦器', scatter: '共振器' }, shortWeapons: { precision: '聚焦', scatter: '共振' },
        health: '稳定度', ammo: '能量', target: '晶格体', fire: '开火', face: { normal: '稳定', alert: '过载', hurt: '偏移', dead: '阵亡' },
        copy: { mode: '校准场', start: '按移动/转向/Space/开火开始', windup: '过载预兆:晶格体正在聚能', empty: '能量耗尽', miss: '脉冲偏移', hit: '校准晶格体', kill: '晶格体已解离', fight: '解离晶格体并抵达出口', clear: '晶格体已解离,前往出口' },
        colors: { primary: [80, 202, 232], secondary: [112, 194, 226], bright: [232, 250, 255], accent: [104, 238, 176], dark: [22, 38, 58], hit: [224, 252, 255], kill: [122, 242, 218] },
        pickup: { weapon: [112, 194, 226], ammo: [112, 224, 255], health: [104, 238, 176] },
        sfx: { wave: 'triangle', precision: 420, scatter: 260, empty: 180, switch: 520, pickup: 720 }
      },
      crystal: {
        family: 'energy', weapons: { precision: '星图刻针', scatter: '星环共振器' }, shortWeapons: { precision: '刻针', scatter: '星环' },
        health: '承载', ammo: '星砂', target: '未定项', fire: '定序', face: { normal: '定标', alert: '聚集', hurt: '扰动', dead: '阵亡' },
        copy: { mode: '定序场', start: '按移动/转向/Space/定序开始', windup: '定序预兆:未定项正在聚集', empty: '星砂已耗尽', miss: '尚未测定', hit: '测定未定项', kill: '未定项已闭合', fight: '测定未定项并抵达出口', clear: '未定项已闭合,前往出口' },
        colors: { primary: [244, 210, 112], secondary: [178, 138, 250], bright: [238, 246, 255], accent: [178, 138, 250], dark: [30, 18, 50], hit: [238, 246, 255], kill: [244, 210, 112] },
        pickup: { weapon: [178, 138, 250], ammo: [244, 210, 112], health: [238, 246, 255] },
        sfx: { crystalShotWave: 'triangle', wave: 'sine', precision: 660, scatter: 330, empty: 196, switch: 494, pickup: 880 }
      }
    };
    function combatThemeFamily(themeName) { return combatHas(COMBAT_THEME_FAMILY, themeName) ? COMBAT_THEME_FAMILY[themeName] : 'ordnance'; }
    function combatPresentation(themeName) { return themeName === 'crystal' ? COMBAT_PRESENTATIONS.crystal : COMBAT_PRESENTATIONS[combatThemeFamily(themeName)]; }
    function combatWeaponKind(value, path) {
      if (!combatHas(WEAPON_PRESETS, value)) combatFail(path, "必须是 'precision' 或 'scatter'");
      return value;
    }
    function parseCombatSpec(maze, grid, start) {
      if (!combatHas(maze, 'combat')) return null;
      var combat = maze.combat, root = 'maze.combat';
      combatObject(combat, root);
      combatFields(combat, ['exitRequires', 'deathKey', 'player', 'loadout', 'equipped', 'guard', 'pickups'], root);
      combatRequired(combat, ['deathKey', 'player', 'loadout', 'equipped'], root);
      if (combatHas(maze, 'monsters')) combatFail('maze.monsters', '不能与 maze.combat.guard 同写');

      var exitRequires = null;
      if (combatHas(combat, 'exitRequires')) {
        if (combat.exitRequires !== 'clear') combatFail(root + '.exitRequires', "必须是 'clear'");
        exitRequires = 'clear';
      }
      var deathKey = combatStateFactKey(combat.deathKey, root + '.deathKey');
      var player = combat.player, playerPath = root + '.player';
      combatObject(player, playerPath);
      combatFields(player, ['maxHealth', 'health'], playerPath);
      combatRequired(player, ['maxHealth', 'health'], playerPath);
      var maxHealth = combatNumber(player.maxHealth, playerPath + '.maxHealth', true, false);
      var health = combatNumber(player.health, playerPath + '.health', false, false);
      if (health > maxHealth) combatFail(playerPath + '.health', '不能大于 maze.combat.player.maxHealth');

      var loadout = combat.loadout, loadoutPath = root + '.loadout';
      if (!Array.isArray(loadout)) combatFail(loadoutPath, '必须是数组');
      if (loadout.length < 1 || loadout.length > 2) combatFail(loadoutPath, '必须含 1..2 项');
      var initialKinds = {}, slots = {
        precision: { kind: 'precision', owned: false, ammo: 0, maxAmmo: 0 },
        scatter: { kind: 'scatter', owned: false, ammo: 0, maxAmmo: 0 }
      };
      for (var li = 0; li < loadout.length; li++) {
        var loadoutItemPath = loadoutPath + '[' + li + ']';
        if (!(li in loadout)) combatFail(loadoutItemPath, '不能是数组空槽');
        var loadoutItem = loadout[li];
        combatObject(loadoutItem, loadoutItemPath);
        combatFields(loadoutItem, ['kind', 'ammo', 'maxAmmo'], loadoutItemPath);
        combatRequired(loadoutItem, ['kind', 'ammo', 'maxAmmo'], loadoutItemPath);
        var loadoutKind = combatWeaponKind(loadoutItem.kind, loadoutItemPath + '.kind');
        if (combatHas(initialKinds, loadoutKind)) combatFail(loadoutItemPath + '.kind', '在 loadout 内必须唯一');
        var loadoutAmmo = combatNumber(loadoutItem.ammo, loadoutItemPath + '.ammo', false, true);
        var loadoutMaxAmmo = combatNumber(loadoutItem.maxAmmo, loadoutItemPath + '.maxAmmo', true, true);
        if (loadoutAmmo > loadoutMaxAmmo) combatFail(loadoutItemPath + '.ammo', '不能大于 ' + loadoutItemPath + '.maxAmmo');
        initialKinds[loadoutKind] = true;
        slots[loadoutKind] = { kind: loadoutKind, owned: true, ammo: loadoutAmmo, maxAmmo: loadoutMaxAmmo };
      }
      var equipped = combatWeaponKind(combat.equipped, root + '.equipped');
      if (!combatHas(initialKinds, equipped)) combatFail(root + '.equipped', '必须属于 maze.combat.loadout');

      var guardPath = root + '.guard', monsterCopy = [];
      if (combatHas(combat, 'guard')) {
        var guard = combat.guard;
        combatObject(guard, guardPath);
        combatFields(guard, ['x', 'y', 'hp', 'hitRadius', 'ai'], guardPath);
        combatRequired(guard, ['x', 'y', 'hp', 'hitRadius', 'ai'], guardPath);
        var gx = combatNumber(guard.x, guardPath + '.x', false, true);
        var gy = combatNumber(guard.y, guardPath + '.y', false, true);
        if (isWall(grid, gx, gy)) combatFail(guardPath, '必须位于 grid 可走格,得到 (' + gx + ',' + gy + ')');
        if (gx === start.x && gy === start.y) combatFail(guardPath, '不能与 maze.start 同格 (' + gx + ',' + gy + ')');
        var guardHp = combatNumber(guard.hp, guardPath + '.hp', true, false);
        var hitRadius = combatNumber(guard.hitRadius, guardPath + '.hitRadius', true, false);
        var guardAi = guard.ai, aiPath = guardPath + '.ai';
        combatObject(guardAi, aiPath);
        combatFields(guardAi, ['sight', 'hear', 'attackRange', 'moveSpeed', 'damage', 'windup', 'cooldown'], aiPath);
        combatRequired(guardAi, ['sight', 'hear', 'attackRange', 'moveSpeed', 'damage', 'windup', 'cooldown'], aiPath);
        var attackRange = combatNumber(guardAi.attackRange, aiPath + '.attackRange', true, false);
        if (attackRange < COMBAT_GUARD_DISTANCE) combatFail(aiPath + '.attackRange', '不得小于 0.50（玩家与存活 guard 的实体包络）');
        var aiState = {
          sight: combatNumber(guardAi.sight, aiPath + '.sight', false, false),
          hear: combatNumber(guardAi.hear, aiPath + '.hear', false, false),
          attackRange: attackRange,
          moveSpeed: combatNumber(guardAi.moveSpeed, aiPath + '.moveSpeed', false, false),
          damage: combatNumber(guardAi.damage, aiPath + '.damage', true, false),
          windup: combatNumber(guardAi.windup, aiPath + '.windup', false, false),
          cooldown: combatNumber(guardAi.cooldown, aiPath + '.cooldown', false, false)
        };
        var guardState = {
          id: 'guard', x: gx, y: gy, sx: gx + 0.5, sy: gy + 0.5,
          hp: guardHp, maxHp: guardHp, hitRadius: hitRadius, ai: aiState,
          active: true, dead: false, combatTarget: true, fadeAlpha: 1, idx: 0, _pathCache: null,
          state: 'idle', stateAt: 0, attackAt: Infinity, recoverUntil: 0, attackCommitted: false, lastHeardShotSeq: 0
        };
        monsterCopy.push(guardState);
      }
      if (exitRequires === 'clear' && !monsterCopy.length) combatFail(root + '.exitRequires', "为 'clear' 时必须声明 maze.combat.guard");

      var pickupsPath = root + '.pickups', pickups = combatHas(combat, 'pickups') ? combat.pickups : [];
      if (!Array.isArray(pickups)) combatFail(pickupsPath, '必须是数组');
      if (combatHas(combat, 'pickups') && !pickups.length) combatFail(pickupsPath, '出现时必须是非空数组');
      var seenPickupCells = {}, weaponPickupPaths = {}, pickupCopy = [];
      for (var pi = 0; pi < pickups.length; pi++) {
        var pickupPath = pickupsPath + '[' + pi + ']';
        if (!(pi in pickups)) combatFail(pickupPath, '不能是数组空槽');
        var pickup = pickups[pi];
        combatObject(pickup, pickupPath);
        var allowedPickupFields = pickup.kind === 'health' ? ['x', 'y', 'kind', 'amount']
          : pickup.kind === 'ammo' ? ['x', 'y', 'kind', 'weapon', 'amount']
            : pickup.kind === 'weapon' ? ['x', 'y', 'kind', 'weapon', 'ammo', 'maxAmmo'] : ['x', 'y', 'kind'];
        combatFields(pickup, allowedPickupFields, pickupPath);
        combatRequired(pickup, ['x', 'y', 'kind'], pickupPath);
        if (pickup.kind !== 'health' && pickup.kind !== 'ammo' && pickup.kind !== 'weapon') combatFail(pickupPath + '.kind', "必须是 'health'、'ammo' 或 'weapon'");
        var px = combatNumber(pickup.x, pickupPath + '.x', false, true);
        var py = combatNumber(pickup.y, pickupPath + '.y', false, true);
        if (cellAt(grid, px, py) !== '.') combatFail(pickupPath, '必须位于 grid 可走 . 格,得到 (' + px + ',' + py + ')');
        if (px === start.x && py === start.y) combatFail(pickupPath, '不能与 maze.start 同格 (' + px + ',' + py + ')');
        for (var pmi = 0; pmi < monsterCopy.length; pmi++) if (monsterCopy[pmi].x === px && monsterCopy[pmi].y === py) combatFail(pickupPath, '不能与 ' + guardPath + ' 同格 (' + px + ',' + py + ')');
        var pickupCell = px + ',' + py;
        if (combatHas(seenPickupCells, pickupCell)) combatFail(pickupPath, '坐标必须唯一;与 ' + seenPickupCells[pickupCell] + ' 同格 (' + px + ',' + py + ')');
        seenPickupCells[pickupCell] = pickupPath;
        if (Array.isArray(maze.events)) for (var sei = 0; sei < maze.events.length; sei++) {
          var mazeEvent = maze.events[sei];
          if (mazeEvent && mazeEvent.x === px && mazeEvent.y === py) combatFail(pickupPath, '不能与 maze.events[' + sei + '] 同格 (' + px + ',' + py + ')');
        }
        var pickupState = { sx: px + 0.5, sy: py + 0.5, kind: pickup.kind, consumed: false, touching: false, isCombatPickup: true, idx: pi };
        if (pickup.kind === 'health') {
          combatRequired(pickup, ['amount'], pickupPath);
          pickupState.amount = combatNumber(pickup.amount, pickupPath + '.amount', true, true);
        } else if (pickup.kind === 'ammo') {
          combatRequired(pickup, ['weapon', 'amount'], pickupPath);
          pickupState.weapon = combatWeaponKind(pickup.weapon, pickupPath + '.weapon');
          pickupState.amount = combatNumber(pickup.amount, pickupPath + '.amount', true, true);
        } else {
          combatRequired(pickup, ['weapon', 'ammo', 'maxAmmo'], pickupPath);
          pickupState.weapon = combatWeaponKind(pickup.weapon, pickupPath + '.weapon');
          if (combatHas(initialKinds, pickupState.weapon)) combatFail(pickupPath + '.weapon', '不能指向初始已拥有武器');
          if (combatHas(weaponPickupPaths, pickupState.weapon)) combatFail(pickupPath + '.weapon', '同一种未拥有武器最多一个 weapon pickup');
          pickupState.ammo = combatNumber(pickup.ammo, pickupPath + '.ammo', true, true);
          pickupState.maxAmmo = combatNumber(pickup.maxAmmo, pickupPath + '.maxAmmo', true, true);
          if (pickupState.ammo > pickupState.maxAmmo) combatFail(pickupPath + '.ammo', '不能大于 ' + pickupPath + '.maxAmmo');
          weaponPickupPaths[pickupState.weapon] = pickupPath;
          slots[pickupState.weapon].maxAmmo = pickupState.maxAmmo;
        }
        pickupCopy.push(pickupState);
      }
      for (var ai = 0; ai < pickupCopy.length; ai++) {
        var ammoPickup = pickupCopy[ai];
        if (ammoPickup.kind === 'ammo' && !combatHas(initialKinds, ammoPickup.weapon) && !combatHas(weaponPickupPaths, ammoPickup.weapon))
          combatFail(pickupsPath + '[' + ai + '].weapon', '必须指向初始 loadout 或可获得的 weapon pickup');
      }
      return {
        exitRequires: exitRequires,
        deathKey: deathKey,
        player: { maxHealth: maxHealth, health: health },
        weapons: { equipped: equipped, slots: slots },
        monsters: monsterCopy,
        pickups: pickupCopy
      };
    }
    // ── /FPS combat 数据闸 ──
    ...
    function combatEventSfx(profile, eventKind, weaponKind) {     // theme family 与动作共用一张确定性 oscillator 图；不接作者字段。
      if (!hbEnsure() || hbMuted()) return false;
      try {
        if (hbCtx.resume) hbCtx.resume();
        var graph = profile.sfx, t = hbCtx.currentTime, o = hbCtx.createOscillator(), gn = hbCtx.createGain();
        var start = eventKind === 'shot' ? graph[weaponKind] : graph[eventKind];
        var end = eventKind === 'shot' ? start * (weaponKind === 'scatter' ? 0.44 : 0.62)
          : eventKind === 'empty' ? start * 0.72 : eventKind === 'switch' ? start * 1.42 : start * 1.50;
        var duration = eventKind === 'shot' ? (weaponKind === 'scatter' ? 0.13 : 0.09) : eventKind === 'pickup' ? 0.18 : 0.11;
        o.type = eventKind === 'shot' && graph.crystalShotWave ? graph.crystalShotWave : graph.wave;
        o.frequency.setValueAtTime(start, t);
        if (o.frequency.exponentialRampToValueAtTime) o.frequency.exponentialRampToValueAtTime(end, t + duration * 0.86);
        gn.gain.setValueAtTime(eventKind === 'shot' ? 0.15 : 0.10, t); gn.gain.exponentialRampToValueAtTime(0.0001, t + duration);
        o.connect(gn); gn.connect(hbMaster); o.start(t); o.stop(t + duration + 0.01); return true;
      } catch (e) { return false; }
    }
    function combatShotSfx(profile, weaponKind) { return combatEventSfx(profile, 'shot', weaponKind); }
    function combatEmptySfx(profile, weaponKind) { return combatEventSfx(profile, 'empty', weaponKind); }
    function combatSwitchSfx(profile, weaponKind) { return combatEventSfx(profile, 'switch', weaponKind); }
    function combatWeaponPickupSfx(profile, weaponKind) { return combatEventSfx(profile, 'pickup', weaponKind); }
    function combatHurtSfx() {                                  // combat 私有受伤确认音:只跟 health commit edge，不跟 recover 帧重播。
      if (!hbEnsure() || hbMuted()) return false;
      try {
        if (hbCtx.resume) hbCtx.resume();
        var t = hbCtx.currentTime, o = hbCtx.createOscillator(), gn = hbCtx.createGain();
        o.type = 'sawtooth'; o.frequency.setValueAtTime(94, t); if (o.frequency.exponentialRampToValueAtTime) o.frequency.exponentialRampToValueAtTime(46, t + 0.14);
        gn.gain.setValueAtTime(0.12, t); gn.gain.exponentialRampToValueAtTime(0.0001, t + 0.16);
        o.connect(gn); gn.connect(hbMaster); o.start(t); o.stop(t + 0.17); return true;
      } catch (e) { return false; }
    }
    ...
    function activateMaze(state, node) {
      ...
      if (node.winKey) validateMazeStateFactKey(node.winKey, 'node.winKey', state);
      if (node.scareKey) validateMazeStateFactKey(node.scareKey, 'node.scareKey', state);
      if (node.winKey && Object.prototype.hasOwnProperty.call(state, node.winKey) && state[node.winKey] === true) return;
      if (node.scareKey && Object.prototype.hasOwnProperty.call(state, node.scareKey) && state[node.scareKey] === true) return;
      try { startMaze(node); }
    ...
    var mod = {
      ...
      render: function (state, node) {
        if (node.winKey) validateMazeStateFactKey(node.winKey, 'node.winKey', state);
        if (node.scareKey) validateMazeStateFactKey(node.scareKey, 'node.scareKey', state);
        var won = !!(node.winKey && Object.prototype.hasOwnProperty.call(state, node.winKey) && state[node.winKey] === true);
        var caught = !!(node.scareKey && Object.prototype.hasOwnProperty.call(state, node.scareKey) && state[node.scareKey] === true);
        var combatDeathKey = node.maze && node.maze.combat && node.maze.combat.deathKey;
        if (combatDeathKey) combatStateFactKey(combatDeathKey, 'maze.combat.deathKey');
        var dead = !!(combatDeathKey && Object.prototype.hasOwnProperty.call(state, combatDeathKey) && state[combatDeathKey] === true);
        ...
        var text = dead ? '你倒在迷宫里。从下面的选项重试或离开。'
          : caught ? (node.caughtText || '它抓住了你。一切归于黑暗。')
        ...
        if (dead) view.audio = { ambient: false, music: false };   // combat 死亡只停旧声床,不新增战斗音频。
        else if (caught) view.audio = { ambient: false, music: false, sfx: [node.scareSfx || 'horror-sting'] };
    ...
    function startMaze(node) {
      ...
      var combatSession = parseCombatSpec(maze, grid, start);   // 纯数据闸先于 DOM 早退:零 DOM assembly-probe 也必须抓非法 FPS spec；不在 graph 复制私有 schema。
      if (!hostDoc || !hostDoc.getElementById) return;          // 数据已验；无 DOM(探针)→ 退化
      var stage = hostDoc.getElementById(node.stageId || STAGE);
      if (!stage) return;                                       // 挂载点由 assembly-probe 静态配对；runtime 仍保留无 DOM 优雅退化
      if (combatSession && api.state && Object.prototype.hasOwnProperty.call(api.state, combatSession.deathKey) && api.state[combatSession.deathKey] === true) { stop(); return; }   // load/restore 已死亡局:只认自有 boolean true，不让原型/核心对象冒充死亡事实；Atlas 外层 death link 接管。
      sessionStage = stage;
      var canvas = hostDoc.createElement('canvas');
      canvas.width = CW; canvas.height = CH;
      canvas.setAttribute('aria-label', '第一人称迷宫画面');
      if (ENABLE_CONTROLS && !STATIC_PREVIEW) canvas.setAttribute('tabindex', '0');
      ...
      var combatFamily = combatThemeFamily(theme), combatProfile = combatPresentation(theme);
      ...
      var zbuf = null;      // 跨帧复用;render 的墙列循环会在精灵 z-test 前写满 [0,CW)
      ...
      var g = { px: start.x + 0.5, py: start.y + 0.5, a: dirToAngle(start.dir), won: false, atDoor: false, tw: 0, hbAcc: 0, prox: 0,
        ...
        monsters: (maze.monsters || []).map(function (m, i) {
          var sp = { sx: m.x + 0.5, sy: m.y + 0.5, active: m.active !== false, fadeAlpha: (m.fadeAlpha != null ? m.fadeAlpha : 1), chase: m.chase !== false, face: m.face, body: m.body, idx: i, _pathCache: null };
        ...
        caught: false };
      if (combatSession) {
        g.combatFamily = combatFamily;
        g.combatPresentation = combatProfile;
        g.combatExitRequires = combatSession.exitRequires;
        g.combatDeathKey = combatSession.deathKey;
        g.player = combatSession.player;
        g.player.dead = false;
        g.player.faceState = 'normal';
        g.player.hurtUntil = 0;
        g.player.hurtFaceUntil = 0;
        g.combatWeapons = combatSession.weapons;
        g.combatMonsters = combatSession.monsters;
        g.combatPickups = combatSession.pickups;
        g.supplyNotice = null;
        g.combatReady = false;
        g.combatReadyAt = null;
        g.combatNotice = null;
        g.fireQueued = false;
        g.fireHeld = {};
        g.shotSeq = 0;
        ...
        g.muzzleUntil = 0;
        g.recoilUntil = 0;
        g.damageFlashUntil = 0;
        g.damageFlashStrength = 0;
        g.deathKeyWritten = false;
        g.lastGunshot = null;
        g.lastShot = null;
        g.combatFeedback = null;
      }
      // ── /FPS Phase 0b combat session skeleton ──
      var keys = {};
      ...
      function pushDecor(x, y, icon, artSpec, scale, who, idx, mode) {
        ...
        if (decorMode === 'sprite' && directFamily) throw new Error("[maze decor] " + who + " icon:'" + icon + "' 只有贴地碎片外观，不能配 mode:'sprite'；竖牌请改用 GLYPHS 名(如 crystal/idol)或提供 art，否则会被误画成钥匙");
      ...
      function isPickupTarget(evx) { return !!(evx && evx.trigger !== 'interact' && evx._visual === 'pickup' && hasTriggerContent(evx)); }   // 显眼独立 token 的形态承诺“可拿”；显式 interact 仍叫互动，examine-only 仍只查看。
      ...
      function findContextTarget() {                                // 归一成私有候选描述，避免把 event 对象误传 acquireKey 或把 K 塞进 world schema。
        ...
        for (var ei = 0; ei < evList.length; ei++) {
          ...
          var score = front * 2 - d2 + (evx._sprite && evx._sprite.isWallPickup ? 0.8 : 0) + ((isInteractTarget(evx) || isPickupTarget(evx)) ? 1.2 : 0);   // 同范围内优先可执行机关/拾取物,否则玩家按 E 却只读旁边纸条会误导。
      ...
      function syncInteractButton() {                                // 触屏上下文按钮=同一主动 intent 的可见入口;显隐/文案只读当前上下文,不写玩法状态。
        ...
        if (target) {
          var key = target.kind === 'key', pickup = !key && isPickupTarget(target.event), use = !key && !pickup && isInteractTarget(target.event);
          interactBtn.textContent = (key || pickup) ? '拾取' : (use ? '互动' : '查看');
          interactBtn.setAttribute('aria-label', (key || pickup) ? '拾取' : (use ? '互动' : '查看')); // 与可见文本一致(rank15 fix: '检视'→'查看')
        }
      }
      function useContextTarget() {
        ...
        if (isPickupTarget(evx) || isInteractTarget(evx)) {             // 显眼 pickup 与主动机关都执行现役事件结算；普通 pickup 的进格自动路径仍不变。
      ...
      function settleMazeEventActions(evx, evj, st) {               // R1-b4:普通事件与 puzzle.success 共享这一套动作结算;避免 run/set/warp/turn/怪物启停/once 分裂成两套执行器。
        ...
        if (evx.set) {
          ...
          var pathConsumers = [g.monsters, g.combatMonsters || []];
      ...
      sessionTeardown = function () { clearMoveIntent(); exitFsIfMine(true); };   // stop/重进统一清持续输入/开火锁存,释放 Pointer Lock、mousemove 与伪全屏页面样式；节点导航期间不另触发 history.back
      ...
      function combatExitBlocked() {
        return g.combatExitRequires === 'clear' && combatAliveCount() > 0;
      }
      function winNow() {                                       // 推开门、通关、停 loop(canvas 停在"门开了"帧;玩家点「走出迷宫」继续)
        if (g.won) return;
        if (g.needKey && !g.hasKey) return;                     // 门锁:此迷宫有钥匙('K')但未拾取 → 不开(HUD 提示去找钥匙)
        if (combatExitBlocked()) return;                        // 清场锁:只读现有敌人 session truth；活敌时不响门声、不写 state、不停 loop。
      ...
      // ── FPS Phase 1:半自动武器 + session-local 敌人状态机/玩家生存闭环；全部由现有唯一 rAF/g.tw 驱动。──
      function setCombatMonsterState(m, state) {
        if (!m || m.state === state) return;
        m.state = state; m.stateAt = g.tw;
        if (state === 'attack-windup') {
          m.attackAt = g.tw + m.ai.windup;
          m.attackCommitted = false;
        } else {
          m.attackAt = Infinity;
          if (state === 'attack-recover') m.recoverUntil = g.tw + m.ai.cooldown;
          if (state === 'dead' || state === 'inactive') m.attackCommitted = true;
        }
      }
      function combatMonsterDistance(m) {
        var dx = g.px - m.sx, dy = g.py - m.sy;
        return Math.sqrt(dx * dx + dy * dy);
      }
      function combatMonsterHasLos(m, distance) {
        if (!(distance > 0)) return true;
        var a = Math.atan2(g.py - m.sy, g.px - m.sx), wall = castRay(m.sx, m.sy, a);
        return distance + 1e-6 < wall.dist;
      }
      function combatMonsterHeardShot(m) {
        var shot = g.lastGunshot;
        if (!shot || shot.seq <= m.lastHeardShotSeq) return false;
        m.lastHeardShotSeq = shot.seq;
        var dx = shot.x - m.sx, dy = shot.y - m.sy;
        return Math.sqrt(dx * dx + dy * dy) <= m.ai.hear;
      }
      function updateCombatFaceState() {
        if (!g.player) return;
        if (g.player.dead) { g.player.faceState = 'dead'; return; }
        if (g.player.hurtFaceUntil > g.tw) { g.player.faceState = 'hurt'; return; }
        var ms = g.combatMonsters || [];
        for (var i = 0; i < ms.length; i++) if (ms[i].active && !ms[i].dead && ms[i].state === 'attack-windup') { g.player.faceState = 'alert'; return; }
        g.player.faceState = 'normal';
      }
      function playerDeathOnce() {
        if (!g.player || g.player.health !== 0) return false;
        if (!g.player.dead) {
          g.player.dead = true;
          g.player.faceState = 'dead';
          g.player.diedAt = g.tw;
          clearMoveIntent();
          syncWeaponCycleButton();
          exitFsIfMine();
        }
        if (g.deathKeyWritten) return false;
        g.deathKeyWritten = true;   // 必须先锁再 apply；同步重渲染也不能重入重复写。
        api.apply({ run: function (st) { st[g.combatDeathKey] = true; } });
        return true;
      }
      function hurtCombatPlayer(damage, attacker) {
        if (!g.player || g.player.dead || !(damage > 0)) return false;
        var before = g.player.health;
        g.player.health = Math.max(0, before - damage);
        if (g.player.health === before) return false;
        g.player.hurtUntil = g.tw + 0.34;
        g.player.hurtFaceUntil = g.tw + 0.48;
        g.player.faceState = g.player.health === 0 ? 'dead' : 'hurt';
        g.damageFlashUntil = g.tw + 0.24;
        g.damageFlashStrength = Math.min(1, 0.34 + damage / g.player.maxHealth * 1.8);
        g.trauma = Math.min(0.78, (g.trauma || 0) + Math.min(0.62, 0.22 + damage / g.player.maxHealth));
        g.lastHurt = { at: g.tw, damage: before - g.player.health, attackerId: attacker && attacker.id, health: g.player.health };
        combatHurtSfx();
        if (g.player.health === 0) playerDeathOnce();
        return true;
      }
      function showSupplyNotice(text) {
        g.supplyNotice = { text: text, at: g.tw, until: g.tw + 1.6 };
      }
      function combatWeaponLabel(kind) { return combatProfile.weapons[kind]; }
      function combatAmmoLabel(kind) { return combatWeaponLabel(kind) + combatProfile.ammo; }
      function combatTargetCountText(count) { return combatProfile.target + ' ' + count; }
      function combatWeaponSlot(kind) { return g.combatWeapons && g.combatWeapons.slots && g.combatWeapons.slots[kind]; }
      function ownedCombatWeaponKinds() {
        var result = [], order = ['precision', 'scatter'];
        for (var i = 0; i < order.length; i++) { var slot = combatWeaponSlot(order[i]); if (slot && slot.owned) result.push(order[i]); }
        return result;
      }
      function syncWeaponCycleButton() {
        var button = g._weaponCycleButton;
        if (!button || !button.style) return;
        var owned = ownedCombatWeaponKinds(), visible = owned.length > 1 && g.player && !g.player.dead && !g.won && !g.caught;
        button.style.display = visible ? 'inline-flex' : 'none';
        if (!visible) return;
        var current = g.combatWeapons.equipped, target = owned[(owned.indexOf(current) + 1) % owned.length];
        button.textContent = '切' + combatProfile.shortWeapons[target];
        button.setAttribute('title', '切换至' + combatWeaponLabel(target));
      }
      function switchCombatWeapon(kind, silentFeedback) {
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen || !g.combatWeapons) return false;
        var slot = combatWeaponSlot(kind);
        if (!slot || !slot.owned) {
          g.combatNotice = { text: '尚未获得' + combatWeaponLabel(kind), at: g.tw, until: g.tw + 1.1 };
          return false;
        }
        if (g.combatWeapons.equipped === kind) return false;
        readyCombat();
        g.combatWeapons.equipped = kind;
        g.switchReadyAt = g.tw + 0.20;
        g.fireQueued = false;
        if (!silentFeedback) {
          g.combatNotice = { text: (theme === 'crystal' ? '已承载' : '已装备') + combatWeaponLabel(kind), at: g.tw, until: g.tw + 0.8 };
          g.combatFeedback = { kind: 'switch', at: g.tw, until: g.tw + 0.24 };
          combatSwitchSfx(combatProfile, kind);
        }
        syncWeaponCycleButton();
        return true;
      }
      function cycleCombatWeapon() {
        var owned = ownedCombatWeaponKinds();
        if (owned.length < 2) return false;
        var current = g.combatWeapons.equipped, at = owned.indexOf(current);
        return switchCombatWeapon(owned[(at + 1) % owned.length]);
      }
      function resolveCombatPickupContacts() {
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen) return false;
        var pickups = g.combatPickups || [], changed = false;
        for (var i = 0; i < pickups.length; i++) {
          var pickup = pickups[i];
          if (!pickup || pickup.consumed) continue;
          var dx = g.px - pickup.sx, dy = g.py - pickup.sy, touching = dx * dx + dy * dy < 0.25;
          if (!touching) { pickup.touching = false; continue; }
          var firstTouch = !pickup.touching;
          pickup.touching = true;
          if (pickup.kind === 'weapon') {
            var weaponSlot = combatWeaponSlot(pickup.weapon);
            if (!weaponSlot || weaponSlot.owned) continue;
            weaponSlot.owned = true;
            weaponSlot.ammo = pickup.ammo;
            weaponSlot.maxAmmo = pickup.maxAmmo;
            pickup.consumed = true;
            switchCombatWeapon(pickup.weapon, true);
            showSupplyNotice((theme === 'crystal' ? '承载' : '获得') + combatWeaponLabel(pickup.weapon) + ' · ' + pickup.ammo + '/' + pickup.maxAmmo);
            g.combatFeedback = { kind: 'pickup', at: g.tw, until: g.tw + 0.34 };
            combatWeaponPickupSfx(combatProfile, pickup.weapon);
            changed = true;
            continue;
          }
          var slot = pickup.kind === 'ammo' ? combatWeaponSlot(pickup.weapon) : null;
          if (pickup.kind === 'ammo' && (!slot || !slot.owned)) {
            if (firstTouch) showSupplyNotice('尚未获得' + combatWeaponLabel(pickup.weapon));
            continue;
          }
          var current = pickup.kind === 'health' ? g.player.health : slot.ammo;
          var cap = pickup.kind === 'health' ? g.player.maxHealth : slot.maxAmmo;
          var next = Math.min(cap, current + pickup.amount);
          if (next === current) {
            if (firstTouch) showSupplyNotice(pickup.kind === 'health' ? combatProfile.health + '已满' : combatAmmoLabel(pickup.weapon) + '已满');
            continue;
          }
          if (pickup.kind === 'health') g.player.health = next;
          else slot.ammo = next;
          pickup.consumed = true;
          showSupplyNotice('+' + (next - current) + ' ' + (pickup.kind === 'health' ? combatProfile.health : combatAmmoLabel(pickup.weapon)));
          g.combatFeedback = { kind: 'pickup', at: g.tw, until: g.tw + 0.28 };
          changed = true;
        }
        return changed;
      }
      function stepCombatMonsters(dt) {
        if (!g.player || g.player.dead) return;
        var ms = g.combatMonsters || [];
        for (var i = 0; i < ms.length && !g.player.dead; i++) {
          var m = ms[i];
          if (!m || !m.active || m.dead) { if (m) setCombatMonsterState(m, m.dead ? 'dead' : 'inactive'); continue; }
          var distance = combatMonsterDistance(m), los = combatMonsterHasLos(m, distance);
          var seen = m.ai.sight > 0 && distance <= m.ai.sight && los;
          var heard = combatMonsterHeardShot(m);
          if (m.state === 'idle') {
            if (!seen && !heard) continue;
            setCombatMonsterState(m, 'chase');
          }
          if (m.state === 'attack-windup') {
            if (g.tw + 1e-9 < m.attackAt) continue;
            if (m.attackCommitted) continue;
            distance = combatMonsterDistance(m); los = combatMonsterHasLos(m, distance);
            if (distance > m.ai.attackRange || !los) { setCombatMonsterState(m, 'chase'); continue; }
            m.attackCommitted = true;
            hurtCombatPlayer(m.ai.damage, m);   // commit 点只在这里；随后立刻推进 recover,重复帧不能重复扣血。
            setCombatMonsterState(m, 'attack-recover');
            continue;
          }
          if (m.state === 'attack-recover') {
            if (g.tw + 1e-9 < m.recoverUntil) continue;
            setCombatMonsterState(m, 'chase');
          }
          if (m.state !== 'chase') continue;
          distance = combatMonsterDistance(m); los = combatMonsterHasLos(m, distance);
          if (distance <= m.ai.attackRange && los) { setCombatMonsterState(m, 'attack-windup'); continue; }
          if (!(m.ai.moveSpeed > 0)) continue;   // 静止 guard 仍能在上面的范围分支攻击。
          var mcx = Math.floor(m.sx), mcy = Math.floor(m.sy), pcx = Math.floor(g.px), pcy = Math.floor(g.py), tx = m.sx, ty = m.sy;
          if (mcx === pcx && mcy === pcy) { tx = g.px; ty = g.py; }
          else {
            var pathCache = m._pathCache, next;
            if (pathCache && pathCache.lastMonCX === mcx && pathCache.lastMonCY === mcy && pathCache.lastPlayerCX === pcx && pathCache.lastPlayerCY === pcy) next = pathCache.cachedNext;
            else { next = astarNext(mcx, mcy, pcx, pcy); m._pathCache = { lastMonCX: mcx, lastMonCY: mcy, lastPlayerCX: pcx, lastPlayerCY: pcy, cachedNext: next }; }
            if (next) { tx = next[0] + 0.5; ty = next[1] + 0.5; }
          }
          var dx = tx - m.sx, dy = ty - m.sy, dl = Math.sqrt(dx * dx + dy * dy);
          if (dl > 0.001) { var step = Math.min(dl, m.ai.moveSpeed * dt, Math.max(0, distance - COMBAT_GUARD_DISTANCE)); m.sx += dx / dl * step; m.sy += dy / dl * step; }
        }
      }
      function firePointerSource(e) { return 'pointer:' + (e && e.pointerId != null ? e.pointerId : 'primary'); }
      function releaseCombatFire(source) {
        if (g.fireHeld && source != null) delete g.fireHeld[source];
      }
      function clearCombatFireIntent() {
        g.fireQueued = false;
        if (g.fireHeld) for (var source in g.fireHeld) if (Object.prototype.hasOwnProperty.call(g.fireHeld, source)) delete g.fireHeld[source];
        g._suppressFireMouseDown = false;
        g._suppressWeaponCycleClick = false;
      }
      function readyCombat() {
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen || g.combatReady) return false;
        g.combatReady = true;
        g.combatReadyAt = g.tw;
        return true;
      }
      function queueCombatFire(source) {
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen) return false;
        readyCombat();
        source = source || 'discrete';
        if (g.fireHeld[source]) return false;
        g.fireHeld[source] = true;
        g.fireQueued = true;   // semi:同一帧多路输入合并为一发；cooldown 内由 fireCombatWeapon 显式 drop,不缓存连发。
        return true;
      }
      function fireCombatWeapon() {
        if (!g.fireQueued) return false;
        g.fireQueued = false;
        if (!g.player || g.player.dead || g.won || g.caught || g.puzzleOpen) return false;
        var weaponKind = g.combatWeapons && g.combatWeapons.equipped;
        var weapon = combatWeaponSlot(weaponKind), preset = WEAPON_PRESETS[weaponKind];
        if (!weapon || !weapon.owned || !preset) return false;
        if (g.tw + 1e-9 < Math.max(g.fireLockUntil, g.switchReadyAt)) {
          g.combatNotice = { text: (theme === 'crystal' ? combatProfile.shortWeapons[weaponKind] : combatWeaponLabel(weaponKind)) + '回稳中', at: g.tw, until: g.tw + 0.48 };
          return false;
        }
        var ammo = weapon.ammo;
        if (!(ammo > 0)) {
          g.lastShot = { kind: 'empty', at: g.tw, weapon: weaponKind, shotSeq: g.shotSeq };
          g.combatFeedback = { kind: 'empty', at: g.tw, until: g.tw + 0.18 };
          combatEmptySfx(combatProfile, weaponKind);
          return false;
        }
        var aggregates = {}, pelletsHit = 0, totalDamage = 0;
        for (var pi = 0; pi < preset.angles.length; pi++) {
          var angle = g.a + preset.angles[pi], dx = Math.cos(angle), dy = Math.sin(angle), wall = castRay(g.px, g.py, angle);
          var hit = resolveHitscan(g.px, g.py, dx, dy, preset.range, wall.dist, g.combatMonsters || []);
          if (!hit) continue;
          var id = hit.target.id, aggregate = aggregates[id];
          if (!aggregate) aggregate = aggregates[id] = { target: hit.target, pelletsHit: 0, damage: 0 };
          aggregate.pelletsHit++;
          aggregate.damage += preset.damage;
          pelletsHit++;
          totalDamage += preset.damage;
        }
        g.combatNotice = null;
        weapon.ammo = ammo - 1;
        g.shotSeq++;
        g.lastGunshot = { seq: g.shotSeq, at: g.tw, x: g.px, y: g.py };
        combatShotSfx(combatProfile, weaponKind);
        g.fireLockUntil = g.tw + preset.cooldown;
        g.muzzleUntil = g.tw + 0.055;
        g.recoilUntil = g.tw + 0.18;
        g.combatFeedback = { kind: 'muzzle', at: g.tw, until: g.tw + 0.13 };
        var targetIds = Object.keys(aggregates).sort();
        if (!targetIds.length) {
          g.lastShot = { kind: 'miss', at: g.tw, weapon: weaponKind, shotSeq: g.shotSeq };
          return true;
        }
        var killed = false;
        for (var ti = 0; ti < targetIds.length; ti++) {
          var committed = aggregates[targetIds[ti]], target = committed.target;
          target.hp = Math.max(0, target.hp - committed.damage);
          target.painUntil = g.tw + 0.14;
          if (target.hp === 0) { target.dead = true; target.active = false; target.killedAt = g.tw; setCombatMonsterState(target, 'dead'); killed = true; }
        }
        var primary = aggregates[targetIds[0]].target;
        g.lastShot = { kind: killed ? 'kill' : 'hit', at: g.tw, weapon: weaponKind, shotSeq: g.shotSeq, targetId: primary.id, hp: primary.hp, maxHp: primary.maxHp };
        g.combatFeedback = { kind: killed ? 'kill' : 'hit', at: g.tw, until: g.tw + (killed ? 0.28 : 0.18) };
        if (weaponKind === 'scatter') { g.lastShot.pelletsHit = pelletsHit; g.lastShot.totalDamage = totalDamage; }
        return true;
      }
      ...
      if (ENABLE_CONTROLS) {
        ...
        var keyDownH = function (e) {
          ...
          if (g.player && (e.key === '1' || e.key === '2') && !(e && e.repeat)) {
            var switched = switchCombatWeapon(e.key === '1' ? 'precision' : 'scatter');
            if (switched && e.preventDefault) e.preventDefault();
            return;
          }
          if (g.player && (e.key === ' ' || e.key === 'Spacebar')) { if (!(e && e.repeat)) queueCombatFire('keyboard'); if (e.preventDefault) e.preventDefault(); return; }
          if ((e.key === 'e' || e.key === 'E' || e.key === 'Enter') && !(e && e.repeat)) { if (useContextTarget() && e.preventDefault) e.preventDefault(); return; }
          var k = mapKey(e.key); if (k) { readyCombat(); keys[k] = 1; e.preventDefault(); }
        };
        var keyUpH = function (e) { if (e.key === ' ' || e.key === 'Spacebar') releaseCombatFire('keyboard'); var k = mapKey(e.key); if (k) keys[k] = 0; };
      ...
      try { if (ENABLE_CONTROLS) {
        if (canvas.addEventListener) canvas.addEventListener('pointerdown', function () { unlockAudio(); readyCombat(); try { canvas.focus(); } catch (e) {} });
        ...
        if (canvas.requestPointerLock) {
          ...
          function requestMouseLook() {
            ...
            g._suppressFireMouseDown = true;                        // 同一物理点击后续还会发 mousedown；即使浏览器同步授予锁也只锁视角、不偷开第一枪。
        ...
        if (g.player && canvas.addEventListener) canvas.addEventListener('mousedown', function (e) {
          if (e && e.button != null && e.button !== 0) return;
          if (g._suppressFireMouseDown) return;
          var fireDoc = canvas.ownerDocument || hostDoc;
          if (canvas.requestPointerLock && fireDoc.pointerLockElement !== canvas) return;   // 第一击只获取 Pointer Lock,绝不偷扣弹。
          if (queueCombatFire('mouse') && e && e.preventDefault) e.preventDefault();
        });
        if (g.player && canvas.addEventListener) {
          var releaseCanvasPointerFire = function (e) { g._suppressFireMouseDown = false; if (!e || !e.pointerType || e.pointerType === 'mouse') releaseCombatFire('mouse'); };
          var releaseCanvasMouseFire = function () { g._suppressFireMouseDown = false; releaseCombatFire('mouse'); };
          canvas.addEventListener('pointerup', releaseCanvasPointerFire); canvas.addEventListener('pointercancel', releaseCanvasPointerFire); canvas.addEventListener('pointerleave', releaseCanvasPointerFire);
          canvas.addEventListener('mouseup', releaseCanvasMouseFire);
          canvas.addEventListener('wheel', function (e) {
            if (!e || !e.deltaY) return;
            if (cycleCombatWeapon() && e.preventDefault) e.preventDefault();
          });
        }
        ...
        var mkBtn = function (dir) {
          ...
          var set = function (e) { unlockAudio(); readyCombat(); keys[dir] = 1; if (e && e.preventDefault) e.preventDefault(); if (b.classList) b.classList.add('is-pressed'); };
          ...
          b.addEventListener('click', function () { unlockAudio(); readyCombat(); keys[dir] = 1; setTimeout(function () { keys[dir] = 0; }, 100); }); // 键盘 Enter/Space 可达
          return b;
        };
        ...
        if (g.player) {
          var fireBtn = hostDoc.createElement('button');
          fireBtn.textContent = combatProfile.fire; fireBtn.setAttribute('aria-label', combatProfile.fire); fireBtn.setAttribute('title', combatProfile.fire);
          fireBtn.className = 'amatlas-maze-fire amatlas-touchpad-key';
          fireBtn.style.cssText = 'position:absolute;right:18px;bottom:18px;z-index:5;width:56px;height:56px;padding:0;border-radius:50%;pointer-events:auto;touch-action:none;font-size:14px';
          fireBtn.style.width = '56px'; fireBtn.style.height = '56px';   // 功能尺寸显式保留,headless style stub 也可核命中区。
          var suppressFireButtonClick = false;
          fireBtn.addEventListener('pointerdown', function (e) { unlockAudio(); suppressFireButtonClick = true; if (queueCombatFire(firePointerSource(e)) && e && e.preventDefault) e.preventDefault(); });
          fireBtn.addEventListener('pointerup', function (e) { releaseCombatFire(firePointerSource(e)); });
          fireBtn.addEventListener('pointercancel', function (e) { releaseCombatFire(firePointerSource(e)); suppressFireButtonClick = false; });
          fireBtn.addEventListener('pointerleave', function (e) { releaseCombatFire(firePointerSource(e)); suppressFireButtonClick = false; });
          fireBtn.addEventListener('click', function () {
            if (suppressFireButtonClick) { suppressFireButtonClick = false; return; }   // 物理 pointerdown 已排沿；吞随后 click，不依赖 MouseEvent.detail 的浏览器/mock 差异。
            unlockAudio(); if (queueCombatFire('accessible-click')) releaseCombatFire('accessible-click');
          });
          stage.appendChild(fireBtn);
          var weaponCycleBtn = hostDoc.createElement('button');
          weaponCycleBtn.setAttribute('aria-label', '切换武器');
          weaponCycleBtn.className = 'amatlas-maze-weapon-cycle amatlas-touchpad-key';
          weaponCycleBtn.style.cssText = 'display:none;position:absolute;right:82px;bottom:18px;z-index:5;min-width:48px;height:48px;padding:0 7px;align-items:center;justify-content:center;pointer-events:auto;touch-action:none;font-size:12px';
          weaponCycleBtn.style.minWidth = '48px'; weaponCycleBtn.style.height = '48px';
          weaponCycleBtn.addEventListener('pointerdown', function (e) {
            unlockAudio(); g._suppressWeaponCycleClick = true;
            if (cycleCombatWeapon() && e && e.preventDefault) e.preventDefault();
          });
          weaponCycleBtn.addEventListener('pointercancel', function () { g._suppressWeaponCycleClick = false; });
          weaponCycleBtn.addEventListener('pointerleave', function () { g._suppressWeaponCycleClick = false; });
          weaponCycleBtn.addEventListener('click', function () {
            if (g._suppressWeaponCycleClick) { g._suppressWeaponCycleClick = false; return; }
            unlockAudio(); cycleCombatWeapon();
          });
          stage.appendChild(weaponCycleBtn);
          g._weaponCycleButton = weaponCycleBtn;
          syncWeaponCycleButton();
        }
        syncInteractButton();
        if (isTouch) {
        ...
        if (opts.fullscreen !== false) {   // 统一伪全屏:总给沉浸按钮(不依赖系统全屏 API,iOS/安卓/桌面一致;opts.fullscreen:false 关)
          ...
          fsBtn.style.cssText = 'position:absolute;top:8px;right:8px;z-index:3;pointer-events:auto;width:44px;height:44px;display:inline-flex;align-items:center;justify-content:center;border-radius:10px;border:1.5px solid rgba(200,170,115,.48);background:linear-gradient(180deg,rgba(46,39,28,.4),rgba(18,14,9,.42));color:#f4dcaa;cursor:pointer;touch-action:none;-webkit-user-select:none;user-select:none;box-shadow:0 1px 4px rgba(0,0,0,.4)';
      ...
      function projectSprite(sp) {
        ...
        if (sp.isKey || sp.isCombatPickup || (sp.isItem && !sp.isDecor && !sp.isFloorPickup && !sp.isWallPickup)) {            // 钥匙/坐标事件道具/combat pickup:独立 token,底部锚到地面线并共享真实 z-buffer。
      ...
      function combatPickupLayers(p) {                          // family 固定 weapon/ammo/health glyph；不接受作者 art/preset。
        var cx = p.screenX, floorY = p.floorY, w = p.w, h = p.h, L = [], C = combatProfile.pickup, B = combatProfile.colors.bright, D = combatProfile.colors.dark;
        if (p.sp.kind === 'weapon') {
          if (combatFamily === 'ordnance') {
            var wide = p.sp.weapon === 'scatter';
            L.push({ x: cx - w * (wide ? 0.50 : 0.36), y: floorY - h * 0.58, w: w * (wide ? 1.00 : 0.72), h: h * 0.17, rgb: C.weapon });
            L.push({ x: cx - w * 0.28, y: floorY - h * 0.40, w: w * 0.56, h: h * 0.20, rgb: B });
            L.push({ x: cx - w * 0.09, y: floorY - h * 0.22, w: w * 0.18, h: h * 0.20, rgb: D });
            if (wide) L.push({ x: cx - w * 0.43, y: floorY - h * 0.67, w: w * 0.86, h: h * 0.07, rgb: C.ammo });
          } else if (combatFamily === 'relic') {
            var fan = p.sp.weapon === 'scatter';
            L.push({ x: cx - w * (fan ? 0.45 : 0.08), y: floorY - h * 0.68, w: w * (fan ? 0.90 : 0.16), h: h * 0.58, rgb: C.weapon });
            L.push({ x: cx - w * 0.32, y: floorY - h * 0.42, w: w * 0.64, h: h * 0.08, rgb: C.ammo });
            if (fan) { L.push({ x: cx - w * 0.36, y: floorY - h * 0.58, w: w * 0.12, h: h * 0.34, rgb: B }); L.push({ x: cx + w * 0.24, y: floorY - h * 0.58, w: w * 0.12, h: h * 0.34, rgb: B }); }
            else L.push({ x: cx - w * 0.18, y: floorY - h * 0.76, w: w * 0.36, h: h * 0.08, rgb: B });
          } else if (combatFamily === 'organic') {
            L.push({ x: cx - w * 0.38, y: floorY - h * 0.54, w: w * 0.76, h: h * 0.36, rgb: C.weapon });
            L.push({ x: cx - w * 0.23, y: floorY - h * 0.66, w: w * 0.46, h: h * 0.54, rgb: D });
            if (p.sp.weapon === 'scatter') { L.push({ x: cx - w * 0.48, y: floorY - h * 0.46, w: w * 0.18, h: h * 0.18, rgb: C.ammo }); L.push({ x: cx + w * 0.30, y: floorY - h * 0.46, w: w * 0.18, h: h * 0.18, rgb: C.ammo }); }
            else L.push({ x: cx - w * 0.05, y: floorY - h * 0.82, w: w * 0.10, h: h * 0.64, rgb: B });
          } else {
            var ring = p.sp.weapon === 'scatter';
            L.push({ x: cx - w * (ring ? 0.48 : 0.10), y: floorY - h * 0.66, w: w * (ring ? 0.96 : 0.20), h: h * 0.56, rgb: C.weapon });
            L.push({ x: cx - w * 0.36, y: floorY - h * 0.49, w: w * 0.72, h: h * 0.10, rgb: D });
            L.push({ x: cx - w * 0.08, y: floorY - h * 0.59, w: w * 0.16, h: h * 0.38, rgb: B });
            if (ring) { L.push({ x: cx - w * 0.40, y: floorY - h * 0.72, w: w * 0.12, h: h * 0.68, rgb: C.ammo }); L.push({ x: cx + w * 0.28, y: floorY - h * 0.72, w: w * 0.12, h: h * 0.68, rgb: C.ammo }); }
          }
        } else if (p.sp.kind === 'ammo') {
          if (combatFamily === 'ordnance') {
            L.push({ x: cx - w * 0.46, y: floorY - h * 0.58, w: w * 0.92, h: h * 0.50, rgb: D });
            for (var i = 0; i < 3; i++) L.push({ x: cx - w * 0.29 + i * w * 0.22, y: floorY - h * 0.48, w: w * 0.10, h: h * 0.31, rgb: C.ammo });
          } else if (combatFamily === 'relic') {
            L.push({ x: cx - w * 0.42, y: floorY - h * 0.55, w: w * 0.84, h: h * 0.12, rgb: C.ammo });
            L.push({ x: cx - w * 0.08, y: floorY - h * 0.72, w: w * 0.16, h: h * 0.52, rgb: C.weapon });
            L.push({ x: cx - w * 0.28, y: floorY - h * 0.42, w: w * 0.56, h: h * 0.07, rgb: B });
          } else if (combatFamily === 'organic') {
            for (var oi = 0; oi < 4; oi++) L.push({ x: cx - w * 0.34 + (oi % 2) * w * 0.36, y: floorY - h * (0.61 - Math.floor(oi / 2) * 0.25), w: w * 0.30, h: h * 0.22, rgb: C.ammo });
            L.push({ x: cx - w * 0.10, y: floorY - h * 0.64, w: w * 0.20, h: h * 0.52, rgb: D });
          } else {
            for (var ei = 0; ei < 5; ei++) L.push({ x: cx - w * 0.34 + ei * w * 0.17, y: floorY - h * (0.58 + (ei % 2) * 0.12), w: w * 0.09, h: h * 0.09, rgb: C.ammo });
            L.push({ x: cx - w * 0.38, y: floorY - h * 0.34, w: w * 0.76, h: h * 0.06, rgb: B });
          }
        } else {
          if (combatFamily === 'ordnance') {
            L.push({ x: cx - w * 0.46, y: floorY - h * 0.64, w: w * 0.92, h: h * 0.56, rgb: B });
            L.push({ x: cx - w * 0.09, y: floorY - h * 0.56, w: w * 0.18, h: h * 0.38, rgb: C.health });
            L.push({ x: cx - w * 0.25, y: floorY - h * 0.45, w: w * 0.50, h: h * 0.15, rgb: C.health });
          } else if (combatFamily === 'relic') {
            L.push({ x: cx - w * 0.30, y: floorY - h * 0.66, w: w * 0.60, h: h * 0.56, rgb: C.health });
            L.push({ x: cx - w * 0.08, y: floorY - h * 0.77, w: w * 0.16, h: h * 0.60, rgb: B });
            L.push({ x: cx - w * 0.28, y: floorY - h * 0.44, w: w * 0.56, h: h * 0.08, rgb: D });
          } else if (combatFamily === 'organic') {
            L.push({ x: cx - w * 0.38, y: floorY - h * 0.56, w: w * 0.34, h: h * 0.38, rgb: C.health });
            L.push({ x: cx + w * 0.04, y: floorY - h * 0.56, w: w * 0.34, h: h * 0.38, rgb: C.health });
            L.push({ x: cx - w * 0.18, y: floorY - h * 0.40, w: w * 0.36, h: h * 0.32, rgb: C.health });
            L.push({ x: cx - w * 0.04, y: floorY - h * 0.76, w: w * 0.08, h: h * 0.66, rgb: D });
          } else {
            L.push({ x: cx - w * 0.08, y: floorY - h * 0.72, w: w * 0.16, h: h * 0.62, rgb: C.health });
            L.push({ x: cx - w * 0.31, y: floorY - h * 0.47, w: w * 0.62, h: h * 0.14, rgb: C.health });
            L.push({ x: cx - w * 0.20, y: floorY - h * 0.62, w: w * 0.40, h: h * 0.40, rgb: D });
          }
        }
        return L;
      }
      function combatGuardLayers(p) {                           // 主题 guard 唯一外观 owner；行为仍只读同一 combat monster state。
        var cx = p.screenX, cy = p.y0 + p.h * 0.50, s = p.h, L = [], state = p.sp.dead ? 'dead' : (p.sp.painUntil > g.tw ? 'hurt' : p.sp.state);
        var C = combatProfile.colors, floorY = floorYAtDepth(p.depth), i;
        if (state === 'dead') {
          if (theme === 'crystal') {
            L.push({ x: cx - s * 0.42, y: floorY - s * 0.035, w: s * 0.84, h: Math.max(1, s * 0.018), rgb: C.primary });
            L.push({ x: cx - s * 0.30, y: floorY - s * 0.09, w: s * 0.60, h: Math.max(1, s * 0.015), rgb: C.bright });
            for (i = 0; i < 5; i++) L.push({ x: cx - s * 0.34 + i * s * 0.17, y: floorY - s * (0.04 + (i % 2) * 0.07), w: Math.max(1, s * 0.025), h: Math.max(1, s * 0.025), rgb: i === 2 ? C.primary : C.secondary });
          } else if (combatFamily === 'ordnance') {
            L.push({ x: cx - s * 0.38, y: floorY - s * 0.10, w: s * 0.76, h: s * 0.10, rgb: C.dark });
            L.push({ x: cx - s * 0.28, y: floorY - s * 0.16, w: s * 0.56, h: s * 0.07, rgb: C.primary });
          } else if (combatFamily === 'relic') {
            for (i = 0; i < 4; i++) L.push({ x: cx - s * 0.38 + i * s * 0.20, y: floorY - s * (0.07 + (i % 2) * 0.06), w: s * 0.16, h: s * 0.06, rgb: i % 2 ? C.secondary : C.primary });
          } else if (combatFamily === 'organic') {
            for (i = 0; i < 5; i++) L.push({ x: cx - s * 0.36 + i * s * 0.15, y: floorY - s * (0.04 + (i % 3) * 0.035), w: s * 0.18, h: s * 0.05, rgb: i % 2 ? C.primary : C.dark });
          } else {
            L.push({ x: cx - s * 0.40, y: floorY - s * 0.055, w: s * 0.80, h: s * 0.025, rgb: C.primary });
            for (i = 0; i < 4; i++) L.push({ x: cx - s * 0.30 + i * s * 0.20, y: floorY - s * 0.10, w: s * 0.04, h: s * 0.04, rgb: C.accent });
          }
          return L;
        }
        if (theme === 'crystal') {
          var rng = mulberry32(0xC7A57A1 ^ ((p.sp.idx || 0) * 131));
          L.push({ x: cx - s * 0.30, y: cy - s * 0.48, w: s * 0.60, h: s * 0.92, rgb: C.dark });
          L.push({ x: cx - s * 0.34, y: cy - s * 0.34, w: s * 0.23, h: s * 0.025, rgb: C.secondary });
          L.push({ x: cx + s * 0.08, y: cy - s * 0.34, w: s * 0.26, h: s * 0.025, rgb: C.secondary });
          L.push({ x: cx - s * 0.27, y: cy + s * 0.03, w: s * 0.20, h: s * 0.025, rgb: C.bright });
          L.push({ x: cx + s * 0.03, y: cy + s * 0.03, w: s * 0.29, h: s * 0.025, rgb: C.secondary });
          L.push({ x: cx - s * 0.18, y: cy - s * 0.44, w: s * 0.025, h: s * 0.32, rgb: C.bright });
          L.push({ x: cx + s * 0.17, y: cy - s * 0.20, w: s * 0.025, h: s * 0.48, rgb: C.secondary });
          for (i = 0; i < 7; i++) L.push({ x: cx + (rng() - 0.5) * s * 0.64, y: cy + (rng() - 0.5) * s * 0.78, w: Math.max(1, s * 0.025), h: Math.max(1, s * 0.025), rgb: i % 3 ? C.secondary : C.bright });
          if (state === 'hurt') {
            L.push({ x: cx - s * 0.28, y: cy - s * 0.40, w: s * 0.56, h: s * 0.025, rgb: C.bright }); L.push({ x: cx - s * 0.28, y: cy + s * 0.34, w: s * 0.56, h: s * 0.025, rgb: C.bright });
            L.push({ x: cx - s * 0.28, y: cy - s * 0.40, w: s * 0.025, h: s * 0.76, rgb: C.bright }); L.push({ x: cx + s * 0.255, y: cy - s * 0.40, w: s * 0.025, h: s * 0.76, rgb: C.bright });
          } else if (state === 'attack-windup') {
            L.push({ x: cx - s * 0.04, y: cy - s * 0.04, w: s * 0.08, h: s * 0.08, rgb: C.primary });
            L.push({ x: cx - s * 0.30, y: cy, w: s * 0.24, h: s * 0.025, rgb: C.primary }); L.push({ x: cx + s * 0.06, y: cy, w: s * 0.24, h: s * 0.025, rgb: C.primary });
            L.push({ x: cx, y: cy - s * 0.35, w: s * 0.025, h: s * 0.29, rgb: C.primary }); L.push({ x: cx, y: cy + s * 0.06, w: s * 0.025, h: s * 0.29, rgb: C.primary });
          }
        } else if (combatFamily === 'ordnance') {
          L.push({ x: cx - s * 0.32, y: cy - s * 0.46, w: s * 0.64, h: s * 0.88, rgb: C.primary });
          L.push({ x: cx - s * 0.24, y: cy - s * 0.33, w: s * 0.48, h: s * 0.18, rgb: C.dark });
          L.push({ x: cx - s * 0.18, y: cy - s * 0.27, w: s * 0.36, h: s * 0.04, rgb: state === 'attack-windup' ? C.kill : C.accent });
          L.push({ x: cx - s * 0.39, y: cy - s * 0.18, w: s * 0.12, h: s * 0.48, rgb: C.secondary }); L.push({ x: cx + s * 0.27, y: cy - s * 0.18, w: s * 0.12, h: s * 0.48, rgb: C.secondary });
        } else if (combatFamily === 'relic') {
          L.push({ x: cx - s * 0.29, y: cy - s * 0.48, w: s * 0.58, h: s * 0.92, rgb: C.primary });
          L.push({ x: cx - s * 0.20, y: cy - s * 0.35, w: s * 0.40, h: s * 0.08, rgb: C.bright });
          L.push({ x: cx - s * 0.04, y: cy - s * 0.43, w: s * 0.08, h: s * 0.72, rgb: C.dark });
          L.push({ x: cx - s * 0.21, y: cy - s * 0.05, w: s * 0.42, h: s * 0.06, rgb: state === 'attack-windup' ? C.accent : C.secondary });
        } else if (combatFamily === 'organic') {
          L.push({ x: cx - s * 0.25, y: cy - s * 0.46, w: s * 0.50, h: s * 0.90, rgb: C.dark });
          for (i = 0; i < 4; i++) L.push({ x: cx - s * 0.36 + (i % 2) * s * 0.38, y: cy - s * 0.39 + Math.floor(i / 2) * s * 0.40, w: s * 0.34, h: s * 0.32, rgb: i % 2 ? C.secondary : C.primary });
          L.push({ x: cx - s * 0.03, y: cy - s * 0.35, w: s * 0.06, h: s * 0.70, rgb: state === 'attack-windup' ? C.kill : C.bright });
        } else {
          L.push({ x: cx - s * 0.30, y: cy - s * 0.46, w: s * 0.60, h: s * 0.90, rgb: C.dark });
          for (i = 0; i < 4; i++) { L.push({ x: cx - s * 0.24 + i * s * 0.16, y: cy - s * 0.36, w: s * 0.035, h: s * 0.66, rgb: C.primary }); L.push({ x: cx - s * 0.25, y: cy - s * 0.30 + i * s * 0.18, w: s * 0.50, h: s * 0.035, rgb: C.secondary }); }
          if (state === 'attack-windup') L.push({ x: cx - s * 0.08, y: cy - s * 0.08, w: s * 0.16, h: s * 0.16, rgb: C.accent });
        }
        if (state === 'hurt' && theme !== 'crystal') L.push({ x: cx - s * 0.34, y: cy - s * 0.03, w: s * 0.68, h: s * 0.06, rgb: C.hit });
        return L;
      }
      ...
      function drawSprites(zbuf) {                               // 墙画完后叠加;逐列 z-test(精灵比该列墙近才画 → 墙遮挡精灵)
        if (!g.monsters.length && !(g.combatMonsters && g.combatMonsters.length) && !(g.combatPickups && g.combatPickups.length) && !(g.items && g.items.length) && !evList.length && !(g.pillars && g.pillars.length) && !(g.decors && g.decors.length)) return;
        var list = [], s, m, pr;
        for (s = 0; s < g.monsters.length; s++) { m = g.monsters[s]; if (!m.active || m.fadeAlpha <= 0) continue; pr = projectSprite(m); if (pr) list.push(pr); }
        if (g.combatMonsters) for (s = 0; s < g.combatMonsters.length; s++) { m = g.combatMonsters[s]; if (!m.active && !m.dead) continue; pr = projectSprite(m); if (pr) list.push(pr); }   // Phase 1a 静态靶:死亡后退出 simulation/hitscan,但 corpse 独立复用投影/z-buffer。
        if (g.combatPickups) for (s = 0; s < g.combatPickups.length; s++) { var pickup = g.combatPickups[s]; if (pickup.consumed) continue; pr = projectSprite(pickup); if (pr) list.push(pr); }
        ...
        for (var k = 0; k < list.length; k++) {
          ...
          if (p.sp.isCombatPickup) ffog = 1;                         // combat glyph 是战斗读数的一部分，主题锚色不被场景雾吞掉。
          else if (p.sp.isKey || (p.sp.isItem && !p.sp.isFloorPickup && !p.sp.isWallPickup)) ffog = Math.max(0.5, ffog);   // floor 物品(钥匙/事件道具)可见性下限;嵌入式隐藏物保持低调
          ...
          if ((p.sp.isKey || p.sp.isCombatPickup || (p.sp.isItem && !p.sp.isMarker && !p.sp.isDecor && !p.sp.isFloorPickup && !p.sp.isWallPickup)) && p.floorY != null) {   // pickup 接触阴影:独立 token 仍保持“能拿”高可见性,但脚点压在地面上
          ...
          var layers;
          if (p.sp.isCombatPickup) {
            layers = combatPickupLayers(p);
          } else if (p.sp.isPillar) {
            ...
            layers = p.sp.art ? artLayers(p) : [];                     // pushDecor 已保证 sprite decor 有竖牌 art；这里不再用金钥匙伪装坏/缺外观。
          } else if (p.sp.combatTarget) {
            layers = combatGuardLayers(p);
          } else {
          ...
          var painFlash = p.sp.combatTarget && p.sp.painUntil > g.tw;
          var attackTelegraph = p.sp.combatTarget && p.sp.state === 'attack-windup';
          for (var Li = 0; Li < layers.length; Li++) {
            ...
            ctx.fillStyle = p.sp.combatTarget ? fogColor(ly.rgb, fog) : painFlash ? 'rgb(255,238,206)' : attackTelegraph ? 'rgb(255,112,58)' : fogColor(ly.rgb, fog);   // combat guard 的 hurt/windup 由 family 几何+色板表达；普通怪仍沿用白闪/橙预兆。
            for (var col = lx0; col <= lx1; col++) { if (lunge || zbuf[col] === undefined || p.depth < zbuf[col]) ctx.fillRect(col, ry, 1, rh); }   // 突脸不被墙遮挡=全脸揭示
          }
        }
      }

      function combatAliveCount() {
        var n = 0, ms = g.combatMonsters || [];
        for (var i = 0; i < ms.length; i++) if (ms[i].active && !ms[i].dead) n++;
        return n;
      }
      function combatWindupCount() {
        var n = 0, ms = g.combatMonsters || [];
        for (var i = 0; i < ms.length; i++) if (ms[i].active && !ms[i].dead && ms[i].state === 'attack-windup') n++;
        return n;
      }
      function combatFaceLabel() {
        return combatProfile.face[g.player && g.player.faceState] || combatProfile.face.normal;
      }
      function combatCss(rgb) { return 'rgb(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ')'; }
      function drawCombatCrosshair(kind, cx, cy) {
        var C = combatProfile.colors;
        ctx.fillStyle = 'rgba(0,0,0,.82)';
        if (combatFamily === 'ordnance') {
          var gap = kind === 'scatter' ? 11 : 4, len = kind === 'scatter' ? 7 : 9;
          ctx.fillRect(cx - gap - len, cy - 1, len, 3); ctx.fillRect(cx + gap, cy - 1, len, 3); ctx.fillRect(cx - 1, cy - gap - len, 3, len); ctx.fillRect(cx - 1, cy + gap, 3, len);
          ctx.fillStyle = combatCss(C.bright);
          ctx.fillRect(cx - gap - len + 1, cy, len - 1, 1); ctx.fillRect(cx + gap, cy, len - 1, 1); ctx.fillRect(cx, cy - gap - len + 1, 1, len - 1); ctx.fillRect(cx, cy + gap, 1, len - 1);
          if (kind === 'scatter') { ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 2, cy - 2, 4, 4); }
        } else if (combatFamily === 'relic') {
          ctx.fillStyle = combatCss(C.primary);
          if (kind === 'precision') {
            ctx.fillRect(cx - 1, cy - 15, 3, 11); ctx.fillRect(cx - 1, cy + 4, 3, 11); ctx.fillRect(cx - 8, cy - 1, 6, 3); ctx.fillRect(cx + 3, cy - 1, 6, 3);
            ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx, cy - 3, 1, 7);
          } else {
            for (var ri = -2; ri <= 2; ri++) ctx.fillRect(cx + ri * 7 - 2, cy + Math.abs(ri) * 3 - 2, 4, 4);
            ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 14, cy - 8, 28, 2); ctx.fillRect(cx - 18, cy + 9, 36, 2);
          }
        } else if (combatFamily === 'organic') {
          ctx.fillStyle = combatCss(C.primary);
          if (kind === 'precision') {
            ctx.fillRect(cx - 2, cy - 14, 4, 10); ctx.fillRect(cx - 2, cy + 4, 4, 10); ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 5, cy - 2, 10, 4);
          } else {
            var spores = [[-13,-7],[0,-11],[13,-7],[-9,8],[9,8]];
            for (var si = 0; si < spores.length; si++) ctx.fillRect(cx + spores[si][0] - 2, cy + spores[si][1] - 2, 5, 5);
            ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 3, cy - 3, 6, 6);
          }
        } else if (theme === 'crystal') {
          ctx.fillStyle = combatCss(C.primary);
          if (kind === 'precision') {
            ctx.fillRect(cx - 1, cy - 17, 3, 13); ctx.fillRect(cx - 1, cy + 4, 3, 13); ctx.fillRect(cx - 17, cy - 1, 13, 3); ctx.fillRect(cx + 4, cy - 1, 13, 3);
            ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 3, cy - 3, 7, 7);
            ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(cx - 13, cy - 13, 3, 3); ctx.fillRect(cx + 11, cy - 13, 3, 3); ctx.fillRect(cx - 13, cy + 11, 3, 3); ctx.fillRect(cx + 11, cy + 11, 3, 3);
          } else {
            ctx.fillRect(cx - 19, cy - 11, 3, 22); ctx.fillRect(cx - 16, cy - 11, 7, 3); ctx.fillRect(cx - 16, cy + 8, 7, 3);
            ctx.fillRect(cx + 16, cy - 11, 3, 22); ctx.fillRect(cx + 9, cy - 11, 7, 3); ctx.fillRect(cx + 9, cy + 8, 7, 3);
            ctx.fillStyle = combatCss(C.secondary); var stars = [[0,-12],[-10,0],[10,0],[-6,10],[6,10]]; for (var ci = 0; ci < stars.length; ci++) ctx.fillRect(cx + stars[ci][0] - 2, cy + stars[ci][1] - 2, 4, 4);
            ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 2, cy - 2, 4, 4);
          }
        } else {
          ctx.fillStyle = combatCss(C.primary);
          if (kind === 'precision') {
            ctx.fillRect(cx - 1, cy - 15, 3, 10); ctx.fillRect(cx - 1, cy + 5, 3, 10); ctx.fillRect(cx - 15, cy - 1, 10, 3); ctx.fillRect(cx + 5, cy - 1, 10, 3); ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 2, cy - 2, 5, 5);
          } else {
            ctx.fillRect(cx - 18, cy - 10, 3, 20); ctx.fillRect(cx + 15, cy - 10, 3, 20); ctx.fillStyle = combatCss(C.secondary); for (var ni = -2; ni <= 2; ni++) ctx.fillRect(cx + ni * 6 - 1, cy + (ni % 2) * 5 - 1, 3, 3);
          }
        }
      }
      function drawCombatWeaponBody(kind, cx, wy) {
        var C = combatProfile.colors;
        if (combatFamily === 'ordnance') {
          var ww = kind === 'scatter' ? 132 : 92, wx = cx - ww / 2;
          ctx.fillStyle = combatCss(C.dark); ctx.fillRect(wx, wy + 18, ww, 40);
          ctx.fillStyle = combatCss(C.primary); ctx.fillRect(wx + (kind === 'scatter' ? 6 : 12), wy + 6, ww - (kind === 'scatter' ? 12 : 24), 24);
          ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(wx + 20, wy + 8, ww - 40, 5);
          ctx.fillStyle = combatCss(C.dark); ctx.fillRect(cx - 13, wy + 34, 26, 30);
          if (kind === 'scatter') { ctx.fillStyle = combatCss(C.accent); ctx.fillRect(wx + 9, wy + 28, ww - 18, 5); ctx.fillRect(wx + 18, wy + 3, ww - 36, 3); }
        } else if (combatFamily === 'relic') {
          ctx.fillStyle = combatCss(C.dark); ctx.fillRect(cx - (kind === 'scatter' ? 67 : 38), wy + 24, kind === 'scatter' ? 134 : 76, 37);
          ctx.fillStyle = combatCss(C.primary);
          if (kind === 'precision') { ctx.fillRect(cx - 7, wy - 1, 14, 49); ctx.fillRect(cx - 31, wy + 17, 62, 9); ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 2, wy - 8, 4, 52); }
          else { ctx.fillRect(cx - 60, wy + 8, 120, 29); ctx.fillStyle = combatCss(C.secondary); for (var ri = 0; ri < 5; ri++) ctx.fillRect(cx - 50 + ri * 24, wy + 1 + Math.abs(2 - ri) * 4, 12, 32); ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 47, wy + 18, 94, 4); }
        } else if (combatFamily === 'organic') {
          ctx.fillStyle = combatCss(C.dark); ctx.fillRect(cx - (kind === 'scatter' ? 64 : 40), wy + 22, kind === 'scatter' ? 128 : 80, 40);
          ctx.fillStyle = combatCss(C.primary); ctx.fillRect(cx - 32, wy + 10, 64, 42);
          ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(cx - 21, wy + 4, 42, 16);
          if (kind === 'precision') { ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 3, wy - 18, 6, 48); }
          else { ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 58, wy + 15, 24, 28); ctx.fillRect(cx + 34, wy + 15, 24, 28); ctx.fillRect(cx - 46, wy + 3, 92, 8); }
        } else {
          var isCrystal = theme === 'crystal', rw = kind === 'scatter' ? 140 : 88;
          ctx.fillStyle = combatCss(C.dark); ctx.fillRect(cx - rw / 2, wy + 22, rw, 40);
          if (kind === 'precision') {
            ctx.fillStyle = combatCss(C.primary); ctx.fillRect(cx - 5, wy - 12, 10, 54); ctx.fillRect(cx - 34, wy + 10, 68, 7);
            ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 2, wy - 18, 4, 56);
            ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(cx - 27, wy + 4, 8, 8); ctx.fillRect(cx + 19, wy + 4, 8, 8);
          } else {
            ctx.fillStyle = combatCss(C.primary); ctx.fillRect(cx - 61, wy + 5, 14, 45); ctx.fillRect(cx + 47, wy + 5, 14, 45);
            ctx.fillRect(cx - 54, wy + 5, 108, 8); ctx.fillRect(cx - 54, wy + 39, 108, 8);
            ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(cx - 45, wy + 15, 90, 22);
            ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 5, wy + 6, 10, 40);
          }
          if (!isCrystal) { ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 18, wy + 26, 36, 6); }
        }
      }
      function drawCombatMuzzle(kind, cx, wy) {
        var C = combatProfile.colors;
        if (combatFamily === 'ordnance') {
          ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 8, wy - 12, 16, 18); ctx.fillRect(cx - 18, wy - 7, 36, 8); ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 4, wy - 18, 8, 16);
        } else if (combatFamily === 'relic') {
          ctx.fillStyle = combatCss(C.accent); ctx.fillRect(cx - 2, wy - 20, 4, 20); ctx.fillRect(cx - 15, wy - 11, 30, 4); ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 7, wy - 15, 14, 14);
        } else if (combatFamily === 'organic') {
          ctx.fillStyle = combatCss(C.accent); var ps = [[-14,-8],[0,-18],[14,-8],[-8,2],[8,2]]; for (var i = 0; i < ps.length; i++) ctx.fillRect(cx + ps[i][0] - 3, wy + ps[i][1] - 3, 6, 6); ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 5, wy - 9, 10, 10);
        } else {
          ctx.fillStyle = combatCss(C.primary); ctx.fillRect(cx - 2, wy - 22, 4, 22); ctx.fillRect(cx - 20, wy - 13, 40, 4); ctx.fillStyle = combatCss(C.bright); ctx.fillRect(cx - 7, wy - 18, 14, 14); ctx.fillStyle = combatCss(C.secondary); ctx.fillRect(cx - 15, wy - 18, 4, 4); ctx.fillRect(cx + 11, wy - 18, 4, 4);
        }
      }
      function drawCombatFeedback(cx, cy, wy) {
        var f = g.combatFeedback;
        if (!f || g.tw >= f.until) return;
        var C = combatProfile.colors, kill = f.kind === 'kill';
        ctx.fillStyle = combatCss(kill ? C.kill : f.kind === 'hit' ? C.hit : C.accent);
        if (f.kind === 'hit' || f.kind === 'kill') {
          if (combatFamily === 'ordnance') {
            var d = kill ? 12 : 9, z = kill ? 5 : 3;
            ctx.fillRect(cx - d, cy - d, z, z); ctx.fillRect(cx + d - z, cy - d, z, z); ctx.fillRect(cx - d, cy + d - z, z, z); ctx.fillRect(cx + d - z, cy + d - z, z, z);
            if (kill) { ctx.fillRect(cx - 1, cy - 17, 3, 8); ctx.fillRect(cx - 1, cy + 9, 3, 8); }
          } else if (combatFamily === 'relic') {
            var rd = kill ? 15 : 10;
            ctx.fillRect(cx - 2, cy - rd, 4, rd - 3); ctx.fillRect(cx - 2, cy + 3, 4, rd - 3); ctx.fillRect(cx - rd, cy - 2, rd - 3, 4); ctx.fillRect(cx + 3, cy - 2, rd - 3, 4);
            if (kill) { ctx.fillRect(cx - 11, cy - 11, 5, 5); ctx.fillRect(cx + 6, cy - 11, 5, 5); ctx.fillRect(cx - 11, cy + 6, 5, 5); ctx.fillRect(cx + 6, cy + 6, 5, 5); }
          } else if (combatFamily === 'organic') {
            var od = kill ? 15 : 10;
            ctx.fillRect(cx - od, cy - 3, od - 3, 6); ctx.fillRect(cx + 3, cy - 3, od - 3, 6);
            ctx.fillRect(cx - 3, cy - od, 6, od - 4); ctx.fillRect(cx - 3, cy + 4, 6, od - 4);
            if (kill) { ctx.fillRect(cx - 17, cy - 12, 6, 6); ctx.fillRect(cx + 11, cy - 12, 6, 6); ctx.fillRect(cx - 17, cy + 6, 6, 6); ctx.fillRect(cx + 11, cy + 6, 6, 6); }
          } else {
            var ed = kill ? 17 : 12, star = theme === 'crystal' ? 4 : 3;
            ctx.fillRect(cx - star / 2, cy - ed, star, 7); ctx.fillRect(cx - star / 2, cy + ed - 7, star, 7); ctx.fillRect(cx - ed, cy - star / 2, 7, star); ctx.fillRect(cx + ed - 7, cy - star / 2, 7, star);
            ctx.fillRect(cx - 2, cy - 2, 4, 4);
            if (kill) { ctx.fillRect(cx - 11, cy - 11, 4, 4); ctx.fillRect(cx + 7, cy - 11, 4, 4); ctx.fillRect(cx - 11, cy + 7, 4, 4); ctx.fillRect(cx + 7, cy + 7, 4, 4); }
          }
        } else if (f.kind === 'switch') {
          if (combatFamily === 'ordnance') {
            ctx.fillRect(cx - 42, wy + 8, 16, 3); ctx.fillRect(cx - 34, wy + 3, 3, 13); ctx.fillRect(cx + 26, wy + 8, 16, 3); ctx.fillRect(cx + 31, wy + 3, 3, 13);
          } else if (combatFamily === 'relic') {
            ctx.fillRect(cx - 44, wy + 5, 18, 4); ctx.fillRect(cx - 29, wy + 1, 4, 12); ctx.fillRect(cx + 26, wy + 5, 18, 4); ctx.fillRect(cx + 25, wy + 1, 4, 12); ctx.fillRect(cx - 2, wy - 3, 4, 18);
          } else if (combatFamily === 'organic') {
            ctx.fillRect(cx - 42, wy + 5, 11, 8); ctx.fillRect(cx - 27, wy + 2, 8, 14); ctx.fillRect(cx + 19, wy + 2, 8, 14); ctx.fillRect(cx + 31, wy + 5, 11, 8);
          } else {
            ctx.fillRect(cx - 45, wy + 7, 14, 3); ctx.fillRect(cx - 34, wy + 1, 3, 15); ctx.fillRect(cx + 31, wy + 7, 14, 3); ctx.fillRect(cx + 31, wy + 1, 3, 15); ctx.fillRect(cx - 4, wy + 4, 8, 8);
          }
        } else if (f.kind === 'pickup') {
          if (combatFamily === 'ordnance') {
            ctx.fillRect(cx - 16, wy - 8, 32, 3); ctx.fillRect(cx - 9, wy - 15, 4, 7); ctx.fillRect(cx + 5, wy - 15, 4, 7); ctx.fillRect(cx - 2, wy - 22, 4, 14);
          } else if (combatFamily === 'relic') {
            ctx.fillRect(cx - 20, wy - 10, 40, 4); ctx.fillRect(cx - 3, wy - 25, 6, 21); ctx.fillRect(cx - 12, wy - 19, 6, 6); ctx.fillRect(cx + 6, wy - 19, 6, 6);
          } else if (combatFamily === 'organic') {
            ctx.fillRect(cx - 17, wy - 13, 9, 9); ctx.fillRect(cx - 5, wy - 22, 10, 12); ctx.fillRect(cx + 8, wy - 13, 9, 9); ctx.fillRect(cx - 9, wy - 7, 18, 5);
          } else {
            ctx.fillRect(cx - 20, wy - 9, 40, 3); ctx.fillRect(cx - 2, wy - 26, 4, 20); ctx.fillRect(cx - 12, wy - 20, 4, 4); ctx.fillRect(cx + 8, wy - 20, 4, 4); ctx.fillRect(cx - 4, wy - 15, 8, 8);
          }
        } else if (f.kind === 'empty') {
          if (combatFamily === 'ordnance') { ctx.fillRect(cx - 9, wy - 5, 5, 5); ctx.fillRect(cx + 4, wy - 5, 5, 5); }
          else if (combatFamily === 'relic') { ctx.fillRect(cx - 10, wy - 7, 4, 9); ctx.fillRect(cx + 6, wy - 7, 4, 9); ctx.fillRect(cx - 2, wy - 3, 4, 4); }
          else if (combatFamily === 'organic') { ctx.fillRect(cx - 11, wy - 6, 7, 7); ctx.fillRect(cx + 4, wy - 6, 7, 7); ctx.fillRect(cx - 2, wy - 1, 4, 4); }
          else { ctx.fillRect(cx - 12, wy - 4, 4, 4); ctx.fillRect(cx - 2, wy - 9, 4, 4); ctx.fillRect(cx + 8, wy - 4, 4, 4); }
        }
      }
      function drawCombatView() {
        if (!g.player || g.won || g.caught) return;
        updateCombatFaceState();
        var weaponKind = g.combatWeapons.equipped, weapon = combatWeaponSlot(weaponKind), preset = WEAPON_PRESETS[weaponKind], ammo = weapon.ammo, cx = CW / 2, cy = CH / 2;
        var kick = (!reducedMotion && g.tw < g.recoilUntil) ? Math.max(0, (g.recoilUntil - g.tw) / 0.18) * preset.recoil * 4 : 0;
        if (ctx.save) ctx.save();
        if (g.damageFlashUntil > g.tw) {
          var flashLife = Math.max(0, Math.min(1, (g.damageFlashUntil - g.tw) / 0.24));
          var flashAlpha = (0.16 + flashLife * 0.34) * (g.damageFlashStrength || 0.5);
          ctx.fillStyle = 'rgba(168,24,24,' + flashAlpha.toFixed(3) + ')';
          var edge = Math.max(10, Math.round(Math.min(CW, CH) * 0.075));
          ctx.fillRect(0, 0, CW, edge); ctx.fillRect(0, CH - edge, CW, edge);
          ctx.fillRect(0, edge, edge, CH - edge * 2); ctx.fillRect(CW - edge, edge, edge, CH - edge * 2);
        }
        // 有限 face 状态:固定左下状态标；crystal 用承载星印而非 Doom 式方脸，其余主题保留像素头像。状态与 HUD 同源,不暴露公共 View 字段。
        var faceX = 10, faceY = CH - 43, faceColor = g.player.faceState === 'dead' ? 'rgb(92,28,32)' : g.player.faceState === 'hurt' ? combatCss(combatProfile.colors.hit) : g.player.faceState === 'alert' ? combatCss(combatProfile.colors.accent) : combatCss(combatProfile.colors.primary);
        ctx.fillStyle = 'rgb(24,28,32)'; ctx.fillRect(faceX - 2, faceY - 2, 38, 36);
        if (theme === 'crystal') {
          ctx.fillStyle = faceColor;
          ctx.fillRect(faceX + 3, faceY + 5, 9, 3); ctx.fillRect(faceX + 3, faceY + 5, 3, 9);
          ctx.fillRect(faceX + 22, faceY + 5, 9, 3); ctx.fillRect(faceX + 28, faceY + 5, 3, 9);
          ctx.fillRect(faceX + 3, faceY + 24, 9, 3); ctx.fillRect(faceX + 3, faceY + 18, 3, 9);
          ctx.fillRect(faceX + 22, faceY + 24, 9, 3); ctx.fillRect(faceX + 28, faceY + 18, 3, 9);
          ctx.fillStyle = combatCss(combatProfile.colors.bright); ctx.fillRect(faceX + 14, faceY + 13, 6, 6);
          if (g.player.faceState === 'alert') { ctx.fillStyle = combatCss(combatProfile.colors.secondary); ctx.fillRect(faceX + 15, faceY + 4, 4, 5); ctx.fillRect(faceX + 15, faceY + 23, 4, 5); }
          else if (g.player.faceState === 'hurt') { ctx.fillStyle = combatCss(combatProfile.colors.dark); ctx.fillRect(faceX + 15, faceY + 10, 3, 5); ctx.fillRect(faceX + 18, faceY + 17, 3, 5); }
          else if (g.player.faceState === 'dead') { ctx.fillStyle = combatCss(combatProfile.colors.dark); ctx.fillRect(faceX + 9, faceY + 15, 16, 3); }
        } else {
          ctx.fillStyle = faceColor; ctx.fillRect(faceX, faceY, 34, 32);
          ctx.fillStyle = 'rgb(24,22,22)'; ctx.fillRect(faceX + 7, faceY + 9, 5, 4); ctx.fillRect(faceX + 22, faceY + 9, 5, 4);
          if (g.player.faceState === 'dead') { ctx.fillRect(faceX + 8, faceY + 23, 18, 3); }
          else if (g.player.faceState === 'hurt') { ctx.fillRect(faceX + 12, faceY + 22, 11, 5); }
          else { ctx.fillRect(faceX + 12, faceY + 23, 11, 2); }
        }
        if (g.player.dead) {
          ctx.fillStyle = 'rgba(58,0,8,.56)'; ctx.fillRect(0, 0, CW, CH);
          ctx.font = 'bold 22px system-ui,sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = 'rgb(255,226,220)';
          ctx.fillText('阵亡', cx, cy);
          if (ctx.restore) ctx.restore();
          return;
        }
        drawCombatCrosshair(weaponKind, cx, cy);
        var wy = CH - 58 + kick;
        drawCombatWeaponBody(weaponKind, cx, wy);
        if (g.tw < g.muzzleUntil) drawCombatMuzzle(weaponKind, cx, wy);   // 因果闪光在 reduced-motion 下保留。
        drawCombatFeedback(cx, cy, wy);
        var statusLeft = combatProfile.health + ' ' + g.player.health + '/' + g.player.maxHealth + '  ' + combatFaceLabel();
        var statusRight = combatWeaponLabel(weaponKind) + ' ' + combatProfile.ammo + ' ' + ammo + '/' + weapon.maxAmmo + '  ' + combatProfile.target + ' ' + combatAliveCount();
        ctx.font = '12px system-ui,sans-serif'; ctx.textBaseline = 'bottom'; ctx.fillStyle = combatCss(combatProfile.colors.bright);
        ctx.textAlign = 'left';
        if (ctx.strokeText) { ctx.strokeStyle = 'rgba(0,0,0,.9)'; ctx.lineWidth = 3; ctx.strokeText(statusLeft, 54, CH - 9); }
        ctx.fillText(statusLeft, 54, CH - 9);
        ctx.textAlign = 'right';
        if (ctx.strokeText) ctx.strokeText(statusRight, CW - 10, CH - 9);
        ctx.fillText(statusRight, CW - 10, CH - 9);
        if (g.supplyNotice && g.tw < g.supplyNotice.until) {
          ctx.fillStyle = combatCss(combatProfile.colors.accent);
          if (ctx.strokeText) ctx.strokeText(g.supplyNotice.text, CW - 10, CH - 27);
          ctx.fillText(g.supplyNotice.text, CW - 10, CH - 27);
        }
        if (ctx.restore) ctx.restore();
      }
      ...
      function render() {
        ...
        if (!zbuf || zbuf.length !== CW) zbuf = new Array(CW);   // per-column z-buffer(持久复用;尺寸变化时才重建)
        ...
        drawCombatView();           // Phase 1a screen-space 层:世界/精灵/震屏之后,准星与武器不被墙遮挡、不随 head-bob 漂。
        var keyLockedDoor = g.atDoor && g.needKey && !g.hasKey;
        var clearLockedDoor = g.atDoor && combatExitBlocked();
        var lockedDoor = keyLockedDoor || clearLockedDoor;
        ...
        if (!g.caught && !g.won && !(g.player && g.player.dead)) {
          // ◎(U+25CE) tofu 风险(rank4) → ●(U+25CF 基本圆点,覆盖良好);先 strokeText 暗描边再 fillText 亮字,亮主题近墙不被吃掉(rank16)
          var objHint = g.player ? (!g.combatReady ? '● 待命:' + combatProfile.copy.start : clearLockedDoor ? '● 出口封锁:剩余 ' + combatTargetCountText(combatAliveCount()) : combatWindupCount() ? '● ' + combatProfile.copy.windup : combatAliveCount() ? '● ' + combatProfile.copy.mode + ':' + combatProfile.copy.fight : '● ' + combatProfile.copy.clear)
            : (g.needKey && !g.hasKey) ? '● 先找那把 ' + kE
            : (g.needKey && g.hasKey) ? '● ' + kE + ' 在手,去开门'
              : '● 寻找出口';
          var objHintY = g.player ? 50 : 9;   // combat 顶部已有 DOM HUD；下移目标锚，避免窄画布两层文字互压。
          ctx.save(); ctx.globalAlpha = 0.6; ctx.font = '12px system-ui,sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
          if (ctx.strokeText) { ctx.strokeStyle = 'rgba(0,0,0,0.85)'; ctx.lineWidth = 3; ctx.lineJoin = 'round'; ctx.strokeText(objHint, 9, objHintY); }   // 亮主题对比描边(headless stub 无 strokeText → guard)
          ctx.fillStyle = 'rgb(238,243,248)'; ctx.fillText(objHint, 9, objHintY); ctx.restore();   // headless ctx 是 stub,save/restore/fillText 退化无操作
          if (g.canMouseLook) {   // 桌面 mouselook 提示；combat 时目标锚位于其下方，非 combat 时保持既有目标锚下方提示。
        ...
        var combatHud = null;
        if (g.player && !g.won && !g.caught) {
          var currentKind = g.combatWeapons.equipped, currentSlot = combatWeaponSlot(currentKind), ca0 = currentSlot.ammo, alive0 = combatAliveCount();
          var combatResources = combatProfile.health + ' ' + g.player.health + '/' + g.player.maxHealth + ' · ' + combatWeaponLabel(currentKind) + ' ' + combatProfile.ammo + ' ' + ca0 + '/' + currentSlot.maxAmmo;
          combatHud = g.player.dead ? '阵亡 · 从下面的选项重试或返回'
            : !g.combatReady ? '待命 · ' + combatResources + ' · ' + combatTargetCountText(alive0) + ' · ' + combatProfile.copy.start
              : (keyLockedDoor && clearLockedDoor) ? '出口封锁 · 先取得 ' + kE + '，并处理剩余 ' + combatTargetCountText(alive0)
              : clearLockedDoor ? '出口封锁 · 仍有 ' + combatTargetCountText(alive0)
                : keyLockedDoor ? '🔒 门锁着 · 先找到那把 ' + kE
                  : combatProfile.copy.mode + ' · ' + combatResources + ' · ' + combatFaceLabel() + ' · ' + combatTargetCountText(alive0);
          if (!g.player.dead && g.supplyNotice && g.tw < g.supplyNotice.until) combatHud += ' · ' + g.supplyNotice.text;
          if (!g.player.dead && g.combatNotice && g.tw < g.combatNotice.until) combatHud += ' · ' + g.combatNotice.text;
          if (!g.player.dead && combatWindupCount()) combatHud += ' · ' + combatProfile.copy.windup;
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'empty') combatHud += ' · ' + combatProfile.copy.empty;
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'miss') combatHud += ' · ' + combatProfile.copy.miss;
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'hit') combatHud += ' · ' + combatProfile.copy.hit + ' (' + g.lastShot.hp + '/' + g.lastShot.maxHp + ')';
          else if (!g.player.dead && g.lastShot && g.lastShot.kind === 'kill') combatHud += ' · ' + combatProfile.copy.kill;
        }
        var hud = combatHud || (g.caught ? '它抓住了你 —— 从下面的选项离开'
        ...
        if (g.eventHint && !g.caught && !g.won && !(g.player && g.player.dead) && !lockedDoor) hud = g.eventHint;   // 坐标事件 hint / R1-b1 检视线索:覆盖常规漫游提示;被抓/通关/死亡/锁门这些关键态优先级更高,故不被盖。
      ...
      function combatGuardBlocks(x, y) {
        var ms = g.combatMonsters || [], minDistance2 = COMBAT_GUARD_DISTANCE * COMBAT_GUARD_DISTANCE;
        for (var i = 0; i < ms.length; i++) {
          var m = ms[i];
          if (!(m && m.active && !m.dead)) continue;
          var dx = x - m.sx, dy = y - m.sy;
          if (dx * dx + dy * dy < minDistance2) return true;
        }
        return false;
      }
      function tryMove(nx, ny) {
        if (!isWall(grid, Math.floor(nx + (nx > g.px ? RAD : -RAD)), Math.floor(g.py)) && !combatGuardBlocks(nx, g.py)) g.px = nx;
        if (!isWall(grid, Math.floor(g.px), Math.floor(ny + (ny > g.py ? RAD : -RAD))) && !combatGuardBlocks(g.px, ny)) g.py = ny;
      }

      // ── 经典 A* 网格寻路(Manhattan 启发):怪物绕墙找向玩家的最短路,返回下一步格 [x,y](无路/同格→null)──
      //   等权网格上 A* 与 BFS 给同一最短路;A* 用启发少展开节点(大迷宫更快)。怪物/玩家整数格不变时复用同一步。
      ...
      if (g.player && g.player.health === 0) playerDeathOnce();   // 合法的 0 初始 HP 也必须立即形成死亡事实,不能先走一帧。
      running = true;
      ...
      function loop(ts) {
        ...
        if (!STATIC_PREVIEW && !g.won && !g.caught && !g.puzzleOpen && !(g.player && g.player.dead)) {
          ...
          if (g.fireQueued) fireCombatWeapon();     // Phase 1a:输入事件只排队；唯一 rAF 在应用本帧转向后原子提交一次开火。
          ...
          resolveCombatPickupContacts();  // 移动/K 接触后、坐标事件与敌人 attack commit 前结算 session-local pickup。
          ...
          if (g.puzzleOpen) { g.walkBob = 0; clearMoveIntent(); }        // 面板打开后当帧立刻停住,不再继续推门/追逐/抓取。
            ...
            if (g.player && g.combatReady && !g.won) stepCombatMonsters(dt);   // ready 前只渲染首帧；首次真实玩法输入后才推进 session-local AI/攻击计时。
            ...
            for (var mi = 0; mi < g.monsters.length && !g.won && !g.caught; mi++) {
              ...
              if (m.chase) {
                ...
                if (mcx === pcx && mcy === pcy) { tx = g.px; ty = g.py; }                       // 同格 → 直接逼近玩家精确位置
                  ...
                  var pathCache = m._pathCache, nc;
                  if (pathCache && pathCache.lastMonCX === mcx && pathCache.lastMonCY === mcy && pathCache.lastPlayerCX === pcx && pathCache.lastPlayerCY === pcy) nc = pathCache.cachedNext;
                  else { nc = astarNext(mcx, mcy, pcx, pcy); m._pathCache = { lastMonCX: mcx, lastMonCY: mcy, lastPlayerCX: pcx, lastPlayerCY: pcy, cachedNext: nc }; }
                  if (nc) { tx = nc[0] + 0.5; ty = nc[1] + 0.5; }
                }   // 否则朝 A* 下一步格中心走;同一双方格内仍每帧继续移动,只省搜索
        ...
        if (g.player && g.player.health === 0) playerDeathOnce();   // 每帧重过死亡闸；deathKeyWritten 保证 api.apply 仅一次。
      ...
      render();
      if (ENABLE_CONTROLS && !STATIC_PREVIEW) {
