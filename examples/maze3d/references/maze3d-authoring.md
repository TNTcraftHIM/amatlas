...
var FPS_RECIPE_NODES = {
  fps_range: {
    ...
    winKey: 'rangeEscaped',
    look: '初始携带精确手枪。先取得近程霰弹枪并近距散射，再按 1 切回精确手枪收尾，最后走到 D 出口。',
    maze: {
      grid: [
        '#############',
        '#...........#',
...
这个三节点骨架的因果才是闭合的：初始 `precision` 2/6 → 获得并自动装备 1/4 的 `scatter` → 一次近距五射线最多聚合 40 伤害 → 按 1 切回 precision 等待全局回稳后以 20 伤害收尾 → `exitRequires:'clear'` 让同一个 `D` 只在 guard 倒下后开放。阵亡后当前 `fps_range` 直接显示“一键重试”，先清 durable 胜负键再原节点重建 session，`range_death` 只保留为可选结果页。只抄 `fps_range` 会留下死链和无法清除的 durable deathKey。
...
这三闸能证明世界图、装配与构建链；graph-audit 不复制完整 `maze.combat` validator，jsdom 也不会真实驱动 canvas。**所以三闸绿不等于 combat 字段正确。**改了 FPS 数据后，必须再用真浏览器打开构建后的 `fps.html`：先等待确认未 ready 时 AI/攻击计时静止，再走“取得 scatter→近距聚合命中→切回 precision 时全局冷却不被绕过→precision 收尾→D”，并走“阵亡→当前节点一键重试→60 HP / 初始 precision 2/6 / scatter 未拥有 / 3 pickups / 1 guard 全新”。同时保留统一 `index.html` 的 graph/assembly/build，确认旧四 recipe 未被 combat 行为污染。
