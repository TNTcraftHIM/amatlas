...
(function () {   // S1 Gallery 单向索引 15 个目标，launcher 不是硬跳转 anchor。
  var expected = [
    ...
    '../text-adventure-demo/index.html', '../tabletop-demo/index.html', '../cutscene-demo/index.html', '../maze3d/fps.html',
...
(function () {   // S1c sidescroller 必须独立标为实验性切片，不混入 7 个正式 reference。
  ...
  ok(experimental.indexOf('id="experimental-slices"') >= 0 &&
    ...
    experimental.indexOf('不是正式 reference 或公共 sidescroller 模块') >= 0 && referenceLaunchers === 7 &&
    references.indexOf('sidescroller') < 0 && indexHtml.indexOf('href="#experimental-slices"') >= 0,
    'S1c 两个 sidescroller 客户必须同处实验切片分区且 7 个正式 reference 计数不变 referenceLaunchers=' + referenceLaunchers);
})();
