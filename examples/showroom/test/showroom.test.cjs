...
(function () {   // S1 Gallery 单向索引 15 个目标，launcher 不是硬跳转 anchor。
  var expected = [
    'ui-skins-gallery.html', 'audio-preview.html', '../maze3d/gallery.html', '../maze3d/audio-gallery.html',
    '../rig-showcase/index.html', '../sidescroller/index.html', '../sidescroller-frostline/index.html',
  ...
  ok(same && unique.length === expected.length && launchers === expected.length && /launchers\.length !== 15/.test(indexHtml) && /15 个入口不完整/.test(indexHtml) && !hard.length && /class="demoport"/.test(indexHtml) && /class="demoport-frame"/.test(indexHtml),
    'S1 embedded dev Gallery 必须精确单向索引 15 个目标并以 button 打开单例 overlay targets=' + JSON.stringify(targets) + ' hard=' + hard.join(',') + ' launchers=' + launchers);
})();

(function () {   // S1a Maze Visual Gallery 卡只更新选材说明，不增加第三helper或launcher。
  var maze = (indexHtml.match(/<section class="catalog-section maze"[\s\S]*?<\/section>/) || [''])[0];
  var mazeLaunchers = (maze.match(/<button\b[^>]*\bdata-src=/g) || []).length;
  ok(mazeLaunchers === 2 && maze.indexOf('六个协调 recipe') >= 0 && maze.indexOf('五张 FPS 主题卡') >= 0 && maze.indexOf('四个 family + crystal 特化') >= 0 && maze.indexOf('../maze3d/gallery.html') >= 0 && maze.indexOf('../maze3d/audio-gallery.html') >= 0,
    'S1a Maze选材仍只有visual/audio两个helper，Visual卡路由协调recipe与四family+crystal特化 mazeLaunchers=' + mazeLaunchers);
})();

(function () {   // S1b rig-showcase 独立归为表现力教学，不混入 7 个玩法 reference。
  var showcase = (indexHtml.match(/<section class="catalog-section showcase"[\s\S]*?<\/section>/) || [''])[0];
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var referenceLaunchers = (references.match(/<button\b[^>]*\bdata-src=/g) || []).length;
  ok(showcase.indexOf('id="expressiveness-showcase"') >= 0 &&
    showcase.indexOf('data-src="../rig-showcase/index.html"') >= 0 &&
    showcase.indexOf('表现力展示') >= 0 && showcase.indexOf('教学材料') >= 0 &&
    showcase.indexOf('不是第八个玩法模块') >= 0 && referenceLaunchers === 7 &&
    references.indexOf('rig-showcase') < 0 && indexHtml.indexOf('href="#expressiveness-showcase"') >= 0,
    'S1b rig-showcase 必须位于独立表现力展示分区且 7 个正式 reference 计数不变 referenceLaunchers=' + referenceLaunchers);
})();

(function () {   // S1b2 R·06 仍是同一正式 reference，文案必须公开真实五果/三败/abort与私有runtime边界。
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var arcade = (references.match(/<button\b[^>]*data-src="\.\.\/arcade-demo\/index\.html"[\s\S]*?<\/button>/) || [''])[0];
  ok(arcade.indexOf('R·06') >= 0 && arcade.indexOf('贪吃蛇门禁范本') >= 0 && arcade.indexOf('5 果') >= 0 && arcade.indexOf('三次失败') >= 0 && arcade.indexOf('任意时刻都能放弃') >= 0 && arcade.indexOf('example 私有模块') >= 0,
    'S1b2 R·06 只校准同一Arcade消费者的五果/三败/abort/private-runtime身份，不新增launcher');
})();

(function () {   // S1b3 R·02 同一正式 Tabletop reference，公开真实资源选择、优势、partial 与双路通关。
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var tabletop = (references.match(/<button\b[^>]*data-src="\.\.\/tabletop-demo\/index\.html"[\s\S]*?<\/button>/) || [''])[0];
  ok(tabletop.indexOf('R·02') >= 0 && tabletop.indexOf('跑团检定范本') >= 0 && tabletop.indexOf('1 状态') >= 0 && tabletop.indexOf('优势') >= 0 && tabletop.indexOf('部分成功') >= 0 && tabletop.indexOf('两路都能通关') >= 0,
    'S1b3 R·02 只校准同一Tabletop消费者的resource→advantage/partial双路，不新增launcher');
})();

(function () {   // S1b4 R·03 同一正式 Cutscene reference，公开高层编译、末拍出口与普通 scene 回接。
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var cutscene = (references.match(/<button\b[^>]*data-src="\.\.\/cutscene-demo\/index\.html"[\s\S]*?<\/button>/) || [''])[0];
  ok(cutscene.indexOf('R·03') >= 0 && cutscene.indexOf('actor / anchor / shot') >= 0 && cutscene.indexOf('确定性演出') >= 0 && cutscene.indexOf('末拍') >= 0 && cutscene.indexOf('普通 scene') >= 0,
    'S1b4 R·03 只校准同一Cutscene消费者的高层compiler→末拍出口→普通scene回接，不新增launcher');
})();

(function () {   // S1b5 R·05 锁custom manifest→十次达标→core exit→普通scene闭环。
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var minimal = (references.match(/<button\b[^>]*data-src="\.\.\/minimal-demo\/index\.html"[\s\S]*?<\/button>/) || [''])[0];
  ok(minimal.indexOf('R·05') >= 0 && minimal.indexOf('manifest.modules') >= 0 && minimal.indexOf('十次达标') >= 0 && minimal.indexOf('核心出口') >= 0 && minimal.indexOf('普通 scene') >= 0,
    'S1b5 R·05 对账custom module正式玩家闭环，不新增launcher');
})();

(function () {   // S1c sidescroller 必须独立标为实验性切片，不混入 7 个正式 reference。
  var experimental = (indexHtml.match(/<section class="catalog-section experimental"[\s\S]*?<\/section>/) || [''])[0];
  var references = (indexHtml.match(/<section class="catalog-section refs"[\s\S]*?<\/section>/) || [''])[0];
  var referenceLaunchers = (references.match(/<button\b[^>]*\bdata-src=/g) || []).length;
  ok(experimental.indexOf('id="experimental-slices"') >= 0 &&
    experimental.indexOf('data-src="../sidescroller/index.html"') >= 0 &&
    experimental.indexOf('data-src="../sidescroller-frostline/index.html"') >= 0 &&
    experimental.indexOf('02 slices') >= 0 && experimental.indexOf('两份作品复用同一') >= 0 &&
    experimental.indexOf('不是正式 reference 或公共 sidescroller 模块') >= 0 &&
    experimental.indexOf('<code>coast</code>') >= 0 && experimental.indexOf('<code>frost</code>') >= 0 &&
    experimental.indexOf('海天背景') >= 0 && experimental.indexOf('极夜山影') >= 0 && referenceLaunchers === 7 &&
    references.indexOf('sidescroller') < 0 && indexHtml.indexOf('href="#experimental-slices"') >= 0,
    'S1c 两个 sidescroller profile 客户必须同处实验切片分区并形成可选材对照，且 7 个正式 reference 计数不变 referenceLaunchers=' + referenceLaunchers);
})();
...
(function () {   // S3 相对资源与 14 个 iframe target 均能在 file:// 源码树解析。
...
(function () {   // S16a present-dom 换节点把焦点移到标题；共享 CSS 留顶部呼吸且保键盘可见焦点，不能全局 outline:none。
  var placeBlock = (uiCss.match(/#place\s*\{[^}]*\}/) || [''])[0];
  var nonKeyboard = (uiCss.match(/#place:focus:not\(:focus-visible\)\s*\{[^}]*\}/) || [''])[0];
  var keyboard = (uiCss.match(/#place:focus-visible\s*\{[^}]*\}/) || [''])[0];
  ok(/scroll-margin-top:\s*24px/.test(placeBlock) && /outline:\s*none/.test(nonKeyboard) && /outline:\s*3px solid/.test(keyboard) && /outline-offset:\s*3px/.test(keyboard) && !/(^|[^#\w-])\*:focus\s*\{[^}]*outline\s*:\s*none/.test(uiCss),
    'S16a #place scrollIntoView 保24px留白，非键盘程序焦点去UA细线，键盘focus-visible保3px焦点环');
})();
