...
(function () {   // S1 Gallery 单向索引 14 个目标，launcher 不是硬跳转 anchor。
  var expected = [
    'ui-skins-gallery.html', 'audio-preview.html', '../maze3d/gallery.html', '../maze3d/audio-gallery.html',
    '../rig-showcase/index.html', '../sidescroller/index.html',
  ...
  ok(same && unique.length === expected.length && launchers === expected.length && /launchers\.length !== 14/.test(indexHtml) && /14 个入口不完整/.test(indexHtml) && !hard.length && /class="demoport"/.test(indexHtml) && /class="demoport-frame"/.test(indexHtml),
    'S1 embedded dev Gallery 必须精确单向索引 14 个目标并以 button 打开单例 overlay targets=' + JSON.stringify(targets) + ' hard=' + hard.join(',') + ' launchers=' + launchers);
})();
...
(function () {   // S1c sidescroller 必须独立标为实验性切片，不混入 7 个正式 reference。
  ...
  ok(experimental.indexOf('id="experimental-slices"') >= 0 &&
    ...
    experimental.indexOf('实验性可玩切片') >= 0 && experimental.indexOf('不是第八个正式 reference') >= 0 &&
    experimental.indexOf('不是公共 sidescroller 模块') >= 0 && referenceLaunchers === 7 &&
    references.indexOf('sidescroller') < 0 && indexHtml.indexOf('href="#experimental-slices"') >= 0,
    'S1c sidescroller 必须位于独立实验切片分区且 7 个正式 reference 计数不变 referenceLaunchers=' + referenceLaunchers);
})();
