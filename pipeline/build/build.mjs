...
/* ════════════════════════════════════════════════════════════════════════
   ...
   用法:  node build.mjs <game/index.html> [out.html] [--smoke] [--version <semver>]
...
export function inlineHtml(indexPath, options = {}) {
  ...
  if (requestedVersion) {
    const declarations = [...html.matchAll(/\bvar\s+AMATLAS_VERSION\s*=\s*'([^']+)'/g)];
    if (declarations.length !== 1) {
      scriptErrors.push('显式版本注入要求恰有一个 AMATLAS_VERSION 声明（实际 ' + declarations.length + ' 个）。');
    } else {
      const declaration = declarations[0];
      const currentVersion = declaration[1];
      if (currentVersion !== '__AMATLAS_VERSION__' && currentVersion !== requestedVersion) {
        scriptErrors.push('AMATLAS_VERSION 已是 ' + currentVersion + '，与请求注入的 ' + requestedVersion + ' 冲突。');
      } else if (currentVersion === '__AMATLAS_VERSION__') {
        const injected = declaration[0].replace("'__AMATLAS_VERSION__'", "'" + requestedVersion + "'");
        html = html.slice(0, declaration.index) + injected + html.slice(declaration.index + declaration[0].length);
      }
    }
  }
...
export function buildToFile(indexPath, outPath, options = {}) {
  const previousOutputPreserved = (() => {
    try { return fs.statSync(outPath).isFile(); } catch (e) { return false; }
  })();
  const inlined = inlineHtml(indexPath, options);
  ...
  //   <img src> / <audio/video/source src>:P1 warn(不阻断构建,作者有意联网情形仍可接受)。
  //   与 script/link 共用 attrsOf，HTML 合法的双引号、单引号、裸值不能再各维护一条会漂移的 regex。
  inlined.html.replace(/<(img|audio|video|source)\b([^>]*?)>/gi, function (_tag, tagName, raw) {
    const target = attrsOf(raw).src;
    if (!target || /^(?:data:|#)/i.test(target)) return '';
    const label = String(tagName).toLowerCase() === 'img' ? '<img src>' : '<audio/video src>';
    result.warns.push(`[P1] 非自包含资产:${label} → ${String(target).slice(0, 80)}(build 只内联 script + 本地 stylesheet;此引用会留在成品里,离线/换机/挪 dist 即失效。改内联 data: URI / 避免二级 CSS 资产,或确认有意联网)。`);
    return '';
  });
...
async function main() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === '--smoke') smoke = true;
      ...
      if (i + 1 >= argv.length) {
        console.error('--version 需要一个 SemVer 参数');
        process.exit(2);
      }
      version = argv[++i];
  ...
  if (!indexArg || positionals.length > 2) {
    console.error('用法: node build.mjs <game/index.html> [out.html] [--smoke] [--version <semver>]');
    process.exit(2);
  }
  if (version && !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version)) {
    console.error('--version 必须是 SemVer（如 0.1.0）');
    process.exit(2);
  }
  ...
  const r = buildToFile(indexPath, candidatePath, { version: version });
