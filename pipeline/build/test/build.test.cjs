...
var CUTSCENE_INDEX = path.join(__dirname, '..', '..', '..', 'examples', 'cutscene-demo', 'index.html');
...
// A7-A10. Phase 3 cutscene authoring closure：正式 demo 必须把 compiler 按 timeline → compiler → world
// 内联到单 HTML；这里锁构建图，不用 source grep 冒充 compiler 运行语义（运行语义由 assembly-probe 覆盖）。
var cutsceneOut = tmp('amatlas-build-cutscene.' + process.pid + '.html');
try { fs.unlinkSync(cutsceneOut); } catch (e) {}
var cutsceneBuild = run([CUTSCENE_INDEX, cutsceneOut]);
var cutsceneBuilt = fs.existsSync(cutsceneOut) ? fs.readFileSync(cutsceneOut, 'utf8') : '';
var cutsceneSource = fs.readFileSync(CUTSCENE_INDEX, 'utf8');
var timelineSrcAt = cutsceneSource.indexOf('presenters/present-timeline.js');
var compilerSrcAt = cutsceneSource.indexOf('modules/cutscene/authoring/compile-performance.js');
var worldSrcAt = cutsceneSource.indexOf('src="world.js"');
var timelineBuiltAt = cutsceneBuilt.indexOf('Atlas presenter timeline: deterministic');
var compilerBuiltAt = cutsceneBuilt.indexOf('Amatlas cutscene performance authoring compiler');
var worldBuiltAt = cutsceneBuilt.indexOf('CUTSCENE_DEMO_WORLD');
ok('A7 cutscene-demo 正式构建退出 0 且 compiler 已内联', cutsceneBuild.status === 0 && compilerBuiltAt >= 0, 'status=' + cutsceneBuild.status + ' | ' + cutsceneBuild.out.slice(0, 500));
ok('A8 cutscene source 顺序闭合为 timeline → compiler → world', timelineSrcAt >= 0 && timelineSrcAt < compilerSrcAt && compilerSrcAt < worldSrcAt, 'timeline=' + timelineSrcAt + ' compiler=' + compilerSrcAt + ' world=' + worldSrcAt);
ok('A9 built 保留 timeline → compiler → world 求值顺序', timelineBuiltAt >= 0 && timelineBuiltAt < compilerBuiltAt && compilerBuiltAt < worldBuiltAt, 'timeline=' + timelineBuiltAt + ' compiler=' + compilerBuiltAt + ' world=' + worldBuiltAt);
ok('A10 cutscene built 无任何外部 script', cutsceneBuilt.length > 0 && !/<script\b[^>]*\bsrc\s*=/i.test(cutsceneBuilt), 'len=' + cutsceneBuilt.length);
...
// D4-D6 媒体 src 与 HTML 属性解析同源：双引号/单引号/裸值都必须报 P1；data URI 不误报。
(function () {
  function worldJs() {
    return "(function(g,f){if(typeof module!=='undefined'&&module.exports)module.exports=f();else g.W=f();})(typeof globalThis!=='undefined'?globalThis:this,function(){return {id:'41414141-4141-4141-8141-414141414141',start:{map:'m',node:'a'},maps:{m:{nodes:{a:{kind:'custom',links:[{label:'回',to:'a'}]}}}}};});\n";
  }
  var dir = fs.mkdtempSync(path.join(os.tmpdir(), 'amatlas-media-src-'));
  fs.writeFileSync(path.join(dir, 'world.js'), worldJs());
  fs.writeFileSync(path.join(dir, 'index.html'), "<!doctype html><body>" +
    "<img src='single.png'><img src=bare.png>" +
    "<audio src='single.ogg'></audio><audio src=bare.ogg></audio>" +
    "<video src='single.mp4'></video><video src=bare.mp4></video>" +
    "<source src='single.webm'><source src=bare.webm>" +
    "<img src='data:image/gif;base64,AAAA'>" +
    "<script src=\"world.js\"></script></body>\n");
  var out = path.join(dir, 'dist', 'index.html');
  var r = run([path.join(dir, 'index.html'), out]);
  var warnings = (r.out.match(/\[P1\] 非自包含资产:<(?:img|audio\/video) src>/g) || []).length;
  ok('D4 单引号/裸值的 img/audio/video/source 共 8 个 src 全报非自包含 P1', r.status === 0 && warnings === 8 && /single\.png/.test(r.out) && /bare\.webm/.test(r.out), 'status=' + r.status + ' warnings=' + warnings + ' ' + r.out.slice(0, 900));
  ok('D5 媒体资产 P1 不阻断构建，产物仍保留作者引用', fs.existsSync(out), r.out.slice(0, 300));
  ok('D6 data:image URI 不误报', r.out.indexOf('data:image/gif') === -1, r.out.slice(0, 500));
  try { fs.rmSync(dir, { recursive: true, force: true }); } catch (e) {}
})();
