...
| **音乐/声景**(气质不对/太单一) | 节点 `audio:{music,ambient}` 换预设 / `{preset,…微调}` / timbre 板 / `{midi}` 嵌现成曲 | `references/audio-system.md` 选乐速查表与现役清单 |
| **成就**(没描述/太少/想加隐藏) | `game.js` manifest 的 `achievements[]`(description 务必写;hidden 防剧透;跨 reset 持久) | `examples/text-adventure-demo/game.js` |
