...
3. 路由:
   ...
   - **过场演出(intro / 结局 / 章节过渡的时间轴演出)** → 用 `modules/cutscene`，常规语义入口是 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`；actor / anchor / shot / action 写法先读 `modules/cutscene/references/cutscene-authoring.md`，正式范例照抄 `examples/cutscene-demo`。低层 `beat.motion` / `beat.cast` / `beat.speaker` 只是专家 escape hatch，只在 beat 边界混排。首拍/中间拍由正文面板逐拍即时快进，末拍才显示 `links` 出口；每次快进仍按序执行目标拍 `run`。对象形 text 需要 timeline + DOM，motion/cast/speaker 还必须加载 SVG、保留 `#scene` 且不能写 `present.svg:false`。正式顺序：core runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game。
...
<script src="../modules/cutscene/authoring/compile-performance.js"></script> <!-- 高层过场：必须在 timeline 后、world 前 -->
