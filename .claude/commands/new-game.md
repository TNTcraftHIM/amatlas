...
3. 路由:
   ...
   - **过场演出(intro / 结局 / 章节过渡的时间轴演出)** → 用 `modules/cutscene`:节点 `kind:'cutscene'` + `beats` 节拍数组(每拍 `{dur秒 | hold:true, text?, motion?, cast?, speaker?, scene?, audio?, run?}`),按时间轴自动推进。`cast` 只允许有限 `dur` 拍，rig 闭合 schema 与预算必须先读 `modules/cutscene/references/cutscene-authoring.md`；`speaker` 只指向本拍 cast 角色并配对象形 typewriter。首拍/中间拍只有「▸」逐拍即时快进，**末拍才显示 `links` 出口**；每次快进仍按序执行目标拍 `run`。正式范例照抄 `examples/cutscene-demo`；`examples/rig-showcase` 仅是同 runtime 的表现力教学材料，不是第八个玩法模块/正式可复制模块。**出口用 `links`(写 exits 会 fail-loud 拦)**。脚本闭包按作者语义配套：对象形 `text` 需要 `present-timeline.js + present-dom.js`；写了 `motion` 或 `cast`（含 `speaker`）则还必须加载 `present-svg.js`、保留 `#scene` 且不能写 `present.svg:false`。加载顺序照正式范例：cutscene runtime → DOM → timeline → SVG → world → boot → game。
