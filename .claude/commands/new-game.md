...
3. 路由:
   ...
   - **第一人称迷宫 / 怪物追逐 / 第一人称 FPS** → 用 `examples/maze3d/` 的同一个自定义 `maze3d` runtime：basic/horror/puzzle/layers/fps-range 是五个 recipes，不是五个模块。先读 `examples/maze3d/references/maze3d-authoring.md`；FPS 照 Recipe 5 的闭合 `maze.combat` 骨架，只写 `player/pistol/guard/supplies` 定稿形状。maze3d **不是 boot 内置 kind**：把 `raycast-maze.js` 复制到 `src/`、index.html 引脚本并保留 `#maze3d-stage`，game.js 通过 `manifest.modules:[A.Maze3d.createMaze3dModule({stageId:'maze3d-stage'})]` 显式装配；漏任一项都不能工作。
