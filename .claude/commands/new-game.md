...
3. 路由:
   ...
   - **过场演出(intro / 结局 / 章节过渡的时间轴演出)** → 用 `modules/cutscene`:节点 `kind:'cutscene'` + `beats` 节拍数组(每拍 `{dur秒 | hold:true, text?, motion?, cast?, speaker?, scene?, audio?, run?}`),按时间轴自动推进。`cast` 只允许有限 `dur` 拍，rig 闭合 schema 与预算必须先读 `modules/cutscene/references/cutscene-authoring.md`；`speaker` 只指向本拍 cast 角色并配对象形 typewriter。首拍/中间拍由正文面板承担逐拍即时快进（鼠标/触屏点击，或聚焦后按 Enter/Space；DOM 不显示独立 `▸`），**末拍才显示 `links` 出口**；每次快进仍按序执行目标拍 `run`。正式范例照抄 `examples/cutscene-demo`；`examples/rig-showcase` 仅是同 runtime 的表现力教学材料，不是第八个玩法模块/正式可复制模块。**出口用 `links`(写 exits 会 fail-loud 拦)**。脚本闭包按作者语义配套：对象形 `text` 需要 `present-timeline.js + present-dom.js`；写了 `motion` 或 `cast`（含 `speaker`）则还必须加载 `present-svg.js`、保留 `#scene` 且不能写 `present.svg:false`。加载顺序照正式范例：cutscene runtime → DOM → timeline → SVG → world → boot → game。
   - **第一人称迷宫 / 怪物追逐 / 第一人称 FPS** → 用 `examples/maze3d/` 的同一个自定义 `maze3d` runtime：basic/horror/puzzle/layers/fps-range 是五个 recipes，不是五个模块。先读 `examples/maze3d/references/maze3d-authoring.md`；FPS 照 Recipe 5 的 v3 闭合 `maze.combat` 骨架，只写 `player/loadout/equipped/guard/pickups`，武器 kind 只用固定 `precision` / `scatter`，完整表现由 `maze.theme` 自动驱动，不写 skin。maze3d **不是 boot 内置 kind**：把 `raycast-maze.js` 复制到 `src/`、index.html 引脚本并保留 `#maze3d-stage`，game.js 通过 `manifest.modules:[A.Maze3d.createMaze3dModule({stageId:'maze3d-stage'})]` 显式装配；漏任一项都不能工作。
   - **引擎里没有的玩法类型**(横版射击 / 模拟经营 / 卡牌 / 打地鼠……但不包括上行已支持的 maze3d / FPS)→ 先读 text-adventure-game skill 的 `references/plugin-development.md` 创建新模块(Level 1 复制 `modules/minimal` 起步),再做游戏。
...
<script src="../presenters/present-timeline.js"></script> <!-- 仅普通 string 文本可省;对象形 text/motion/cast/speaker 必需 -->
<script src="../presenters/present-svg.js"></script>     <!-- 普通纯文字可选;scene/motion/cast/speaker 用到视觉意图时必需，且保留 #scene、勿设 present.svg:false -->
