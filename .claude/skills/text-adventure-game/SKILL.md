...
- **过场演出(intro / outro / 结局 / 章节过渡)** → 用 `modules/cutscene`:节点 `kind:'cutscene'` + `beats:[{dur秒 | hold:true, text?, motion?, cast?, speaker?, scene?, audio?, run?}]` 按时间轴自动推进(**dur 单位是秒**,别写毫秒);`cast` 只允许有限 dur 拍，rig 的 `parts/drawOrder/tracks/variants/secondary` 闭合 schema 与硬预算先读 `modules/cutscene/references/cutscene-authoring.md`，`speaker` 只指向本拍 cast 并配对象形 typewriter。每拍复用既有 scene/audio 词汇——画幅黑边=`elements:[{kind:'letterbox'}]`、音乐走 v15 缺键继承(第 0 拍起乐、后面的拍不写 audio 就一直播)、拍不写 scene=继承上一拍(画面连续不闪,写新 scene=重建出场)。首拍/中间拍由正文面板承担逐拍即时快进（鼠标/触屏点击，或聚焦后按 Enter/Space；DOM 不显示独立 `▸`），**末拍才显示 `links` 出口**；连续激活正文面板仍会按序经过每拍并执行对应 `run`，不能绕过中间状态。**出口用 `links`、别写 exits(fail-loud 拦)**;「看完过场」成就用 `on:'action'` 查末拍 run 置的 flag(演出期间不再发 enter)。正式范例照抄 `examples/cutscene-demo`(intro→正文→结局三段全示范)；`examples/rig-showcase` 只是同 runtime 的表现力教学材料，不是第八个玩法模块/正式可复制模块。脚本闭包按语义配套：对象形 text 需要 timeline + DOM；motion/cast/speaker 还必须加载 SVG、保留 `#scene` 且不能设 `present.svg:false`；顺序照正式范例的 cutscene runtime → DOM → timeline → SVG。
