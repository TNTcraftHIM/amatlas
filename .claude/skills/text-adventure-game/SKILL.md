...
本 skill 整合了一款约 10 万字中文互动小说（《制图师的挽歌》）10 轮迭代的完整经验。
...
- **跑团 / 检定 / 资源管理 / 角色卡** → 用 `modules/tabletop` 模块:先读 `modules/tabletop/references/tabletop-design.md`(**怎么设计**:何时掷骰 / 角色卡 / 资源时钟平衡 / 定 DC),再照 `modules/tabletop/references/few-shot.md`(填好的 schema 例子)动手；正式 resource→advantage/partial 双路玩家旅程见 `examples/tabletop-demo` R·02。**骰子外观/换皮**(骰形随面数、暴击/大失败特效、材质/配色/真 3D)见 `modules/tabletop/references/dice-styles.md`。
...
- **过场演出(intro / outro / 结局 / 章节过渡)** → 用 `modules/cutscene`。细节演出的常规语义入口是 `Amatlas.CutsceneAuthoring.compilePerformance(spec, options?)`：写 actor / anchor / shot 与八种闭合 action，让 compiler 生成现役 beats；先读 `modules/cutscene/references/cutscene-authoring.md`，完整可运行 Rig 与高低层对照看 `examples/cutscene-demo`。低层 `beat.motion` / `beat.cast` / `beat.speaker` 是专家 escape hatch，只在 beat 边界与高层结果混排，单拍内不维护两套作者真相。节点仍是 `kind:'cutscene'` + `beats`，首拍/中间拍由正文面板承担逐拍即时快进（鼠标/触屏点击，或聚焦后按 Enter/Space；DOM 不显示独立 `▸`），末拍才显示 `links` 出口；连续快进仍按序执行 `run`。对象形 text 需要 timeline + DOM，motion/cast/speaker 还必须加载 SVG、保留 `#scene` 且不能设 `present.svg:false`；混合 `scene + cutscene` 依赖主干是 core runtime → text-adventure runtime → cutscene runtime → DOM → timeline → compile-performance → SVG → world → boot → game，现役 R·03 的 audio/plugins 完整顺序见该 reference 与正式 `index.html`。
- **恐怖游戏** → 恐怖是**基调(tone)横切所有玩法,不是单一玩法类型**——别一听"恐怖"就上迷宫。先分流:**① 纯叙事/心理恐怖**(阅读/悬疑慢热/克苏鲁/邮件体/visual novel 风/不可名状)= **走上面文字冒险主线**,恐怖靠表现层(present-svg `eyes`/`dread`/`letterbox`/`transition` 见 `references/visual-system.md` + `ambient-unease` 见 `references/audio-system.md` + mood `dread`/`eerie`/`horror-climax`),**不用 maze3d**。**② 第一人称迷宫探险/怪物实时追逐/贴脸 jump-scare** = 读 `references/horror-game-design.md` 用 `maze3d`(先看 `examples/maze3d/index.html`:一个统一入口内含 basic / horror / puzzle / layers / fps-range recipes;取钥匙后苏醒/分阶段追逐优先照抄正式 Recipe 2 的 quiet route → K + wake → familiar return cookbook,入场即追只作为 always-on 变体;视觉先在 `examples/maze3d/gallery.html` 选协调 recipe 再查原子,声音核听看 `examples/maze3d/audio-gallery.html`;这些 recipes 都是同一 `raycast-maze.js` runtime,不是不同模块;主题/怪物/proxFx/突脸/deathFx 全数据驱动;Wolf3D 级 fillRect lo-fi、非手绘贴图但声画演出到位)。**③ 恐怖检定**(理智/逃跑 DC)= 叠 tabletop `encounter`。**多数完整恐怖短篇 = 混合**:大量 scene 铺心理叙事 + 1-3 个 maze3d 节点做追逐高潮 + 可选检定;maze3d 是**可选高潮武器**非默认形态(纯堆迷宫=游戏感盖过恐怖叙事)。题材→theme/face 速配表见 `horror-game-design.md` §0/§8。
- **第一人称 FPS / 枪战迷宫 / 生存射击** → 仍用上行同一个 `maze3d` 私有 runtime 的 `fps-range` Recipe 5，**不是新模块**、也不是 boot 自动内置 kind。动手前先在 `examples/maze3d/gallery.html` 的五张主题卡（四个 family + crystal 特化）选择 `maze.theme`，再读 `examples/maze3d/references/maze3d-authoring.md` §7，照抄 v3 闭合 `maze.combat` 骨架与 `raycast-maze.js + #maze3d-stage + manifest.modules` 装配；AI 只写 `player/loadout/equipped/guard/pickups` 与可选 clear 数据，武器 kind 只用固定 `precision` / `scatter`，枪体/准星/HUD/SFX统一由 `maze.theme` 驱动。runtime 负责 hitscan、LOS/A*、获得/切换、全局冷却、HP/ammo clamp、pickup 单次 commit 与 session/durable 分账。不要因“射击”就走 unknown custom module，也不要发明 skin、第三武器、多 guard、普通门、objective 或作者战斗音频。
- **第一人称空间谜题 / 机关迷宫 / 分支线索库** → 用上行同一个 `maze3d` 私有 runtime；先读 `examples/maze3d/references/maze3d-authoring.md` §5.5–§5.6，再从统一入口的正式 Recipe 3“分支仪式库”复制 `grid/events/pages/puzzle/set/warp/turn` cookbook。先画无环 dependency DAG 与三条 clue wings，至少证明两种收集顺序；partial pages 让 0/1/2/3/solved 可读，wrong/cancel 保持零副作用，最后仍由唯一 `D` 收口。不要把它路由成普通 scene 节点锁，也不要发明 custom puzzle module、第四模板或 debug state API。
- **普通谜题 / 小游戏关卡(组合锁 / 拨号锁 / 序列·Simon / 撬锁检定 / 实时 arcade 如贪吃蛇)** → 先读 `references/puzzles-and-minigames.md`。小游戏=一等可组合内嵌玩法模块,与正常玩法混排。**分层**:不依赖第一人称空间的组合锁/序列=节点图 + 门控写法(**零引擎改动**、§A,照抄即可);张力检定走 tabletop;真·实时 arcade 写自定义玩法模块(与内置模块**同一等机制**,引擎一行不加),照抄 `examples/arcade-demo`(最小贪吃蛇,§B)。⚠️ 逃生口必须**无条件 + to 别的节点**(`to:self` 清零不算逃生口、会掩盖真 soft-lock);闸不验证谜题逻辑可解性,靠人工试玩。**小游戏别 `enter` 即起局**(前置 ready 待机屏让玩家 opt-in);**限次/锁死必 fail-forward**(锁一扇门=同时开另一扇,别接死墙)——见 §B.7。
- **2D 横版跑跳 + 向右半自动射击 + 单固定目标** → 先读 `references/puzzles-and-minigames.md` §B.8，复用 `examples/sidescroller/` 的 example 私有固定 tick runtime；按题材可选闭合的 `presentation.profile` (`coast`/`frost`，缺失为 neutral)，表现与 SFX 由 runtime 成套拥有、没有作者旋钮。它不是公共模块，超出单目标/右射边界才走下一条。
- **引擎里没有的玩法类型**(模拟经营 / 卡牌 / 打地鼠，或超出既有横版窄边界的玩法)→ 先读 `references/plugin-development.md` 创建新模块(Level 1 复制 minimal 起步),再做游戏。
...
| `examples/maze3d/`(统一入口 + `references/maze3d-authoring.md` 作者手册 + `gallery.html` 协调recipe/FPS family/原子选材 + `audio-gallery.html` 试听) | maze3d 自定义 runtime 的 basic / horror / puzzle / layers / fps-range recipes(同一 `raycast-maze.js`,非公共模块);手册说明 grid、`maze.combat`、局部态 vs Amatlas state、Gallery 边界 | 做第一人称迷宫 / 怪物追逐恐怖 / 第一人称 FPS 时 |
...
| `references/puzzles-and-minigames.md` | 谜题(组合锁/序列/检定=节点图写法)+ 实时 arcade + 实验性固定 tick 横版切片(`examples/sidescroller*`) | 做谜题 / 小游戏关卡 / 窄横版跑跳射击时 |
