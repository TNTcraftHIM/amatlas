...
- **第一人称 FPS / 枪战迷宫 / 生存射击** → 仍用上行同一个 `maze3d` 私有 runtime 的 `fps-range` Recipe 5，**不是新模块**、也不是 boot 自动内置 kind。动手前读 `examples/maze3d/references/maze3d-authoring.md` §7，照抄闭合 `maze.combat` 骨架与 `raycast-maze.js + #maze3d-stage + manifest.modules` 装配；AI 只写 `player/pistol/guard/supplies` 与可选 clear 数据，runtime 负责 hitscan、LOS/A*、输入边沿、HP/ammo clamp、攻击/补给单次 commit 与 session/durable 分账。不要因“射击”就走 unknown custom module，也不要顺手发明普通门、objective、第二敌人、战斗音频或另一把武器。
