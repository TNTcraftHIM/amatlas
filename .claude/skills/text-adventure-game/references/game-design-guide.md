...
/* ── 存档/成就/地图浮窗:居中模态 + 显式背景遮罩 + ✕ 关闭(插件自行渲染 sibling .amatlas-plugin-backdrop)── */
...
.amatlas-plugin-backdrop{position:fixed;inset:0;z-index:59;background:rgba(0,0,0,.72);backdrop-filter:blur(3px);pointer-events:auto;touch-action:none}  /* 插件已创建 DOM；只换皮，不要自造或改回 ::before */
.amatlas-plugin-backdrop[hidden]{display:none}
