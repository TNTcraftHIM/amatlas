...
├─ 需要 2D 横版跑跳 + 向右半自动射击 + 单固定目标
│  └→ 先走 `puzzles-and-minigames.md` §B.8，复用 `examples/sidescroller/` 的实验性私有 runtime
├─ 需要引擎里没有的**玩法类型**(模拟经营 / 卡牌 / 打地鼠，或超出上述横版窄 profile)
...
引擎自带 `modules/minimal/`(一个完整、能跑、~30 行的计数器模块)+ `examples/minimal-demo/`。正式 R·05 同时含 custom `counter` 与普通 `scene`：index 保留 text-adventure runtime，custom kind 经 `manifest.modules:[create你的Module()]` 显式装配，普通 scene 仍由 boot 自动拉；十次达标后的核心 exit 展示两类节点自然回接。照下面**每步都跑验证**:
