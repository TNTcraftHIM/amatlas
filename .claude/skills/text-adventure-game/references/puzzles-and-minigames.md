...
`examples/sidescroller` 是比贪吃蛇更严格的一份实时岛教学材料：

- 作者数据只有闭合的 `viewport/map/player/weapon/sentry/clear`；`rows` 是等长 `.`/`#` tile，`clear.defeat` 必须引用 `sentry.id`，`clear.exposeLink` 必须引用真实 `node.links[].id`。
...
- 当前例子只有“跑/跳/向右半自动射击/固定哨戒炮/双方 3HP/死亡重开/清关”，**没有** waves、boss phases、拾取、多武器、音频、checkpoint、shmup 或 beat-em-up profile。不要根据文件名猜这些能力存在。

复制前先读 `examples/sidescroller/world.js` 与 `sidescroller-module.js`；修改数据后运行：

```bash
node examples/sidescroller/test/sidescroller.test.cjs
node core/tooling/graph-audit.mjs examples/sidescroller/world.js
node core/tooling/assembly-probe.mjs examples/sidescroller/index.html
node pipeline/build/build.mjs examples/sidescroller/index.html examples/sidescroller/dist/index.html --smoke
```
