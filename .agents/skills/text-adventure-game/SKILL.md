...
- **2D 横版跑跳 + 向右半自动射击 + 单固定目标** → 先读 `references/puzzles-and-minigames.md` §B.8，复用 `examples/sidescroller/` 的 example 私有固定 tick runtime；它不是公共模块，超出单目标/右射边界才走下一条。
...
| `examples/maze3d/`(统一入口 + `references/maze3d-authoring.md` 作者手册 + `gallery.html` 选材 + `audio-gallery.html` 试听) | maze3d 自定义 runtime 的 basic / horror / puzzle / layers / fps-range recipes(同一 `raycast-maze.js`,非公共模块);手册说明 grid、`maze.combat`、局部态 vs Amatlas state、Gallery 边界 | 做第一人称迷宫 / 怪物追逐恐怖 / 第一人称 FPS 时 |
