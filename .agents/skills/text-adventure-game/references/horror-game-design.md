...
horror_maze: {
  kind: 'maze3d',
  title: 'Recipe 2 · 地底回廊',
  winKey: 'horrorEscaped',           // 走到 'D' 出口门 → 写这个 key
  scareKey: 'horrorCaught',          // 被怪贴脸 → 写这个 key + 自动播 scareSfx
  ...
  links: [
    // 1. 逃出:winKey 门控
    { to: 'escaped_ending', requires: function (s) { return !!s.horrorEscaped; },
      showWhenLocked: true, lockHint: '你还没拿到钥匙、推开那扇门', label: '推开门,逃出去' },
    // 2. 被抓:scareKey 门控(被抓后唯一活的链接;走向结局节点)
    { to: 'caught_ending', requires: function (s) { return !!s.horrorCaught; },
      showWhenLocked: true, lockHint: '——', label: '……(你被它抓住了)' },
    // 3. 放弃:只在尚未终局时可用
    { to: 'fled_ending', requires: function (s) { return !s.horrorEscaped && !s.horrorCaught; }, label: '放弃,原路退回' }
  ]
},
fled_ending: {
  kind: 'scene',
...
- `winKey` / `scareKey` **必须在 `initState` 里声明为 false**:`initState: { horrorEscaped: false, horrorCaught: false }`(否则 graph-audit 报死键)。
- 正式 Recipe 2 是 `quiet route → K + wake → familiar return route → D / caught / give-up`:初始 grid 为 0 个 `K`,唯一怪物严格 `active:false`;进入 `(1,9)` 死路石座时,同一事件先在同格 `set K`,再 `activateMonsters:[0]`,下一帧才取得钥匙。photo 保持 quiet phase 可达且 `run` 去重。
- 动态 `K`、怪物 active/位置与 event once 都属于 session-local,不写 durable pursuit-started flag。只把 escaped/caught 作为持久结果;从 hub 重入时只清这两个结果事实。
- 先用 blocked-cell BFS 证明 photo 非必经、石座真死路,再用 runtime-equivalent `1/120`、`1/60`、`0.05` 三档证明激活前安全、回程可感知且能逃、另一路线确实能被抓。不要只比较 `2.6` 与 `chaseSpeed`。
- 唤醒不能靠 TTS 暗示:保留非空 `hint`、可见 marker 与动态 K 交接。放弃结果页显式写 `audio:{ambient:false,music:false}`,避免继承迷宫的 ambient/music。
- 保持单怪、无 combat;不要为追逐新增第二怪、phase、timer、sensor、debug API、新声音资产或 durable state。
...
- **「再玩一次」要回迷宫?必须在 link 的 `run` 里重置 winKey/scareKey**:结局通常是**终局场景**,玩家用工具栏「重新开始」重玩(引擎 `reset` 自动清所有 flag)。但**如果你写一条 link 从结局/中途**回到迷宫或迷宫前的节点,**必须**在那条 link 的 `run` 里把这个迷宫的 winKey/scareKey 清回 false——否则 `winKey` 残留为 true,重进迷宫会**直接显"通关"文字而不重玩**(winKey 是世界 flag、会持续;引擎不替你清,因为它常兼作跨节点进度门控)。写法:`{ to:'porch', label:'再玩一次', run:function(s){ s.horrorEscaped=false; s.horrorCaught=false; } }`。
- `scareSfx` 是字符串,不是数组(present-audio 内部包成 sfx 数组)。

**always-on 变体**:若产品意图确实是从入场第一秒持续追逐,可省略 `active:false` 与石座 activation,直接在初始 grid 放可达 `K`。它不是正式 Recipe 2 的默认锚,也不能描述成“取钥匙后苏醒 / 分阶段追逐”。
...
- 总入口:`examples/maze3d/index.html` 是 maze3d 的统一入口:basic / horror / puzzle / layers / fps-range 都是同一 `raycast-maze.js` runtime 的 recipes,不是不同模块。作者手册在 `examples/maze3d/references/maze3d-authoring.md`,先读它确认字段和状态边界；要做第一人称 FPS 用其中 §7 的 Recipe 5 闭合 `maze.combat`，不要把射击误判成引擎没有的玩法。选材入口:`examples/maze3d/gallery.html` 是 maze3d 素材总览页。先打开它看全套 `theme`、`wallTex`、`floorTex`、`ceilTex`、`wallDecor`、地面 decor、`events[i].visual`、出口和柱子,再把对应字段抄进 `maze:{...}`。gallery 是作者预览页,不是新玩法流程。
...
- 环境层与可交互层不要靠错误 fallback 混写；pillar / floor decor / sprite decor / pickup / marker 的默认选择、`floor-only + sprite` 硬边界、有意雕塑/诱饵/巨型物件的合法写法及同组物件的家族线索，统一以 `examples/maze3d/references/maze3d-authoring.md` §8 的视觉角色矩阵为准。这里不再重复一份易漂移规则。
...
先从 `examples/maze3d/index.html` 选 recipe:basic / horror / puzzle / layers / fps-range 都在同一个外部入口里,且都用同一份 `raycast-maze.js` runtime。追逐照抄 horror recipe,多层/多主题照抄 layers recipe,机关/隐藏物照抄 puzzle recipe,基础闭环照抄 basic recipe,第一人称 FPS 照抄 fps-range Recipe 5 与作者手册 §7。**不要**把这些 recipe 当成不同模块;模块边界看 `modules/` 与 `manifest.modules`。
...
| 闸/现象 | 真因 | 修 |
|---|---|---|
| graph-audit 报 `'horrorCaught' 死键(读从不写)` | scareKey 没在 initState 声明 | 加 `initState: { horrorCaught: false }` |
...
- **`activateMonsters` / `deactivateMonsters`**(可选,两键独立、可同现)—— 走到该格时批量**唤醒 / 停用**怪物。两种形态:`true`=作用全部怪;`[0, 2, …]`=整数索引数组,精确到 `maze.monsters` 下标(下标从 0、按数组声明顺序)。停用的怪**当帧起**不追、不渲染、不进最近怪心跳/压迫声场(等同"消失");激活后在同一 rAF 帧后续的怪物阶段即可追逐。**两键同现时固定顺序=先 deactivate 后 activate**(同一下标两边都写 → 最终态是激活)。经典用法——「进禁区唤醒怪」:入口处怪物 `active:false`(显式不追),玩家踏入禁区那一格触发 `activateMonsters:true`,怪物瞬间从静止转为追猎;反过来也能做"解开机关后怪物撤退"(`deactivateMonsters`)。v1 是瞬时开关,不做渐隐/渐显动画。

```js
maze: {
  grid: [/* …出口门附近故意用 '#' 封死…(示意占位:下方各事件坐标请按你的真实网格调整——warp/set 目标落在墙上会在进迷宫那一刻直接抛错,fail-loud 保护) */], start: { x: 1, y: 1, dir: 'E' },
  monsters: [{ x: 8, y: 8, body: 'slender', active: false }],   // 禁区守卫:显式 active:false = 静止不追,埋伏等触发
  events: [
...
- **完整可跑范本:`examples/maze3d/` 的 basic recipe** —— 压力板开石门 + 探索死路捡草图,照抄它起步。若不确定该复制哪一个 maze3d 变体,先打开 `examples/maze3d/index.html` 看 basic / horror / puzzle / layers / fps-range 分流；FPS 另先读 `references/maze3d-authoring.md` §7 的状态分账和停止线。
